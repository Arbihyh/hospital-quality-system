...
//                ->queryByMust(['term' => ['MED_REC_ID' => '729906']])
                ->queryByMust(['range' => ['AAC01' => ['gt' => '2023-01-01 00:00:00']]])
...
//                                    获取7天所有病程中的所有正副科的数据
    ...
    function getZbv2($type = 0, $where = [])
    {
        ...
        $dateType = [
            ...
            59 => ['numerator_jjbl', 'denominator_jjbl'],
    ...
    static function zbCount($resArr = [])
    {
        ...
        $count = [
            ...
            'res' => $denominator ? round($numerator / $denominator, 2) : 0,
            'time' => '全年', 'source' => '系统提取'];
        return $count;
    }
    ...
    function bingliData(array $where = [])
    {
        ...
        while (1) {
            ...
            $data = PatientInfo::query()
                ...
                ->where('patient_info.MED_REC_ID', '>', $lastId)
//                ->where('patient_info.MED_REC_ID', '=', '727924')
                ->orderBy('patient_info.MED_REC_ID', 'asc')
                ->limit($pageSize)
    ...
    function kjywData()
    {
        ...
        while (1) {
            ...
            $data = PatientInfo::query()
                ...
                ->where('patient_info.MED_REC_ID', '>', $lastId)
                ->where("patient_info.AAC01", '>', '2023-01-01')
                ->limit($pageSize)
                ->orderBy('patient_info.MED_REC_ID', 'asc')
                ->groupBy(['yzb.ZYH', 'yzb.YZMC', 'yzb.JLDW', 'yzb.YCJL', 'yzb.SYPC'])
                ->get(['yzb.XMLB', 'yzb.YZQX', 'yzb.ZYH', 'yzb.YZMC', 'yzb.KZSJ', 'yzb.kjyw_name', 'yzb.JLDW', 'yzb.YCJL', 'yzb.SYPC'])->toArray();
            ...
            foreach ($newData as $med => $data) {
                ...
                foreach ($data as $y) {
                    ...
                    if (!$bingcheng) {
                        ...
                        if ($ZXSJTime > $eTime) {
                            $flag = false;
                            $kjywError[] = '医嘱【' . $kjyw_name . '】病程【' . $y['YZMC'] . '】但是病程记录【 时间超过24小时 】';
                        } else {
    ...
    function exzlhxzlData()
    {
        ...
        while (1) {
            ...
            $data = $bgdObj = PatientInfo::query()
                ...
                ->get(['yzb.YZQX', 'yzb.ZYH', 'yzb.YZMC', 'yzb.KZSJ', 'yzb.hlyw_name', 'yzb.SYPC', 'yzb.YCJL'])->toArray();
            ...
            foreach ($newData as $med => $data) {
                ...
                foreach ($data as $y) {
                    ...
                    if (!$bingcheng) {
                        ...
                        if ($ZXSJTime > $eTime) {
                            $flag = false;
                            $exzlhxzlError[] = '医嘱【' . $hlyw_name . '】' . '病程记录【 时间超过24小时 】';
                        } else {
                ...
                foreach ($c1 as $c2) {
                    foreach ($lsyzArr[$k] as $l1) {
                        if ($c2['hlyw_name'] == $l1['hlyw_name'] && $c2['YCJL'] == $l1['YCJL'] && $c2['JLDW'] == $l1['JLDW']) {
                            Yzb::query()->where('id', '=', $l1['id'])->update(['is_has_hlyw' => 0]);
                        }
                    }
                }
            }
    ...
    public function cleanPreFymc()
    {
        ...
        while (true) {
            $start = ($page - 1) * $limit;
            $end = $start + $limit;
            $let = DB::update("update `fee_detailed` set pre_FYMC=SUBSTRING_INDEX(FYMC,'/', 1) where id between {$start} and {$end}");
            var_dump($let);
            if ($end > 100130601) {
                break;
            }
    ...
    function feeClean()
    {
        ...
        foreach ($blKeyword as $item) {
            ...
            while (true) {
                $start = ($page - 1) * $limit;
                $end = $start + $limit;
                $let = DB::update("update `fee_detailed` set is_bingli=1 where pre_FYMC='" . $item . "' and id between {$start} and {$end}");
                if ($end > 100130601) {
                    break;
                }
                $page++;
    ...
    static function operationClean()
    {
        ...
        if ($res) {
            ...
            $res = DB::table('yzb')->whereIn('id', $YzbIds)->update(['is_operation' => 1]);
            var_dump($res);
        }
    }
    ...
    function getZbList($where = [])
    {
        ...
        $dateType = [
            ...
            59 => ['numerator_jjbl', 'denominator_jjbl', 'jjbl_error'],
    ...
    function operationComplete()
    {
        ...
        while (1) {
            ...
            $patientInfo = $patientInfoCopy = PatientInfo::query()
                ->where('MED_REC_ID', '>', $lastId)
//                ->where('MED_REC_ID', '=', '723836')
                ->orderBy('MED_REC_ID', 'asc')
                ->where("AAC01", '>', '2023-01-01')
                ->limit($pageSize)
                ->get(['MED_REC_ID', 'AAC01'])->toArray();
            if (!$patientInfo) {
                break;
            }
...
//                        }
                        ...
                        $ssjl = EMR_BL_BL01::query()
                            ...
                            ->where('EMR_BL_BL01.BLZT', '<>', 9)
                            ->limit(1)
                            ->get(['CJSJ', 'operation_time'])->toArray();
                        if (!$ssjl) {
