...
//                $bcjlData[] = $lcyxBcjl3['msg'];
    ...
    public function depZbStatistics($type,$where,$page,$pageSize)
    {
        $zbService = new ElasticsearchService('patient_info_target');
...
//            $depList = Department::query()->pluck('dep_name')->toArray();
    ...
    public function depZbStatisticsList($type,$where,$page,$pageSize)
    {
        // 获取要统计得指标字段
        $zbFieldList = config('zb.zb_field_list');
        $zbField = $zbFieldList[$type];
        ...
        if ($type == 60 || $type == 61) {
            $field = ['patient_info.AAA28','patient_info.AAA01','patient_info.AAC11N','patient_info.AAC01','patient_info.AAB01','patient_info_target.'.$zbField[0],'patient_info_target.'.$zbField[1],'patient_info_target.'.$zbField[2],'patient_info_target.NG_BLBH','patient_info_target.XG_BLBH'];

        }else{
            $field = ['patient_info.AAA28','patient_info.MED_REC_ID','patient_info.AAA01','patient_info.AAC11N','patient_info.AAC01','patient_info.AAB01','patient_info_target.'.$zbField[0],'patient_info_target.'.$zbField[1],'patient_info_target.'.$zbField[2]];
        }

        if ($where['status'] == 1) {
            $query = PatientInfo::query()
                ->leftJoin('patient_info_target','patient_info.MED_REC_ID','=','patient_info_target.ZYH')
                ->where('patient_info_target.'.$zbField[1],'=',1)
                ->where('patient_info_target.'.$zbField[0],'=',1)
                ->whereBetween('AAC01',[$startTime,$endTime]);
            if ($depName) {
                $query->where('AAC11N', '=', $depName);
            }
            $data = $query->paginate($pageSize,$field,'page',$page)
                ->toArray();
        } elseif ($where['status'] === 0) {
            $query = PatientInfo::query()
                ->leftJoin('patient_info_target', 'patient_info.MED_REC_ID', '=', 'patient_info_target.ZYH')
                ->where('patient_info_target.' . $zbField[1], '=', 1)
                ->where('patient_info_target.' . $zbField[0], '=', 0)
                ->whereBetween('AAC01', [$startTime, $endTime]);
            if ($depName) {
                $query->where('AAC11N', '=', $depName);
            }
