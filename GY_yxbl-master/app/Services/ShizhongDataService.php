...
//            self::insertDataMain($rData, $info['inData'], $ABC03C, $config);
    ...
    public static function sssq($list = "")
    {
        $sql = "SELECT SQDH, JGID, ZYH, SSKS, SQKS, SQYS, SQRQ, SSRQ, SSNM, SSYS, SSYZ, SSSZ, SSEZ, SSYQ, MZDM, MZYS, TJBZ, APBZ, ZFBZ, TXKS, CZGH, SQTL, SQZD, NSSMC, FYBQ, QKDJ, THYY, ZFYY, ZFGH, LRBZ, YXJS, NLTR, HBQTJB, QTTSQK, TSQKNR, SSJB, CRBZ, CRBG, BXBZ, BXSM, BZXX, SSTW, SPBZ, CFSS, ZLXZ, RJSS FROM PORTAL_HIS.SM_SSSQ  WHERE ZYH IN ($list)";
        $inResult = oci_parse(self::$con, $sql);
        ...
        \App\Model\SSSQ::query()->whereIn('ZYH', $zyh)->delete();
        \App\Model\SSSQ::query()->insert($inData);
    }

    public static function pacs($list = "")
    {
        ...
        $sql = "SELECT StudyUid, ZYH, YLJGDM, JZLX, JZLSH, MZZYBZ, KH, KLX, BRXM, BRXB, PatientID, PatientID1, JCXMDM, JCXMDMYB, SQDH, KDSJ, JYSJ, ExamType, SBBM, YQBM, SQKS, SQKSMC, SQRGH, SQRXM, JCBGJGMC, JCKS, JCKSMC, JCYSGH, JCYS, BGSJ, BGRQ, BGRGH, BGRXM, SHRGH, SHRXM, JCBW, BWACR, JCMC, ZYJCXX1, ZYJCXX2, ZYJCXX3, YXBX, YXZD, BZHJY, SFYYY, XGBZ, MJ, YLYL1, YLYL2, csrq, F_PDF_PATH, F_RPT_JPG_PATH FROM TB_RIS_Report  WHERE ZYH IN ($list)";
        ...
        foreach ($data as $item) {
            $inData[] = [
                ...
                'YLYL2' => $item['YLYL2'],
                'csrq' => $item['csrq'],
                'F_PDF_PATH' => $item['F_PDF_PATH'],
                'F_RPT_JPG_PATH' => $item['F_RPT_JPG_PATH'],
            ];
        }

        $zyh = array_column($data, 'ZYH');
        \App\Model\PACS::query()->whereIn('ZYH', $zyh)->delete();
        \App\Model\PACS::query()->insert($inData);
    }
    ...
    // 查询患者信息
    public static function selectInsertData($list = [])
    {
        if (empty($list)) {
            return ['data' => [], 'inData' => []];
        }
        $list = implode(',', $list);
        echo $list . "\n";
        $sql = "SELECT * FROM PORTAL_HIS.INIT_MED_REC_MAIN a WHERE MED_REC_ID IN ($list)";
        ...
        $inResult = oci_parse(self::$con, $in_sql);
        oci_execute($inResult, OCI_DEFAULT);
        $inData = [];
        while ($row = oci_fetch_assoc($inResult)) {
            $inData[] = $row;
    ...
    public static function insertDataMain($data, $inData, $ABC03C, $config)
    {
        foreach ($data as $v) {
            ...
            if (PatientInfo::query()->where('MED_REC_ID', '=', $v['MED_REC_ID'])->exists()) {
                PatientInfo::query()->where('MED_REC_ID', '=', $v['MED_REC_ID'])->update($patient_info);
            } else {
            ...
            if (PatientInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->exists()) {
                PatientOtherInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->update($patient_other_info);
            } else {
            ...
            if (PatientInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->exists()) {
                PatientMedicalInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->update($patient_medical_info);
            } else {
            ...
            if (PatientHospitalInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->exists()) {
                PatientHospitalInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->update($patient_hospital_info);
            } else {
            ...
            if (PatientDoctorInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->exists()) {
                PatientDoctorInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->update($patient_doctor_info);
            } else {
            ...
            if (PatientAddressInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->exists()) {
                PatientAddressInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->update($patient_address_info);
            } else {
            ...
            if (PatientWorkInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->exists()) {
                PatientWorkInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->update($patient_work_info);
            } else {
            ...
            if (PatientContactsInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->exists()) {
                PatientContactsInfo::query()->where('AAA28', '=', $v['MED_REC_ID'])->update($patient_contacts_info);
            } else {
    ...
    {
        $sql = "SELECT ZYH,FYXH,FYMC,to_char(ZFJE,'fm9999990.000') as ZFJE,to_char(JFRQ,'yyyy-mm-dd hh24:mi:ss') as JFRQ,FYSL,to_char(FYDJ,'fm9999990.000') as FYDJ,FYKS FROM PORTAL_HIS.V_ZY_FYMX WHERE ZYH IN (" . $list . ") ";
...
//                'SYFYGB' => array_search($item['SYFYGB'], $config) ?? '',//首页费用归并
        ...
        while (1) {
            ...
            $zyh = array_column($temFee, 'AAA28');
            FeeDetailed::query()->whereIn('AAA28', $zyh)->delete();
    ...
    public static function mainOperation($data, $config)
    {
        ...
        $zyh = array_column($data, 'ZYH');
        MainOperation::query()->whereIn('AAA28', $zyh)->delete();
        SecondaryOperation::query()->whereIn('AAA28', $zyh)->delete();
    ...
    {
        $zyh = array_column($data, 'ZYH');
        \App\Model\Yzb::query()->whereIn('ZYH', $zyh)->delete();
        foreach ($data as $item) {
            $list = [
                'ZYH' => $item['ZYH'],
                'YZBXH' => $item['YZBXH'] ?? '',
     ...
     * 获取病例数据
     */
    public static function getBl01($list = '')
    {

        $sql = "SELECT * FROM PORTAL55_EMR.V_JMGS_BASY_QBL WHERE ZYH IN (" . $list . ") ";
        $result = oci_parse(self::$con, $sql);
        oci_execute($result, OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
     ...
     * 获取病例数据
     */
    public static function getBl012($list)
    {
    ...
    public static function blsy($blbh = '')
    {
        $sql = "SELECT BLBH,SYYS,to_char(SYSJ,'yyyy-mm-dd hh24:mi:ss') as SYSJ,to_char(JLSJ,'yyyy-mm-dd hh24:mi:ss') as JLSJ FROM PORTAL55_EMR.EMR_BL_BLSY WHERE BLBH IN ($blbh)";
        $result = oci_parse(self::$con, $sql);
        oci_execute($result, OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
    ...
    public static function zyhcmx($ids = '')
    {

        $sql = "SELECT ZYH, to_char(HCRQ,'yyyy-mm-dd hh24:mi:ss') as HCRQ, ZZRQ, HCLX, HQCH, HHCH, HQKS, HHKS, HQBQ, HHBQ, JSCS, CZGH, JGID FROM PORTAL_HIS.ZY_HCMX WHERE ZYH IN ($ids)";
        $result = oci_parse(self::$con, $sql);
        oci_execute($result, OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
        ...
        $zyh = array_column($data, 'ZYH');
        ZY_HCMX::query()->whereIn('ZYH', $zyh)->delete();
        foreach ($data as $item) {
            $insertData = [
                'ZYH' => $item['ZYH'],
                'HCRQ' => $item['HCRQ'],
            ...
            ZY_HCMX::query()->insert($insertData);
        }
    }
