...
class QueueChaoSheng extends Command
{
    /**
     ...
     */
    protected $signature = 'queue:chao-sheng';
     ...
     * @var string
     */
    protected $description = 'Command 超生的消息队列消费脚本';
    ...
    public function handle()
    {
        ...
        while (true) {
            ...
            $data = QueueList::query()->where(["type" => "fee-detailed", "status" => 0])->orderBy("id", "desc")->first();
            if (!$data) {
                sleep(10);
                continue;
            }
            ...
            $homeDataService->getChaoShengData($data["data"]);
