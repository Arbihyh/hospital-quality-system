...
class QueueJianYan extends Command
{
    /**
     ...
     */
    protected $signature = 'queue:jianyan';
     ...
     * @var string
     */
    protected $description = 'Command 检验的消息队列消费脚本';
    ...
    public function handle()
    {
        ...
        while (true) {
            ...
            $data = QueueList::query()->where(["type" => "jianyan", "status" => 0])->orderBy("id", "desc")->first();
            if (!$data) {
                sleep(10);
                continue;
            }
            ...
            try{
                ...
                $homeDataService->getJianYanData($data["data"]);
                // 删除
