...
class Zb_szfhsqmlyzl extends Command
{
    protected $signature = 'zb_szfhsqmlyzl {zyh?} {startTime?} {endTime?}';

    protected $description = '计算术者符合授权目录一致率指标';
    ...
    public function handle()
    {
        ...
        $this->info('开始计算术者符合授权目录一致率: ' . date("Y-m-d H:i:s"));
        ...
        $indicatorCalcService->calculateSzfhsqmlyzl($zyh, $startTime, $endTime);
        $e = time();
        $diff = $e - $s;
        $h = floor($diff / 3600);
