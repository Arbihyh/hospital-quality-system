...
class Ryjl2 extends Command
{
     ...
     * @var string
     */
    protected $signature = 'shizhong:ryjl2';
    ...
    public function ZY_BRRY($zyh = '')
    {
        ...
        foreach ($data as $item) {
            ...
            //异步更新入库
            AsynHomeData::dispatch(['yzb' => [], 'bl01' => $hisData['bl01'][$item['ZYH']] ?? [], 'zyh'=>$item['ZYH']])->onQueue('asynHomeData');

            $err = [];
            $err[] = $shizhongService->checkRy8($hisData, $item['ZYH'], $wanchengshijian);
            $ry = $shizhongService->ruleRuYuan(99, $item['ZYH'],$hisData, $wanchengshijian);
            $err = array_merge($err, $ry);
            foreach ($err as $v) {
