...
class Cyjl extends Command
{
     ...
     * @var string
     */
    protected $signature = 'shizhong:cyjl';
    ...
    public function ZY_BRRY($zyh = '')
    {
        ...
        $sql = "SELECT CYPB,ZYH,ZYHM,BRKS,BRXM,ZZYS,ZSYS,to_char(RYRQ,'yyyy-mm-dd HH24:mi:ss') as RYRQ,to_char(CYRQ,'yyyy-mm-dd HH24:mi:ss') as CYRQ FROM PORTAL_HIS.ZY_BRRY WHERE CYRQ BETWEEN to_date(".$start.",'yyyy-MM-dd HH24:mi:ss') AND to_date(".$end.",'yyyy-MM-dd HH24:mi:ss')";
        ...
        foreach ($data as $item) {
            ...
            $res = $homeData->getData($item['ZYH'], ['yzb','bl01','bl01New']);
            if ($res === false) {
                continue;
            }

            $banhzkgzService->rule95($caseRule, 95, $item['ZYH']);
            $banhzkgzService->rule96($caseRule, 96, $item['ZYH']);
            $res = $banhzkgzService->insertData;
            foreach ($res as $v) {
