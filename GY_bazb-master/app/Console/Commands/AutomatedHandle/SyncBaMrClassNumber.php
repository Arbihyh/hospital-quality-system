...
class SyncBaMrClassNumber extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sync:baMrClassNumber';
    ...
    {
        $setName = 'es_index_ba_mr_class_number_2023';
        $lastId = Setting::query()->where('name', '=', $setName)->value('content');
        $lastId = $lastId ?: 0;

        $data = BaMrClassNumber::query()
            ->where('id','>',$lastId)
            ->orderBy('id')
            ->get()->toArray();
        $index = 'ba_mr_class_number_2023';
        if (!empty($data)) {
            $es_params = [];
