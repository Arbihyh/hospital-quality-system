...
class ZbBaglService
{
    ...
    public function getPatientInfo($cyzs, $zyh, $start, $end, $page)
    {
        return DB::table($cyzs->table_name)
            ...
            ->orderBy('patient_info.AAC01')
            ->paginate(500, [$cyzs->table_name . '.' . $cyzs->MED_REC_ID, 'patient_info.AAB01', $cyzs->table_name . '.AAA28', 'patient_info.AAA01', 'patient_info.AAC01', 'patient_info.AAC04', 'patient_info.score', 'patient_doctor_info.AEE03', DB::raw('COALESCE(NULLIF(TRIM(department.dep_name), \'\'), \'\') as AAC11N')], 'page', $page);
    }
        ...
        while (true) {
            ...
            foreach ($data->items() as $value) {
                ...
                if ($HCRQ) {
                    ...
                    $params = $bl01Service->clearMust()->queryByMustBatch($must)->queryByShouldBatch($should)->minimumShouldMatch(1)->queryByMustNot(["term" => ['BLZT' => 9]])->paginate(1, 1000)->getParams();
                    $restful = app('es')->search($params);
                    ...
                    if (!empty($bgsj_start)) {
                       ...
                       foreach ($yzbData[0] as $yzb) {
                        ...
                        if (!$isYzb) {
                            ...
                            //2026-03-23 14:56:16.013 处理bgsj为YYYY-MM-DD HH:MM:SS格式
                            $bgsj = date('Y-m-d H:i:s', strtotime($bgsj));
                            $bgsj_end = date('Y-m-d H:i:s', strtotime($bgsj) + (72 * 3600));
                            ...
                            if (!empty($bl01Data[0])) {
                                ...
                                $orderGroup['content'][] = [
                                    'status' => 0,
                                    'content' => "报告出具后24小时内，病程记录中未见对检查结果的描述"
                                ];
                            }
                        }
                    }
    ...
    public function bljcjl($zyh, $start, $end)
    {
        ...
        while (true) {
            ...
            foreach ($data->items() as $value) {
                ...
                // 改为MySQL查询，查询fee_detailed表，FYMC做模糊匹配
                $feeQuery = \App\Model\FeeDetailed::query()
                    ->where('AAA28', $ZYH)
                    ->where(function ($query) use ($blfy) {
                        foreach ($blfy as $item) {
                            if (!empty($item['fymc'])) {
                                $query->orWhere('FYMC', 'like', $item['fymc'] . '%');
                            }
                        }
                    });
                $feeDataArr = $feeQuery->limit(1000)->get()->toArray();
                // 按原有结构兼容，$feeData[0]为结果集
                $feeData = [];
                if (!empty($feeDataArr)) {
                    $feeData[0] = $feeDataArr;
                } else {
                    $feeData[0] = [];
                }
    ...
    public function xjpyjcjl($zyh, $start, $end)
    {
        ...
        while (true) {
            ...
            foreach ($data->items() as $value) {
                ...
                // 住院收费明细中包含病理检查相关收费项目，例如：一般细菌涂片检查、一般细菌涂片检查(图文报告）、抗酸杆菌涂片检查等"
                ...
                $yzb = [];
                $yzbAll = Yzb::query()->where('ZYH', $ZYH)->get(['YZMC'])->toArray();
                foreach ($yzbAll as $item) {
                ...
                // 有细菌培养检查医嘱，且该医嘱对应的检验项目与药敏试验项目一致
                $ymresult = [];
                foreach ($ymresultList as $item) {
                    $EXAMINAIM = $item['EXAMINAIM'];
                    if (in_array($EXAMINAIM, $yzb)) {
                        $ymresult[] = $item;
                    }
                }
                ...
                echo 'ymresultList:' . count($ymresultList) . PHP_EOL;

                $insert['xjpyjcjl_fm'] = 1;

                // 分子处理
                $allMatch = true;
                foreach ($ymresult as $key => $ymValue) {
                    // 分子 - 报告单
                    $reportGroup = [
                        'status' => 1,
                        'content' => []
                    ...
                    $reportGroup['content'][] = [
                        'status' => 1,
                        'content' => '医嘱【' . $ymValue['EXAMINAIM'] . '】'
                    ];
                    if (empty($ymValue['YMJG'])) {
                        $allMatch = false;
                        $reportGroup['status'] = 0;
                    }
                    $reportGroup['content'][] = [
                        'status' => $ymValue['YMJG'] ? 1 : 0,
                        'content' => '药敏结果【' . ($ymValue['YMJG'] ?: '无') . '】'
                    ];
                    $reportGroup['content'][] = [
                        'status' => 1,
                        'content' => '报告时间【' . $ymValue['BGSJ'] . '】'
                    ];
                    $flag = false;
                    $firstBlsyTime = "";
                    $blmc = "";
                    foreach ($BL01 as $item) {
                        ...
                        if (strtotime($item['ZXSJ']) < strtotime($ymValue['BGSJ']) + 86400 && (strpos($item['HJNR'], '培养') !== false || strpos($item['HJNR'], $ymValue['XJMC']) !== false)) {
                            $flag = true;
                    ...
                    $reportGroup['content'][] = [
                        'status' => $flag === true ? 1 : 0,
                        'content' => '病程记录【' . ($flag === true ? '有培养结果描述' : '无培养结果描述') . '】'
                    ];

                    if ($flag) {
                        if (strtotime($firstBlsyTime) > strtotime($ymValue['BGSJ']) + 864000) {
                            $reportGroup['content'][] = [
                                'status' => 0,
                                'content' => '首次签名时间【' . $blmc . '】超24小时'
                            ];
                            $reportGroup['status'] = 0;
                        }
                    } else {
