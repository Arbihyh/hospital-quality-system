...
class IndicatorService
{
    ...
    public function handleHzry8xsncf($data, $startTime)
    {
        ...
        foreach ($data as $item) {
            ...
            if (empty($AAC01)) {
                //当前时间
                $AAC01 = date('Y-m-d H:i:s');
            }
            if (strtotime($AAC01) - strtotime($item['AAB01']) <= 86400) {
                continue;
            }
            ...
            $yzInfo = Yzb::query()
                ->where('ZYH', '=', $insert['zyh'])
                //->where('YZZT', '!=', $keyword2035)
                ->where('KZSJ', '<=', date('Y-m-d H:i:s', $endTime))
                ->orderBy('KZSJ', 'asc')
                ->get()
                ->toArray();
    ...
    public function handleQjcg($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $yzbInfo = Yzb::query()
                ->where('ZYH', '=', $insert['zyh'])
                ->where('YZMC', 'LIKE', $ruleMap8026['keyword'])
                ->get()
                ->toArray();
        ...
        foreach ($data as $item) {
            ...
            foreach ($yzbList as $idx => $y) {
                ...
                if (!$foundConsultOpinion) {
                    $grp['content'][] = ['status' => 0, 'content' => '会诊申请【无】'];
                }
