...
class BanhzkgzService
{
    ...
    public function zkgzHandle($caseRule, $info, $isSz = 0)
    {
        ...
        foreach ($methodList as $ruleId => $method) {
            ...
            // 调用质控规则
            if ($ruleId == 1003) {
                $this->$method($caseRule, $ruleId, $ZYH, $isSz);
            } else {
      ...
      * 缺少死亡病例讨论结论记录
      */
    public function rule1211($caseRule = [], $ruleId, $ZYH = "")
    {
        $yzbDataList = Yzb::query()->where('ZYH', (string)$ZYH)->where('YZMC', 'like', '%死亡%')->get()->toArray();
        if (empty($yzbDataList)) {
            return [];
        }

        $basisList = [];
        $bl01 = EMR_BL_BL01::query()->where('MBLB', 4302)->where("JZHM", (string)$ZYH)->get(["EMR_BL_BL01.BLMC"])->toArray();
        if (empty($bl01)) {
            $this->insertData[] = [
    ...
    public function rule1029($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $newData = $newData->toArray();
        $RYSJ = date("Y-m-d H", strtotime($newData["RYSJ"]));
        ...
        $AAB01 = date("Y-m-d H", strtotime($AAB01));
    ...
    public function rule1028($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $newData = $newData->toArray();
        $CYRQ = date("Y-m-d H", strtotime($newData["CYRQ"]));
        ...
        $AAC01 = date("Y-m-d H", strtotime($AAC01));
    ...
    public function rule1027($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $newData = $newData->toArray();
        $RYRQ = date("Y-m-d H", strtotime($newData["RYRQ"]));
    ...
    public function rule102($caseRule, $ruleId, $ZYH, $isYj = 0)
    {
        ...
        $pacs = \App\Model\PACS::query()->where("ZYH", (string)$ZYH)->get(["JCMC", "BGSJ"])->toArray();
        $yzb = \App\Model\Yzb::query()->where("ZYH", (string)$ZYH)->orderBy('KZSJ', 'desc')->get(["YZMC", "KZSJ"])->toArray(); //按开嘱时间倒序
        $isnum = 1;
        //$pacsStartTime = 0;
        foreach ($pacs as $p) {
            $basis = [];
            ...
            $p['BGSJ'] = date('Y-m-d H:i:s', strtotime($p['BGSJ']));
            ...
            if ($isnum == 1) {
                $yzb1 = \App\Model\Yzb::query()->where("ZYH", (string)$ZYH)->orderBy('KZSJ', 'asc')->get(["YZMC", "KZSJ"])->toArray();
                foreach ($yzb1 as $y) {
                    // 不包含CT或者包含的是OCT则跳过
    ...
    public function rule109($caseRule, $ruleId, $ZYH, $isYj = 0): array
    {
        ...
        if (!empty($resData)) {
            ...
            foreach ($resData as $v) {
                ...
                $hjnrKeywords = array_values(array_filter($hjnrKeywords, function ($keyword) {
                    return trim((string)$keyword) !== '';
                }));
                // 24小时内的病程记录
                $resDataList1 = EMR_BL_BL01::query()
    ...
    public function rule95($caseRule, $ruleId, $ZYH, $isYj = 0)
    {
        ...
        // 2. 查询医嘱是否包含出院/离院关键字
        $exitKeywords = array_values(array_filter($exitKeywords, function ($keyword) {
            return trim((string)$keyword) !== '';
        }));
    ...
    public function rule99($caseRule, $ruleId, $ZYH, $isYj = 0)
    {
        ...
        if (empty($firstBlsyTime)) {
            ...
            if ($diffTime > 0 && $diffTime < 2 * 3600) {
                $res = remainderTime($diffTime);
                $content = '请在' . $res . '内完成入院记录';
                $msgYj = ['入院时间【' . $enterTime . '】', '入院记录【已创建，未签名】'];
                $this->caseService->sendMsg($ZYH, $content, 99, $msgYj);
            }
            return true;
        }

        //如果firstBlsyTime在入院时间到入院时间+24小时之间，则不质控，在之前就是提前创建，之后就是超时
        if (strtotime($firstBlsyTime) < strtotime($enterTime)) {
            $basis[] = "入院记录首次签名时间【" . $firstBlsyTime . "（提前创建）】";
            //加入质控的病案首页的BLBH
            $basis["BLBH"] = $bl01Data[0]['BLBH'];
            $this->insertData[] = [
                'JZHM' => $ZYH,
        ...
        } elseif (strtotime($firstBlsyTime) > strtotime($enterTime) + 24 * 3600) {
    ...
    public function rule98($caseRule, $ruleId, $ZYH)
    {
        ...
        if (!empty($deathbl01)) {
            ...
            $resData = $blDataFormatService->cleanDataFilter("clean_bllb_288", [], $hjnr);
            if (!empty($resData['SWSJ'])) {
                $deathTime = $resData['SWSJ'];
                $deathTime = substr($deathTime, 0, 10) . ' ' . substr($deathTime, 10);
                Log::info('$ZYH', ['$ZYH' => $ZYH, '$deathTime' => $deathTime]);
            } elseif (!empty($hjnr)) {
    ...
    public function rule1000($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($yzbData as $key => $item) {
            ...
            foreach ($ruleMap8003 as $keyword) {
                if (stripos($item['YZMC'], $keyword) !== false) {
                    $flag = true;
                    break;
                }
            }
    ...
    public function rule101($caseRule, $ruleId, $ZYH)
    {
        ...
        if (empty($bl01Data[0])) {
            ...
            if ($diffTime > 0 && $diffTime < 2 * 3600) {
                ...
                $msgYj = ['入院时间【' . $enterTime . '】', '首次病程记录【未创建】'];
        ...
        if (empty($firstBlsyTime)) {
            ...
            if ($diffTime > 0 && $diffTime < 2 * 3600) {
                $res = remainderTime($diffTime);
                $content = '请在' . $res . '内完成首次病程记录';
                $msgYj = ['入院时间【' . $enterTime . '】', '首次病程记录【已创建，未签名】'];
                $this->caseService->sendMsg($ZYH, $content, 101, $msgYj);
            }
            return true;
        }
        ...
        if (strtotime($firstBlsyTime) < strtotime($enterTime)) {
            // 签名时间早于入院时间，提前创建
            $basis[] = "首次病程记录创建时间【" . $firstBlsyTime . "（提前创建）】";
            $basis["BLBH"] = $bl01Data[0]['BLBH']; //加入被质控病程BLBH
            $this->insertData[] = [
                'JZHM' => $ZYH,
                'rule_id' => $ruleId,
        ...
        } else if (strtotime($firstBlsyTime) > $enterTimeEnd) {
            // 签名时间超过入院后8小时
            $basis[] = "首次病程记录创建时间【" . $firstBlsyTime . "（超8小时）】";
            $basis["BLBH"] = $bl01Data[0]['BLBH']; //加入被质控病程BLBH
            $this->insertData[] = [
                'JZHM' => $ZYH,
                'rule_id' => $ruleId,
    ...
    public function rule114($caseRule, $ruleId, $ZYH, $isYj = 0)
    {
        ...
        // 完成时间
        $ruleMap3048 = RuleWordMap::query()->where('id', '=', 3048)->value('keyword');

        //转换结束时间成YYYY-MM-DD HH:MM:SS
        $enterTimeEnd = date('Y-m-d H:i:s', $enterTimeEnd);

        $bl01Data = EMR_BL_BL01::query()->where('JZHM', $ZYH)->whereIn('BLLB', $excludeDiagnosis)->whereNotIn('MBLB', $excludeDiagnosis1)->where($ruleMap8011, '>=', $enterTime)->where($ruleMap8011, '<=', $enterTimeEnd)->get()->toArray();
        ...
        if (empty($bl01Data)) {
            // 按照ruleMap8011没找到，按照ZXSJ回退查找
            $bl01Data = EMR_BL_BL01::query()->where('JZHM', $ZYH)->whereIn('BLLB', $excludeDiagnosis)->whereNotIn('MBLB', $excludeDiagnosis1)->where('ZXSJ', '>=', $enterTime)->where('ZXSJ', '<=', $enterTimeEnd)->orderBy('ZXSJ', 'desc')->get()->toArray();
            ...
            if (empty($bl01Data)) {
                ...
                $hasHighLevelDoctor1 = false;
                $zxsjrecord = [];
                foreach ($bl01Data as $record) {
                    $blmc = $record['BLMC'];
                    $hjnr = EMR_BL_BLXG::query()->where('BLBH', $record['BLBH'])->value('HJNR');
                    foreach ($highLevelTitles as $title) {
                        if (strpos($blmc, $title) !== false || strpos($hjnr, $title) !== false) {
                            $hasHighLevelDoctor1 = true;
                            $zxsjrecord = $record;
                            break;
                        }
                    }
                }
                if (!$hasHighLevelDoctor1) {
                    $basis[] = "上级医师查房记录【无】";
                    $this->insertData[] = [
                        'JZHM' => $ZYH,
                        'rule_id' => $ruleId,
                    ...
                    // 检查创建时间与标题时间的关系
                    $cjsj = $zxsjrecord[$ruleMap3048] ?? '';
                    $allBasisGroups = [];
                    if (strtotime($cjsj) < strtotime($enterTime)) {
                        $basis = [];
                        $basis[] = "入院时间【" . $enterTime . "】";
                        $basis[] = "【" . $zxsjrecord['BLMC'] . "】完成时间【" . $cjsj . "（提前创建）】";
                        $basis["BLBH"] = $zxsjrecord['BLBH'];
                        $allBasisGroups[] = $basis;
                    } elseif (strtotime($cjsj) > strtotime($enterTime) + 48 * 3600) {
                        $basis = [];
                        $basis[] = "入院时间【" . $enterTime . "】";
                        $basis[] = "【" . $zxsjrecord['BLMC'] . "】完成时间【" . $cjsj . "（超时创建）】";
                        $basis["BLBH"] = $zxsjrecord['BLBH'];
                        $allBasisGroups[] = $basis;
                    }
                    if (!empty($allBasisGroups)) {
        ...
        foreach ($bl01Data as $value) {
            ...
            foreach ($blsy as $blsyValue) {
                ...
                //判断是否包含逗号
                if (strpos($ruleMap8012, ',') !== false) {
                    $excludeDiagnosis3 = explode(',', $ruleMap8012);
                } else {
                    $excludeDiagnosis3 = [$ruleMap8012];
                }
                //判断医生职称是否包含'主任'或'主治'
                $isMatched = false;
                foreach ($excludeDiagnosis3 as $title) {
        ...
        if (!$hasLeader) {
            // ruleMap8011找到了记录但没有上级医师，按照ZXSJ回退查找
            $bl01DataZxsj = EMR_BL_BL01::query()->where('JZHM', $ZYH)->whereIn('BLLB', $excludeDiagnosis)->whereNotIn('MBLB', $excludeDiagnosis1)->where('ZXSJ', '>=', $enterTime)->where('ZXSJ', '<=', $enterTimeEnd)->orderBy('ZXSJ', 'desc')->get()->toArray();

            if (!empty($bl01DataZxsj)) {
                // 通过ZXSJ找到了记录，检查是否包含上级医师关键字
                $ruleMap8012z = RuleWordMap::query()->where('id', '=', 8012)->value('keyword');
                $ruleMap8012z = !empty($ruleMap8012z) ? $ruleMap8012z : '主治,主任';
                if (strpos($ruleMap8012z, ',') !== false) {
                    $highLevelTitles = explode(',', $ruleMap8012z);
                } else {
                    $highLevelTitles = [$ruleMap8012z];
                }

                $hasHighLevelDoctor1 = false;
                $zxsjrecord = [];
                foreach ($bl01DataZxsj as $record) {
                    $blmc = $record['BLMC'];
                    $hjnr = EMR_BL_BLXG::query()->where('BLBH', $record['BLBH'])->value('HJNR');
                    foreach ($highLevelTitles as $title) {
                        if (strpos($blmc, $title) !== false || strpos($hjnr, $title) !== false) {
    ...
    public function rule1047($caseRule, $ruleId, $ZYH)
    {
        ...
        if (empty($bl01Data)) {
            ...
            if (empty($bl01Data)) {
                ...
                if (!$hasHighLevelDoctor1) {
                    ...
                    // 检查创建时间与标题时间的关系
                    $wcsj = $zxsjrecord[$ruleMap3048] ?? '';
                    $allBasisGroups = [];
                    if (!empty($wcsj) && !empty($zxsjrecord[$ruleMap8011])) {
                        if (strtotime($wcsj) < strtotime($zxsjrecord[$ruleMap8011])) {
                            $basis = [];
                            $basis[] = "入院时间【" . $enterTime . "】";
                            $basis[] = "【" . $zxsjrecord['BLMC'] . "】创建时间【" . $wcsj . "（提前创建）】";
                            $basis["BLBH"] = $zxsjrecord['BLBH'];
                            $allBasisGroups[] = $basis;
                        } elseif (strtotime($wcsj) > strtotime($zxsjrecord['ZXSJ'])) {
                            $basis = [];
                            $basis[] = "入院时间【" . $enterTime . "】";
                            $basis[] = "【" . $zxsjrecord['BLMC'] . "】完成时间【" . $wcsj . "（超时创建）】";
                            $basis["BLBH"] = $zxsjrecord['BLBH'];
                            $allBasisGroups[] = $basis;
                        }
                    } else {
        ...
        foreach ($bl01Data as $value) {
            ...
            $blmc = $value['BLMC'];
            $hjnr = EMR_BL_BLXG::query()->where('BLBH', $value['BLBH'])->value('HJNR');
        ...
        if (!$hasLeader) {
            ...
            if (!empty($bl01DataZxsj)) {
                ...
                if ($hasHighLevelDoctor1) {
                    // 检查创建时间与标题时间的关系
                    $cjsj = $zxsjrecord['CJSJ'] ?? '';
                    $allBasisGroups = [];
                    if (!empty($cjsj) && !empty($zxsjrecord['ZXSJ'])) {
                        if (strtotime($cjsj) < strtotime($zxsjrecord['ZXSJ'])) {
                            $basis = [];
                            $basis[] = "入院时间【" . $enterTime . "】";
                            $basis[] = "【" . $zxsjrecord['BLMC'] . "】创建时间【" . $cjsj . "（提前创建）】";
                            $basis["BLBH"] = $zxsjrecord['BLBH'];
                            $allBasisGroups[] = $basis;
                        } elseif (strtotime($cjsj) > strtotime($zxsjrecord['ZXSJ'])) {
                            $basis = [];
                            $basis[] = "入院时间【" . $enterTime . "】";
                            $basis[] = "【" . $zxsjrecord['BLMC'] . "】创建时间【" . $cjsj . "（超时创建）】";
                            $basis["BLBH"] = $zxsjrecord['BLBH'];
                            $allBasisGroups[] = $basis;
                        }
                    } else {
    ...
    public function rule132($caseRule, $ruleId, $ZYH)
    {
        ...
        // 新增：遍历所有病程记录，检查 72 小时窗口内是否存在 bllb=294 的匹配记录
        $allRecords = EMR_BL_BL01::query()
            ->where('JZHM', $ZYH)
            ->where('BLLB', 294)
            ->get()
            ->toArray();

        foreach ($allRecords as $current) {
            $zxsj = $current['ZXSJ'] ?? null;
            $cjsj = $current[$ruleMap3048] ?? null;
            if (!$zxsj || !$cjsj) {
                continue;
            }
            // 如果出院时间 - ZXSJ 小于 3*24 小时，跳过
            if ((strtotime($dischargeTime) - strtotime($zxsj)) < 3 * 24 * 3600) {
                continue;
            }
            $zxsjEnd = Carbon::parse($zxsj)->addHours(72)->toDateTimeString();
            $cjsjEnd = Carbon::parse($cjsj)->addHours(72)->toDateTimeString();

            $exists = EMR_BL_BL01::query()
                ->where('JZHM', $ZYH)
                ->where('BLLB', 294)
                ->where('ZXSJ', '>=', $zxsj)
                ->where('ZXSJ', '<=', $zxsjEnd)
                ->where('CJSJ', '>=', $cjsj)
                ->where('CJSJ', '<=', $cjsjEnd)
                ->where('BLBH', '<>', $current['BLBH'])
                ->exists();

            if (!$exists) {
                $basis = [];
                $basis[] = "【{$current['BLMC']}】标题时间【{$zxsj}】完成时间【{$cjsj}】之后72小时内不存在符合病程";
                $allBasisGroups[] = $basis;
            }
        }
    ...
    public function rule1002($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($ssapData as $surgery) {
            ...
            if (empty($records)) {
                // 使用ZXSJ回退查找
                $zxsjRecords = EMR_BL_BL01::query()
                    ->where('JZHM', $ZYH)
                    ->where('BLLB', 294)
                    ->where('ZXSJ', '>=', $surgeryEndTime)
                    ->where('ZXSJ', '<=', $sixHoursAfterSurgery)
                    ->orderBy('ZXSJ', 'desc')
                    ->get()
                    ->toArray();

                if (empty($zxsjRecords)) {
                    // ZXSJ也没找到，再尝试通过BLMC查找
                    $zxsjRecords = EMR_BL_BL01::query()
                        ->where('JZHM', $ZYH)
                        ->where('BLLB', $ruleMap8010)
                        ->where('BLMC', 'like', '%' . $ruleMap8018 . '%')
                ...
                if (empty($zxsjRecords)) {
                    ...
                    // 通过ZXSJ找到了，检查创建时间与标题时间的关系
                    $zxsjrecord = $zxsjRecords[0];
                    $cjsj = $zxsjrecord['CJSJ'] ?? '';
                    if (!empty($cjsj) && !empty($zxsjrecord['ZXSJ'])) {
                        if (strtotime($cjsj) < strtotime($zxsjrecord['ZXSJ'])) {
                            $basis = [];
            ...
            if (!$hasValidRecord && !empty($invalidRecords)) {
                ...
                if (!empty($zxsjRecords)) {
                    ...
                    if (!empty($cjsj) && !empty($zxsjrecord['ZXSJ'])) {
                        ...
                        // ZXSJ找到了但CJSJ或ZXSJ为空，使用原始invalid逻辑
                        foreach ($invalidRecords as $record) {
                            $basis = [];
                            $basis[] = "手术名称【" . $surgeryName . "】";
                            $basis[] = "(手麻)手术结束时间【" . $surgeryEndTime . "】";
     ...
     * @datetime 2025-05-10
     */
    public function rule1003($caseRule, $ruleId, $ZYH, $isSz = 0)
    {
    ...
    public function rule1004($caseRule, $ruleId, $ZYH)
    {
        ...
        for ($i = 0; $i < count($ssapData); $i++) {
            ...
            foreach ($records as $record) {
                ...
                $hjnr = EMR_BL_BLXG::query()->where('BLBH', $record['BLBH'])->value('HJNR');
                if (!empty($hjnr) && strpos($hjnr, '手术者术前查看患者相关情况') !== false) {
                    $issign = true;
                }
                ...
                if ($issign) {
                    ...
                    if (!empty($signTime)) {
                        if (strtotime($signTime) <= strtotime($surgeryStartTime)) {
            ...
            if (!$hasSurgeonSignature) {
                ...
                if (!empty($untimelySignedRecords)) {
                    foreach ($untimelySignedRecords as $record) {
                        ...
                        $problemRecords[] = "创建时间【" . $record[$ruleMap8002] . "（超时）】";
                    }
                }
    ...
    public function rule1005($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($ssapData as $surgery) {
            ...
            // 计算查房时间范围
            $endCheckTime = date('Y-m-d 23:59:59', strtotime($surgeryEndTime) + 24 * 3600);
    ...
    public function rule1060($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($ssapData as $surgery) {
            ...
            foreach ($records as $record) {
                ...
                //查看是否有术者签名
                $signatureData = false;
                Log::info('$ZYH', ['$ZYH' => $ZYH, '$blmc' => $blmc, '$surgeon1' => $surgeon1]);
                if (empty($blmc) || empty($surgeon1) || strpos($blmc, $surgeon1) !== false) {
                    $signatureData = true;
                }
    ...
    public function rule1008($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($ssapData as $surgery) {
            ...
            //如果第三天出院也不质控
            if (!empty($dischargeDay) && ($thirdDay == $dischargeDay)) {
                continue; // 第三天出院，跳过质控
            }
    ...
    public function rule1051($caseRule, $ruleId, $ZYH)
    {
        ...
        if (empty($bl01Data)) {
            $bl01Data = EMR_BL_BL01::query()
                ->where('JZHM', $ZYH)
                ->where('BLLB', '294')
                ->where('ZXSJ', '>=', $exitTimeStart)
                ->where('ZXSJ', '<=', $exitTimeEnd)
                ->orderBy('ZXSJ', 'desc')
                ->get()
                ->toArray();
            ...
            if (empty($bl01Data)) {
                ...
                foreach ($bl01Data as $record) {
                    ...
                    foreach ($highLevelTitles as $title) {
                        if (strpos($blmc, $title) !== false || strpos($hjnr, $title) !== false) {
                            Log::info('hasHighLevelDoctor1--------su');
                            $hasHighLevelDoctor1 = true;
                            $zxsjrecord = $record;
                            break;
                        }
    ...
    public function rule1010($caseRule, $ruleId, $ZYH)
    {
        ...
        $records = EMR_BL_BL01::query()
            ...
            ->where($ruleMap8011, '<=', $deadlineTime)
            ->get()
            ->toArray();
    ...
    public function rule1013($caseRule, $ruleId, $ZYH)
    {
        ...
        for ($cycleIndex = 0; $cycleIndex < $cycleTotalCount; $cycleIndex++) {
            ...
            $yzKeywords = array_values(array_filter($yzKeywords, function ($keyword) {
                return trim((string)$keyword) !== '';
            }));
    ...
    public function rule1014($caseRule, $ruleId, $ZYH)
    {
        ...
        $zcKeywords = array_values(array_filter($zcKeywords, function ($keyword) {
            return trim((string)$keyword) !== '';
        }));
        ...
        foreach ($yzbData[0] as $order) {
            ...
            $excludeKeywords = array_values(array_filter($excludeKeywords, function ($keyword) {
                return trim((string)$keyword) !== '';
            }));
    ...
    public function rule1045($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($ssjl as $surgery) {
            $ssmc = $surgery['SSMC'] ?? '';
            //示例 右侧腹股无张力手术或者右侧腹股无张力术，去除手术名称中的“手术”或者“术”
            $ssmc = preg_replace('/手术|术/', '', $ssmc);
            if (empty($ssmc)) {
                continue;
            }
    ...
    public function rule1064($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $rule8011 = RuleWordMap::query()->where('id', 8011)->value('keyword');
        $rule8011 = !empty($rule8011) ? $rule8011 : 'ZXSJ';
        // 查询符合条件的病程记录：BLLB=294且BLMC含"操作记录"
        $blmcKeywords = [];
        if (!empty($rule8081)) {
            foreach ($rule8081 as $item) {
                $blmcKeywords[] = $item;
            }
        }
        $blmcKeywords = array_values(array_filter($blmcKeywords, function ($keyword) {
            return trim((string)$keyword) !== '';
        }));

        if (empty($blmcKeywords)) {
            $resData = [[], 0];
        } else {
            $resDataList = EMR_BL_BL01::query()
                ->leftJoin('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
                ...
                ->where(function ($query) use ($blmcKeywords) {
                    foreach ($blmcKeywords as $keyword) {
    ...
    public function rule1224($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $blmcKeywords = array_values(array_filter($rule8091, function ($keyword) {
            return trim((string)$keyword) !== '';
        }));
        if (empty($blmcKeywords)) {
            return true;
    ...
    public function rule1251($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $blmcKeywords = array_values(array_filter($rule8081, function ($keyword) {
            return trim((string)$keyword) !== '';
        }));
        if (empty($blmcKeywords)) {
            return true;
