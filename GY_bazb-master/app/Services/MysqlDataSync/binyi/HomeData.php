...
class HomeData
{
    ...
    public static $sqlsrvService1;

    public function getSqlsrvConnect1()
    {
        if (self::$sqlsrvService1) {
            return;
        }
        ...
        self::$sqlsrvService1 = $sqlsrvService;
    }
    public function getSqlsrvConnect()
    {
    ...
    public function getBlData($zyh = "", $type = [], $blbh = "", $startTime = "")
    {
        ...
        $patientData = $this->getPatientInfo($zyh);
        ...
        if (!empty($diagnosisData)) {
            $this->addDiagnosis($diagnosisData);
        }
        ...
        if (!empty($buChongData)) {
            $this->addBuChong($zyh, $buChongData);
        }
     ...
     * @return array
     */
    public function getPatientInfo($ZYH = "", $startTime = "", $day = 1)
    {
    ...
    public function addPatientInfo($data = [])
    {
        ...
        $patient_info = [
            ...
            'CYPB' => $data['CYPB'] ?? 0,                          //0：在院病人 1：出院证明 2：预结出院 8：正常出院 9：终结出院 99 注销出院
            'patient_name' => $data['AAA01'] ?? '',        //患者姓名
            'id_card' => $data['AAA07'] ?? '',        //身份证号
            'IS_CATA' => 1,
        ];
        PatientInfo::query()->updateOrInsert(['MED_REC_ID' => $data['MED_REC_ID']], $patient_info);
        //        if(!$res) Log::error("主表信息插入更新失败::{$data['MED_REC_ID']}",$patient_info);
    ...
    public function getSssqData($ZYH)
    {
        ...
        $data = oci_parse(self::$conHis, $sql);
        oci_execute($data, OCI_DEFAULT);
        $result = [];
        while ($row = oci_fetch_assoc($data)) {
            $result[] = $row;
    ...
    public function getFyData($ZYH)
    {
        ...
        $sql = "SELECT A.*,to_char(JFRQ,'yyyy-mm-dd hh24:mi:ss') as JFRQ,to_char(DSG_LDR_TIME,'yyyy-mm-dd hh24:mi:ss') as DSG_LDR_TIME FROM PORTAL_HIS.V_JMGS_BASY_FYMX A WHERE ZYH=" . $ZYH;
        $data = oci_parse(self::$con, $sql);
        oci_execute($data, OCI_DEFAULT);
        $result = [];
        while ($row = oci_fetch_assoc($data)) {
        ...
        foreach ($result as $val) {
            ...
            FeeDetailed::query()->updateOrInsert(['AAA28' => $ZYH, 'JLXH' => $item['JLXH']], $item);
        }
    }
    ...
    public function getOperationData($ZYH)
    {
        ...
        //修改后sql
        $sql = "SELECT A.*,to_char(SSKSSJ,'yyyy-mm-dd hh24:mi:ss') as SSKSSJ,to_char(SSJSSJ,'yyyy-mm-dd hh24:mi:ss') as SSJSSJ,to_char(SSCZRQ,'yyyy-mm-dd hh24:mi:ss') as SSCZRQ FROM PORTAL_HIS.V_JMGS_BASY_SS A WHERE A.ZYH=" . $ZYH;
        $data = oci_parse(self::$con, $sql);
        oci_execute($data, OCI_DEFAULT);
        $result = [];
        while ($row = oci_fetch_assoc($data)) {
    ...
    public function addOperation($data = [])
    {
        ...
        foreach ($data as $val) {
            $addData = [
                ...
                'OPE_DATE' => $val['SSCZRQ'] ?? '', //手术或操作日期  ---oracle没有字段SSCZRQ,更改为字段SSCZRI
    ...
    {
        //        $sql = "SELECT A.*,to_char(TZSJ,'yyyy-mm-dd hh24:mi:ss') as TJ,to_char(XZJDSJ,'yyyy-mm-dd hh24:mi:ss') as XJ,to_char(TZQRSJ,'yyyy-mm-dd hh24:mi:ss') as TZJ,to_char(APSJ,'yyyy-mm-dd hh24:mi:ss') as AJ,to_char(KZSJ,'yyyy-mm-dd hh24:mi:ss') as KJ,to_char(ZXSJ,'yyyy-mm-dd hh24:mi:ss') as ZJ FROM PORTAL_HIS.BTF_EMR_YZB A WHERE ZYH=" . $ZYH;
        //修改后sql
        $sql = "SELECT A.*,to_char(TZSJ,'yyyy-mm-dd hh24:mi:ss') as TJ,to_char(XZJDSJ,'yyyy-mm-dd hh24:mi:ss') as XJ,to_char(TZQRSJ,'yyyy-mm-dd hh24:mi:ss') as TZJ,to_char(APSJ,'yyyy-mm-dd hh24:mi:ss') as AJ,to_char(KZSJ,'yyyy-mm-dd hh24:mi:ss') as KJ FROM PORTAL_HIS.EMR_YZB A WHERE ZYH=" . $ZYH;
        $data = oci_parse(self::$con, $sql);
        oci_execute($data, OCI_DEFAULT);
        $result = [];
        while ($row = oci_fetch_assoc($data)) {
    ...
    public function addYzb($data)
    {
        ...
        foreach ($data as $val) {
            ...
            Yzb::query()->updateOrInsert(['ZYH' => $insertData['ZYH'], 'YZBXH' => $insertData['YZBXH']], $insertData);
        }
    }
    ...
    public function addBLSY($zyh = "", $blbh3 = "")
    {
        ...
        foreach ($data as $item) {
            $result = [
                ...
                'SBBZ' => $item['SBBZ'] ?? '',
                'WZZT' => $item['WZZT'] ?? '',
                //'HTML_PRINT' => $this->blobToStr($item['BLNR1']) ?? '',
                'bl_type' => 0
            ];
            $str = $this->blobToStr($item['BLNR']);
            if (empty($str)) {
                continue;
            }
            // 从 $item['BLMC'] 中提取时间，兼容多种时间格式
            $blmc = $item['BLMC'] ?? '';
            $first_blsy_time = null;
            if (!empty($blmc)) {
                // 常见时间格式正则：2023-06-01 12:30、2023/06/01 12:30、2023.06.01 12:30、2023年06月01日 12:30、2023-6-1 1:2、2023-06-01、06-01 12:30
                $patterns = [
                    '/(\d{4}年\d{1,2}月\d{1,2}日\s*\d{1,2}:\d{1,2})/', // 2023年06月01日 12:30
                    '/(\d{4}[-\/\.年]\d{1,2}[-\/\.月]\d{1,2}[日]?\s*\d{1,2}:\d{1,2})/', // 2023-06-01 12:30、2023/06/01 12:30、2023.06.01 12:30、2023年06月01日 12:30
                    '/(\d{4}[-\/\.年]\d{1,2}[-\/\.月]\d{1,2}[日]?)/', // 2023-06-01、2023/06/01、2023.06.01、2023年06月01日
                    '/(\d{1,2}[-\/\.月]\d{1,2}[日]?\s*\d{1,2}:\d{1,2})/', // 06-01 12:30、06/01 12:30、06.01 12:30、06月01日 12:30
                    '/(\d{1,2}[-\/\.月]\d{1,2}[日]?)/', // 06-01、06/01、06.01、06月01日
                ];
                foreach ($patterns as $pattern) {
                    if (preg_match($pattern, $blmc, $matches)) {
                        $first_blsy_time = $matches[1];
                        $first_blsy_time = str_replace(['年', '月', '日'], ['-', '-', ''], $first_blsy_time);
                        break;
                    }
                }
            }

            // 如果手术记录没有时间，则从手术记录中提取时间
            if (empty($first_blsy_time) && $item['BLLB'] == 303) {
                $hjnr = $str;
                $hjnr = str_replace([' ', "\r", "\t", "\n", "\r\n", ' '], '', $hjnr);
                if (preg_match('/记录时间(.*?)滨州医学院烟台附属医院/', $hjnr, $matches)) {
                    $first_blsy_time = $matches[1];
                    $first_blsy_time = substr($first_blsy_time, 0, 10) . ' ' . substr($first_blsy_time, 10);
                    $first_blsy_time = str_replace(['年', '月', '日'], ['-', '-', ''], $first_blsy_time);
                }
            }
            $result['first_blsy_time'] = !empty($first_blsy_time) ? date("Y-m-d H:i:s", strtotime($first_blsy_time)) : "";
            if (
        ...
        $sql = "select JLXH,BLBH,SYYS,to_char(SYSJ,'yyyy-mm-dd hh24:mi:ss') as SYSJ,to_char(JLSJ,'yyyy-mm-dd hh24:mi:ss') as JLSJ,QMLX,QMYS from PORTAL55_EMR.EMR_BL_BLSY WHERE BLBH in ($blbhStr) AND DSG_OPERATION <> 'D'";
        $result = oci_parse(self::$con, $sql);
        oci_execute($result, OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
    ...
    public function ZY_HCMX($zyh)
    {
        ...
        $result = oci_parse(self::$conHis, $sql);
        oci_execute($result, OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
            $data[] = $row;
    ...
    public function YS_ZY_HZYJ($zyh)
    {
        ...
        foreach ($data as $item) {
            ...
            $SQXH[] = $inData['SQXH'];
            YS_ZY_HZSQ::query()->updateOrInsert(['SQXH' => $item['SQXH'],], $inData);
        }
    ...
    public function YJ_ZY01($zyh = null)
    {
        ...
        //        $sql = "SELECT YJXH,TJHM,ZYH,ZYHM,BRXM,to_char(KDRQ,'yyyy-mm-dd hh24:mi:ss') as KDRQ,KSDM,YSDM,to_char(ZXRQ,'yyyy-mm-dd hh24:mi:ss') as ZXRQ,ZXKS,ZXPB,HJGH,BBBM,ZYSX,ZFPB,HYMX,YJPH,SQDH,BWID,JBID,DJZT,SQWH,FYBQ,SQID,YQDH,JGID,SSYS,SSYZ,SSEZ,SSSZ,JZBZ FROM PORTAL_HIS.BTF_YJ_ZY01 WHERE ZYH={$zyh}";
        //修改后sql
        $sql = "SELECT YJXH,TJHM,ZYH,ZYHM,BRXM,to_char(KDRQ,'yyyy-mm-dd hh24:mi:ss') as KDRQ,KSDM,YSDM,to_char(ZXRQ,'yyyy-mm-dd hh24:mi:ss') as ZXRQ,ZXKS,ZXPB,HJGH,BBBM,ZYSX,ZFPB,HYMX,YJPH,SQDH,BWID,JBID,DJZT,SQWH,FYBQ,SQID,JGID FROM PORTAL_HIS.BTF_YJ_ZY01 WHERE ZYH={$zyh}";
        $result = oci_parse(self::$conHis, $sql);
        oci_execute($result, OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
        ...
        //        $sql = "SELECT SBXH, YJXH, YLXH, XMLX, YJZX, YLDJ, YLSL, FYGB, ZFBL, YZXH, TPLJ, YEPB, TMDY_LQ, TMH, JGID, ZTMC, JHH, XDH, JHSJ, JFID FROM PORTAL_HIS.BTF_YJ_ZY02 WHERE YJXH in ($YJXHstr) and DSG_OPERATION <> 'D'";
        //修改后sql
        $sql = "SELECT SBXH, YJXH, YLXH, XMLX, YJZX, YLDJ, YLSL, FYGB, ZFBL, YZXH, TPLJ, YEPB FROM PORTAL_HIS.BTF_YJ_ZY02 WHERE YJXH in ($YJXHstr)";
        $result = oci_parse(self::$con, $sql);
        oci_execute($result, OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
    ...
    public function PACS($startTime = "")
    {
        if (empty($startTime)) {
            $startTime = date('Y-m-d 00:00:00', strtotime('-1 day'));
        }
        $this->getSqlsrvConnect();

        $startTime = strtotime($startTime);
        while (true) {
            if ($startTime + 86400 > time()) {
                break;
            }
            $endTime = date('Y-m-d H:i:s', $startTime + 86400);
            $startTime = date('Y-m-d H:i:s', $startTime);
            echo $startTime . ' - ' . $endTime . PHP_EOL;

            // 使用直接查询获取数据
            $sql = "SELECT StudyUid, YLJGDM, JZLX, JZLSH, MZZYBZ, KH, KLX, CASE WHEN LEN(BRXM) = 2 THEN SUBSTRING ( BRXM, 1, 1 ) + '*' + SUBSTRING ( BRXM, 3 ,2) WHEN LEN( BRXM ) >= 3 THEN SUBSTRING ( BRXM, 1, 1 ) + '*' + SUBSTRING ( BRXM, 3 ,2) ELSE BRXM END as BRXM, BRXB, PatientID, PatientID1, JCXMDM, JCXMDMYB, SQDH, KDSJ, JYSJ, ExamType, SBBM, YQBM, SQKS, SQKSMC, SQRGH, SQRXM, JCBGJGMC, JCKS, JCKSMC, JCYSGH, JCYS, BGSJ, BGRQ, BGRGH, BGRXM, SHRGH, SHRXM, JCBW, BWACR, JCMC, ZYJCXX1, ZYJCXX2, ZYJCXX3, YXBX, YXZD, BZHJY, SFYYY, XGBZ, MJ, YLYL1, YLYL2 FROM dbo.TB_RIS_Report where KDSJ>'{$startTime}' and KDSJ<'{$endTime}'";
            $rows = self::$sqlsrvService->query($sql);
            // 打印数据行
            foreach ($rows as $row) {
                echo $row['StudyUid'] . PHP_EOL;

                // 清洗ZYH
                $zyh = "";
                if ($row['JZLX'] == 400) {
                    $kdsj = strtotime($row['KDSJ']);
                    $patientInfo = PatientInfo::query()->where('AAA28', '=', $row['PatientID'])->get(['AAB01', 'AAC01', 'MED_REC_ID'])->toArray();
                    foreach ($patientInfo as $key => $value) {
                        $start = strtotime($value['AAB01']);
                        $end = strtotime($value['AAC01']);
                        if ($start < $kdsj && $end > $kdsj) {
                ...
                $insert = [
                    ...
                    "PatientID1"      => $row['PatientID1'] ?? '',
                    "JCXMDM"      => $row['JCXMDM'] ?? '',
                    "JCXMDMYB"      => $row['JCXMDMYB'] ?? '',
                    "SQDH"      => $row['SQDH'] ?? '',
                    "KDSJ"      => $row['KDSJ'] ?? '',
                    "JYSJ"      => $row['JYSJ'] ?? '',
                    ...
                    "YLYL2"      => $row['YLYL2'] ?? '',
                    "csrq"      => $row['csrq'] ?? '',
                    "F_PDF_PATH"      => $row['F_PDF_PATH'] ?? '',
                    "F_RPT_JPG_PATH"      => $row['F_RPT_JPG_PATH'] ?? '',
                ];
                \App\Model\PACS::query()->updateOrInsert(["StudyUid" => $insert['StudyUid']], $insert);
            }
    ...
    public function PACS1($startTime = "")
    {
        if (empty($startTime)) {
            $startTime = date('Y-m-d 00:00:00', strtotime('-1 day'));
        }
        $this->getSqlsrvConnect1();
        $startTime = strtotime($startTime);
        ...
        while (true) {
            ...
            $offset = 0;
            while (true) {
                $sql = "SELECT StudyUid, YLJGDM, JZLX, JZLSH, MZZYBZ, KH, KLX, CASE WHEN LEN(BRXM) = 2 THEN SUBSTRING ( BRXM, 1, 1 ) + '*' + SUBSTRING ( BRXM, 3 ,2) WHEN LEN( BRXM ) >= 3 THEN SUBSTRING ( BRXM, 1, 1 ) + '*' + SUBSTRING ( BRXM, 3 ,2) ELSE BRXM END as BRXM, BRXB, PatientID, PatientID1, JCXMDM, JCXMDMYB, SQDH, KDSJ, JYSJ, ExamType, SBBM, YQBM, SQKS, SQKSMC, SQRGH, SQRXM, JCBGJGMC, JCKS, JCKSMC, JCYSGH, JCYS, BGSJ, BGRQ, BGRGH, BGRXM, SHRGH, SHRXM, JCBW, BWACR, JCMC, ZYJCXX1, ZYJCXX2, ZYJCXX3, YXBX, YXZD, BZHJY, SFYYY, XGBZ, MJ, YLYL1, YLYL2 FROM dbo.TB_RIS_Report where KDSJ>'{$startTime}' and KDSJ<'{$endTime}' ORDER BY KDSJ OFFSET {$offset} ROWS FETCH NEXT 10 ROWS ONLY";
                $rows = self::$sqlsrvService1->query($sql);
                ...
                if (!is_array($rows)) {
                    ...
                    // 使用正则把2025-01-01 07:32:48.513替换成"2025-01-01 07:32:48.513"
                    $rows = preg_replace('/(\d{4}-\d{1,2}-\d{1,2}\s*\d{1,2}:\d{1,2}:\d{1,2}\.\d{1,3})/', '"$1"', $rows);
                ...
                $rows = $rows["data"]; // 获取第一条（也是唯一一条）记录
                foreach ($rows as $row) {
                    echo $row['StudyUid'] . PHP_EOL;
    ...
    function cleanPacsZYH($time = 0)
    {
        $id = 0;
        while (true) {
            $rows = PACS::query()->where('id', '>', $id)->orderBy('id', 'asc')->where('ZYH', '')->where('JZLX', 400)->limit(100)->get(['id', 'JZLX', 'KDSJ', 'PatientID'])->toArray();
            if (empty($rows)) {
                break;
            }
            foreach ($rows as $row) {
                ...
                // 清洗ZYH
                $zyh = "";
                if ($row['JZLX'] == 400 && empty($row['ZYH'])) {
                    echo $row['id'] . PHP_EOL;
                    $kdsj = strtotime($row['KDSJ']);
                    $patientInfo = PatientInfo::query()->where('AAA28', '=', $row['PatientID'])->get(['AAB01', 'AAC01', 'MED_REC_ID'])->toArray();
                    foreach ($patientInfo as $key => $value) {
                        $start = strtotime($value['AAB01']);
                        $end = !empty($value['AAC01']) ? strtotime($value['AAC01']) : time();
                        if ($start < $kdsj && $end > $kdsj) {
                            $zyh = $value['MED_REC_ID'];
                            break;
                        }
                    }
                    PACS::query()->where('id', $row['id'])->update(['ZYH' => $zyh]);
                }
            }
