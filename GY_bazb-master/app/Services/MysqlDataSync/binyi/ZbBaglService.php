...
class ZbBaglService
{
    ...
    public function zqtys($zyh, $start, $end)
    {
        ...
        $cyzs = ZbBagl::getFirstById(1, true); //出院总数 分母
        $surgeryConsentKeywords = $this->getRuleWordKeywordsById(
            20052,
            ['滨州医学院烟台附属医院手术同意书']
        );
        $transfusionConsentKeywords = $this->getRuleWordKeywordsById(
            20053,
            ['输血同意书', '输血治疗知情同意书']
        );
        ...
        while (true) {
            ...
            foreach ($data->items() as $value) {
                ...
                $consentForms = EMR_BL_BL01::query()
                    ->where('JZHM', $ZYH)
                    ->whereIn('BLLB', [303, 329])
                    ->where('BLZT', '!=', '9')
                    ->get()
                    ->toArray();

                $surgeryConsent = $this->getZqtysConsentForm($consentForms, 'surgery', $surgeryConsentKeywords);
                $transfusionConsent = $this->getZqtysConsentForm($consentForms, 'transfusion', $transfusionConsentKeywords);
                if (empty($surgeryConsent) && empty($transfusionConsent)) {
                    continue;
                }
                ...
                if (!empty($surgeryConsent)) {
                    $surgeryResult = $this->checkZqtysConsentSignTime($surgeryConsent, '手术知情同意书');
                    $errorContent[] = $surgeryResult;
                    if (empty($surgeryResult['status'])) {
                        $allConsentOk = false;
                    }
                }

                if (!empty($transfusionConsent)) {
                    $transfusionResult = $this->checkZqtysConsentSignTime($transfusionConsent, '输血治疗同意书');
                    $errorContent[] = $transfusionResult;
                    if (empty($transfusionResult['status'])) {
                        $allConsentOk = false;
                    }
                }
    ...
    /**
     * 读取 rule_word_map 关键词，未配置时使用默认值。
     */
    private function getRuleWordKeywordsById($id, $default = [])
    {
     ...
     * 按新口径筛选手术/输血知情同意书。
     */
    private function getZqtysConsentForm($records, $type, $keywords)
    {
        foreach ($records as $record) {
            $bllb = intval($record['BLLB'] ?? 0);
            $blmc = $record['BLMC'] ?? '';
            $blbh = $record['BLBH'] ?? '';

            if ($type === 'surgery' && $bllb !== 303) {
                continue;
            }
            if ($type === 'transfusion' && $bllb !== 329) {
                continue;
            }

            if ($type === 'surgery') {
                $record['HJNR'] = $this->getZqtysHjnrByBlbh($blbh);
            }

            $haystack = $type === 'surgery' ? ($record['HJNR'] ?? '') : $blmc;
            if ($this->zqtysContainsAnyKeyword($haystack, $keywords)) {
                return $record;
            }
        }
    ...
    private function zqtysContainsAnyKeyword($content, $keywords)
    {
        $content = $this->normalizeZqtysText($content);
        foreach ($keywords as $keyword) {
            if ($keyword !== '' && strpos($content, $this->normalizeZqtysText($keyword)) !== false) {
                return true;
            }
        }
    ...
    private function getZqtysHjnrByBlbh($blbh)
    {
        if (empty($blbh)) {
            return '';
        }

        return EMR_BL_BLXG::query()->where('BLBH', $blbh)->value('HJNR') ?? '';
    }

    /**
     * 校验医生签名时间必须早于等于患者签名时间，且签名表中存在对应角色签名。
     */
    private function checkZqtysConsentSignTime($record, $title)
    {
        $blbh = $record['BLBH'] ?? '';
        $blmc = $record['BLMC'] ?? '';
        $hjnr = !empty($record['HJNR']) ? $record['HJNR'] : $this->getZqtysHjnrByBlbh($blbh);
        $doctorTime = $this->extractZqtysSignTime($hjnr, 'doctor');
        $patientTime = $this->extractZqtysSignTime($hjnr, 'patient');
        $hasDoctorSign = $this->hasZqtysBlsySign($blbh, 5);
        $hasPatientSign = $this->hasZqtysBlsySign($blbh, 3);

        $group = [
            'status' => 0,
            'content' => []
        ];
        $group['content'][] = ['status' => 1, 'content' => $title . '【' . ($blmc ?: $blbh) . '】'];
        $group['content'][] = ['status' => $hasDoctorSign ? 1 : 0, 'content' => '医生签名【' . ($hasDoctorSign ? '有' : '无') . '】'];
        $group['content'][] = ['status' => $hasPatientSign ? 1 : 0, 'content' => '患者签名【' . ($hasPatientSign ? '有' : '无') . '】'];
        $group['content'][] = ['status' => !empty($doctorTime) ? 1 : 0, 'content' => '医生签名时间【' . ($doctorTime ?: '无') . '】'];
        $group['content'][] = ['status' => !empty($patientTime) ? 1 : 0, 'content' => '患者签名时间【' . ($patientTime ?: '无') . '】'];

        if ($hasDoctorSign && $hasPatientSign && !empty($doctorTime) && !empty($patientTime)) {
            if (strtotime($patientTime) < strtotime($doctorTime)) {
                $group['content'][] = ['status' => 0, 'content' => '患者签名时间早于医生签名时间'];
            } else {
                $group['status'] = 1;
                //$group['content'][] = ['status' => 1, 'content' => '患者签名时间晚于医生签名时间'];
            }
        }
    ...
    private function hasZqtysBlsySign($blbh, $qmmc)
    {
        if (empty($blbh)) {
            return false;
        }
     ...
     * 从同意书全文中提取医生或患者签名时间。
     */
    private function extractZqtysSignTime($hjnr, $type)
    {
        $text = $this->normalizeZqtysText($hjnr);
        if (empty($text)) {
            return '';
        }

        $signatureTimePattern = '[0-9]{4}[年\-\/][0-9]{1,2}[月\-\/][0-9]{1,2}[日]?\s*[0-9]{1,2}(?:[:：时][0-9]{1,2})?(?:分)?';

        if ($type === 'doctor') {
            $pattern = '/(?:经治医师签名|手术医师签名|医师签名).*?(?:签署时间|签字时间)[：:]?(' . $signatureTimePattern . ')/u';
        } else {
            $patientSignatureLabels = [
                '患者签名',
                '患方明确意见',
                '患者的监护人/委托代理人/近亲属签名',
            ];
            $patientSignaturePattern = implode('|', array_map(function ($patientSignatureLabel) {
                return preg_quote($patientSignatureLabel, '/');
            }, $patientSignatureLabels));
            $pattern = '/(?:' . $patientSignaturePattern . ').*?(?:签署时间|签字时间)[：:]?(' . $signatureTimePattern . ')/u';
        }

        if (preg_match($pattern, $text, $matches)) {
            return $this->formatZqtysSignTime($matches[1]);
        }

        return '';
    }

    private function normalizeZqtysText($text)
    {
        return str_replace([' ', "\r", "\t", "\n", "\r\n", '　', ' '], '', $text ?? '');
    }

    private function formatZqtysSignTime($time)
    {
        $time = str_replace(['年', '月', '日', '时', '分', '/', '：'], ['-', '-', ' ', ':', '', '-', ':'], $time);
        $time = trim($time);
        if (preg_match('/^(\d{4}-\d{1,2}-\d{1,2})\s*(\d{1,2}:\d{1,2})$/', $time, $matches)) {
