...
class BanhzkgzService
{
    ...
    {
        $discussionRecord = EMR_BL_BL01::query()
            ->where('MBLB', 82)
            ->where("JZHM", $ZYH)
            ->orderBy('ZXSJ', 'asc')
            ->first(["BLBH", "BLMC"]);
        ...
        $hjnr = (string)EMR_BL_BLXG::query()->where('BLBH', $discussionRecord['BLBH'] ?? '')->value('HJNR');
        if ($hjnr === '') {
            return [];
        }
        ...
        $discussionInfo = [
            'time' => $discussionTime,
            'timestamp' => strtotime($discussionTime),
            'blmc' => $discussionRecord['BLMC'] ?? '',
        ];

        if (empty($discussionInfo['timestamp'])) {
            return [];
        }
        ...
        foreach ($yzb as $yzb) {
            $yzb_name = $yzb['YZMC']; // 假设医嘱名称在name字段中
        ...
        $orderTime = $this->normalizeConsentComparableTime($firstYzb['KZSJ'] ?? '');
        if ($orderTime === '') {
            return [];
        }
        ...
        foreach ($consentForms as $form) {
            ...
            if ($doctorTime !== '') {
                ...
                if ($doctorTimestamp <= $discussionInfo['timestamp']) {
                    $basis[] = '医师签字时间【' . $doctorTime . '】未在术前讨论时间之后';
                    $hasViolation = true;
                }

                if ($orderTimestamp <= $doctorTimestamp) {
                    $basis[] = '手术医嘱开具时间【' . $orderTime . '】未在医师签字时间【' . $doctorTime . '】之后';
                    $hasViolation = true;
                }
            }

            if (!empty($patientSignInfo)) {
                if ($patientSignInfo['timestamp'] <= $discussionInfo['timestamp']) {
                    $basis[] = $patientSignInfo['label'] . '【' . $patientSignInfo['time'] . '】未在术前讨论时间之后';
                    $hasViolation = true;
                }

                if ($orderTimestamp <= $patientSignInfo['timestamp']) {
                    $basis[] = '手术医嘱开具时间【' . $orderTime . '】未在' . $patientSignInfo['label'] . '【' . $patientSignInfo['time'] . '】之后';
                    $hasViolation = true;
                }
            }
    ...
    public function rule10316($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        foreach ($surgeryPlans as $plan) {
            ...
            foreach ($consentForms as $form) {
                ...
                if (
                    strtotime($doctorTime) <= $surgeryStartTimestamp &&
                    $patientSignInfo['timestamp'] <= $surgeryStartTimestamp
                ) {
                    $hasTimelyConsent = true;
                    break;
                }

                if (strtotime($doctorTime) > $surgeryStartTimestamp) {
                    $formBasis[] = '医师签字时间【' . $doctorTime . '】晚于手术开始时间';
                }

                if ($patientSignInfo['timestamp'] > $surgeryStartTimestamp) {
    ...
    public function rule10312($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        foreach ($records as $record) {
            ...
            if ($plannedSurgeryTimestamp < $discussionTimestamp) {
                ...
                $basis[] = '拟手术时间【' . $plannedSurgeryTime . '】，早于术前讨论时间【' . $discussionTime . '】';
            }
    ...
    public function rule1064($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        if (empty($resData[0]) || empty($resData[0][0]) || empty($resData[0][0]['BLBH'])) {
            return true;
        }
    ...
    public function rule1402($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        ...
        $tszlConsentForms = $this->getTszlConsentForms($ZYH);
        $consentForms = !empty($tszlConsentForms) ? $tszlConsentForms : $this->getSszqtysConsentForms($ZYH);
        $basisList = [];
