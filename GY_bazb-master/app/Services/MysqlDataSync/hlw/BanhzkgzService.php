...
class BanhzkgzService
{
    ...
    protected $consentPreQualitySkipRuleIds = [
        1306, 1307, 1308, 1313, 1314, 1317, 1318, 1319, 1322, 1323,
        1326, 1327, 1328, 1330, 1331, 1335, 1336, 1338, 1340, 1341,
        1346, 1347, 1348, 1350, 1351, 1355, 1356, 1358, 1359, 1360,
        1361, 1362, 1363, 1365, 1366, 1370, 1371, 1372, 1373, 1374,
        1378, 1379, 1380, 1381, 1382, 1386, 1387, 1388, 1389, 1391,
        1392, 1393, 1396, 1397, 1414, 1415, 1417, 1420, 1421,
    ];

    protected $consentPreQualitySkipCache = [];

    protected $consentConsciousnessKeywordMapId = 20076;
     ...
     * 校验病程是否已签名。
     *
     * @param mixed $BLBH 病历编号
     * @return bool
     */
    private function isMedicalRecordSigned($BLBH)
    {
        if (empty($BLBH)) {
            return false;
        }

        return EMR_BL_BLSY::query()
            ->where('BLBH', '=', $BLBH)
            ->exists();
    }
     ...
     * @return bool
     */
    protected function shouldSkipConsentPreQualityRule($ruleId = 0, $ZYH = '')
    {
        if (!in_array((int)$ruleId, $this->consentPreQualitySkipRuleIds, true)) {
            return false;
        }

        if (isset($this->consentPreQualitySkipCache[$ZYH]['underage'])) {
            return $this->consentPreQualitySkipCache[$ZYH]['underage'];
        }

        $this->consentPreQualitySkipCache[$ZYH]['underage'] = $this->isBllb292PatientUnderage($ZYH);
     ...
     * 过滤满足前置不质控条件的同意书。
     *
     * @param array $forms 同意书列表
     * @param string $ZYH 住院号
     * @return array
     */
    protected function filterConsentFormsByPreQualityConditions($forms = [], $ZYH = '')
    {
        if (empty($forms)) {
            return [];
        }
        ...
        foreach ($forms as $form) {
            if ($this->shouldSkipConsentFormByPreQualityConditions($form, $ZYH)) {
                continue;
            }
    ...
    /**
     * 单份同意书满足打印、意识异常关键词任一条件时不质控。
     *
     * @param array $form 同意书数据
     * @param string $ZYH 住院号
     * @return bool
     */
    protected function shouldSkipConsentFormByPreQualityConditions($form = [], $ZYH = '')
    {
        $blbh = $form['BLBH'] ?? '';
        if (!empty($blbh) && $this->isConsentFormPrinted($blbh)) {
            return true;
        }

        return $this->hasConsciousnessKeywordBeforeConsent($ZYH, $form['CJSJ'] ?? '');
    }

    /**
     * bllb292 首条年龄包含“岁”且小于18岁时不质控。
     *
     * @param string $ZYH 住院号
     * @return bool
     */
    protected function isBllb292PatientUnderage($ZYH = '')
    {
        if ($ZYH === '') {
            return false;
        }

        $record = Bllb292::query()
            ->where('ZYH', $ZYH)
            ->first(['NL']);
        ...
        $age = trim((string)$age);
        if ($age === '' || strpos($age, '岁') === false) {
            return false;
        }

        if (!preg_match('/(\d+(?:\.\d+)?)\s*岁/u', $age, $matches)) {
            return false;
        }

        return (float)$matches[1] < 18;
    }
     ...
     * @return bool
     */
    protected function isConsentFormPrinted($BLBH = '')
    {
        if ($BLBH === '') {
            return false;
        }

        return (int)EMR_BL01_DYJL::query()
            ->where('BLBH', $BLBH)
            ->value('SFDY') === 1;
    }

    /**
     * 同意书创建前三天内病程含意识异常关键词时不质控。
     *
     * @param string $ZYH 住院号
     * @param string $CJSJ 同意书创建时间
     * @return bool
     */
    protected function hasConsciousnessKeywordBeforeConsent($ZYH = '', $CJSJ = '')
    {
        $consentTime = $this->normalizeConsentComparableTime($CJSJ);
        if ($ZYH === '' || $consentTime === '') {
            return false;
        }

        $cacheKey = $ZYH . '|' . $consentTime;
        if (isset($this->consentPreQualitySkipCache['consciousness'][$cacheKey])) {
            return $this->consentPreQualitySkipCache['consciousness'][$cacheKey];
        }

        $keywords = $this->getConsentConsciousnessKeywords();
        if (empty($keywords)) {
            $this->consentPreQualitySkipCache['consciousness'][$cacheKey] = false;

            return false;
        }

        $startTime = date('Y-m-d 00:00:00', strtotime($consentTime . ' -3 days'));
        $records = EMR_BL_BL01::query()
            ->where('JZHM', $ZYH)
            ->where('ZXSJ', '>=', $startTime)
            ->where('ZXSJ', '<=', $consentTime)
            ->get(['BLBH'])
            ->toArray();
        ...
        $blbhList = array_filter(array_unique(array_column($records, 'BLBH')));
        if (empty($blbhList)) {
            $this->consentPreQualitySkipCache['consciousness'][$cacheKey] = false;
        ...
        $hasKeyword = false;
        foreach (array_chunk($blbhList, 100) as $chunk) {
            $hjnrList = EMR_BL_BLXG::query()
                ->whereIn('BLBH', $chunk)
                ->pluck('HJNR')
                ->toArray();

            foreach ($hjnrList as $hjnr) {
                if ($this->textContainsAnyKeyword($hjnr, $keywords)) {
                    $hasKeyword = true;
                    break 2;
                }
            }
        ...
        $this->consentPreQualitySkipCache['consciousness'][$cacheKey] = $hasKeyword;
     ...
     * @return array
     */
    protected function getConsentConsciousnessKeywords()
    {
        $keyword = RuleWordMap::query()
            ->where('id', $this->consentConsciousnessKeywordMapId)
            ->value('keyword');

        if (empty($keyword)) {
            $keyword = '神志模糊,神志昏睡,神志谵妄,神志昏迷,神志不清,意识模糊,嗜睡,昏睡,谵妄,昏迷,意识不清,意识模糊,呼唤无应答,言语混乱,语无伦次,无法有效沟通,失语';
        }

        return array_values(array_unique(array_filter(array_map('trim', explode(',', $keyword)))));
    }
     ...
     * @return bool
     */
    protected function textContainsAnyKeyword($text = '', $keywords = [])
    {
        $text = (string)$text;
        if ($text === '') {
            return false;
    ...
    public function zkgzHandle($caseRule, $info, $isSz = 0)
    {
        ...
        $methodList = [
            80 => 'rule80',     // 病案首页
            95 => 'rule95',     // 病案首页
            20008 => 'rule20008', // 病案首页完成时间超24小时
            96 => 'rule96',     // 出院记录
            20002 => 'rule20002', // 出院记录完成时间超24小时
            99 => 'rule99',     // 入院记录
            97 => 'rule97',     // 24小时出入院记录
            20004 => 'rule20004', // 24小时入出院记录完成时间超24小时
            278 => 'rule278', // 病案首页，死亡患者在死亡后1周内要完成病案首页
            20009 => 'rule20009', // 病案首页完成时间超7天（死亡患者）
            98 => 'rule98', // 患者死亡后24小时内未完成死亡记录
            20003 => 'rule20003', // 死亡记录完成时间超24小时
            1000 => 'rule1000', //24小时入院死亡记录
            20005 => 'rule20005', // 24小时入院死亡记录完成时间超24小时
            101 => 'rule101', // 患者入院后，8小时内未完成【首次病程记录】
            102 => 'rule102', // 开具CT检查的医嘱，未在CT检查报告单结果出具后的72小时内完成相关病程记录
            20017 => 'rule20017', // CT检查相关病程记录完成时间超72小时
            103 => 'rule103', // 开具MR检查的医嘱（MR、磁共振、DWI、TRICKS、SWI），未在MR检查报告单结果出具后的72小时内完成相关病程记录
            20018 => 'rule20018', // MR检查相关病程记录完成时间超72小时
            109 => 'rule109',   // 有抗菌药，开嘱时间+24小时 内要有病程记录
            20019 => 'rule20019', // 抗菌药物相关病程记录完成时间超72小时
            110 => 'rule110',   // 有化疗药，开嘱时间+24小时 内要有病程记录
            20020 => 'rule20020', // 化疗药物相关病程记录完成时间超72小时
            219 => 'rule219', //
            ...
            20012 => 'rule20012', // 病情稳定患者每3天1次查房完成时间超时
            ...
            20014 => 'rule20014', // 死亡病例讨论结论记录完成时间超7天
            1011 => 'rule1011', // 危急值接收后24小时内未完成危急值记录
            20006 => 'rule20006', // 危急值记录完成时间超24小时
            1012 => 'rule1012', // 输血结束后24小时内未完成输血记录
            20007 => 'rule20007', // 输血记录完成时间超24小时
            1013 => 'rule1013', // 住院三十天以上未书写阶段小结
            20013 => 'rule20013', // 阶段小结完成时间超30天
            1014 => 'rule1014', // 转出前未完成转出记录
            1015 => 'rule1015', // 转入后未完成转入记录
            20015 => 'rule20015', // 转入记录完成时间超24小时
            ...
            20010 => 'rule20010', // 病危患者上级医师每天1次查房完成时间超时
            1050 => 'rule1050', // 医嘱名称含"病重"，每两天1次上级医师查房 新（查blmc）
            20011 => 'rule20011', // 病重患者上级医师每2天1次查房完成时间超时
            1051 => 'rule1051', // 出院前上级医师查房记录 新（查blmc）
            20016 => 'rule20016', // 出院前一日或当日上级医师查房记录书写超时
            ...
            1260 => 'rule1260', // 知情同意书未体现先告后知（所有知情同意书医师先签名，患者后签名）
            1276 => 'rule1276', // 首次病程记录超时
            20001 => 'rule20001', // 上级医师首次查房超48小时
            1278 => 'rule1278', // 入院记录超时
            1286 => 'rule1286', // 缺病危（病重）通知书
            1290 => 'rule1290', // 缺化疗知情同意书
            1306 => 'rule1306', // 特殊检查/特殊治疗知情同意书无医师签字
            1307 => 'rule1307', // 特殊检查/特殊治疗知情同意书无患方签字
            1308 => 'rule1308', // 特殊检查/特殊治疗知情同意书内容填写不全，有空项
            1309 => 'rule1309', // 特殊检查/特殊治疗知情同意书中“拟施特殊检查/特殊治疗项目名称”未填写
            1310 => 'rule1310', // 特殊检查/特殊治疗知情同意书中“拟施特殊检查/特殊治疗项目目的”未填写
            1311 => 'rule1311', // 特殊检查/特殊治疗知情同意书中替代方案未填写
            1312 => 'rule1312', // 特殊检查/特殊治疗知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
            1313 => 'rule1313', // 特殊检查/特殊治疗知情同意书中医师签字时间未填写
            1314 => 'rule1314', // 特殊检查/特殊治疗知情同意书中患方签字时间未填写
            1315 => 'rule1315', // 特殊检查/特殊治疗知情同意书中患方意见未填写
            1316 => 'rule1316', // 特殊检查/特殊治疗知情同意书中代理人与患者关系未填
            1317 => 'rule1317', // 术中冰冻切片检查知情同意书无医师签字
            1318 => 'rule1318', // 术中冰冻切片检查知情同意书无患方签字
            1319 => 'rule1319', // 术中冰冻切片检查知情同意书内容填写不全，有空项
            1320 => 'rule1320', // 术中冰冻切片检查知情同意书中“手术中冰冻切片检查目的”未填写
            1321 => 'rule1321', // 术中冰冻切片检查知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
            1322 => 'rule1322', // 术中冰冻切片检查知情同意书中医师签字时间未填写
            1323 => 'rule1323', // 术中冰冻切片检查知情同意书中患方签字时间未填写
            1324 => 'rule1324', // 术中冰冻切片检查知情同意书中患方意见未填写
            1325 => 'rule1325', // 术中冰冻切片检查知情同意书中代理人与患者关系未填
            1326 => 'rule1326', // 化疗知情同意书无医师签字
            1327 => 'rule1327', // 化疗知情同意书无患方签字
            1328 => 'rule1328', // 化疗知情同意书内容填写不全，有空项
            1329 => 'rule1329', // 化疗知情同意书中替代方案未填写
            1330 => 'rule1330', // 化疗知情同意书中医师签字时间未填写
            1331 => 'rule1331', // 化疗知情同意书中患方签字时间未填写
            1332 => 'rule1332', // 化疗知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
            1333 => 'rule1333', // 化疗知情同意书中患方意见未填写
            1334 => 'rule1334', // 化疗知情同意书中代理人与患者关系未填
            1335 => 'rule1335', // CT增强检查知情同意书无医师签字
            1336 => 'rule1336', // CT增强检查知情同意书无患方签字
            1337 => 'rule1337', // 未在检查开始前签署CT增强检查知情同意书
            1338 => 'rule1338', // CT增强检查知情同意书内容填写不全，有空项
            1339 => 'rule1339', // CT增强检查知情同意书中替代医疗方案未填写
            1340 => 'rule1340', // CT增强检查知情同意书中医师签字时间未填写
            1341 => 'rule1341', // CT增强检查知情同意书中患方签字时间未填写
            1342 => 'rule1342', // CT增强检查知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
            1343 => 'rule1343', // CT增强检查知情同意书中患方意见未填写
            1344 => 'rule1344', // CT增强检查知情同意书中为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
            1345 => 'rule1345', // CT增强检查知情同意书中代理人与患者关系未填
            1346 => 'rule1346', // 病危（病重）通知书无医师签字
            1347 => 'rule1347', // 病危（病重）通知书无代理人签字
            1348 => 'rule1348', // 病危（病重）通知书内容填写不全，有空项
            1349 => 'rule1349', // 病危（病重）通知书中目前诊断未填写
            1350 => 'rule1350', // 病危（病重）通知书中医师签字时间未填写
            1351 => 'rule1351', // 病危（病重）通知书中代理人签字时间未填写
            1352 => 'rule1352', // 病危（病重）通知书中医师签字时间晚于患方签字时间，未体现先告后知
            1353 => 'rule1353', // 病危（病重）通知书中患方意见未填写
            1354 => 'rule1354', // 病危（病重）通知书中代理人与患者关系未填
            1355 => 'rule1355', // 授权委托书内容填写不全，有空项
            1356 => 'rule1356', // 授权委托书代理人信息填写不全
            1357 => 'rule1357', // 授权委托书无委托人（患者）签名
            1358 => 'rule1358', // 授权委托书委托人（患者）签字时间未填写
            1359 => 'rule1359', // 授权委托书无代理人签字
            1360 => 'rule1360', // 授权委托书中代理人签字时间未填写
            1361 => 'rule1361', // 拒绝检查/治疗告知书无医师签字
            1362 => 'rule1362', // 拒绝检查/治疗告知书无患方签字
            1363 => 'rule1363', // 拒绝检查/治疗告知书内容填写不全，有空项
            1364 => 'rule1364', // 拒绝检查/治疗告知书中“拒绝检查/治疗项目名称”未填写
            1365 => 'rule1365', // 拒绝检查/治疗告知书中医师签字时间未填写
            1366 => 'rule1366', // 拒绝检查/治疗告知书中患方签字时间未填写
            1367 => 'rule1367', // 拒绝检查/治疗告知书中医师签字时间晚于患方签字时间，未体现先告后知
            1368 => 'rule1368', // 拒绝检查/治疗告知书中患方意见未填写
            1369 => 'rule1369', // 拒绝检查/治疗告知书中代理人与患者关系未填
            1370 => 'rule1370', // 自动出院/转院风险告知书无医师签字
            1371 => 'rule1371', // 自动出院/转院风险告知书无患方签字
            1372 => 'rule1372', // 自动出院/转院风险告知书内容填写不全，有空项
            1373 => 'rule1373', // 自动出院/转院风险告知书中医师签字时间未填写
            1374 => 'rule1374', // 自动出院/转院风险告知书中患方签字时间未填写
            1375 => 'rule1375', // 自动出院/转院风险告知书中医师签字时间晚于患方签字时间，未体现先告后知
            1376 => 'rule1376', // 自动出院/转院风险告知书中患方意见未填写
            1377 => 'rule1377', // 自动出院/转院风险告知书中代理人与患者关系未填
            1378 => 'rule1378', // 尸检告知书无医师签字
            1379 => 'rule1379', // 尸检告知书无代理人签字
            1380 => 'rule1380', // 尸检告知书内容填写不全，有空项
            1381 => 'rule1381', // 尸检告知书中医师签字时间未填写
            1382 => 'rule1382', // 尸检告知书中患方签字时间未填写
            1383 => 'rule1383', // 尸检告知书中医师签字时间晚于患方签字时间，未体现先告后知
            1384 => 'rule1384', // 尸检告知书中患方意见未填写
            1385 => 'rule1385', // 尸检告知书中代理人与患者关系未填
            1386 => 'rule1386', // 医保外付费项目知情同意书无医师签字
            1387 => 'rule1387', // 医保外付费项目知情同意书无患方签字
            1388 => 'rule1388', // 医保外付费项目知情同意书内容填写不全，有空项
            1389 => 'rule1389', // 医保外付费项目知情同意书中签字时间未填写
            1390 => 'rule1390', // 医保外付费项目知情同意书中患方意见未填写
            1391 => 'rule1391', // 植入类或Ⅲ级医用耗材知情同意书无医师签字
            1392 => 'rule1392', // 植入类或Ⅲ级医用耗材知情同意书无患方签字
            1393 => 'rule1393', // 植入类或Ⅲ级医用耗材知情同意书内容填写不全，有空项
            1394 => 'rule1394', // 植入类或Ⅲ级医用耗材知情同意书中“拟用耗材名称”等未填写
            1395 => 'rule1395', // 植入类或Ⅲ级医用耗材知情同意书中替代耗材名称未填写
            1396 => 'rule1396', // 植入类或Ⅲ级医用耗材知情同意书中医师签字时间未填写
            1397 => 'rule1397', // 植入类或Ⅲ级医用耗材知情同意书中患方签字时间未填写
            1398 => 'rule1398', // 植入类或Ⅲ级医用耗材知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
            1399 => 'rule1399', // 植入类或Ⅲ级医用耗材知情同意书中患方意见未填写
            1400 => 'rule1400', // 植入类或Ⅲ级医用耗材知情同意书中代理人与患者关系未填
            1401 => 'rule1401', // 缺特殊检查/特殊治疗知情同意书
            1402 => 'rule1402', // 未在操作开始前签署特殊检查/特殊治疗知情同意书
            1403 => 'rule1403', // 特殊检查/特殊治疗知情同意书中为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
            1404 => 'rule1404', // 缺术中冰冻切片检查知情同意书
            1405 => 'rule1405', // 未在检查前签署术中冰冻切片检查知情同意书
            1406 => 'rule1406', // 术中冰冻切片检查知情同意书为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
            1407 => 'rule1407', // 未在化疗开始前签署化疗知情同意书
            1408 => 'rule1408', // 化疗知情同意书中为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
            1409 => 'rule1409', // 缺CT增强检查知情同意书
            1410 => 'rule1410', // 拒绝检查/治疗告知书中为代理人签字，无授权委托书
            1411 => 'rule1411', // 自动出院/转院风险告知书中为代理人签字，无授权委托书
            1412 => 'rule1412', // 植入类或Ⅲ级医用耗材知情同意书中为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
            1413 => 'rule1413', // 缺腰椎穿刺术知情同意书
            1414 => 'rule1414', // 腰椎穿刺术知情同意书无医师签字
            1415 => 'rule1415', // 腰椎穿刺术知情同意书无患方签字
            1416 => 'rule1416', // 未在操作开始前签署腰椎穿刺术知情同意书
            1417 => 'rule1417', // 腰椎穿刺术知情同意书内容填写不全，有空项
            1418 => 'rule1418', // 腰椎穿刺术知情同意书中“拟施特殊检查/特殊治疗项目目的”未填写
            1419 => 'rule1419', // 腰椎穿刺术知情同意书中替代方案未填写
            1420 => 'rule1420', // 腰椎穿刺术知情同意书中医师签字时间未填写
            1421 => 'rule1421', // 腰椎穿刺术知情同意书中患方签字时间未填写
            1422 => 'rule1422', // 腰椎穿刺术知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
            1423 => 'rule1423', // 腰椎穿刺术知情同意书中患方意见未填写
            1424 => 'rule1424', // 腰椎穿刺术知情同意书中为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
            1425 => 'rule1425', // 腰椎穿刺术知情同意书中代理人与患者关系未填
            1426 => 'rule1426', // 特殊检查/特殊治疗知情同意书中代理人签字时间早于授权委托书代理人签字时间
            1427 => 'rule1427', // 术中冰冻切片检查知情同意书中代理人签字时间早于授权委托书代理人签字时间
            1428 => 'rule1428', // 化疗知情同意书中代理人签字时间早于授权委托书代理人签字时间
            1429 => 'rule1429', // CT增强检查知情同意书中代理人签字时间早于授权委托书代理人签字时间
            1430 => 'rule1430', // 病危（病重）通知书中代理人签字时间早于授权委托书代理人签字时间
            1431 => 'rule1431', // 拒绝检查/治疗告知书中代理人签字时间早于授权委托书代理人签字时间
            1432 => 'rule1432', // 自动出院/转院风险告知书中代理人签字时间早于授权委托书代理人签字时间
            1433 => 'rule1433', // 尸检告知书中代理人签字时间早于授权委托书代理人签字时间
            1434 => 'rule1434', // 植入类或Ⅲ级医用耗材知情同意书中代理人签字时间早于授权委托书代理人签字时间
            1435 => 'rule1435', // 腰椎穿刺术知情同意书中代理人签字时间早于授权委托书代理人签字时间
            1436 => 'rule1436', // 输血同意书中代理人签字时间早于授权委托书代理人签字时间
            10312 => 'rule10312', // 拟手术时间早于术前讨论及术前小结及术前讨论结论记录时间，不符合逻辑
            20021 => 'rule20021', // 标题时间晚于拟手术时间，不符合逻辑
            20022 => 'rule20022', // 病程记录引用检验报告内容时禁止使用符号*
            20023 => 'rule20023', // 会诊记录不得使用不规范表述
            20024 => 'rule20024', // 病程记录不得复制检查报告非诊疗相关内容
            20025 => 'rule20025', // 病程记录不得复制检查报告提示性内容
            10313 => 'rule10313', // 手术同意书中患方未签字
            10314 => 'rule10314', // 缺手术同意书
            10315 => 'rule10315', // 手术同意书中经治医师未签字
            10316 => 'rule10316', // 未在手术开始前签署手术同意书
            10317 => 'rule10317', // 手术同意书内容填写不全，有空项
            10318 => 'rule10318', // 手术同意书中“术前诊断”未填写
            10319 => 'rule10319', // 手术同意书中“拟施手术名称”未填写
            10320 => 'rule10320', // 手术同意书中无替代医疗方案
            10321 => 'rule10321', // 手术同意书中患方意见未填写
            10322 => 'rule10322', // 手术同意书中经治医师未签字 
            10323 => 'rule10323', // 手术同意书中术者未签字
            10324 => 'rule10324', // 手术同意书中医师签字时间未填写
            10325 => 'rule10325', // 手术同意书中患方签字时间未填写
            10326 => 'rule10326', // 手术同意书中代理人签字时间早于授权委托书代理人签字时间
            10327 => 'rule10327', // 手术同意书中医师签字时间晚于患方签字时间，未体现先告后知
            10328 => 'rule10328', // 手术同意书违反“术前讨论完成后，方可签署手术知情同意书、开具手术医嘱”的逻辑关系
            10329 => 'rule10329', // 缺输血同意书
            10330 => 'rule10330', // 输血同意书无医师签字
            10331 => 'rule10331', // 输血同意书无患方签字
            10332 => 'rule10332', // 未在输血开始前签署输血同意书
            10333 => 'rule10333', // 输血同意书内容填写不全，有空项
            10334 => 'rule10334', // 输血同意书中临床诊断未填写
            10335 => 'rule10335', //  输血同意书中医师签字时间未填写
            10336 => 'rule10336', //  输血同意书中患方签字时间未填写
            10337 => 'rule10337', //  输血同意书中替代医疗方案未填写
            10338 => 'rule10338', //  输血同意书中医师签字时间晚于患方签字时间，未体现先告后知
            10339 => 'rule10339', //  输血同意书中患方意见未填写
            10340 => 'rule10340', //  输血同意书为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
            10341 => 'rule10341', //  输血同意书中代理人与患者关系未填
            10350 => 'rule10350', //  病史陈述者未填写
        ];
        ...
        foreach ($methodList as $ruleId => $method) {
            ...
            // 4、如果规则不存在，则不进行质控
            if (empty($caseRule[$ruleId]) || empty($caseRule[$ruleId]['status']) || in_array($ruleId, $appealRuleIds) || (($isSz == 1 || $isSz == 4) && $caseRule[$ruleId]['is_shizhong'] == 0)) {
                continue;
                //Log::info('ruleids---->', $ruleId);
            }

            if ($this->shouldSkipConsentPreQualityRule($ruleId, (string)$ZYH)) {
                continue;
            }
            ...
            if (method_exists($this, $method)) {
                $this->$method($caseRule, $ruleId, (string)$ZYH);
            }
        }
        ...
        if (empty($appealids)) {
            Appeal::where('ZYH', $ZYH)->whereIn('error_id', $methodList)->where('status', '=', '0')->update(['status' => 3]);
        } else {
            Appeal::whereNotIn('id', is_array($appealids) ? $appealids : [$appealids])->where('ZYH', $ZYH)->whereIn('error_id', $methodList)->where('status', '=', '0')->update(['status' => 3]);
        }
     ...
     * 输血同意书中代理人与患者关系未填
     */
    public function rule10341($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        }

        $basisList = [];
        $consentForms = $this->getSxzqtysConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            if ((int)($form['SFDLRQM'] ?? 0) !== 1) {
                continue;
            }

            if ($this->consentFieldEmpty($form['DLRGX'] ?? '')) {
                $basisList[] = ['【' . $blmc . '】代理人与患者关系未填'];
            }
        }
     ...
     * 病史陈述者未填写
     */
    public function rule10350($caseRule = [], $ruleId, $ZYH = "")
    {
        $bllb292 = Bllb292::query()->where('ZYH', $ZYH)->limit(1)->get()->toArray();
        if (!$bllb292) {
            return [];
        }
        $blsy = EMR_BL_BLSY::query()->where('BLBH', $bllb292[0]['BLBH'])->where(function ($query) {
            $query->orWhere('QMMC', 7)->orWhere('QMYS', '陈述者签名');
        })->limit(1)->get()->toArray();
        if (empty($blsy)) {
            $this->insertData[] = [
                'basis' => json_encode([], JSON_UNESCAPED_UNICODE),
                'JZHM' => $ZYH,
                'rule_id' => $ruleId,
                'code' => '',
                'error_field' => $caseRule[$ruleId]['title']
    ...
    /**
     * 输血同意书为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
     */
    public function rule10340($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        $sqwtsForms = $this->getSqwtsConsentForms($ZYH);
        $authorizationSummary = $this->getSqwtsAuthorizationSummary($sqwtsForms, 'signed');
        ...
        foreach ($consentForms as $form) {
            ...
            if (!$authorizationSummary['has_sqwts']) {
                $basisList[] = ['【' . $blmc . '】为代理人签字，无授权委托书'];
                continue;
            }

            if (!$authorizationSummary['has_valid']) {
                $basis = ['【' . $blmc . '】授权委托书中无代理人姓名及签字'];
                $basisList[] = $this->appendSqwtsInvalidBlmcBasis($basis, $authorizationSummary);
            }
        }
     ...
     * 输血同意书中患方意见未填写
     */
    public function rule10339($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        $basisList = $this->getConsentPatientOpinionMissingForms($consentForms);
    ...
    /**
     * 输血同意书中医师签字时间晚于患方签字时间，未体现先告后知
     */
    public function rule10338($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            $basis = $this->getConsentTimeOrderViolationBasis($form);

            if (!empty($basis)) {
                array_unshift($basis, '病历名称：【' . $blmc . '】');
                $basisList[] = $basis;
            }
        }
     ...
     * 输血同意书中替代医疗方案未填写
     */
    public function rule10337($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            if ($this->consentFieldEmpty($form['TDFA'] ?? '')) {
                $basisList[] = ['【' . $blmc . '】替代医疗方案未填写'];
            }
        }
     ...
     * 输血同意书中患方签字时间未填写
     */
    public function rule10336($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        foreach ($consentForms as $form) {
            ...
            if ($this->shouldSkipDoctorPatientSignatureRuleGroup($form)) {
                continue;
            }

            $basis = $this->getConsentSignTimeMissingFields($form);
     ...
     * 输血同意书中医师签字时间未填写
     */
    public function rule10335($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        foreach ($consentForms as $form) {
            ...
            if ($this->consentFieldEmpty($form['YSQMSJ'] ?? '')) {
                $basisList[] = ['【' . $blmc . '】医师签字时间未填写'];
            }
        }
     ...
     * 输血同意书中临床诊断未填写
     */
    public function rule10334($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            if ($this->consentFieldEmpty($form['MQZD'] ?? '')) {
                $basisList[] = ['【' . $blmc . '】临床诊断未填写'];
            }
        }
     ...
     * 输血同意书内容填写不全，有空项
     */
    public function rule10333($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            $basis = $this->getConsentHeaderMissingFields($form);
     ...
     * 未在输血开始前签署输血同意书
     */
    public function rule10332($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        $hasTimelyConsent = false;
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            $basis = ['输血同意书：【' . $blmc . '】'];

            $doctorTime = $this->normalizeConsentComparableTime($form['YSQMSJ'] ?? '');
            $patientSignInfo = $this->getConsentPatientSignTimeInfo($form);

            if (
                $doctorTime !== '' && strtotime($doctorTime) <= $startTimestamp &&
                !empty($patientSignInfo) && $patientSignInfo['timestamp'] <= $startTimestamp
            ) {
                $hasTimelyConsent = true;
                break;
            }

            $hasLateIssue = false;
            if ($doctorTime !== '' && strtotime($doctorTime) > $startTimestamp) {
                $basis[] = '医师签字时间【' . $doctorTime . '】晚于输血开始时间';
                $hasLateIssue = true;
            }

            if (!empty($patientSignInfo) && $patientSignInfo['timestamp'] > $startTimestamp) {
                $basis[] = $patientSignInfo['label'] . '【' . $patientSignInfo['time'] . '】晚于输血开始时间';
                $hasLateIssue = true;
            }
        ...
        if (!$hasTimelyConsent && !empty($basisList)) {
            $transfusionItem = trim((string)($startInfo['cfx'] ?? ''));
            array_unshift($basisList, ['【' . $transfusionItem . '】首次输血开始时间【' . $startInfo['time'] . '】']);
            $this->insertData[] = [
                'basis' => json_encode($basisList, JSON_UNESCAPED_UNICODE),
                'JZHM' => $ZYH,
                'rule_id' => $ruleId,
     ...
     * 输血同意书无患方签字
     */
    public function rule10331($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        foreach ($consentForms as $form) {
            ...
            if (!$this->hasPatientSignatureByBlsy($blbh)) {
                $basisList[] = ['【' . $blmc . '】无患方签字'];
            }
        }
     ...
     * 输血同意书无医师签字
     */
    public function rule10330($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        foreach ($consentForms as $form) {
            ...
            if (!$this->hasDoctorSignatureByBlsy($blbh)) {
                $basisList[] = ['【' . $blmc . '】无医师签字'];
            }
        }

        if (!empty($basisList)) {
            array_unshift($basisList, [$startInfo['basis']]);
            $this->insertData[] = [
                'basis' => json_encode($basisList, JSON_UNESCAPED_UNICODE),
                'JZHM' => $ZYH,
                'rule_id' => $ruleId,
     ...
     * 缺输血同意书
     */
    public function rule10329($caseRule = [], $ruleId, $ZYH = "")
    {
        $startInfo = $this->getFirstTransfusionStartInfo($ZYH);
        if (empty($startInfo)) {
            return [];
        ...
        if (empty($consentForms)) {
            $this->insertData[] = [
                'basis' => json_encode([[$startInfo['basis'], '缺少输血治疗知情同意书']], JSON_UNESCAPED_UNICODE),
                'JZHM' => $ZYH,
                'rule_id' => $ruleId,
                'code' => '',
                'error_field' => $caseRule[$ruleId]['title']
    ...
    /**
     * 手术同意书违反“术前讨论完成后，方可签署手术知情同意书、开具手术医嘱”的逻辑关系
     */
    public function rule10328($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        $surgeryOrders = [];
        // rule_word_map 10328 配置不参与本规则判断的医嘱关键词，英文逗号分割。
        $skipOrderKeywordText = (string)RuleWordMap::query()->where('id', 10328)->value('keyword');
        $skipOrderKeywords = array_filter(array_map('trim', explode(',', $skipOrderKeywordText)), function ($keyword) {
            return $keyword !== '';
        });
        $yzbList = Yzb::query()
            ->where('ZYH', $ZYH)
        ...
        foreach ($yzbList as $yzb) {
            $orderName = trim((string)($yzb['YZMC'] ?? ''));
            foreach ($skipOrderKeywords as $skipOrderKeyword) {
                if (strpos($orderName, $skipOrderKeyword) !== false) {
                    continue 2;
                }
            }

            $orderTime = $this->normalizeConsentComparableTime($yzb['KZSJ'] ?? '');
            if ($orderName === '' || $orderTime === '' || !preg_match('/拟.*术/u', $orderName)) {
                continue;
            }
        ...
        foreach ($discussionRecords as $discussionRecord) {
            $hjnr = (string)EMR_BL_BLXG::query()
                ->where('BLBH', $discussionRecord['BLBH'] ?? '')
                ->value('HJNR');
            if ($hjnr === '') {
                continue;
            }

            $discussionTime = $this->extractPreoperativeSummaryDiscussionTime($hjnr);
            if ($discussionTime === '') {
                continue;
            }
            ...
            $discussionName = trim((string)($discussionRecord['BLMC'] ?? ''));
            $recordTime = $this->normalizeConsentComparableTime($discussionRecord['ZXSJ'] ?? '');
            if ($recordTime !== '' && strpos($discussionName, substr($recordTime, 0, 16)) === false) {
                $discussionName = trim(substr($recordTime, 0, 16) . '  ' . $discussionName);
            }
        ...
        $consentFormName = '手术知情同意书';
        $consentForms = $this->getSszqtysConsentForms($ZYH);
        if (empty($consentForms)) {
            $consentFormName = '特殊治疗同意书';
            $consentForms = $this->getTszlConsentForms($ZYH);
        }
        $previousOrderTimestamp = null;

        foreach ($surgeryOrders as $order) {
            $currentPreviousOrderTimestamp = $previousOrderTimestamp;
            $previousOrderTimestamp = $order['timestamp'];
            $matchedDiscussion = [];

            foreach ($discussions as $discussion) {
                if ($discussion['timestamp'] >= $order['timestamp']) {
                    continue;
                }

                if ($currentPreviousOrderTimestamp !== null && $discussion['timestamp'] <= $currentPreviousOrderTimestamp) {
                    continue;
                }
            ...
            $discussionName = $matchedDiscussion['blmc'] ?: $matchedDiscussion['time'];
            $orderBasis = [
                '手术医嘱【' . $order['name'] . '】',
                '开嘱时间【' . $order['time'] . '】',
                '术前讨论【' . $discussionName . '】',
                '讨论时间【' . $matchedDiscussion['time'] . '】',
            ];

            $hasValidConsentForm = false;
            $invalidConsentBasisList = [];
            foreach ($consentForms as $form) {
                $consentBasis = [$consentFormName . '【' . ($form['BLMC'] ?? '') . '】'];
                $doctorSignTimeValid = false;
                $doctorTime = $this->normalizeConsentComparableTime($form['YSQMSJ'] ?? '');
                if ($doctorTime === '') {
                    $consentBasis[] = '医师签名时间【无】';
                } else {
                    $doctorTimestamp = strtotime($doctorTime);
                    if ($doctorTimestamp === false) {
                        $consentBasis[] = '医师签名时间【无】';
                    } elseif ($doctorTimestamp <= $matchedDiscussion['timestamp']) {
                        $consentBasis[] = '医师签名时间【' . $doctorTime . '】早于讨论时间';
                    } elseif ($doctorTimestamp >= $order['timestamp']) {
                        $consentBasis[] = '医师签名时间【' . $doctorTime . '】晚于开嘱时间';
                    } else {
                        $doctorSignTimeValid = true;
                    }
                }

                $patientSignTimeValid = false;
                $patientSignTime = $this->normalizeConsentComparableTime($form['HZQMSJ'] ?? '');
                if ($patientSignTime === '') {
                    foreach (['DLRQMSJ', 'DLRQMSJ2'] as $agentSignTimeField) {
                        $patientSignTime = $this->normalizeConsentComparableTime($form[$agentSignTimeField] ?? '');
                        if ($patientSignTime !== '') {
                            break;
                        }
                    }
                ...
                if ($patientSignTime === '') {
                    $consentBasis[] = '患方签名时间【无】';
                } else {
                    $patientSignTimestamp = strtotime($patientSignTime);
                    if ($patientSignTimestamp === false) {
                        $consentBasis[] = '患方签名时间【无】';
                    } elseif ($patientSignTimestamp <= $matchedDiscussion['timestamp']) {
                        $consentBasis[] = '患方签名时间【' . $patientSignTime . '】早于讨论时间';
                    } elseif ($patientSignTimestamp >= $order['timestamp']) {
                        $consentBasis[] = '患方签名时间【' . $patientSignTime . '】晚于开嘱时间';
                    } else {
                        $patientSignTimeValid = true;
                    }
                }

                if ($doctorSignTimeValid && $patientSignTimeValid) {
                    $hasValidConsentForm = true;
                    break;
                }
                ...
                $invalidConsentBasisList[] = $consentBasis;
            }

            if ($hasValidConsentForm) {
                continue;
            }

            if (empty($invalidConsentBasisList)) {
                $basisList[] = array_merge($orderBasis, [$consentFormName . '【无或签名不符合要求】']);
                continue;
            }

            foreach ($invalidConsentBasisList as $invalidConsentBasis) {
                $basisList[] = array_merge($orderBasis, $invalidConsentBasis);
            }
        }
    ...
    /**
     * 手术同意书中医师签字时间晚于患方签字时间，未体现先告后知
     */
    public function rule10327($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSszqtysConsentForms($ZYH);
    ...
    /**
     *  手术同意书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule10326($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSszqtysConsentForms($ZYH);
        ...
        $authorizationTime = $this->getSqwtsEarliestAgentSignTime($this->getSqwtsConsentForms($ZYH));
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            if (!$this->hasSszqtysAgentSign($form)) {
                continue;
            }

            $consentAgentTime = $this->normalizeConsentComparableTime($form['DLRQMSJ'] ?? '');
            if ($consentAgentTime === '') {
                continue;
            }

            if (strtotime($consentAgentTime) < $authorizationTime['timestamp']) {
                $basisList[] = [
                    '手术同意书【' . $blmc . '】',
                    '代理人签字时间【' . $consentAgentTime . '】',
                    '授权委托书【' . ($authorizationTime['blmc'] ?? '') . '】',
                    '代理人签字时间【' . $authorizationTime['time'] . '】',
                ];
            }
        }
    ...
    /**
     * 特殊检查/特殊治疗知情同意书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule1426($caseRule = [], $ruleId, $ZYH = "")
    {
        return $this->checkConsentAgentSignTimeBeforeAuthorization(
            $caseRule,
            $ruleId,
            $ZYH,
            '特殊检查/特殊治疗知情同意书',
            $this->getTszlConsentForms($ZYH),
            'hasTszlAgentSign'
        );
    }

    /**
     * 术中冰冻切片检查知情同意书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule1427($caseRule = [], $ruleId, $ZYH = "")
    {
        return $this->checkConsentAgentSignTimeBeforeAuthorization(
            $caseRule,
            $ruleId,
            $ZYH,
            '术中冰冻切片检查知情同意书',
            $this->getBdqpConsentForms($ZYH),
            'hasBdqpAgentSign'
        );
    }

    /**
     * 化疗知情同意书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule1428($caseRule = [], $ruleId, $ZYH = "")
    {
        return $this->checkConsentAgentSignTimeBeforeAuthorization(
            $caseRule,
            $ruleId,
            $ZYH,
            '化疗知情同意书',
            $this->getHltysConsentForms($ZYH)
        );
    }

    /**
     * CT增强检查知情同意书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule1429($caseRule = [], $ruleId, $ZYH = "")
    {
        return $this->checkConsentAgentSignTimeBeforeAuthorization(
            $caseRule,
            $ruleId,
            $ZYH,
            'CT增强检查知情同意书',
            $this->getCtzqConsentForms($ZYH)
        );
    }

    /**
     * 病危（病重）通知书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule1430($caseRule = [], $ruleId, $ZYH = "")
    {
        return $this->checkConsentAgentSignTimeBeforeAuthorization(
            $caseRule,
            $ruleId,
            $ZYH,
            '病危（病重）通知书',
            $this->getBwbztysConsentForms($ZYH)
        );
    }

    /**
     * 拒绝检查/治疗告知书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule1431($caseRule = [], $ruleId, $ZYH = "")
    {
        return $this->checkConsentAgentSignTimeBeforeAuthorization(
            $caseRule,
            $ruleId,
            $ZYH,
            '拒绝检查/治疗告知书',
            $this->getJjjctysConsentForms($ZYH)
        );
    }

    /**
     * 自动出院/转院风险告知书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule1432($caseRule = [], $ruleId, $ZYH = "")
    {
        return $this->checkConsentAgentSignTimeBeforeAuthorization(
            $caseRule,
            $ruleId,
            $ZYH,
            '自动出院/转院风险告知书',
            $this->getZdcygzsConsentForms($ZYH)
        );
    }

    /**
     * 尸检告知书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule1433($caseRule = [], $ruleId, $ZYH = "")
    {
        return $this->checkConsentAgentSignTimeBeforeAuthorization(
            $caseRule,
            $ruleId,
            $ZYH,
            '尸检告知书',
            $this->getSjgzsConsentForms($ZYH)
        );
    }

    /**
     * 植入类或Ⅲ级医用耗材知情同意书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule1434($caseRule = [], $ruleId, $ZYH = "")
    {
        return $this->checkConsentAgentSignTimeBeforeAuthorization(
            $caseRule,
            $ruleId,
            $ZYH,
            '植入类或Ⅲ级医用耗材知情同意书',
            $this->getZrltysConsentForms($ZYH)
        );
    }

    /**
     * 腰椎穿刺术知情同意书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule1435($caseRule = [], $ruleId, $ZYH = "")
    {
        return $this->checkConsentAgentSignTimeBeforeAuthorization(
            $caseRule,
            $ruleId,
            $ZYH,
            '腰椎穿刺术知情同意书',
            $this->getYzccstysConsentForms($ZYH)
        );
    }

    /**
     * 输血同意书中代理人签字时间早于授权委托书代理人签字时间
     */
    public function rule1436($caseRule = [], $ruleId, $ZYH = "")
    {
        return $this->checkConsentAgentSignTimeBeforeAuthorization(
            $caseRule,
            $ruleId,
            $ZYH,
            '输血同意书',
            $this->getSxzqtysConsentForms($ZYH)
        );
    }

    /**
     * 校验同意书代理人签字时间是否早于授权委托书代理人签字时间
     */
    protected function checkConsentAgentSignTimeBeforeAuthorization(
        $caseRule = [],
        $ruleId = 0,
        $ZYH = "",
        $consentName = '',
        $consentForms = [],
        $agentSignMethod = 'hasConsentAgentSign'
    ) {
        $basisList = [];
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            $hasAgentSign = method_exists($this, $agentSignMethod)
                ? $this->{$agentSignMethod}($form)
                : $this->hasConsentAgentSign($form);
     ...
     * 手术同意书中患方签字时间未填写
     */
    public function rule10325($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSszqtysConsentForms($ZYH);
     ...
     * 手术同意书中医师签字时间未填写
     */
    public function rule10324($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSszqtysConsentForms($ZYH);
     ...
     * 手术同意书中术者未签字
     */
    public function rule10323($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSszqtysConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            ...
            if (!$this->hasSurgeonSignatureByBlsy($blbh)) {
                $basisList[] = ['【' . $blmc . '】术者未签字'];
            }
        }
     ...
     *  手术同意书中经治医师未签字
     */
    public function rule10322($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSszqtysConsentForms($ZYH);
     ...
     *  手术同意书中患方意见未填写
     */
    public function rule10321($caseRule = [], $ruleId, $ZYH = "")
    {
        $consentForms = $this->getSszqtysConsentForms($ZYH);
     ...
     * 手术同意书中无替代医疗方案
     */
    public function rule10320($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSszqtysConsentForms($ZYH);
     ...
     * 手术同意书中“拟施手术名称”未填写
     */
    public function rule10319($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSszqtysConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            if ($this->consentFieldEmpty($form['NSSSMC'] ?? '')) {
                $basisList[] = ['【' . $blmc . '】拟施手术名称未填写'];
            }
        }
     ...
     * 手术同意书中“术前诊断”未填写
     */
    public function rule10318($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSszqtysConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            if ($this->consentFieldEmpty($form['SQZD'] ?? '')) {
                $basisList[] = ['【' . $blmc . '】术前诊断未填写'];
            }
        }
    ...
    public function rule10317($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSszqtysConsentForms($ZYH);
    ...
    /**
     * 未在手术开始前签署手术同意书
     * 1、有手术SM_SSAP 且类型是（手术+介入）
     * 2、手术开始时间 SM_SSAP
     * 3、术前有没有同意书 EMR_BL_BL01  bllb=329 blmc包含手术同意书,手术知情同意书
     * 手术知情同意书 ZXSJ
     */
    public function rule10316($caseRule = [], $ruleId, $ZYH = "")
    {
        $surgeryPlans = $this->getSszqtysRequiredSurgeryPlans($ZYH);
        if (empty($surgeryPlans)) {
            return [];
        }
        ...
        foreach ($surgeryPlans as $plan) {
            $surgeryStartTime = $this->normalizeConsentComparableTime($plan['SSRQ'] ?? '');
            if ($surgeryStartTime === '') {
                continue;
            }

            $surgeryStartTimestamp = strtotime($surgeryStartTime);
            if ($surgeryStartTimestamp === false) {
                continue;
            }

            $recordBasis = [];
            $surgeryName = trim((string)($plan['ICD9_SSCZMC'] ?? ''));

            if ($surgeryName !== '') {
                $recordBasis[] = '手术名称【' . $surgeryName . '】';
            }

            $recordBasis[] = '手术开始时间【' . $surgeryStartTime . '】';
            ...
            foreach ($consentForms as $form) {
                ...
                if ($doctorTime === '' && empty($patientSignInfo)) {
                    continue;
                }
                ...
                if ($doctorTime !== '') {
                    $doctorTimestamp = strtotime($doctorTime);
                    $doctorTimeValid = $doctorTimestamp !== false && $doctorTimestamp <= $surgeryStartTimestamp;
                }

                $patientTimeValid = !empty($patientSignInfo) && $patientSignInfo['timestamp'] <= $surgeryStartTimestamp;

                if ($doctorTimeValid || $patientTimeValid) {
                    $hasTimelyConsent = true;
                    break;
                }
            }
            ...
            $recordBasis[] = '缺少手术同意书或签名不符合要求';
            $basisList[] = $recordBasis;
        }
     ...
     * 手术同意书中患方未签字
     */
    public function rule10313($caseRule = [], $ruleId, $ZYH = "")
    {
        $consentForms = $this->getSszqtysConsentForms($ZYH);
        if (empty($consentForms)) {
            return [];
     ...
     * 手术同意书中经治医师未签字
     */
    public function rule10315($caseRule = [], $ruleId, $ZYH = "")
    {
        $consentForms = $this->getSszqtysConsentForms($ZYH);
        if (empty($consentForms)) {
            return [];
     ...
     * 手术知情同意书 ZXSJ
     */
    public function rule10314($caseRule = [], $ruleId, $ZYH = "")
    {
        $surgeryPlans = $this->getSszqtysRequiredSurgeryPlans($ZYH);
        if (empty($surgeryPlans)) {
            return [];
        ...
        foreach ($surgeryPlans as $plan) {
            $basis = [];
            $surgeryName = trim((string)($plan['ICD9_SSCZMC'] ?? ''));
            $startTime = $this->normalizeConsentComparableTime($plan['SSRQ'] ?? '');
    ...
    public function rule10312($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        ...
        foreach ($records as $record) {
            ...
            $hjnr = (string)EMR_BL_BLXG::query()->where('BLBH', $blbh)->value('HJNR');
            if ($hjnr === '') {
                continue;
            }

            // $recordTime = $this->extractPreoperativeSummaryRecordTitleTime($hjnr);
            $discussionTime = $this->extractPreoperativeSummaryDiscussionTime($hjnr);
            $plannedSurgeryTime = $this->extractPreoperativeSummaryPlannedSurgeryTime($hjnr);

            if ($discussionTime === '' || $plannedSurgeryTime === '') {
                continue;
            }

            // $recordTimestamp = strtotime($recordTime);
            $discussionTimestamp = strtotime($discussionTime);
            $plannedSurgeryTimestamp = strtotime($plannedSurgeryTime);

            if ($discussionTimestamp === false || $plannedSurgeryTimestamp === false) {
                continue;
            }

            $basis = [];
            // if ($plannedSurgeryTimestamp < $recordTimestamp) {
            //     $basis[] = '病历名称【' . $blmc . '】';
            //     $basis[] = '拟手术时间【' . $plannedSurgeryTime . '】，早于记录时间【' . $recordTime . '】';
            // }

            if ($plannedSurgeryTimestamp < $discussionTimestamp) {
                if (empty($basis)) {
                    $basis[] = '病历名称【' . $blmc . '】';
                }
                $basis[] = '术前讨论时间【' . $discussionTime . '】，晚于拟手术时间【' . $plannedSurgeryTime . '】';
            }
    ...
    public function rule20021($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];
        ...
        foreach ($records as $record) {
            ...
            if ($plannedSurgeryTimestamp < $discussionTimestamp) {
                ...
                $basis[] = '标题时间【' . $discussionTime . '】，晚于拟手术时间【' . $plannedSurgeryTime . '】';
            }
    ...
    public function rule20022($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];

        $keywordText = (string)RuleWordMap::query()->where('id', 20072)->value('keyword');
        if ($keywordText === '') {
            $keywordText = '*长期';
        }

        $keywords = array_values(array_filter(array_map('trim', explode(',', str_replace('，', ',', $keywordText))), function ($keyword) {
            return $keyword !== '';
        }));
        ...
        foreach ($records as $record) {
            $hjnr = (string)($record['HJNR'] ?? '');
            if ($hjnr === '') {
                continue;
            }

            $illegalSegments = $this->extractIllegalAsteriskSegments($hjnr, $keywords);
            if (empty($illegalSegments)) {
                continue;
            }

            $recordTime = !empty($record['ZXSJ']) ? date('Y-m-d H:i', strtotime($record['ZXSJ'])) : '';
            $recordName = trim($recordTime . ' ' . ($record['BLMC'] ?? ''));

            $basis = [];
            $basis[] = '病程【' . $recordName . '】';
            foreach ($illegalSegments as $segment) {
                $basis[] = '【' . $segment . '】不能出现符号 *';
            }
    ...
    public function rule20023($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];

        $keywordText = (string)RuleWordMap::query()->where('id', 20073)->value('keyword');
        if ($keywordText === '') {
            $keywordText = '谢邀,贵科,我科随诊,我科随访';
        }
        ...
        $records = EMR_BL_BL01::query()
            ...
            ->where('EMR_BL_BL01.JZHM', $ZYH)
            ->where('EMR_BL_BL01.BLLB', 294)
            ->where('EMR_BL_BL01.BLMC', 'like', '%会诊%')
            ->orderBy('EMR_BL_BL01.ZXSJ', 'asc')
            ->get([
                'EMR_BL_BL01.BLBH as BLBH',
                'EMR_BL_BL01.BLMC as BLMC',
        ...
        foreach ($records as $record) {
            ...
            $matchedSegments = $this->extractForbiddenKeywordSegments($hjnr, $keywords);
            if (empty($matchedSegments)) {
                continue;
            }
            ...
            $basis = [];
            $basis[] = '会诊记录【' . $recordName . '】';
            foreach ($matchedSegments as $matchedSegment) {
                $basis[] = '【' . $matchedSegment['segment'] . '】不能写“' . $matchedSegment['keyword'] . '”';
            }
            $basis['BLBH'] = $record['BLBH'] ?? '';
            $basisList[] = $basis;
        }
    ...
    public function rule20024($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];

        $keywordText = (string)RuleWordMap::query()->where('id', 20074)->value('keyword');
        if ($keywordText === '') {
            $keywordText = '加收,夜间急诊使用';
        }
    ...
    public function rule20025($caseRule = [], $ruleId, $ZYH = "")
    {
        $basisList = [];

        $keywordText = (string)RuleWordMap::query()->where('id', 20075)->value('keyword');
        if ($keywordText === '') {
            $keywordText = '请结合临床,请结合病史';
        }
    ...
    protected function extractForbiddenKeywordSegments($hjnr, array $keywords)
    {
        $segments = [];

        foreach ($keywords as $keyword) {
            $pattern = '/' . preg_quote($keyword, '/') . '/u';
            if (!preg_match_all($pattern, $hjnr, $matches, PREG_OFFSET_CAPTURE)) {
                continue;
            }

            foreach ($matches[0] as $match) {
                $segment = $this->extractTextAroundOffsetByPunctuation($hjnr, $match[1]);
                if ($segment === '') {
                    continue;
                }
    ...
    protected function extractIllegalAsteriskSegments($hjnr, array $keywords)
    {
        $segments = [];

        foreach ($keywords as $keyword) {
            $pattern = $this->buildIllegalAsteriskPattern($keyword);
            if ($pattern === '') {
                continue;
            }
            ...
            foreach ($matches[0] as $match) {
                $offset = $match[1];
                if ($this->isExcludedAsteriskKeyword($hjnr, $offset, $keywords)) {
                    continue;
                }

                // * 前后任一侧为数字或字母时不质控，例如：2*3、2.2*3、2.2*3.3、2*3.3、A*、*A。
                if ($this->isAllowedAsteriskExpression($hjnr, $offset)) {
                    continue;
                }

                $segment = $this->extractTextAroundOffsetByPunctuation($hjnr, $offset);
                if ($segment !== '') {
                    $segments[] = $segment;
                }
            }
    ...
    protected function buildIllegalAsteriskPattern($keyword)
    {
        return '/\*/u';
    }

    protected function isExcludedAsteriskKeyword($text, $asteriskOffset, array $keywords)
    {
        foreach ($keywords as $keyword) {
            if ($keyword === '') {
                continue;
            ...
            if (substr($text, $asteriskOffset, strlen($keyword)) === $keyword) {
                return true;
            }
        }
    ...
    protected function isAllowedAsteriskExpression($text, $asteriskOffset)
    {
        $before = substr($text, 0, $asteriskOffset);
        $after = substr($text, $asteriskOffset + 1);

        if (preg_match('/([A-Za-z0-9])\s*$/u', $before)) {
            return true;
        }

        if (preg_match('/^\s*([A-Za-z0-9])/u', $after)) {
            return true;
        }
    ...
    protected function extractTextAroundOffsetByPunctuation($text, $offset)
    {
        $start = 0;
        $end = strlen($text);
        ...
        if (preg_match_all('/[。！？!?；;，,、：:\r\n【】\[\]{}《》]/u', $text, $matches, PREG_OFFSET_CAPTURE)) {
            foreach ($matches[0] as $match) {
                $punctuation = $match[0];
                $punctuationOffset = $match[1];
                if ($punctuationOffset < $offset) {
                    $start = $punctuationOffset + strlen($punctuation);
                    continue;
                }

                $end = $punctuationOffset;
                break;
            }
        }

        $segment = substr($text, $start, $end - $start);
        $segment = preg_replace('/^\s+|\s+$/u', '', strip_tags($segment));

        return $segment ?: '';
    }

    protected function extractPreoperativeSummaryRecordTitleTime($hjnr = '')
    {
        $hjnr = trim((string)$hjnr);
        if ($hjnr === '') {
            return '';
        }

        if (preg_match('/^\s*(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?)/u', $hjnr, $matches)) {
            return $this->normalizeConsentComparableTime($matches[1]);
        }

        return '';
    }

    protected function extractPreoperativeSummaryDiscussionTime($hjnr = '')
    {
        $hjnr = trim((string)$hjnr);
        if ($hjnr === '') {
            return '';
        ...
        $patterns = [
            '/术前小结及术前讨论结论记录\s*(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?)/u',
            '/术前讨论结论记录\s*(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?)/u',
            '/(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?)，?在.*?主持下进行了术前讨论/u',
        ];
    ...
    protected function extractPreoperativeSummaryPlannedSurgeryTime($hjnr = '')
    {
        $hjnr = trim((string)$hjnr);
        if ($hjnr === '') {
            return '';
        ...
        $patterns = [
            '/手术拟定于\s*(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?)\s*进行/u',
            '/拟定于\s*(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?)\s*进行/u',
        ];
    ...
    public function rule1052($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        })->where("JZHM", $ZYH)->get(["BLMC", 'BLBH', 'ZXSJ'])->toArray();
        if (empty($bl01)) {
            return [];
        }
        $basisList = [];
        foreach ($bl01 as $b) {
            $zxsjTimestamp = strtotime((string)($b['ZXSJ'] ?? ''));
            if ($zxsjTimestamp !== false && time() <= $zxsjTimestamp + 48 * 3600) {
                continue;
            }
    ...
    public function rule1039($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        if (!$this->isMedicalRecordSigned($newData['BLBH'] ?? '')) {
            return [];
        }
        $ZYTS = $newData["ZYTS"] ?? '';
    ...
    {
        $basisList = [];
        // jzhm关联，mblb = 27；HJNR 需通过 BLBH 关联 EMR_BL_BLXG 获取
        ...
        foreach ($newData as $item) {
            $basis = [];
            $hjnr = (string)EMR_BL_BLXG::query()->where('BLBH', $item['BLBH'] ?? '')->value('HJNR');
            if (empty($hjnr)) {
                continue;
            }
        ...
        foreach ($newData as $item) {
            ...
            foreach ($blsy as $blsyItem) {
                ...
                if(!empty($ssz)){
                 ...
                 if(!empty($yizhu)){
                    if (strpos($name, $yizhu) !== false || strpos($yizhu, $name) !== false) {
                        $isHas = 1;
                        break;
                    }
                }
                if(!empty($erzhu)){
                    if (strpos($name, $erzhu) !== false || strpos($erzhu, $name) !== false) {
                        $isHas = 1;
                        break;
                    }
                }
            ...
            if (!$isHas) {
                ...
                $basis[] = $item["BLMC"];
                $basis[] = '术者：【' . $ssz . '】';
                $basis[] = '手术记录签名医师：【' . rtrim($doctornams, ',') . '】非术者或一助、二助';
    ...
    public function rule261($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($yzb as $y) {
            ...
            $resDataList = EMR_BL_BL01::query()
                ...
                ->get(['BLBH', 'CJSJ', 'BLMC', 'BLLB', 'first_blsy_time'])
                ->toArray();
            $resData = [$resDataList, count($resDataList)];
            // 病程记录无
            if (empty($resData[1])) {
                $basis[] = '手术同意书【无】';
            } else {
                $basis[] = '手术同意书【' . $resData[0][0]['BLMC'] . '】';
            }

            $resDataList1 = EMR_BL_BL01::query()
                ->where('JZHM', $ZYH)
                ->where('MBLB', 82)
                ->where('operation_time', '<', strtotime($y['KZSJ']))
                ->where(function ($query) {
                    $query->where('BLZT', '!=', 9)
                        ->orWhereNull('BLZT');
                })
                ->get(['BLBH', 'CJSJ', 'BLMC', 'BLLB', 'first_blsy_time'])
                ->toArray();
            $resData1 = [$resDataList1, count($resDataList1)];
            // 病程记录无
            if (empty($resData1[1])) {
                $basis[] = '术前小结【无】';
                $flag = false;
            } else {
                $basis[] = '手术同意书【' . $resData1[0][0]['BLMC'] . '】';

                // 检查首次签名时间是否超时
                if (strtotime($resData1[0][0]["first_blsy_time"]) > strtotime($y['KZSJ'])) {
                    $basis[] = '【未在手术医嘱之前】';
                    $basis["BLBH"] = $resData1[0][0]["BLBH"];
                    $flag = false;
                }
                // 检查首次签名时间是否超时
                if (!empty($resData[0]) && strtotime($resData1[0][0]["first_blsy_time"]) > strtotime($resData[0][0]["first_blsy_time"])) {
                    $basis[] = '【未在手术同意书之前】';
                    $basis["BLBH"] = $resData1[0][0]["BLBH"];
                    $flag = false;
                }
    ...
    public function rule260($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($yzb as $y) {
            ...
            if (empty($resData[1])) {
                ...
                // 检查首次签名时间是否超时
                if (strtotime($resData[0][0]['first_blsy_time']) > strtotime($y['KZSJ'])) {
     ...
     */
    public function rule102($caseRule, $ruleId, $ZYH, $isYj = 0, $onlyTimeout = false)
    {
        $basisList = [];
        //替换ct、oct 字典
        ...
        foreach ($pacs as $p) {
            ...
            if (!$hasYzb) {
                ...
                $ctKeywords = array_values(array_filter($ctFirst, function ($keyword) {
                    return trim($keyword) !== '';
                }));
                $octKeywords = array_values(array_filter($octFirst, function ($keyword) {
                    return trim($keyword) !== '';
                }));
                $resDataList = EMR_BL_BL01::query()
                    ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
                    ->where('EMR_BL_BL01.JZHM', $ZYH)
                    ->whereIn('EMR_BL_BL01.BLLB', $map9009)
                    ->where('EMR_BL_BL01.ZXSJ', '>', $p['BGSJ'])
                    ->where('EMR_BL_BL01.ZXSJ', '<', $bgsj)
                    ->where(function ($query) use ($ctKeywords) {
                        foreach ($ctKeywords as $keyword) {
                            $query->orWhere('EMR_BL_BLXG.HJNR', 'like', '%' . $keyword . '%');
                        }
                    })
                    ->where(function ($query) use ($octKeywords) {
                        foreach ($octKeywords as $keyword) {
                    ...
                    ->get([
                        ...
                        'EMR_BL_BL01.first_blsy_time as first_blsy_time',
                        'EMR_BL_BLXG.HJNR as HJNR',
                    ])
                    ->toArray();
                $resData = [$resDataList, count($resDataList)];
                ...
                // 病程记录无
                if (empty($resData[0][0])) {
                    if (!$onlyTimeout) {
                        $basis[] = '报告出具后72小时内的病程记录未记录检查结果';
                        $basisList[] = $basis;
                    }
                    $exitTimeEnd = $bgsj;
                    //先转换时间戳
                    ...
                    //移除空元素
                    $resData[0] = array_values($resData[0]);
                    ...
                    if (empty($resData[0])) {
                        ...
                        $firstUnsignedRecord = null;
                        $firstTimeoutRecord = null;

                        foreach ($resData[0] as $item) {
                            if (empty($item["first_blsy_time"])) {
                                if (empty($firstUnsignedRecord)) {
                                    $firstUnsignedRecord = $item;
                                }
                                continue;
                            }
                            ...
                            if (empty($firstTimeoutRecord)) {
                                $firstTimeoutRecord = $item;
                            }
                        }
                        ...
                        if (!$isZk) {
                            if ($onlyTimeout) {
                                if (!empty($firstTimeoutRecord)) {
                                    $recordTime = !empty($firstTimeoutRecord["ZXSJ"]) ? date('Y-m-d H:i', strtotime($firstTimeoutRecord["ZXSJ"])) : '';
                                    $recordName = trim($firstTimeoutRecord["BLMC"]);
                                    $basis[] = '病程记录【' . $recordName . '】';
                                    $basis[] = '首次签名时间【' . $firstTimeoutRecord["first_blsy_time"] . ' （超时）】';
                                    $basis["BLBH"] = $firstTimeoutRecord["BLBH"];
                                    $basisList[] = $basis;
                                }
                            } elseif (empty($firstTimeoutRecord) && !empty($firstUnsignedRecord)) {
                                $recordTime = !empty($firstUnsignedRecord["ZXSJ"]) ? date('Y-m-d H:i', strtotime($firstUnsignedRecord["ZXSJ"])) : '';
                                $recordName = trim($firstUnsignedRecord["BLMC"]);
                                $basis[] = '病程记录【' . $recordName . '】，未签名';
                                $basis["BLBH"] = $firstUnsignedRecord["BLBH"];
                                $basisList[] = $basis;
                            }
                        }
                    }
    ...
    /**
     * CT检查相关病程记录首次签名时间超72小时。
     */
    public function rule20017($caseRule, $ruleId, $ZYH)
    {
        return $this->rule102($caseRule, $ruleId, $ZYH, 0, true);
    }

    /*
     * 检查报告单有mr结果，24小时内要写病程记录，或 24小时内出入院记录：关键字 "MR” 或 “磁共振”
     */
    public function rule103($caseRule, $ruleId, $ZYH, $isYj = 0, $onlyTimeout = false)
    {
        $basisList = [];
        //替换ct、oct 字典
        ...
        foreach ($pacs as $p) {
            ...
            if (!$hasYzb) {
                ...
                // 检查病程信息
                $bgsj = date("Y-m-d H:i:s", strtotime($p['BGSJ']) + intval($map9010['keyword']) * 3600);
                $mrKeywords = array_values(array_filter($map25, function ($keyword) {
                ...
                if (!empty($mrKeywords)) {
                    $resDataList = EMR_BL_BL01::query()
                        ...
                        ->where('EMR_BL_BL01.ZXSJ', '>=', $p['BGSJ'])
                        ->where('EMR_BL_BL01.ZXSJ', '<=', $bgsj)
                        ->where(function ($query) use ($mrKeywords) {
                            foreach ($mrKeywords as $keyword) {
                                $query->orWhere('EMR_BL_BLXG.HJNR', 'like', '%' . $keyword . '%');
                            }
                        })
                        ->orderBy('EMR_BL_BL01.ZXSJ', 'asc')
    ...
    /**
     * MR检查相关病程记录首次签名时间超72小时。
     */
    public function rule20018($caseRule, $ruleId, $ZYH)
    {
        return $this->rule103($caseRule, $ruleId, $ZYH, 0, true);
    }
     ...
     */
    public function rule109($caseRule, $ruleId, $ZYH, $isYj = 0, $onlyTimeout = false): array
    {
        ...
        if (!empty($resData)) {
            ...
            foreach ($resData as $v) {
                ...
                $endTime = date("Y-m-d H:i:s", strtotime($v['KZSJ']) + 72 * 3600);
                ...
                //}
                //$shouldListNot[] = ['match_phrase' => ['BLMC' => $map9015['keyword']]];
                $hjnrKeywords[] = $kjywName;
                $hjnrKeywords = array_values(array_unique(array_filter($hjnrKeywords, function ($keyword) {
                    return trim((string)$keyword) !== '';
                })));
                $resDataList1 = [];
                if (!empty($hjnrKeywords)) {
                    $resDataList1 = EMR_BL_BL01::query()
                        ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
                        ->where('EMR_BL_BL01.JZHM', $ZYH)
                        ->whereIn('EMR_BL_BL01.BLLB', $map9009)
                        ->where('EMR_BL_BL01.ZXSJ', '>=', $startTime)
                        ->where('EMR_BL_BL01.ZXSJ', '<=', $endTime)
                        ->where(function ($query) use ($hjnrKeywords) {
                            foreach ($hjnrKeywords as $keyword) {
                                $query->orWhere('EMR_BL_BLXG.HJNR', 'like', '%' . $keyword . '%');
                            }
                        })
                        ->orderBy('EMR_BL_BL01.ZXSJ', 'asc')
                ...
                if (empty($resData1[1])) {
                    ...
                    foreach ($resData1[0] as $key => $item) {
                        $blmc = $item["BLMC"];
                        $hjnr = $item["HJNR"];
                        if (strpos($blmc, '会诊') !== false) {
                            //把痕迹内容中  “会诊意见给予”之后的内容全部删除
                            ...
                            if (strpos($hjnr, $kjywName) === false) {
                                //移除这条
                                unset($resData1[0][$key]);
                            }
                        }
                    }
                    //移除空元素
                    $resData1[0] = array_values($resData1[0]);
                    if (empty($resData1[0])) {
                        if (!$onlyTimeout) {
                            $basis[] = '医嘱开具后72小时内的病程记录未记录使用原因';
                            $basisList[] = $basis;
    ...
    /**
     * 抗菌药物相关病程记录首次签名时间超72小时。
     */
    public function rule20019($caseRule, $ruleId, $ZYH): array
    {
        return $this->rule109($caseRule, $ruleId, $ZYH, 0, true);
    }

    /**
     * 化疗药开嘱时间前24小时至开嘱后72小时，病程中搜索化疗药
     * 先查ZXSJ是否符合，再看首次签名时间是否符合
     */
    public function rule110($caseRule, $ruleId, $ZYH, $isYj = 0, $onlyTimeout = false): array
    {
        $errorNotice = [];
        $basisList = [];
        $map9009 = RuleWordMap::query()->where('id', 8068)->value('keyword');
        ...
        if (!empty($resData)) {
            foreach ($resData as $v) {
                ...
                /* $mustNot = [
                    ['match_phrase' => ['BLMC' => $map9015['keyword']]]
                ]; */
                $hjnrKeywords = [];
                //从数据表获取抗菌药名称，和2053中的满足一项即可表medicinal_info  且 type=1
    ...
    /**
     * 化疗药物相关病程记录首次签名时间超72小时。
     */
    public function rule20020($caseRule, $ruleId, $ZYH): array
    {
        return $this->rule110($caseRule, $ruleId, $ZYH, 0, true);
    }
    ...
    public function rule95($caseRule, $ruleId, $ZYH, $isYj = 0)
    {
        ...
        // 2. 查询MySQL医嘱表，按住院号关联并匹配出院/离院医嘱，排除死亡医嘱
        $exitKeywords = array_filter(array_map('trim', $exitKeywords));
     ...
     * @return true
     */
    public function rule20008($caseRule, $ruleId, $ZYH, $isYj = 0)
    {
        // 1. 获取出院/离院医嘱配置项
        $ruleMap8000 = RuleWordMap::query()->where('id', '=', 8000)->value('keyword');
        $ruleMap8000 = !empty($ruleMap8000) ? $ruleMap8000 : "出院,离院";  // 医嘱名称包含的关键字
    ...
    public function rule96($caseRule, $ruleId, $ZYH)
    {
        ...
        // rule96仅质控是否创建、是否签名，签名超时由rule20002处理
     ...
     * @return true
     */
    public function rule20002($caseRule, $ruleId, $ZYH)
    {
        $exitTime = ZY_BRRY::query()->where('ZYH', '=', $ZYH)->value('AAC01');
        if (empty($exitTime)) {
            $exitTime = Carbon::now()->toDateTimeString();
        ...
        if (time() - strtotime($exitTime) < 24 * 3600) {
            return true;
        }
        ...
        $exitTimeEnd = strtotime($exitTime) + 24 * 3600;
        if (strtotime(Carbon::now()->toDateTimeString()) < $exitTimeEnd) {
            return true;
        }

        $ruleMap8000 = RuleWordMap::query()->where('id', '=', 8000)->value('keyword');
        $ruleMap8000 = !empty($ruleMap8000) ? $ruleMap8000 : "出院,离院";
        if (strpos($ruleMap8000, ',') !== false) {
            $exitKeywords = explode(',', $ruleMap8000);
        } else {
            $exitKeywords = [$ruleMap8000];
        ...
        $ruleMap8001 = RuleWordMap::query()->where('id', '=', 8004)->value('keyword');
        $ruleMap8001 = !empty($ruleMap8001) ? $ruleMap8001 : 1;
        if (strpos($ruleMap8001, ',') !== false) {
            $bllbKeywords = explode(',', $ruleMap8001);
        } else {
            $bllbKeywords = [$ruleMap8001];
        ...
        $bl01Data = EMR_BL_BL01::query()
            ->where(['JZHM' => $ZYH])
            ->whereIn('BLLB', $bllbKeywords)
            ->whereNotNull($ruleMap8002)
            ->where($ruleMap8002, '<>', '')
            ->orderBy($ruleMap8002, 'asc')
            ->get()
            ->toArray();
        ...
        $firstRecord = $bl01Data[0];
        $firstBlsyTime = $firstRecord[$ruleMap8002];
        if (strtotime($firstBlsyTime) <= $exitTimeEnd) {
            return true;
        }

        $basis = [];
        $basis[] = "出院时间【" . $exitTime . "】";
        $basis[] = "【" . $firstRecord['BLMC'] . "】首次签名时间【" . $firstBlsyTime . "】超24小时";
        $basis["BLBH"] = $firstRecord['BLBH'];
        $this->insertData[] = [
            'JZHM' => $ZYH,
            'rule_id' => $ruleId,
            'code' => 'rule_' . $ruleId,
     ...
     * @return bool
     */
    public function rule20009($caseRule, $ruleId, $ZYH)
    {
        ...
        $exitTime = $patientInfo['AAC01'];
        if (empty($exitTime) || $exitTime == '1970-01-01 00:00:00' || $exitTime == '0000-00-00 00:00:00') {
            return true; // 没有有效的死亡时间
        }

        //获取当前时间，如果当前时间-死亡时间小于7天，则不质控
        $currentDate = Carbon::now()->toDateTimeString();
        $diffTime = strtotime($currentDate) - strtotime($exitTime);
        if ($diffTime < 7 * 24 * 3600) {
            return true;
    ...
    public function rule98($caseRule, $ruleId, $ZYH)
    {
        ...
        $ruleMap8003 = !empty($ruleMap8003) ? $ruleMap8003 : "死亡";  // 医嘱名称包含的关键字
        //判断是否包含逗号
        if (strpos($ruleMap8003, ',') !== false) {
            $exitKeywords = explode(',', $ruleMap8003);
        } else {
            $exitKeywords = [$ruleMap8003];
        }

        // 5. 查询MySQL医嘱表，按住院号关联并匹配死亡医嘱
        $exitKeywords = array_filter(array_map('trim', $exitKeywords));
        if (empty($exitKeywords)) {
            return true; // 如果没有有效的关键词，跳过
        }
        ...
        $deathTimeEnd = strtotime($deathTime) + 24 * 3600;
        // 死亡后未满24小时不质控，避免使用出院时间误判死亡记录时效。
        if (time() < $deathTimeEnd) {
            return true;
        }
        ...
        if (empty($bl01Data)) {
            ...
            $diffTime = $deathTimeEnd - time();
            if ($diffTime > 0 && $diffTime < 2 * 3600) {
                // 预警发送已由其他流程负责，此处不再发送。
                // $res = remainderTime($diffTime);
                // $content = '请在' . $res . '内完成死亡记录';
                // $msgYj = ['死亡时间【' . $deathTime . '】', '死亡记录【未创建】'];
        ...
        $firstBlsyTime = $bl01Data[0][$ruleMap8002];
        // rule98仅质控是否创建、是否签名，签名超时由rule20003处理
        if (empty($firstBlsyTime)) {
            $basis[] = "【" . $bl01Data[0]['BLMC'] . "】 未签名";
            //加入质控的病案首页的BLBH
            $basis["BLBH"] = $bl01Data[0]['BLBH'];
            ...
            if ($diffTime > 0 && $diffTime < 2 * 3600) {
                ...
                // $res = remainderTime($diffTime);
                // $content = '请在' . $res . '内完成死亡记录';
                // $msgYj = ['死亡时间【' . $deathTime . '】', '死亡记录【已创建，未签名】'];
                // $this->caseService->sendMsg($ZYH, $content, 100, $msgYj);
            }
            return true;
        }
     ...
     * @return bool
     */
    public function rule20003($caseRule, $ruleId, $ZYH)
    {
        ...
        $deathTimestamp = strtotime($deathTime);
        if (empty($deathTimestamp)) {
            return true;
        }

        $deathTimeEnd = $deathTimestamp + 24 * 3600;
        if (time() <= $deathTimeEnd) {
            return true;
        }

        $ruleMap8005 = RuleWordMap::query()->where('id', '=', 8005)->value('keyword');
        $ruleMap8005 = !empty($ruleMap8005) ? $ruleMap8005 : 288;
        if (strpos($ruleMap8005, ',') !== false) {
            $exitKeywords8005 = explode(',', $ruleMap8005);
        } else {
            $exitKeywords8005 = [$ruleMap8005];
        ...
        $firstBlsyTimestamp = strtotime($firstBlsyTime);
        if (empty($firstBlsyTimestamp) || $firstBlsyTimestamp <= $deathTimeEnd) {
            return true;
        }

        $basis = [];
        $basis[] = '死亡时间【' . date('Y-m-d H:i:s', $deathTimestamp) . '】';
        $basis[] = '【' . ($bl01Data[0]['BLMC'] ?? '死亡记录') . '】首次签名时间【' . date('Y-m-d H:i:s', $firstBlsyTimestamp) . '】超24小时';
        if (!empty($bl01Data[0]['BLBH'])) {
            $basis['BLBH'] = $bl01Data[0]['BLBH'];
        }
    ...
    private function getDeathRecordTime($ZYH)
    {
        ...
        $blbh = EMR_BL_BL01::query()->where('JZHM', $ZYH)->whereIn('BLLB', $exitKeywords8007)->value('BLBH');
        if (empty($blbh)) {
            return null;
        }
        ...
        if (preg_match('/死亡时间：(?:\{)?(\d{4}-\d{2}-\d{2} \d{2}:\d{2})(?:\})?/', $hjnr, $matches)) {
            ...
            $deathTime = $matches[1];
        } elseif (preg_match('/死亡时间：(\d{4}年\d{1,2}月\d{1,2}日 \d{1,2}:\d{1,2})/', $hjnr, $matches)) {
            $deathTime = $matches[1];
        }
    ...
    public function rule1000($caseRule, $ruleId, $ZYH)
    {
        ...
        if (empty($deathTime) || $deathTime == '无') {
            return true;
        }
     ...
     * @return true
     */
    public function rule20005($caseRule, $ruleId, $ZYH)
    {
        $exitTime = ZY_BRRY::query()->where('ZYH', '=', $ZYH)->value('AAC01');
        if (empty($exitTime)) {
            $exitTime = Carbon::now()->toDateTimeString();
        ...
        $firstBlsyTime = $bl01Data[0][$ruleMap8002] ?? '';
        if (empty($firstBlsyTime) || strtotime($firstBlsyTime) <= $deathTimeEnd) {
            return true;
        }

        $basis = [];
        $basis[] = "死亡时间【" . $deathTime . "】";
        $basis[] = "【" . $bl01Data[0]['BLMC'] . "】首次签名时间【" . $firstBlsyTime . "】超24小时";
        $basis["BLBH"] = $bl01Data[0]['BLBH'];
        $this->insertData[] = [
            'JZHM' => $ZYH,
            'rule_id' => $ruleId,
     ...
     * @return true
     */
    public function rule20004($caseRule, $ruleId, $ZYH)
    {
        $exitTime = ZY_BRRY::query()->where('ZYH', '=', $ZYH)->value('AAC01');
        if (empty($exitTime)) {
            $exitTime = Carbon::now()->toDateTimeString();
        ...
        $ruleMap8051 = RuleWordMap::query()->where('id', '=', 8051)->value('keyword');
        $ruleMap8051 = !empty($ruleMap8051) ? $ruleMap8051 : '18,1';
        if (strpos($ruleMap8051, ',') !== false) {
            $exitKeywords = explode(',', $ruleMap8051);
        } else {
            $exitKeywords = [$ruleMap8051];
        ...
        $isExclude = false;
        foreach ($exitKeywords8023 as $keyword) {
            if (strpos($bl01Mc, $keyword) !== false) {
                foreach ($exitKeywords8024 as $keyword8024) {
                    if (strpos($bl01Mc, $keyword8024) !== false) {
                        $isExclude = true;
                        break;
                    }
                }
        ...
        $firstBlsyTime = $bl01Data[0][$ruleMap8002] ?? '';
        if (empty($firstBlsyTime) || strtotime($firstBlsyTime) <= $exitTimeEnd) {
            return true;
        }

        $basis = [];
        $basis[] = "出院时间【" . $exitTime . "】";
        $basis[] = "【" . $bl01Mc . "】首次签名时间【" . $firstBlsyTime . "】超24小时";
        $basis["BLBH"] = $bl01Data[0]['BLBH'];
        $this->insertData[] = [
            'JZHM' => $ZYH,
            'rule_id' => $ruleId,
    ...
    public function rule101($caseRule, $ruleId, $ZYH)
    {
        ...
        //获取出院时间
        $AAC01 = ZY_BRRY::query()->where('ZYH', '=', $ZYH)->value('AAC01');

        $exitTime = time();
        if(!empty($AAC01)){
            $exitTime = strtotime($AAC01);
        }


        //当前时间-入院时间小于8小时，则不质控
        $diffTime = $exitTime - strtotime($enterTime);
        if ($diffTime < 8 * 3600) {
            return true;
        }
    ...
    public function rule114($caseRule, $ruleId, $ZYH, $isYj = 0)
    {
        ...
        $bl01Data = EMR_BL_BL01::query()->where('JZHM', $ZYH)->whereIn('BLLB', $excludeDiagnosis)->whereNotIn('MBLB', $excludeDiagnosis1)->where($ruleMap8011, '>=', $enterTime)->where($ruleMap8011, '<=', $enterTimeEnd)->orderBy($ruleMap8011, 'asc')->get()->toArray();
        ...
        foreach ($bl01Data as $value) {
            ...
            foreach ($blsy as $blsyValue) {
                ...
                if (strpos($ruleMap8012, ',') !== false) {
                    $excludeDiagnosis = explode(',', $ruleMap8012);
                } else {
                    $excludeDiagnosis = [$ruleMap8012];
                }
                //判断医生职称是否包含'主任'或'主治'
                $isMatched = false;
                ...
                if ($isMatched) {
                    $hasLeader = true;
                    //包含主任，查看首次签名时间
                    $firstBlsyTime = $value[$firstBlsyTimeField];
                    $basis[] = "上级医师查房记录【" . $value['BLMC'] . "】";
                    //如果首次签名时间为空，则不质控
                    if (empty($firstBlsyTime)) {
                        $basis[] = "首次签名时间【未签名】";
                        $basis["BLBH"] = $value['BLBH']; //加入被质控病程BLBH
                        $insertData[] = [
                            'JZHM' => $ZYH,
                            'rule_id' => $ruleId,
                            'code' => 'rule_' . $ruleId,
                        ...
                        //如果首次签名时间不为空，则查看是否在入院时间到入院时间+48小时之间,之前就是提前创建，之后就是超时
                        if ($firstBlsyTime < $enterTime) {
                            $basis[] = "首次签名时间【" . $firstBlsyTime . "（提前创建）】";
                        } elseif ($firstBlsyTime > $enterTimeEnd) {
                            $basis[] = "首次签名时间【" . $firstBlsyTime . "（超48小时）】";
                        } else {
                            //如果有符合的，情况关于这个rule_id=114的质控
                            return true;
                            break;
                        }
                        $basis["BLBH"] = $value['BLBH']; //加入被质控病程BLBH
    ...
    // rule20001：只质控上级医师首次查房首次签名时间是否超48小时
    public function rule20001($caseRule, $ruleId, $ZYH)
    {
        $exitTime = ZY_BRRY::query()->where('ZYH', '=', $ZYH)->value('AAC01');
        if (empty($exitTime)) {
            $exitTime = Carbon::now()->toDateTimeString();
        ...
        $enterTimeEnd = strtotime($enterTime) + 48 * 3600;
        if (strtotime(Carbon::now()->toDateTimeString()) < $enterTimeEnd) {
            return true;
        }
        $enterTimeEnd = date('Y-m-d H:i:s', $enterTimeEnd);
        ...
        $bl01Data = EMR_BL_BL01::query()
            ->where('JZHM', $ZYH)
            ->whereIn('BLLB', $excludeDiagnosis)
            ->whereNotIn('MBLB', $excludeDiagnosis1)
            ->where($ruleMap8011, '>=', $enterTime)
            ->where($ruleMap8011, '<=', $enterTimeEnd)
            ->orderBy($ruleMap8011, 'asc')
            ->get()
            ->toArray();
        ...
        $ruleMap8012 = RuleWordMap::query()->where('id', '=', 8012)->value('keyword');
        $ruleMap8012 = !empty($ruleMap8012) ? $ruleMap8012 : '主治,主任';
        if (strpos($ruleMap8012, ',') !== false) {
            $leaderTitles = explode(',', $ruleMap8012);
        } else {
            $leaderTitles = [$ruleMap8012];
        }

        $firstTimeoutRecord = [];
        $firstTimeoutSignedTime = '';
        foreach ($bl01Data as $value) {
            $blmc = $value['BLMC'];
            $isMatched = false;
            foreach ($leaderTitles as $title) {
                if ($title !== '' && strpos($blmc, $title) !== false) {
                    $isMatched = true;
                    break;
                }
            }
            ...
            $firstSignedTime = isset($value[$firstBlsyTimeField]) ? $value[$firstBlsyTimeField] : '';
            if (empty($firstSignedTime)) {
                continue;
            }

            // 范围内只要有一条符合条件且签名未超48小时，即视为通过。
            if (strtotime($firstSignedTime) <= strtotime($enterTimeEnd)) {
                return true;
            }

            if (empty($firstTimeoutRecord)) {
                $firstTimeoutRecord = $value;
                $firstTimeoutSignedTime = $firstSignedTime;
            }
        }
        ...
        $basis = [];
        $basis[] = "入院时间【" . $enterTime . "】";
        $basis[] = "【" . $firstTimeoutRecord['BLMC'] . "】首次签名时间【" . $firstTimeoutSignedTime . "】超48小时";
        $basis["BLBH"] = $firstTimeoutRecord['BLBH'];
        $this->insertData[] = [
            'JZHM' => $ZYH,
            'rule_id' => $ruleId,
    ...
    public function rule1047($caseRule, $ruleId, $ZYH)
    {
        ...
        $currentDate = Carbon::now()->toDateTimeString();
        $enterTimeEnd = strtotime($enterTime) + 48 * 3600;
        //如果当前时间小于入院时间+48小时就跳过
        if (strtotime($currentDate) < $enterTimeEnd) {
            return true;
        }
        ...
        // 范围内只要有一条上级医师查房记录已签名，即视为满足1047。
        $hasLeader = false;
        $firstUnsignedRecord = [];
        foreach ($bl01Data as $value) {
            $blmc = isset($value['BLMC']) ? $value['BLMC'] : '';
            $isMatched = false;
            foreach ($leaderTitles as $title) {
                if ($title !== '' && strpos($blmc, $title) !== false) {
                    $isMatched = true;
            ...
            if (!$isMatched) {
                ...
                foreach ($blsy as $doctor) {
                    ...
                    $staff = isset($staffInfo->ygjb_text) ? $staffInfo->ygjb_text : '';
                    foreach ($leaderTitles as $title) {
                        if ($title !== '' && strpos($staff, $title) !== false) {
                            $isMatched = true;
                            break;
                        }
                    }
            ...
            $hasLeader = true;
            $firstBlsyTime = isset($value[$firstBlsyTimeField]) ? $value[$firstBlsyTimeField] : '';
            if (!empty($firstBlsyTime)) {
                return true;
            }

            if (empty($firstUnsignedRecord)) {
                $firstUnsignedRecord = $value;
            }
        }
        ...
        if (!empty($firstUnsignedRecord)) {
            $basis[] = "【" . $firstUnsignedRecord['BLMC'] . "】 未签名";
            $basis["BLBH"] = $firstUnsignedRecord['BLBH']; //加入被质控病程BLBH
            $this->insertData[] = [
                'JZHM' => $ZYH,
                'rule_id' => $ruleId,
                'code' => 'rule_' . $ruleId,
    ...
    public function rule1048($caseRule, $ruleId, $ZYH)
    {
        ...
        // 如果没有出院时间或出院时间为默认值，使用当前时间
        if (empty($dischargeTime) || $dischargeTime == '1970-01-01 00:00:00' || $dischargeTime == '0000-00-00 00:00:00') {
            $dischargeTime = date('Y-m-d H:i:s', strtotime('-1 day'));
        }
        ...
        // 获取blmc包含“主任”或“三级”的最早zxsj
        $earliestZxsj = null;
        try {
            // 构建blmc关键字数组（主任和三级）
            $blmcKeywords = ['主任', '三级','主治','二级'];
            ...
            // 动态添加blmc模糊查询，包含主任或三级任意一个
            $query->where(function ($q) use ($blmcKeywords) {
                foreach ($blmcKeywords as $kw) {
                    $q->orWhere('BLMC', 'like', '%' . $kw . '%');
                }
            });

            // 按zxsj升序，获取最早一条
            $firstRecord = $query->orderBy('ZXSJ')->first();

            if ($firstRecord && isset($firstRecord->ZXSJ)) {
                $earliestZxsj = $firstRecord->ZXSJ;
            }
        } catch (\Exception $e) {
            Log::error("rule-$ruleId-$ZYH-获取最早zxsj异常: " . $e->getMessage());
            $earliestZxsj = null;
        }

        if(empty($earliestZxsj)){
            return false;
        }else{
            $enterTime = date('Y-m-d H:i:s', strtotime($earliestZxsj . ' +1 day'));
    ...
    public function rule133($caseRule, $ruleId, $ZYH)
    {
        ...
        // 执行查询
        try {
            $yzbDataList = Yzb::query()
                ->where('ZYH', '=', $ZYH)
                ->where('YZMC', 'like', '%' . $ruleMap8015 . '%')
                ->get()
                ->toArray();
            $yzbData = [$yzbDataList, count($yzbDataList)];
            if (empty($yzbData[0])) {
        ...
        foreach ($yzbData[0] as $order) {
            ...
            if (empty($validRoundsInSpecialCycle)) {
                $targetInvalidRounds = array_values(array_filter($invalidRoundsInSpecialCycle, function ($record) use ($onlyTimeout) {
                    return $onlyTimeout ? $record['sign_status'] == '超时' : $record['sign_status'] == '未签名';
                }));

                if (($onlyTimeout && empty($targetInvalidRounds)) || (!$onlyTimeout && !empty($invalidRoundsInSpecialCycle) && empty($targetInvalidRounds))) {
                    continue;
                }
                ...
                // 添加医师信息
                if (!empty($targetInvalidRounds)) {
                    foreach ($targetInvalidRounds as $record) {
                        $doctorInfo = implode("，", $record['doctor_info']);
                        $signInfo = "";
                ...
                // 添加BLBH（如果有）
                if (!empty($targetInvalidRounds)) {
                    $cycleItem["BLBH"] = $targetInvalidRounds[0]['BLBH'];
                }
            ...
            for ($i = 0; $i < $totalRemainingCycles; $i++) {
                ...
                if (empty($validRoundsInCycle)) {
                    $targetInvalidRounds = array_values(array_filter($invalidRounds, function ($record) use ($onlyTimeout) {
                        return $onlyTimeout ? $record['sign_status'] == '超时' : $record['sign_status'] == '未签名';
                    }));

                    if (($onlyTimeout && empty($targetInvalidRounds)) || (!$onlyTimeout && !empty($invalidRounds) && empty($targetInvalidRounds))) {
                        continue;
                    }
     ...
     * @author lzh
     */
    public function rule1049($caseRule, $ruleId, $ZYH, $onlyTimeout = false)
    {
        ...
        foreach ($yzbData[0] as $order) {
            ...
            // 按自然日跳过开嘱当天和停嘱当天，只质控中间自然日
            $startDate = date('Y-m-d', strtotime($startTime));
            $endDate = date('Y-m-d', strtotime($endTime));
            $remainingStartDate = date('Y-m-d', strtotime($startDate . ' +1 day'));
            $remainingEndDate = date('Y-m-d', strtotime($endDate . ' -1 day'));
            ...
            try {
                $bl01Data = EMR_BL_BL01::query()
                    ->where('JZHM', $ZYH)
                    ->whereIn('MBLB', $recordTypes)
                    ->where($ruleMap8011, '>=', $remainingStartDate . ' 00:00:00')
                    ->where($ruleMap8011, '<=', $remainingEndDate . ' 23:59:59')
                    ->get()
                    ->toArray();
            } catch (\Exception $e) {
                Log::error("rule-$ruleId-$ZYH-error: " . $e->getMessage());
            ...
            // 医嘱信息
            $orderInfo = "医嘱名称【" . $yzmc . "】";
            $orderInfo1 = "开嘱时间【" . $startTime . "】";
            $orderInfo2 = "停嘱时间【" . $endTime . "】";

            // 为当前医嘱创建质控记录组
            $cycleGroups = [];

            // 5. 计算中间自然日周期（跳过开嘱当天和停嘱当天）
     ...
     * @author lzh
     */
    public function rule20010($caseRule, $ruleId, $ZYH)
    {
        return $this->rule1049($caseRule, $ruleId, $ZYH, true);
    }
     ...
     * @author lzh
     */
    public function rule134($caseRule, $ruleId, $ZYH, $onlyTimeout = false)
    {
     ...
     * @author lzh
     */
    public function rule1050($caseRule, $ruleId, $ZYH, $onlyTimeout = false)
    {
                ...
                if (empty($validRoundsInCycle)) {
                    ...
                    if (!empty($targetInvalidRounds)) {
                        foreach ($targetInvalidRounds as $record) {
                            ...
                            $cycleItem[] = "【" . $record['BLMC'] . "】首次签名时间【" . $signInfo . "】";
                        }
                    } else {
                        // 当前周期没有任何查房记录
                        $cycleItem[] = "上级医师未查房";
     ...
     * @author lzh
     */
    public function rule20011($caseRule, $ruleId, $ZYH)
    {
        return $this->rule1050($caseRule, $ruleId, $ZYH, true);
    }
     ...
     * @author lzh
     */
    public function rule132($caseRule, $ruleId, $ZYH, $onlyTimeout = false)
    {
        ...
        try {
            $yzbDataList = Yzb::query()
                ->where('ZYH', '=', $ZYH)
                ->where(function ($query) use ($ruleMap8014, $ruleMap8015) {
                    $query->where('YZMC', 'like', '%' . $ruleMap8014 . '%')
                        ->orWhere('YZMC', 'like', '%' . $ruleMap8015 . '%');
                })
                ->get()
                ->toArray();
            $yzbData = [$yzbDataList, count($yzbDataList)];
        ...
        for ($i = 0; $i < $totalCycles; $i++) {
            ...
            if (empty($validRoundsInCycle)) {
                ...
                if (!empty($targetInvalidRounds)) {
                    ...
                    // 添加BLBH（如果有）
                    if (!empty($targetInvalidRounds)) {
                        $basis["BLBH"] = $targetInvalidRounds[0]['BLBH'];
                    }
                } else {
                    // 当前周期没有任何查房记录
                    $basis[] = "未查房";
     ...
     * @author lzh
     */
    public function rule20012($caseRule, $ruleId, $ZYH)
    {
        return $this->rule132($caseRule, $ruleId, $ZYH, true);
    }
    ...
    public function rule1002($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($ssapData as $surgery) {
            // 获取手术结束时间
            $surgeryEndTime = date('Y-m-d H:i', strtotime($surgery['JSRQ']));
            //获取当前时间
            $currentDate = Carbon::now()->toDateTimeString();
            $sixHoursAfterSurgery = date('Y-m-d H:i', strtotime($surgeryEndTime) + 6 * 3600);
            ...
            foreach ($records as $record) {
                ...
                else if (strtotime($firstBlsyTime) > strtotime($sixHoursAfterSurgery)) {
                    // 签名时间超过规定时间
                    $record['sign_status'] = '超时';
                    $invalidRecords[] = $record;
                } else {
    ...
    public function rule1004($caseRule, $ruleId, $ZYH)
    {
        ...
        for ($i = 0; $i < count($ssapData); $i++) {
            ...
            $records = EMR_BL_BL01::query()
                ...
                ->where($ruleMap8011, '>=', date('Y-m-d H:i', strtotime($startCheckTime))) //精确到分钟就行
            ...
            if (empty($records)) {
                ...
                if (!empty($sssqData)) {
                    $jjbz = $sssqData['JJBZ'];
                    if ($jjbz == 1) {
                        //紧急手术，查询一下有没有术前小结；HJNR 需关联 EMR_BL_BLXG 获取
                        $records8016 = EMR_BL_BL01::query()
                            ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
                            ->where('EMR_BL_BL01.JZHM', $ZYH)
                            ->whereIn('EMR_BL_BL01.MBLB', $recordTypes8016)
                            ->where('EMR_BL_BL01.' . $ruleMap8011, '>=', $startCheckTime)
                            ->where('EMR_BL_BL01.' . $ruleMap8011, '<=', $surgeryStartTime)
                            ->where('EMR_BL_BLXG.HJNR', 'like', '%' . $surgeon1 . '%')
                            ->get([
                                'EMR_BL_BL01.*',
                                'EMR_BL_BLXG.HJNR as HJNR',
                            ])
    ...
    public function rule1006($caseRule, $ruleId, $ZYH)
    {
        ...
        // 2. 从MySQL查询医嘱名称含"抢救"的医嘱

        // 执行查询
        try {
            $yzbDataList = Yzb::query()
                ->where('ZYH', '=', $ZYH)
                ->where('YZMC', 'like', '%' . $ruleMap8026 . '%')
                ->get()
                ->toArray();
            $yzbData = [$yzbDataList, count($yzbDataList)];
            if (empty($yzbData[0])) {
    ...
    public function rule1051($caseRule, $ruleId, $ZYH)
    {
        ...
        // 5. 查询MySQL医嘱表是否包含出院/离院关键字，排除死亡医嘱
        $exitKeywords = array_filter(array_map('trim', $exitKeywords));
        if (empty($exitKeywords)) {
            return true; // 如果没有有效的关键词，跳过
        }
        ...
        if (empty($validRecords)) {
            ...
            if (!empty($invalidRecords)) {
                $basis[] = "出院前一日或当日上级医师查房记录【已创建,未签名】";
            } else {
                $basis[] = "出院前一日或当日上级医师查房记录【未创建】";
            }
     ...
     * @author lzh
     */
    public function rule20016($caseRule, $ruleId, $ZYH)
    {
        $patientInfo = ZY_BRRY::query()->where('ZYH', '=', $ZYH)->first();
        if (empty($patientInfo)) {
            return true;
        ...
        if (empty($yzbDataList)) {
            return true;
        }

        $ruleMap8049 = RuleWordMap::query()->where('id', '=', 8049)->value('keyword');
        $ruleMap8049 = !empty($ruleMap8049) ? $ruleMap8049 : '296,50';
        ...
        foreach ($bl01Data as $record) {
            $blmc = isset($record['BLMC']) ? $record['BLMC'] : '';
            $hasHighLevelDoctor = false;
            ...
            if (strtotime($firstBlsyTime) > strtotime($exitTimeEnd)) {
                $timeoutRecords[] = [
                    'blmc' => $blmc,
                    'time' => $firstBlsyTime,
                    'BLBH' => isset($record['BLBH']) ? $record['BLBH'] : ''
                ];
            }
        }

        if (empty($timeoutRecords)) {
            return true;
        }
        ...
        foreach ($timeoutRecords as $record) {
            $basis[] = "出院前一日或当日上级医师查房记录【" . $record['blmc'] . "】";
            $basis[] = "首次签名时间【" . $record['time'] . "（超时）】";
        }

        if (!empty($timeoutRecords[0]['BLBH'])) {
            $basis['BLBH'] = $timeoutRecords[0]['BLBH'];
        }
     ...
     * @author lzh
     */
    public function rule1010($caseRule, $ruleId, $ZYH, $onlyTimeout = false)
    {
        ...
        if (!empty($deathbl01)) {
            ...
            if (!empty($hjnr)) {
                ...
                if (preg_match('/死亡时间：(?:\{)?(\d{4}-\d{2}-\d{2} \d{2}:\d{2})(?:\})?/', $hjnr, $matches)) {
                    ...
                    //死亡时间：2025年8月28日 22:35
                    else if (preg_match('/死亡时间：(\d{4}年\d{1,2}月\d{1,2}日 \d{1,2}:\d{1,2})/', $hjnr, $matches)) {
                        $deathTime = $matches[1];
        ...
        if ($deathTime == '无') {
            ...
            // 4. 计算死亡后7天的截止时间
            $deadlineTime = date('Y-m-d H:i:s', strtotime($deathTime . ' +7 day'));

            //获取当前时间
            $currentDate = Carbon::now()->toDateTimeString();
            //$deadlineTimeEnd = date('Y-m-d H:i:s', strtotime($deadlineTime) + 7 * 24 * 3600);
            //如果当前时间小于死亡后7天截止时间就跳过
            if (strtotime($currentDate) < strtotime($deadlineTime)) {
                return true;
            }
        }


        // 3. 获取死亡病例讨论结论记录MBLB（使用id8029表示病例讨论结论记录类型）
        ...
        if (empty($records)) {
            ...
            foreach ($records as $record) {
                ...
                // 检查签名时间是否在期限内
                if (!empty($signTime)) {
                    if (strtotime($signTime) <= strtotime($deadlineTime)) {
                        $hasValidRecord = true;
                        break;
                    } else {
                        // 签名时间超过期限
                        if ($onlyTimeout) {
                            ...
                            // 超时由rule20014处理，rule1010只质控是否创建或签名。
                            $hasValidRecord = true;
                            break;
                        }
                    }
     ...
     * @author lzh
     */
    public function rule20014($caseRule, $ruleId, $ZYH)
    {
        return $this->rule1010($caseRule, $ruleId, $ZYH, true);
    }
        ...
        foreach ($wjzData as $wjz) {
            ...
            if (!empty($bl01Data)) {
                foreach ($bl01Data as $record) {
                    ...
                    if (empty($firstBlsyTime)) {
                        ...
                        // rule1011只质控是否创建、是否签名，签名超时由rule20006处理。
                        $validRecords[] = $record;
                    }
                }
            }
            ...
            if (empty($validRecords)) {
                ...
                if (!empty($invalidRecords)) {
                    ...
                    // 没有任何危急值记录
                    $detailItems[] = "对应“危急值记录”未在接收后24小时内完成，或文书标题时间早于危急值接收时间";
                }
     ...
     * @return bool
     */
    public function rule20006($caseRule, $ruleId, $ZYH)
    {
        $ruleMap8010 = RuleWordMap::query()->where('id', '=', 8010)->value('keyword');
        $ruleMap8010 = !empty($ruleMap8010) ? $ruleMap8010 : '294';
        if (strpos($ruleMap8010, ',') !== false) {
        ...
        foreach ($wjzData as $wjz) {
            ...
            $wjznrStr = '';
            if (!empty($wjznr)) {
                foreach ($wjznr as $wjzItem) {
                    $wjznrStr .= '(' . $wjzItem['WJZNR'] . ')';
                }
            }
            ...
            foreach ($bl01Data as $record) {
                ...
                if (strtotime($firstBlsyTime) <= strtotime($timeLimit)) {
                    ...
                    $timeoutRecords[] = $record;
                }
            }

            if (!empty($validRecords) || empty($timeoutRecords)) {
                continue;
            }

            $record = $timeoutRecords[0];
            $firstBlsyTime = $record[$firstBlsyTimeField];
            $basis = [];
            $basis[] = "危急值【" . $wjznrStr . "】";
            $basis[] = "接收时间【" . $wjzsj . "】";
            $basis[] = "【" . $record['BLMC'] . "】首次签名时间【" . $firstBlsyTime . "】超24小时";
            if (!empty($record['BLBH'])) {
                $basis['BLBH'] = $record['BLBH'];
            }
    ...
    public function rule1012($caseRule, $ruleId, $ZYH)
    {
        ...
        $firstBlsyTimeField = !empty($firstBlsyTimeField) ? $firstBlsyTimeField : 'first_blsy_time';
        //8053
        $ruleMap8053 = RuleWordMap::query()->where('id', '=', 8053)->value('keyword');
        $ruleMap8053 = !empty($ruleMap8053) ? $ruleMap8053 : '手术自体血'; // 输血排除关键词
        if (strpos($ruleMap8053, ',') !== false) {
            $sxExcludeKeywords = explode(',', $ruleMap8053);
        } else {
            $sxExcludeKeywords = [$ruleMap8053];
        }
        //8054
        $ruleMap8054 = RuleWordMap::query()->where('id', '=', 8054)->value('keyword');
        $ruleMap8054 = !empty($ruleMap8054) ? $ruleMap8054 : '输'; // 输血手术记录排除关键词1
        if (strpos($ruleMap8054, ',') !== false) {
            $sxBl01ExcludeKeywords = explode(',', $ruleMap8054);
        } else {
            $sxBl01ExcludeKeywords = [$ruleMap8054];
        }
        //8055 红细胞,血浆,凝血因子,血小板
        $ruleMap8055 = RuleWordMap::query()->where('id', '=', 8055)->value('keyword');
        $ruleMap8055 = !empty($ruleMap8055) ? $ruleMap8055 : '红细胞,血浆,凝血因子,血小板'; // 输血手术记录排除关键词2
        if (strpos($ruleMap8055, ',') !== false) {
            $sxBl01ExcludeKeywords2 = explode(',', $ruleMap8055);
        } else {
            $sxBl01ExcludeKeywords2 = [$ruleMap8055];
        }
        //8056 输血
        $ruleMap8056 = RuleWordMap::query()->where('id', '=', 8056)->value('keyword');
        $ruleMap8056 = !empty($ruleMap8056) ? $ruleMap8056 : '输血'; // 输血手术记录排除关键词3
        if (strpos($ruleMap8056, ',') !== false) {
            $sxBl01ExcludeKeywords3 = explode(',', $ruleMap8056);
        } else {
            $sxBl01ExcludeKeywords3 = [$ruleMap8056];
        }
        ...
        foreach ($sxData as $sx) {
            if(empty($sx['JSSJ']) || $sx['JSSJ'] == '1970-01-01 00:00:00' || $sx['JSSJ'] == '0000-00-00 00:00:00' || $sx['JSSJ'] == 'NULL' || $sx['JSSJ'] === null || $sx['JSSJ'] === '' || $sx['JSSJ'] === 'null') {
                continue;
            }
            // 提取输血结束时间
            ...
            if (!empty($bl01Data)) {
                foreach ($bl01Data as $record) {
                    ...
                    if (empty($firstBlsyTime)) {
                        ...
                        // rule1012只质控是否创建、是否签名，签名超时由rule20007处理。
                        $validRecords[] = $record;
                    }
                }
            }
            ...
            if (empty($validRecords)) {
                ...
                if (!empty($invalidRecords)) {
                    ...
                    // 没有任何输血病程记录
                    $detailItems[] = "对应“输血记录”未在输血结束后24小时内完成，或文书标题时间早于输血结束时间";
                }
     ...
     * @return bool
     */
    public function rule20007($caseRule, $ruleId, $ZYH)
    {
        $ruleMap8031 = RuleWordMap::query()->where('id', '=', 8031)->value('keyword');
        $ruleMap8031 = !empty($ruleMap8031) ? $ruleMap8031 : '45';
        if (strpos($ruleMap8031, ',') !== false) {
            $blTypes = explode(',', $ruleMap8031);
        } else {
            $blTypes = [$ruleMap8031];
        ...
        $ruleMap8053 = RuleWordMap::query()->where('id', '=', 8053)->value('keyword');
        $ruleMap8053 = !empty($ruleMap8053) ? $ruleMap8053 : '手术自体血';
        if (strpos($ruleMap8053, ',') !== false) {
            $sxExcludeKeywords = explode(',', $ruleMap8053);
        } else {
            $sxExcludeKeywords = [$ruleMap8053];
        ...
        $ruleMap8054 = RuleWordMap::query()->where('id', '=', 8054)->value('keyword');
        $ruleMap8054 = !empty($ruleMap8054) ? $ruleMap8054 : '输';
        if (strpos($ruleMap8054, ',') !== false) {
            $sxBl01ExcludeKeywords = explode(',', $ruleMap8054);
        } else {
            $sxBl01ExcludeKeywords = [$ruleMap8054];
        ...
        $ruleMap8055 = RuleWordMap::query()->where('id', '=', 8055)->value('keyword');
        $ruleMap8055 = !empty($ruleMap8055) ? $ruleMap8055 : '红细胞,血浆,凝血因子,血小板';
        if (strpos($ruleMap8055, ',') !== false) {
            $sxBl01ExcludeKeywords2 = explode(',', $ruleMap8055);
        } else {
            $sxBl01ExcludeKeywords2 = [$ruleMap8055];
        ...
        $ruleMap8056 = RuleWordMap::query()->where('id', '=', 8056)->value('keyword');
        $ruleMap8056 = !empty($ruleMap8056) ? $ruleMap8056 : '输血';
        if (strpos($ruleMap8056, ',') !== false) {
            $sxBl01ExcludeKeywords3 = explode(',', $ruleMap8056);
        } else {
            $sxBl01ExcludeKeywords3 = [$ruleMap8056];
     ...
     * @author lzh
     */
    public function rule1013($caseRule, $ruleId, $ZYH, $onlyTimeout = false)
    {
        ...
        // 转科医嘱配置
        $ruleMap8034 = RuleWordMap::query()->where('id', '=', 8034)->value('keyword');
        $ruleMap8034 = !empty($ruleMap8034) ? $ruleMap8034 : '转科'; // 转科医嘱关键词
        ...
        for ($cycleIndex = 0; $cycleIndex < $cycleTotalCount; $cycleIndex++) {
            ...
            // 查询转科医嘱
            if (strpos($ruleMap8034, ',') !== false) {
                $yzKeywords = explode(',', $ruleMap8034);
            } else {
                $yzKeywords = [$ruleMap8034];
            }
            $yzKeywords = array_filter(array_map('trim', $yzKeywords));
            ...
            try {
                ...
                if (!empty($yzKeywords)) {
                    $yzbDataList = Yzb::query()
                        ...
                        ->where('KZSJ', '<=', $cycleEndTime)
                        ->where(function ($query) use ($yzKeywords) {
     ...
     * @author lzh
     */
    public function rule20013($caseRule, $ruleId, $ZYH)
    {
        return $this->rule1013($caseRule, $ruleId, $ZYH, true);
    }
    ...
    public function rule1014($caseRule, $ruleId, $ZYH)
    {
        ...
        // 1. 获取配置字段映射
        // 获取转出医嘱关键词
        $ruleMap8035 = RuleWordMap::query()->where('id', '=', 8035)->value('keyword');
        $ruleMap8035 = !empty($ruleMap8035) ? $ruleMap8035 : '转出';
        ...
        if (strpos($ruleMap8035, ',') !== false) {
            $zcKeywords = explode(',', $ruleMap8035);
        } else {
            $zcKeywords = [$ruleMap8035];
        }
        $zcKeywords = array_filter(array_map('trim', $zcKeywords));
        if (empty($zcKeywords)) {
            return true;
        }
        ...
        try {
            $yzbDataList = Yzb::query()
                ->where('ZYH', '=', $ZYH)
                ->where(function ($query) use ($zcKeywords) {
        ...
        foreach ($yzbData[0] as $order) {
            ...
            $excludeKeywords = array_filter(array_map('trim', $excludeKeywords));

            try {
                $excludeDataList = [];
                if (!empty($excludeKeywords)) {
                    $excludeDataList = Yzb::query()
                        ->where('ZYH', '=', $ZYH)
                        ->where('KZSJ', '>=', $beforeStartTime)
                        ->where('KZSJ', '<=', $startTime)
                        ->where(function ($query) use ($excludeKeywords) {
                            foreach ($excludeKeywords as $keyword) {
                                $query->orWhere('YZMC', 'like', '%' . $keyword . '%');
                            }
                        })
                ...
                $excludeData = [$excludeDataList, count($excludeDataList)];

                if (!empty($excludeData[0])) {
                    // 存在抢救/病危/急会诊医嘱，跳过此医嘱的质控
                    continue;
                }
            } catch (\Exception $e) {
                Log::error("rule-$ruleId-$ZYH-exclude-check-error: " . $e->getMessage());
                // 出错时继续进行质控，避免漏检
            }
            ...
            try {
                ...
                $bl01Data = EMR_BL_BL01::query()
                    ...
                    ->where($ruleMap8011, '<=', $startTime) // 执行时间小于等于开嘱时间
     ...
     * 检查规则：
     * 1. 换床类型HCLX=2作为转入时间
     * 2. 转入后超过24小时仍未创建转入记录，或已创建但未签名
     * 3. 已签名但首次签名时间超24小时由rule20015处理
     * @param $caseRule
     * @param $ruleId
     * @param $ZYH
     * @return bool
        ...
        foreach ($hcmx as $hcmxitem) {
            ...
            try {
                ...
                $bl01Data = EMR_BL_BL01::query()
                    ...
                    ->where($ruleMap8011, '>=', $startTime) // 执行时间大于等于换床时间
                    ->where($ruleMap8011, '<=', $oneDayAfterSurgery)
                    ->get()
                    ->toArray();
                ...
                if (empty($bl01Data)) {
                    ...
                    //$orderBasis[] = "医嘱名称【" . $yzmc . "】";
                    //$orderBasis[] = "开嘱时间【" . $kzsj . "】";
                    $orderBasis[] = "转入时间【" . $startTime . "】";

                    // 添加无转入记录信息
                    $orderBasis[] = "转入记录【未创建或提前创建】";
                    //$orderBasis[] = "首次签名时间【无】";

                    // 将这个医嘱的质控结果添加到总结果中
                    $allBasisGroups[] = $orderBasis;
                } else {
                    // 1015只处理未签名，已签名但超时由rule20015处理
                    $hasSignedRecord = false;

                    foreach ($bl01Data as $record) {
                        $firstBlsyTime = isset($record[$firstBlsyTimeField]) ? $record[$firstBlsyTimeField] : '';

                        if (!empty($firstBlsyTime)) {
                            $hasSignedRecord = true; // 已签名记录不在1015中报超时
                            break;
                        }
                    }
     ...
     * 检查规则：
     * 1. 换床类型HCLX=2作为转入时间
     * 2. 已创建转入记录且首次签名时间超过转入时间24小时
     * 3. 未创建或未签名由rule1015处理
     * @param $caseRule
     * @param $ruleId
     * @param $ZYH
     * @return bool
     * @author lzh
     */
    public function rule20015($caseRule, $ruleId, $ZYH)
    {
        // 获取转入记录MBLB值
        $ruleMap8033 = RuleWordMap::query()->where('id', '=', 8033)->value('keyword');
        $ruleMap8033 = !empty($ruleMap8033) ? $ruleMap8033 : '30';
        ...
        foreach ($hcmx as $hcmxitem) {
            ...
            foreach ($bl01Data as $record) {
                ...
                if (strtotime($firstBlsyTime) <= strtotime($endCheckTime)) {
                    $hasValidRecord = true;
                    break;
                }
            ...
            if (!$hasValidRecord && !empty($timeoutRecords)) {
                $orderBasis = [];
                $orderBasis[] = "转入时间【" . $startTime . "】";

                foreach ($timeoutRecords as $timeoutRecord) {
                    $orderBasis[] = "转入记录【" . $timeoutRecord['blmc'] . "】";
                    $orderBasis[] = "首次签名时间【" . $timeoutRecord['time'] . "（超时）】";
                }
    ...
    public function rule1063($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($cultivationResults as $result) {
            ...
            // 5. 查询报告时间到72小时内的病程记录；HJNR 需关联 EMR_BL_BLXG 获取
            $hjnrKeywords = array_values(array_filter(array_map('trim', $rule8080)));
            if (!empty($xjmc)) {
                $hjnrKeywords[] = $xjmc;
            }
            $hjnrKeywords = array_values(array_unique(array_filter($hjnrKeywords, function ($keyword) {
            ...
            if (!empty($hjnrKeywords)) {
                $resDataList = EMR_BL_BL01::query()
                    ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
                    ->where('EMR_BL_BL01.JZHM', $ZYH)
                    ->whereIn('EMR_BL_BL01.MBLB', $bcRecordTypes)
                    ->where('EMR_BL_BL01.' . $rule8011, '>=', $reportTime)
                    ->where('EMR_BL_BL01.' . $rule8011, '<=', $deadline)
                    ->where(function ($query) use ($hjnrKeywords) {
                        foreach ($hjnrKeywords as $keyword) {
                            $query->orWhere('EMR_BL_BLXG.HJNR', 'like', '%' . $keyword . '%');
                        }
    ...
    public function rule1064($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $rule8011 = RuleWordMap::query()->where('id', 8011)->value('keyword');
        $rule8011 = !empty($rule8011) ? $rule8011 : 'ZXSJ';
        // 查询符合条件的病程记录：BLLB=294且BLMC含"操作记录"；HJNR 需关联 EMR_BL_BLXG 获取
        $operationRecordKeywords = is_array($rule8081) ? $rule8081 : [$rule8081];
        $operationRecordKeywords = array_values(array_filter(array_map('trim', $operationRecordKeywords)));
        $resDataList = [];
        if (!empty($operationRecordKeywords)) {
            $resDataList = EMR_BL_BL01::query()
                ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
                ->where('EMR_BL_BL01.JZHM', $ZYH)
                ->whereIn('EMR_BL_BL01.BLLB', $rule8010)
                ->where(function ($query) use ($operationRecordKeywords) {
                    foreach ($operationRecordKeywords as $keyword) {
                        $query->orWhere('EMR_BL_BL01.BLMC', 'like', '%' . $keyword . '%');
                    }
                })
                ->get([
                    'EMR_BL_BL01.*',
        ...
        if (empty($resData[0]) || empty($resData[0][0])) {
            return true;
        }
    ...
    public function rule1215($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        if (!$this->isMedicalRecordSigned($SWJL[0]['BLBH'] ?? '')) {
            return [];
        }
    ...
    public function rule1219($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $basisList = [];
        //获取所有病程记录；HJNR 需关联 EMR_BL_BLXG 获取
        $newDataList = EMR_BL_BL01::query()
            ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
            ->where('EMR_BL_BL01.JZHM', $ZYH)
            ->whereIn('EMR_BL_BL01.MBLB', $rule8087)
            ->get([
                'EMR_BL_BL01.*',
                'EMR_BL_BLXG.HJNR as HJNR',
            ])
            ->toArray();
        $newData = [$newDataList, count($newDataList)];
        if (empty($newData[0]) || empty($newData[0][0])) {
            return [];
        }


        foreach ($newData[0] as $item) {
            if (!$this->isMedicalRecordSigned($item['BLBH'] ?? '')) {
                continue;
            }
    ...
    public function rule1223($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $newDataList = EMR_BL_BL01::query()
            ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
            ->where('EMR_BL_BL01.JZHM', $ZYH)
            ->whereIn('EMR_BL_BL01.MBLB', $rule8090)
            ->get([
                'EMR_BL_BL01.*',
                'EMR_BL_BLXG.HJNR as HJNR',
            ])
    ...
    public function rule1224($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $rule8091 = array_filter(array_map('trim', $rule8091));
        $newDataList = [];
        if (!empty($rule8091)) {
            $newDataList = EMR_BL_BL01::query()
                ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
                ->where('EMR_BL_BL01.JZHM', $ZYH)
                ->where('EMR_BL_BL01.MBLB', 294)
                ->where(function ($query) use ($rule8091) {
                    foreach ($rule8091 as $keyword) {
                        $query->orWhere('EMR_BL_BL01.BLMC', 'like', '%' . $keyword . '%');
                    }
                })
                ->get([
    ...
    public function rule1226($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        try {
            ...
            foreach ($bl01Records as $bl01Record) {
                ...
                if (!$this->isMedicalRecordSigned($blbh)) {
                    continue;
                }
    ...
    public function rule1230($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        foreach ($BCJL as $item) {
            ...
            $firstBlsyTime = $item['first_blsy_time'] ?? '';
            $firstBlsyTimestamp = !empty($firstBlsyTime) ? strtotime($firstBlsyTime) : false;
            if ($firstBlsyTimestamp !== false && time() < $firstBlsyTimestamp + 24 * 60 * 60) {
                continue;
            }
    ...
    public function rule1239($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $cyjl = $cyjl->toArray();
        if (!$this->isMedicalRecordSigned($cyjl['BLBH'] ?? '')) {
            return [];
        }
        $rykq = $cyjl['RYQK'] ?? ''; // 入院情况
    ...
    public function rule1242($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $swjl = $swjl->toArray();
        if (!$this->isMedicalRecordSigned($swjl['BLBH'] ?? '')) {
            return [];
        }
        $zljg = $swjl['ZLJG'] ?? ''; // 诊疗经过
    ...
    public function rule1247($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $newDataList = EMR_BL_BL01::query()
            ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
            ->where('EMR_BL_BL01.JZHM', $ZYH)
            ->whereIn('EMR_BL_BL01.MBLB', $mblbTypes)
            ->get([
                'EMR_BL_BL01.*',
                'EMR_BL_BLXG.HJNR as HJNR',
            ])
    ...
    public function rule1251($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        // 2. 查询符合条件的操作记录：BLLB=294且BLMC含"操作记录"
        $rule8081 = array_filter(array_map('trim', $rule8081));
        $resDataList = [];
        if (!empty($rule8081)) {
            $resDataList = EMR_BL_BL01::query()
                ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
                ->where('EMR_BL_BL01.JZHM', $ZYH)
                ->whereIn('EMR_BL_BL01.BLLB', $rule8010)
                ->where(function ($query) use ($rule8081) {
                    foreach ($rule8081 as $keyword) {
                        $query->orWhere('EMR_BL_BL01.BLMC', 'like', '%' . $keyword . '%');
                    }
                })
                ->get([
        ...
        foreach ($bl01 as $record) {
            ...
            //hjnr提取blmc1之后的内容
            /* if (!empty($BLMC1)) {
                $hjnrStartPos = strpos($HJNR, $BLMC1);
                if ($hjnrStartPos === false) {
                    continue;
                }
                $HJNR = substr($HJNR, $hjnrStartPos);
            } */
    ...
    public function rule1256($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($surgeryData as $surgery) {
            ...
            //查询在当前手术的开始时间前两个小时结束的手术
            $previousSurgery = Bllb303::query()
                ->where('ZYH', '=', $ZYH)
                ->where('SSKSSJ', '<', $surgeryStartTime)
                ->where('SSJSSJ', '>', date('Y-m-d H:i:s', strtotime($surgeryStartTime) - 2 * 3600))
                ->get()
                ->toArray();

            //查询在当前手术的结束时间后两个小时开始的手术
            $nextSurgery = Bllb303::query()
                ->where('ZYH', '=', $ZYH)
                ->where('SSKSSJ', '>', $surgeryEndTime)
                ->where('SSKSSJ', '<', date('Y-m-d H:i:s', strtotime($surgeryEndTime) + 2 * 3600))
                ->get()
                ->toArray();

            //查询手术开始时间或结束时间在当前手术的开始时间和结束时间之间的
            $betweenSurgery = Bllb303::query()
                ->where('ZYH', '=', $ZYH)
                ->where(function ($query) use ($surgeryStartTime, $surgeryEndTime) {
                    $query->where('SSKSSJ', '>=', $surgeryStartTime)
                        ->where('SSKSSJ', '<=', $surgeryEndTime)
                        ->orWhere('SSJSSJ', '>=', $surgeryStartTime)
                        ->where('SSJSSJ', '<=', $surgeryEndTime);
                })
                ->get()
                ->toArray();
            ...
            if (!empty($betweenSurgery)) {
                foreach ($betweenSurgery as $item) {
                    if (empty($item['SSZ'])) {
                        continue;
                    }
                    $basis[] = "手术名称【" . $item['SSMC'] . "】";
                    $basis[] = "术者【" . $item['SSZ'] . "】";
                    if (!empty($item['YIZHU'])) {
                        $basis[] = "一助【" . $item['YIZHU'] . "】";
                    }
            ...
            if (!empty($previousSurgery)) {
                foreach ($previousSurgery as $item) {
                    if (empty($item['SSZ'])) {
                        continue;
                    }
                    $basis[] = "手术名称【" . $item['SSMC'] . "】";
            ...
            if (!empty($nextSurgery)) {
                foreach ($nextSurgery as $item) {
                    if (empty($item['SSZ'])) {
                        continue;
                    }
                    $basis[] = "手术名称【" . $item['SSMC'] . "】";
                        ...
                        foreach ($blsyNames as $blsyName) {
                            ...
                            if (!empty($previousSurgery)) {
                                foreach ($previousSurgery as $item) {
                                    ...
                                    if (strpos($blsyName, $item['SSZ']) !== false || strpos($item['SSZ'], $blsyName) !== false) {
                                        $isbh = true;
                                        break;
                                    }
                                    if (!empty($item['YIZHU'])) {
                                        if (strpos($blsyName, $item['YIZHU']) !== false || strpos($item['YIZHU'], $blsyName) !== false) {
                                            $isbh = true;
                                            break;
                                        }
                                    }
                                    if (!empty($item['ERZHU'])) {
                                        if (strpos($blsyName, $item['ERZHU']) !== false || strpos($item['ERZHU'], $blsyName) !== false) {
                                            $isbh = true;
                                            break;
                                        }
                                    }
    ...
    public function rule1257($caseRule, $ruleId, $ZYH)
    {
        ...
        if (strpos($ruleMap8026, ',') !== false) {
            $rescueOrderKeywords = explode(',', $ruleMap8026);
        } else {
            $rescueOrderKeywords = [$ruleMap8026];
        }
        $rescueOrderKeywords = array_filter(array_map('trim', $rescueOrderKeywords));

        if (empty($rescueOrderKeywords)) {
            return true;
        }

        // 4. 查询所有抢救医嘱并统计数量
        try {
            $rescueOrders = Yzb::query()
                ->where('ZYH', '=', $ZYH)
                ->where(function ($query) use ($rescueOrderKeywords) {
                    foreach ($rescueOrderKeywords as $keyword) {
                        $query->orWhere('YZMC', 'like', '%' . $keyword . '%');
                    }
                })
                ->orderBy('KZSJ', 'asc')
            ...
            Log::error("rule-$ruleId-$ZYH-查询医嘱error: " . $e->getMessage());
            return true;
        }

        // 5. 数量一致不质控，数量不一致时输出列表依据
        if (count($rescueRecords) == count($rescueOrders)) {
            return true;
        }
        ...
        if(!empty($rescueOrders)){
            foreach ($rescueOrders as $order) {
                $basis[] = "医嘱名称【" . ($order['YZMC'] ?? '') . "】开嘱时间【" . ($order['KZSJ'] ?? '') . "】";
            }
        }else{
            $basis[] = "无抢救医嘱";
        }
    ...
    public function rule1258($caseRule, $ruleId, $ZYH)
    {
        ...
        try {
            $records = EMR_BL_BL01::query()
                ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
                ->where('EMR_BL_BL01.JZHM', $ZYH)
                ->whereIn('EMR_BL_BL01.MBLB', $recordTypes)
                ->get([
                    'EMR_BL_BL01.*',
                    'EMR_BL_BLXG.HJNR as HJNR',
                ])
    ...
    /**
     * 规则1260：知情同意书未体现先告后知（所有知情同意书医师先签名，患者后签名）
     * @param array $caseRule 规则配置
     * @param int $ruleId 规则ID
     * @param string $ZYH 住院号
     * @return array
     */
    public function rule1260($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];

        // 1. 从RuleWordMap获取BLMC匹配关键词配置（id=8105）
        $blmcKeyword = RuleWordMap::query()->where('id', 8105)->value('keyword');
        if (empty($blmcKeyword)) {
            // 如果没有配置，使用默认值"同意书"
            $blmcKeyword = '同意书';
        }

        // 2. 查询BLLB=329且BLMC包含指定关键词的病历记录，需要获取HTML_PRINT字段
        $consentForms = EMR_BL_BL01::query()
            ->where('JZHM', $ZYH)
            ->where('BLLB', 329)
            ->where('BLMC', 'like', '%' . $blmcKeyword . '%')
            ->get(['BLBH', 'BLMC', 'HTML_PRINT'])
            ->toArray();
        ...
        foreach ($consentForms as $form) {
            $blbh = $form['BLBH'] ?? '';
            $blmc = $form['BLMC'] ?? '';
            $htmlPrint = $form['HTML_PRINT'] ?? '';

            if (empty($blbh) || empty($htmlPrint)) {
                continue;
            }

            // 判断是否为手术同意书或手术知情同意书
            $isSurgeryConsent = false;
            if (strpos($blmc, '手术同意书') !== false || strpos($blmc, '手术知情同意书') !== false) {
                $isSurgeryConsent = true;
            }

            // 如果是手术同意书，执行签名时间提取和质控逻辑
            if ($isSurgeryConsent) {
                // 4. 从HTML_PRINT中提取医生签名时间（主刀医师签名时间）
                $doctorSignTime = null;
                // 匹配：data-id="主刀医师签名时间" 后面的时间值
                // 示例：<span title="主刀医师签名时间" data-id="主刀医师签名时间" ... tabindex="12" readonly="readonly">2026-01-13 10:43</span>
                if (preg_match('/data-id="主刀医师签名时间"[^>]*>([^<]*)</', $htmlPrint, $matches)) {
                    $doctorSignTime = trim($matches[1]);
                    // 过滤无效时间
                    if (empty($doctorSignTime) || $doctorSignTime == '主刀医师签名时间') {
                        $doctorSignTime = null;
                    }
                }

                // 5. 从HTML_PRINT中提取患者签名时间
                $patientSignTime = null;

                // 首先尝试提取"患者签名时间"
                if (preg_match('/data-id="患者签名时间"[^>]*>([^<]*)</', $htmlPrint, $matches)) {
                    $patientSignTime = trim($matches[1]);
                    if (empty($patientSignTime) || $patientSignTime == '患者签名时间' || $patientSignTime == '签名时间') {
                        $patientSignTime = null;
                    }
                }

                // 如果没有患者签名时间，尝试提取"被授权人签名时间"
                if (is_null($patientSignTime)) {
                    if (preg_match('/data-id="被授权人签名时间"[^>]*>([^<]*)</', $htmlPrint, $matches)) {
                        $patientSignTime = trim($matches[1]);
                        if (empty($patientSignTime) || $patientSignTime == '被授权人签名时间' || $patientSignTime == '签名时间') {
                            $patientSignTime = null;
                        }
                    }
                }

                // 6. 判断：如果医生签名或患者签名任意一个提取不到，跳过不质控
                if (is_null($doctorSignTime) || is_null($patientSignTime)) {
                    continue;
                }

                // 7. 比对签名时间：医生应该先签名，患者后签名
                $doctorTimestamp = strtotime($doctorSignTime);
                $patientTimestamp = strtotime($patientSignTime);

                // 违规：医生签名时间晚于或等于患者签名时间
                if ($doctorTimestamp >= $patientTimestamp) {
                    $basis = [];
                    $basis[] = '病历名称【' . $blmc . '】';
                    $basis[] = '医师签名时间【' . $doctorSignTime . '】';
                    $basis[] = '患者签名时间【' . $patientSignTime . '】';
                    $basis[] = '患者签名时间早于或等于医生签名时间，未体现先告后知原则（应为医生先签名，患者后签名）';
    ...
    /**
     * 规则1306：特殊检查/特殊治疗知情同意书无医师签字
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“特殊检查/特殊治疗知情同意书”的文书进行质控
     * 3. 按 BLBH 关联 BLSY，仅判断 FG_ACTIVE=1 且 QMLX=1 的医师签名是否存在
     */
    public function rule1306($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getTszlConsentForms($ZYH);
    ...
    /**
     * 规则1307：特殊检查/特殊治疗知情同意书无患方签字
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“特殊检查/特殊治疗知情同意书”的文书进行质控
     * 3. 按 BLBH 关联 BLSY，仅判断 FG_ACTIVE=1 且 QMLX=2 的患方/代理人签名是否存在
     */
    public function rule1307($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getTszlConsentForms($ZYH);
    ...
    /**
     * 规则1308：特殊检查/特殊治疗知情同意书内容填写不全，有空项
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“特殊检查/特殊治疗知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 提取纯文本，判断头部基础信息是否存在空项
     */
    public function rule1308($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getTszlConsentForms($ZYH);
    ...
    /**
     * 规则1309：特殊检查/特殊治疗知情同意书中“拟施特殊检查/特殊治疗项目名称”未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“特殊检查/特殊治疗知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 提取纯文本，判断“拟施特殊检查/特殊治疗项目名称”是否为空
     */
    public function rule1309($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getTszlConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            if ($this->consentFieldEmpty($form['XMMC'] ?? '')) {
                $basisList[] = ['【' . $blmc . '】拟施特殊检查/特殊治疗项目名称未填写'];
            }
        }
    ...
    /**
     * 规则1310：特殊检查/特殊治疗知情同意书中“拟施特殊检查/特殊治疗项目目的”未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“特殊检查/特殊治疗知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 提取纯文本，判断“拟施特殊检查/特殊治疗项目目的”是否为空
     */
    public function rule1310($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getTszlConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            if ($this->consentFieldEmpty($form['XMMD'] ?? '')) {
                $basisList[] = ['【' . $blmc . '】拟施特殊检查/特殊治疗项目目的未填写'];
            }
        }
    ...
    /**
     * 规则1311：特殊检查/特殊治疗知情同意书中替代方案未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“特殊检查/特殊治疗知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 提取纯文本，判断替代方案是否为空
     */
    public function rule1311($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getTszlConsentForms($ZYH);
    ...
    /**
     * 规则1312：特殊检查/特殊治疗知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“特殊检查/特殊治疗知情同意书”的文书进行质控
     * 3. 签名时间统一取 BLSY 中 FG_ACTIVE=1 的 JLSJ，对比 QMLX=1 与 QMLX=2
     */
    public function rule1312($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getTszlConsentForms($ZYH);
    ...
    /**
     * 规则1313：特殊检查/特殊治疗知情同意书中医师签字时间未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“特殊检查/特殊治疗知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 提取纯文本，判断“医师签名：签名时间：”后是否缺少时间值
     */
    public function rule1313($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getTszlConsentForms($ZYH);
    ...
    /**
     * 规则1314：特殊检查/特殊治疗知情同意书中患方签字时间未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“特殊检查/特殊治疗知情同意书”的文书进行质控
     * 3. 若患者或代理人已签字，则对应签字时间不能为空；若患方侧完全未签且时间也为空，同样质控
     */
    public function rule1314($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getTszlConsentForms($ZYH);
    ...
    /**
     * 规则1315：特殊检查/特殊治疗知情同意书中患方意见未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“特殊检查/特殊治疗知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT / 文本中判断患方意见是否仍为占位或空白
     */
    public function rule1315($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $consentForms = $this->getTszlConsentForms($ZYH);
    ...
    /**
     * 规则1316：特殊检查/特殊治疗知情同意书中代理人与患者关系未填
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“特殊检查/特殊治疗知情同意书”的文书进行质控
     * 3. 如存在代理人签字时间，则判断“被授权人与患者关系”是否为空；无代理人签字时间则不质控
     */
    public function rule1316($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getTszlConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            if ($this->consentFieldEmpty($form['DLRQMSJ'] ?? '')) {
                continue;
            }
    ...
    protected function getBdqpConsentKeyword()
    {
        return '术中冰冻切片检查知情同意书';
    }

    protected function getSxzqtysConsentKeyword()
    {
        return '输血治疗知情同意书';
    }

    protected function getTszlConsentKeyword()
    {
        return '特殊检查/特殊治疗知情同意书';
    }

    protected function consentFieldEmpty($value = '')
    {
        return trim((string)$value) === '';
    }

    protected function hasDoctorSignatureByBlsy($blbh = '')
    {
        if (empty($blbh)) {
            return false;
        }
    ...
    protected function hasSurgeonSignatureByBlsy($blbh = '')
    {
        if (empty($blbh)) {
            return false;
        }
    ...
    protected function hasPatientSignatureByBlsy($blbh = '')
    {
        if (empty($blbh)) {
            return false;
        }
    ...
    protected function hasAgentSignatureByBlsy($blbh = '')
    {
        if (empty($blbh)) {
            return false;
        }
    ...
    protected function hasPatientOpinionByBlsy($blbh = '')
    {
        if (empty($blbh)) {
            return false;
        }
    ...
    protected function getConsentPatientOpinionMissingForms($consentForms = [])
    {
        $basisList = [];
        ...
        foreach ($consentForms as $form) {
            ...
            if (!$this->hasPatientOpinionByBlsy($blbh)) {
                $basisList[] = ['【' . $blmc . '】患方意见未填写'];
            }
        }
    ...
    protected function getConsentHeaderMissingFields($form = [])
    {
        $basis = [];

        if ($this->consentFieldEmpty($form['XM'] ?? '')) {
            $basis[] = '姓名为空';
        }

        if ($this->consentFieldEmpty($form['XB'] ?? '')) {
            $basis[] = '性别为空';
        }

        if ($this->consentFieldEmpty($form['NL'] ?? '')) {
            $basis[] = '年龄为空';
        }

        if ($this->consentFieldEmpty($form['KS'] ?? '')) {
            $basis[] = '科室为空';
        }

        if ($this->consentFieldEmpty($form['BAH'] ?? '')) {
            $basis[] = '病案号为空';
        }
    ...
    protected function getConsentSignTimeMissingFields($form = [])
    {
        $basis = [];
        $hasPatientSign = (int)($form['SFHZQM'] ?? 0) === 1;
        $hasAgentSign = (int)($form['SFDLRQM'] ?? 0) === 1;
        $patientTimeEmpty = $this->consentFieldEmpty($form['HZQMSJ'] ?? '');
        $agentTimeEmpty = $this->consentFieldEmpty($form['DLRQMSJ'] ?? '');

        if ($hasPatientSign && $patientTimeEmpty) {
            $basis[] = '患者签字时间未填写';
        }

        if ($hasAgentSign && $agentTimeEmpty) {
            $basis[] = '代理人签字时间未填写';
        }

        if (!$hasPatientSign && !$hasAgentSign && $patientTimeEmpty && $agentTimeEmpty) {
            $basis[] = '患方签字时间未填写';
        }
    ...
    protected function shouldSkipDoctorPatientSignatureRuleGroup($form = [])
    {
        $blbh = $form['BLBH'] ?? '';
        if (empty($blbh)) {
            return false;
        ...
        $hasDoctorSignature = $this->hasDoctorSignatureByBlsy($blbh);
        $hasPatientSignature = $this->hasPatientSignatureByBlsy($blbh);
        $hasDoctorSignTime = $this->normalizeConsentComparableTime($form['YSQMSJ'] ?? '') !== '';
        $hasPatientSignTime = !empty($this->getConsentPatientSignTimeInfo($form));
        $hasPatientOpinion = $this->hasPatientOpinionByBlsy($blbh);

        return !$hasDoctorSignature
            && !$hasPatientSignature
            && !$hasDoctorSignTime
            && !$hasPatientSignTime
            && !$hasPatientOpinion;
    ...
    protected function normalizeConsentComparableTime($value = '')
    {
    ...
    protected function getConsentPatientSignTimeInfo($form = [])
    {
        $candidates = [];
        $fieldMap = [
            ['field' => 'HZQMSJ', 'label' => '患者签字时间'],
            ['field' => 'DLRQMSJ', 'label' => '代理人签字时间'],
            ['field' => 'DLRQMSJ2', 'label' => '代理人签字时间'],
        ];

        foreach ($fieldMap as $field) {
            $normalizedTime = $this->normalizeConsentComparableTime($form[$field['field']] ?? '');
            if ($normalizedTime === '') {
                continue;
            }
    ...
    protected function getConsentTimeOrderViolationBasis($form = [])
    {
        $doctorSignTime = $this->normalizeConsentComparableTime($form['YSQMSJ'] ?? '');
        $patientSignInfo = $this->getConsentPatientSignTimeInfo($form);

        if ($doctorSignTime === '' || empty($patientSignInfo)) {
            return [];
        }

        if (strtotime($doctorSignTime) <= $patientSignInfo['timestamp']) {
            return [];
        }

        return [
            '医师签字时间【' . $doctorSignTime . '】',
            $patientSignInfo['label'] . '【' . $patientSignInfo['time'] . '】',
            '医师签字时间晚于患方签字时间，未体现先告后知',
        ];
    }

    protected function getTszlConsentForms($ZYH = "")
    {
        $forms = Bllb329Tszl::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();

        return $this->filterConsentFormsByPreQualityConditions($forms, $ZYH);
    }

    protected function getYzccstysConsentForms($ZYH = "")
    {
        $forms = Bllb329Yzccstys::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getTszlOrSszqtysConsentForms($ZYH = "")
    {
        return array_merge(
            $this->getTszlConsentForms($ZYH),
            $this->getSszqtysConsentForms($ZYH)
        );
    }

    protected function isTszlConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getTszlConsentKeyword();

        return strpos($blmc, $keyword) !== false || strpos($htmlPrint, $keyword) !== false;
    }

    protected function getTszlConsentText($htmlPrint = '')
    {
        if (empty($htmlPrint)) {
            return '';
        }

        $dom = removeHtmlAndHiddenElements($htmlPrint, true);
        $text = $dom->textContent ?? '';

        return preg_replace('/\s+/u', '', $text);
    }

    protected function normalizeRuleMatchText($text = '')
    {
    ...
    protected function getTszlMissingRuleKeywords()
    {
        return $this->getRuleWordMapKeywordList(8200);
    }

    protected function getTszlOperationRecordKeywords()
    {
        return $this->getRuleWordMapKeywordList(8201);
    }

    protected function getBdqpMissingRuleKeywords()
    {
        return $this->getRuleWordMapKeywordList(8202);
    }

    protected function getBwbztysOrderKeywords()
    {
        return $this->getRuleWordMapKeywordList(8205);
    }

    protected function getYzccstysKeywords()
    {
        return $this->getRuleWordMapKeywordList(8206);
    }

    protected function getCtzqOrderCtKeywords()
    {
        return $this->getRuleWordMapKeywordList(8203);
    }

    protected function getCtzqOrderEnhanceKeywords()
    {
        return $this->getRuleWordMapKeywordList(8204);
    }

    protected function getRuleWordMapKeywordList($ruleWordMapId = 0)
    {
        static $keywordCache = [];

        $ruleWordMapId = (int)$ruleWordMapId;
        if ($ruleWordMapId <= 0) {
            return [];
        }

        if (array_key_exists($ruleWordMapId, $keywordCache)) {
            return $keywordCache[$ruleWordMapId];
        }

        $keywordText = (string)RuleWordMap::query()->where('id', $ruleWordMapId)->value('keyword');
        $keywordList = preg_split('/[,，]/u', $keywordText);
        $keywords = [];

        foreach ($keywordList as $keyword) {
            $keyword = $this->normalizeRuleMatchText($keyword);
            if ($keyword !== '') {
                $keywords[] = $keyword;
            }
        }

        $keywordCache[$ruleWordMapId] = array_values(array_unique($keywords));

        return $keywordCache[$ruleWordMapId];
    }

    protected function getYcczNameMap()
    {
        static $nameMap = null;
        ...
        $nameMap = [];
        $names = YCCZ::query()->pluck('name')->toArray();

        foreach ($names as $name) {
            $normalizedName = $this->normalizeRuleMatchText($name);
            if ($normalizedName !== '') {
                $nameMap[$normalizedName] = trim((string)$name);
            }
        }
    ...
    protected function getTszlRequiredSurgeryMatches($ZYH = "")
    {
        if (empty($ZYH)) {
            return [];
        }
        ...
        $matches = [];
        $keywordList = $this->getTszlMissingRuleKeywords();
        $ycczNameMap = $this->getYcczNameMap();
        ...
        foreach ($ssapList as $ssap) {
            $surgeryName = trim((string)($ssap['ICD9_SSCZMC'] ?? ''));
            $normalizedSurgeryName = $this->normalizeRuleMatchText($surgeryName);
            ...
            if (isset($ycczNameMap[$normalizedSurgeryName])) {
                $matched = true;
            }

            foreach ($keywordList as $keyword) {
                if (mb_strpos($normalizedSurgeryName, $keyword) !== false) {
                    $matched = true;
                    break;
                }
            }
            ...
            if ($matched) {
                $matches[$normalizedSurgeryName] = ['手麻手术名称【' . $surgeryName . '】'];
            }
        }
    ...
    protected function getTszlOperationStartRecords($ZYH = "")
    {
        if (empty($ZYH)) {
            return [];
        }

        $keywords = $this->getTszlOperationRecordKeywords();
        if (empty($keywords)) {
            return [];
        }
        ...
        foreach ($records as $record) {
            ...
            $hjnr = (string)EMR_BL_BLXG::query()->where('BLBH', $blbh)->value('HJNR');
            $operationStartTime = $this->extractTszlOperationStartTime($record['ZXSJ'] ?? '', $hjnr);
    ...
    protected function getYzccstysOperationStartRecords($ZYH = "")
    {
        if (empty($ZYH)) {
            return [];
        }

        $operationKeywords = $this->getTszlOperationRecordKeywords();
        $yzccsKeywords = $this->getYzccstysKeywords();
        if (empty($operationKeywords) || empty($yzccsKeywords)) {
            return [];
        }
        ...
        foreach ($records as $record) {
            $blmc = (string)($record['BLMC'] ?? '');
            $matchedOperationKeyword = '';

            foreach ($operationKeywords as $keyword) {
                if ($this->containsNormalizedKeyword($blmc, $keyword)) {
                    $matchedOperationKeyword = $keyword;
                    break;
                }
            }

            if ($matchedOperationKeyword === '') {
                continue;
            }
            ...
            $hjnr = (string)EMR_BL_BLXG::query()->where('BLBH', $blbh)->value('HJNR');
            $matchedYzccsKeyword = '';
            foreach ($yzccsKeywords as $keyword) {
                if ($this->containsNormalizedKeyword($blmc, $keyword) || $this->containsNormalizedKeyword($hjnr, $keyword)) {
                    $matchedYzccsKeyword = $keyword;
                    break;
                }
            }

            if ($matchedYzccsKeyword === '') {
                continue;
            }
            ...
            $result[] = [
                ...
                'ZXSJ' => $record['ZXSJ'] ?? '',
                'HJNR' => $hjnr,
                'operation_keyword' => $matchedOperationKeyword,
                'matched_keyword' => $matchedYzccsKeyword,
                'operation_start_time' => $operationStartTime,
            ];
        }
    ...
    protected function getBdqpRequiredSurgeryRecords($ZYH = "")
    {
        if (empty($ZYH)) {
            return [];
        }

        $keywords = $this->getBdqpMissingRuleKeywords();
        if (empty($keywords)) {
            return [];
        }

        $records = Bllb303::query()
            ->where('ZYH', $ZYH)
            ->get(['BLBH', 'BLMC', 'SSJG', 'SSMC', 'SSKSSJ'])
            ->toArray();

        $result = [];
        foreach ($records as $record) {
            $ssjg = (string)($record['SSJG'] ?? '');
            if ($ssjg === '') {
                continue;
            }
            ...
            $matchedKeywords = [];
            foreach ($keywords as $keyword) {
                if ($this->containsNormalizedKeyword($ssjg, $keyword)) {
    ...
    protected function extractTszlOperationStartTime($zxsj = '', $hjnr = '')
    {
        $recordDate = $this->normalizeConsentComparableTime($zxsj);
        $hjnr = trim((string)$hjnr);
        ...
        $datePart = date('Y-m-d', strtotime($recordDate));
        $patterns = [
            '/今日\s*(\d{1,2})时(\d{1,2})分/u',
            '/于今日\s*(\d{1,2}):(\d{1,2})/u',
            '/(\d{1,2})时(\d{1,2})分/u',
            '/(\d{1,2}):(\d{1,2})/u',
        ];
        ...
        foreach ($patterns as $pattern) {
            ...
            $hour = isset($matches[1]) ? str_pad($matches[1], 2, '0', STR_PAD_LEFT) : '';
            $minute = isset($matches[2]) ? str_pad($matches[2], 2, '0', STR_PAD_LEFT) : '';
            if ($hour === '' || $minute === '') {
                continue;
            }
    ...
    protected function getBdqpConsentForms($ZYH = "")
    {
        $forms = Bllb329Bdqp::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getSxzqtysConsentForms($ZYH = "")
    {
        $forms = Bllb329Sxzqtys::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getSszqtysConsentForms($ZYH = "")
    {
        $forms = Bllb329Sszqtys::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function isBdqpConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getBdqpConsentKeyword();

        return strpos($blmc, $keyword) !== false || strpos($htmlPrint, $keyword) !== false;
    }

    protected function isSxzqtysConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getSxzqtysConsentKeyword();

        return $this->containsNormalizedKeyword($blmc, $keyword) || $this->containsNormalizedKeyword($htmlPrint, $keyword);
    }

    protected function getBdqpConsentText($htmlPrint = '')
    {
        if (empty($htmlPrint)) {
            return '';
        }
    ...
    protected function getHtmlFieldInnerHtml($htmlPrint = '', $dataId = '')
    {
        if (empty($htmlPrint) || empty($dataId)) {
            return '';
        }

        $pattern = '/data-id="' . preg_quote($dataId, '/') . '"[^>]*>.*?<span[^>]*class="tag-value[^"]*"[^>]*>(.*?)<\/span>/su';
        if (preg_match($pattern, $htmlPrint, $matches)) {
            return $matches[1] ?? '';
        }
    ...
    protected function getHtmlFieldText($htmlPrint = '', $dataId = '')
    {
        $innerHtml = $this->getHtmlFieldInnerHtml($htmlPrint, $dataId);
        if ($innerHtml === '') {
            return '';
        }

        $text = html_entity_decode(strip_tags($innerHtml), ENT_QUOTES | ENT_HTML5);
        $text = preg_replace('/[\s\x{00A0}]+/u', '', $text);

        return trim($text);
    }

    protected function htmlFieldHasContent($htmlPrint = '', $dataId = '')
    {
        $innerHtml = $this->getHtmlFieldInnerHtml($htmlPrint, $dataId);
        if ($innerHtml === '') {
            return false;
        ...
        if (preg_match('/background-image\s*:|<img\b|<svg\b/u', $innerHtml)) {
            return true;
        }

        $text = $this->getHtmlFieldText($htmlPrint, $dataId);

        return $text !== '' && $text !== $dataId;
    }

    /**
     * 规则1317：术中冰冻切片检查知情同意书无医师签字
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“术中冰冻切片检查知情同意书”的文书进行质控
     * 3. 按 BLBH 关联 BLSY，仅判断 FG_ACTIVE=1 且 QMLX=1 的医师签名是否存在
     */
    public function rule1317($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBdqpConsentForms($ZYH);
    ...
    /**
     * 规则1318：术中冰冻切片检查知情同意书无患方签字
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“术中冰冻切片检查知情同意书”的文书进行质控
     * 3. 按 BLBH 关联 BLSY，仅判断 FG_ACTIVE=1 且 QMLX=2 的患方/代理人签名是否存在
     */
    public function rule1318($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBdqpConsentForms($ZYH);
    ...
    /**
     * 规则1319：术中冰冻切片检查知情同意书内容填写不全，有空项
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“术中冰冻切片检查知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 提取纯文本，判断头部基础信息是否存在空项
     */
    public function rule1319($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBdqpConsentForms($ZYH);
    ...
    /**
     * 规则1320：术中冰冻切片检查知情同意书中“手术中冰冻切片检查目的”未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“术中冰冻切片检查知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 提取纯文本，判断“手术中冰冻切片检查目的”是否为空
     */
    public function rule1320($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBdqpConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            if ($this->consentFieldEmpty($form['JCMD'] ?? '')) {
                $basisList[] = ['【' . $blmc . '】手术中冰冻切片检查目的未填写'];
            }
        }
    ...
    /**
     * 规则1321：术中冰冻切片检查知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“术中冰冻切片检查知情同意书”的文书进行质控
     * 3. 签名时间统一取 BLSY 中 FG_ACTIVE=1 的 JLSJ，对比 QMLX=1 与 QMLX=2
     */
    public function rule1321($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBdqpConsentForms($ZYH);
    ...
    /**
     * 规则1322：术中冰冻切片检查知情同意书中医师签字时间未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“术中冰冻切片检查知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 中读取“医师签名时间”字段判断是否为空
     */
    public function rule1322($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBdqpConsentForms($ZYH);
    ...
    /**
     * 规则1323：术中冰冻切片检查知情同意书中患方签字时间未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“术中冰冻切片检查知情同意书”的文书进行质控
     * 3. 若患者或代理人存在签字，则对应签字时间不能为空；若患方侧完全未签且时间也为空，同样质控
     */
    public function rule1323($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBdqpConsentForms($ZYH);
    ...
    /**
     * 规则1324：术中冰冻切片检查知情同意书中患方意见未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“术中冰冻切片检查知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 中读取“患方手写意见”字段，判断是否为空
     */
    public function rule1324($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $consentForms = $this->getBdqpConsentForms($ZYH);
    ...
    /**
     * 规则1325：术中冰冻切片检查知情同意书中代理人与患者关系未填
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“术中冰冻切片检查知情同意书”的文书进行质控
     * 3. 如存在代理人签字时间，则判断“被授权人与患者关系”是否为空；无代理人签字时间则不质控
     */
    public function rule1325($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBdqpConsentForms($ZYH);
    ...
    protected function getHltysConsentKeyword()
    {
        return '化疗知情同意书';
    }

    protected function getCtzqConsentKeyword()
    {
        return 'CT增强检查知情同意书';
    }

    protected function getBwbztysConsentKeyword()
    {
        return '病危（病重）通知书';
    }

    protected function getSqwtsConsentKeyword()
    {
        return '授权委托书';
    }

    protected function getJjjctysConsentKeyword()
    {
        return '拒绝检查/治疗告知书';
    }

    protected function getZdcygzsConsentKeyword()
    {
        return '自动出院/转院风险告知书';
    }

    protected function getSjgzsConsentKeyword()
    {
        return '尸检告知书';
    }

    protected function getYbwffgzsConsentKeyword()
    {
        return '医保外付费项目知情同意书';
    }

    protected function getZrltysConsentKeyword()
    {
        return '植入类或Ⅲ级医用耗材知情同意书';
    }

    protected function getHltysConsentForms($ZYH = "")
    {
        $forms = Bllb329Hltys::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getBwbztysConsentForms($ZYH = "")
    {
        $forms = Bllb329Bwbztys::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getCtzqConsentForms($ZYH = "")
    {
        $forms = Bllb329Ctzq::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getSqwtsConsentForms($ZYH = "")
    {
        $forms = Bllb329Sqwts::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getJjjctysConsentForms($ZYH = "")
    {
        $forms = Bllb34Jjjctys::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getZdcygzsConsentForms($ZYH = "")
    {
        $forms = Bllb34Zdcygzs::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getSjgzsConsentForms($ZYH = "")
    {
        $forms = Bllb34Sjgzs::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getYbwffgzsConsentForms($ZYH = "")
    {
        $forms = Bllb34Ybwffgzs::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getZrltysConsentForms($ZYH = "")
    {
        $forms = Bllb329Zrltys::query()
            ->where('ZYH', $ZYH)
            ->get()
            ->toArray();
    ...
    protected function getFirstTransfusionStartInfo($ZYH = "")
    {
        if (empty($ZYH)) {
            return [];
        }
        ...
        if (empty($record) || empty($record['KSSJ'])) {
            return [];
        }

        return [
            'time' => $record['KSSJ'],
            'cfx' => trim((string)($record['CFX'] ?? '')),
            'basis' => '首次输血开始时间【' . $record['KSSJ'] . '】',
        ];
    }

    protected function getFirstChemoOrderInfo($ZYH = "")
    {
        if (empty($ZYH)) {
            return [];
        }
        ...
        if (empty($order) || empty($order['YZMC'])) {
            return [];
        }

        return [
            'yzmc' => trim((string)($order['YZMC'] ?? '')),
            'kzsj' => $order['KZSJ'] ?? '',
        ];
    }

    protected function getFirstYzccstysOrderInfo($ZYH = "")
    {
        if (empty($ZYH)) {
            return [];
        }

        $keywords = $this->getYzccstysKeywords();
        if (empty($keywords)) {
            return [];
        }
        ...
        foreach ($orders as $order) {
            ...
            foreach ($keywords as $keyword) {
                if ($this->containsNormalizedKeyword($yzmc, $keyword)) {
                    return [
                        'yzmc' => $yzmc,
                        'kzsj' => $order['KZSJ'] ?? '',
                    ];
    ...
    protected function getFirstBwbztysOrderInfo($ZYH = "")
    {
        if (empty($ZYH)) {
            return [];
        }

        $keywords = $this->getBwbztysOrderKeywords();
        if (empty($keywords)) {
            return [];
        }
    ...
    protected function getFirstCtzqOrderInfo($ZYH = "")
    {
        if (empty($ZYH)) {
            return [];
        }

        $ctKeywords = $this->getCtzqOrderCtKeywords();
        $enhanceKeywords = $this->getCtzqOrderEnhanceKeywords();

        if (empty($ctKeywords) || empty($enhanceKeywords)) {
            return [];
        }
        ...
        foreach ($orders as $order) {
            ...
            $normalizedYzmc = mb_strtolower($this->normalizeRuleMatchText($yzmc), 'UTF-8');
            $hasCtKeyword = false;
            $hasEnhanceKeyword = false;

            foreach ($ctKeywords as $keyword) {
                $normalizedKeyword = mb_strtolower($this->normalizeRuleMatchText($keyword), 'UTF-8');
                if ($normalizedKeyword !== '' && strpos($normalizedYzmc, $normalizedKeyword) !== false) {
                    $hasCtKeyword = true;
                    break;
                }
            }
            ...
            foreach ($enhanceKeywords as $keyword) {
                $normalizedKeyword = mb_strtolower($this->normalizeRuleMatchText($keyword), 'UTF-8');
                if ($normalizedKeyword !== '' && strpos($normalizedYzmc, $normalizedKeyword) !== false) {
                    $hasEnhanceKeyword = true;
                    break;
                }
            }

            if (!$hasEnhanceKeyword) {
                continue;
            }
    ...
    protected function isHltysConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getHltysConsentKeyword();

        return strpos($blmc, $keyword) !== false || strpos($htmlPrint, $keyword) !== false;
    }

    protected function isCtzqConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getCtzqConsentKeyword();

        return strpos($blmc, $keyword) !== false || strpos($htmlPrint, $keyword) !== false;
    }

    protected function isBwbztysConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getBwbztysConsentKeyword();

        return strpos($blmc, $keyword) !== false || strpos($htmlPrint, $keyword) !== false;
    }

    protected function isSqwtsConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getSqwtsConsentKeyword();

        return strpos(preg_replace('/[\p{Z}\s]+/u', '', (string)$blmc), $keyword) !== false
            || strpos(preg_replace('/[\p{Z}\s]+/u', '', (string)$htmlPrint), $keyword) !== false;
    }

    protected function isJjjctysConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getJjjctysConsentKeyword();

        return strpos($blmc, $keyword) !== false || strpos($htmlPrint, $keyword) !== false;
    }

    protected function isZdcygzsConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getZdcygzsConsentKeyword();

        return $this->containsNormalizedKeyword($blmc, $keyword) || $this->containsNormalizedKeyword($htmlPrint, $keyword);
    }

    protected function isSjgzsConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getSjgzsConsentKeyword();

        return $this->containsNormalizedKeyword($blmc, $keyword) || $this->containsNormalizedKeyword($htmlPrint, $keyword);
    }

    protected function isYbwffgzsConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getYbwffgzsConsentKeyword();

        return $this->containsNormalizedKeyword($blmc, $keyword) || $this->containsNormalizedKeyword($htmlPrint, $keyword);
    }

    protected function isZrltysConsentForm($blmc = '', $htmlPrint = '')
    {
        $keyword = $this->getZrltysConsentKeyword();

        return $this->containsNormalizedKeyword($blmc, $keyword) || $this->containsNormalizedKeyword($htmlPrint, $keyword);
    }

    protected function getYbwffgzsRows($form = [])
    {
        $rows = json_decode($form['XMJSON'] ?? '[]', true);

        return is_array($rows) ? $rows : [];
    }

    protected function getSqwtsAgentSlots($form = [])
    {
        return [
            1 => [
                'name' => $form['DLRXM1'] ?? '',
                'age' => $form['DLRNL1'] ?? '',
                'relation' => $form['DLRGX1'] ?? '',
                'phone' => $form['DLRDH1'] ?? '',
                'signed' => (int)($form['SFDLRQM'] ?? 0) === 1,
                'sign_time' => $form['DLRQMSJ'] ?? '',
            ],
            2 => [
                'name' => $form['DLRXM2'] ?? '',
                'age' => $form['DLRNL2'] ?? '',
                'relation' => $form['DLRGX2'] ?? '',
                'phone' => $form['DLRDH2'] ?? '',
                'signed' => (int)($form['SFDLRQM2'] ?? 0) === 1,
                'sign_time' => $form['DLRQMSJ2'] ?? '',
            ],
        ];
    }

    protected function getSqwtsActiveAgentSlots($form = [])
    {
        $activeSlots = [];
        foreach ($this->getSqwtsAgentSlots($form) as $index => $slot) {
            $hasInfo = !$this->consentFieldEmpty($slot['name'])
                || !$this->consentFieldEmpty($slot['age'])
                || !$this->consentFieldEmpty($slot['relation'])
                || !$this->consentFieldEmpty($slot['phone']);

            if ($hasInfo || $slot['signed'] || !$this->consentFieldEmpty($slot['sign_time'])) {
    ...
    protected function hasValidSqwtsAgentAuthorization($sqwtsForms = [])
    {
        return $this->getSqwtsAuthorizationSummary($sqwtsForms)['has_valid'];
    }

    protected function getSqwtsAuthorizationSummary($sqwtsForms = [], $validationType = 'sign_time')
    {
        $summary = [
            'has_sqwts' => !empty($sqwtsForms),
            'has_valid' => false,
            'invalid_blmcs' => [],
        ];

        foreach ($sqwtsForms as $sqwtsForm) {
            $hasValidSlot = false;

            foreach ($this->getSqwtsActiveAgentSlots($sqwtsForm) as $slot) {
                $hasName = !$this->consentFieldEmpty($slot['name'] ?? '');
                $hasSign = $validationType === 'signed'
                    ? !empty($slot['signed'])
                    : !$this->consentFieldEmpty($slot['sign_time'] ?? '');
            ...
            if (!$hasValidSlot) {
                $summary['invalid_blmcs'][] = $sqwtsForm['BLMC'] ?? '';
            }
        }

        $summary['invalid_blmcs'] = array_values(array_filter(array_unique($summary['invalid_blmcs'])));

        return $summary;
    }

    protected function appendSqwtsInvalidBlmcBasis(array $basis = [], array $authorizationSummary = [])
    {
        foreach (($authorizationSummary['invalid_blmcs'] ?? []) as $sqwtsBlmc) {
            if ($sqwtsBlmc === '') {
                continue;
            }

            $basis[] = '授权委托书：【' . $sqwtsBlmc . '】';
        }

        return $basis;
    }

    protected function hasConsentAgentSign($form = [])
    {
        return (int)($form['SFDLRQM'] ?? 0) === 1 || !$this->consentFieldEmpty($form['DLRQMSJ'] ?? '');
    }

    protected function hasSszqtysAgentSign($form = [])
    {
        return $this->hasConsentAgentSign($form);
    }

    protected function hasTszlAgentSign($form = [])
    {
        return $this->hasConsentAgentSign($form);
    }

    protected function hasBdqpAgentSign($form = [])
    {
        return $this->hasConsentAgentSign($form);
    }

    protected function getSqwtsEarliestAgentSignTime($sqwtsForms = [])
    {
        $earliest = [];

        foreach ($sqwtsForms as $sqwtsForm) {
            foreach ($this->getSqwtsActiveAgentSlots($sqwtsForm) as $slot) {
                $signTime = $this->normalizeConsentComparableTime($slot['sign_time'] ?? '');
                if ($signTime === '') {
                    continue;
                }

                $timestamp = strtotime($signTime);
                if (empty($earliest) || $timestamp < $earliest['timestamp']) {
                    $earliest = [
                        'time' => $signTime,
                        'timestamp' => $timestamp,
                        'blmc' => $sqwtsForm['BLMC'] ?? '',
    ...
    protected function getSszqtysRequiredSurgeryPlans($ZYH = "")
    {
        if (empty($ZYH)) {
            return [];
        }
        ...
        $ssapList = SM_SSAP::query()
            ...
            ->whereNotNull('SSRQ')
            ->where('SSRQ', '<>', '')
            ->get(['ICD9_SSLB', 'ICD9_SSCZMC', 'SSRQ'])
            ->toArray();
        ...
        $result = [];
        foreach ($ssapList as $ssap) {
            $surgeryType = trim((string)($ssap['ICD9_SSLB'] ?? ''));
            if ($surgeryType === '' || (mb_strpos($surgeryType, '手术') === false && mb_strpos($surgeryType, '介入') === false)) {
                continue;
            }

            $startTime = $this->normalizeConsentComparableTime($ssap['SSRQ'] ?? '');
            if ($startTime === '') {
                continue;
            }

            $result[] = [
                'ICD9_SSLB' => $surgeryType,
                'ICD9_SSCZMC' => trim((string)($ssap['ICD9_SSCZMC'] ?? '')),
                'SSRQ' => $startTime,
            ];
        }
    ...
    protected function getCtzqCheckStartTime($ZYH = "")
    {
        if (empty($ZYH)) {
            return '';
        }
        ...
        foreach ($pacs as $item) {
            $time = $item['JYSJ'] ?: ($item['KDSJ'] ?: ($item['BGSJ'] ?: ''));
            if (!empty($time)) {
     ...
     * 规则1290：缺化疗知情同意书
     */
    public function rule1290($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $orderInfo = $this->getFirstChemoOrderInfo($ZYH);
        if (empty($orderInfo)) {
            return [];
        }

        $consentForms = $this->getHltysConsentForms($ZYH);
        if (!empty($consentForms)) {
            return [];
        }
     ...
     * 规则1326：化疗知情同意书无医师签字
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“化疗知情同意书”的文书进行质控
     * 3. 按 BLBH 关联 BLSY，仅判断 FG_ACTIVE=1 且 QMLX=1 的医师签名是否存在
     */
    public function rule1326($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getHltysConsentForms($ZYH);
     ...
     * 3. 按 BLBH 关联 BLSY，仅判断 FG_ACTIVE=1 且 QMLX=2 的患方/代理人签名是否存在
     */
    public function rule1327($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getHltysConsentForms($ZYH);
    ...
    /**
     * 规则1328：化疗知情同意书内容填写不全，有空项
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“化疗知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 提取纯文本，判断头部基础信息是否存在空项
     */
    public function rule1328($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getHltysConsentForms($ZYH);
    ...
    /**
     * 规则1329：化疗知情同意书中替代方案未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“化疗知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 中读取替代治疗方案字段判断是否为空
     */
    public function rule1329($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getHltysConsentForms($ZYH);
    ...
    /**
     * 规则1330：化疗知情同意书中医师签字时间未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“化疗知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 中读取“医师签名时间”字段判断是否为空
     */
    public function rule1330($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getHltysConsentForms($ZYH);
    ...
    /**
     * 规则1331：化疗知情同意书中患方签字时间未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“化疗知情同意书”的文书进行质控
     * 3. 若患者或代理人存在签字，则对应签字时间不能为空；若患方侧完全未签且时间也为空，同样质控
     */
    public function rule1331($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getHltysConsentForms($ZYH);
    ...
    /**
     * 规则1332：化疗知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“化疗知情同意书”的文书进行质控
     * 3. 签名时间统一取 BLSY 中 FG_ACTIVE=1 的 JLSJ，对比 QMLX=1 与 QMLX=2
     */
    public function rule1332($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getHltysConsentForms($ZYH);
    ...
    /**
     * 规则1333：化疗知情同意书中患方意见未填写
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“化疗知情同意书”的文书进行质控
     * 3. 从 HTML_PRINT 中读取“患方手写意见”字段，判断是否为空
     */
    public function rule1333($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $consentForms = $this->getHltysConsentForms($ZYH);
    ...
    /**
     * 规则1334：化疗知情同意书中代理人与患者关系未填
     * 1. 查询当前病历 BLLB=329 的文书
     * 2. 仅对 BLMC 或 HTML_PRINT 命中“化疗知情同意书”的文书进行质控
     * 3. 如存在代理人签字，则判断“被授权人与患者关系”是否为空；无代理人签字则不质控
     */
    public function rule1334($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getHltysConsentForms($ZYH);
    ...
    /**
     * 规则1335：CT增强检查知情同意书无医师签字
     */
    public function rule1335($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getCtzqConsentForms($ZYH);
    ...
    /**
     * 规则1336：CT增强检查知情同意书无患方签字
     */
    public function rule1336($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getCtzqConsentForms($ZYH);
    ...
    /**
     * 规则1337：未在检查开始前签署CT增强检查知情同意书
     */
    public function rule1337($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $orderInfo = $this->getFirstCtzqOrderInfo($ZYH);
        if (empty($orderInfo)) {
            return [];
        }

        $orderTime = $this->normalizeConsentComparableTime($orderInfo['kzsj'] ?? '');
        if ($orderTime === '') {
            return [];
        }
        ...
        foreach ($consentForms as $form) {
            ...
            $doctorTime = $this->normalizeConsentComparableTime($form['YSQMSJ'] ?? '');
            $patientTime = $this->normalizeConsentComparableTime($form['HZQMSJ'] ?? '');
            $agentTime = $this->normalizeConsentComparableTime($form['DLRQMSJ'] ?? '');

            $doctorValid = $doctorTime !== '' && strtotime($doctorTime) >= $orderTimestamp;
            $patientValid = $patientTime !== '' && strtotime($patientTime) >= $orderTimestamp;
            $agentValid = $agentTime !== '' && strtotime($agentTime) >= $orderTimestamp;

            if ($doctorValid && ($patientValid || $agentValid)) {
                $hasValidConsent = true;
                break;
            }

            if ($doctorTime !== '' && strtotime($doctorTime) < $orderTimestamp) {
                $formBasis[] = '医师签字时间【' . $doctorTime . '】，在开具医嘱之前';
            }

            if ($patientTime !== '' && strtotime($patientTime) < $orderTimestamp) {
                $formBasis[] = '患方签字时间【' . $patientTime . '】，在开具医嘱之前';
            }

            if ($agentTime !== '' && strtotime($agentTime) < $orderTimestamp) {
                $formBasis[] = '代理人签字时间【' . $agentTime . '】，在开具医嘱之前';
            }

            if (count($formBasis) > 1) {
                $basisList[] = $formBasis;
            }
        }

        if ($hasValidConsent || empty($basisList)) {
            return [];
        }

        array_unshift($basisList, [
            '医嘱名称【' . ($orderInfo['yzmc'] ?? '') . '】',
            '开嘱时间【' . $orderTime . '】',
        ]);
    ...
    /**
     * 规则1338：CT增强检查知情同意书内容填写不全，有空项
     */
    public function rule1338($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getCtzqConsentForms($ZYH);
    ...
    /**
     * 规则1339：CT增强检查知情同意书中替代医疗方案未填写
     */
    public function rule1339($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getCtzqConsentForms($ZYH);
    ...
    /**
     * 规则1340：CT增强检查知情同意书中医师签字时间未填写
     */
    public function rule1340($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getCtzqConsentForms($ZYH);
    ...
    /**
     * 规则1341：CT增强检查知情同意书中患方签字时间未填写
     */
    public function rule1341($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getCtzqConsentForms($ZYH);
    ...
    /**
     * 规则1342：CT增强检查知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
     */
    public function rule1342($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getCtzqConsentForms($ZYH);
    ...
    /**
     * 规则1343：CT增强检查知情同意书中患方意见未填写
     */
    public function rule1343($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $consentForms = $this->getCtzqConsentForms($ZYH);
    ...
    /**
     * 规则1344：CT增强检查知情同意书中为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
     */
    public function rule1344($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getCtzqConsentForms($ZYH);
        ...
        $sqwtsForms = $this->getSqwtsConsentForms($ZYH);
        $authorizationSummary = $this->getSqwtsAuthorizationSummary($sqwtsForms);

        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';

            if (!$this->hasConsentAgentSign($form)) {
                continue;
            }
    ...
    /**
     * 规则1345：CT增强检查知情同意书中代理人与患者关系未填
     */
    public function rule1345($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getCtzqConsentForms($ZYH);
     ...
     * 规则1286：缺病危（病重）通知书
     */
    public function rule1286($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $orderInfo = $this->getFirstBwbztysOrderInfo($ZYH);
        if (empty($orderInfo)) {
            return [];
        }

        $consentForms = $this->getBwbztysConsentForms($ZYH);
        if (!empty($consentForms)) {
            return [];
        }
    ...
    /**
     * 规则1346：病危（病重）通知书无医师签字
     */
    public function rule1346($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBwbztysConsentForms($ZYH);
    ...
    /**
     * 规则1347：病危（病重）通知书无代理人签字
     */
    public function rule1347($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBwbztysConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            ...
            if (!$this->hasAgentSignatureByBlsy($blbh)) {
                $basisList[] = ['【' . $blmc . '】无代理人签字'];
            }
        }
    ...
    /**
     * 规则1348：病危（病重）通知书内容填写不全，有空项
     */
    public function rule1348($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBwbztysConsentForms($ZYH);
    ...
    /**
     * 规则1349：病危（病重）通知书中目前诊断未填写
     */
    public function rule1349($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBwbztysConsentForms($ZYH);
    ...
    /**
     * 规则1350：病危（病重）通知书中医师签字时间未填写
     */
    public function rule1350($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBwbztysConsentForms($ZYH);
    ...
    /**
     * 规则1351：病危（病重）通知书中代理人签字时间未填写
     */
    public function rule1351($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBwbztysConsentForms($ZYH);
    ...
    /**
     * 规则1352：病危（病重）通知书中医师签字时间晚于患方签字时间，未体现先告后知
     */
    public function rule1352($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBwbztysConsentForms($ZYH);
    ...
    /**
     * 规则1353：病危（病重）通知书中患方意见未填写
     */
    public function rule1353($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBwbztysConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            ...
            if ($this->consentFieldEmpty($form['HFYJ'] ?? '')) {
                $basisList[] = ['【' . $blmc . '】患方意见未填写'];
            }
        }
    ...
    /**
     * 规则1354：病危（病重）通知书中代理人与患者关系未填
     */
    public function rule1354($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBwbztysConsentForms($ZYH);
    ...
    /**
     * 规则1355：授权委托书内容填写不全，有空项
     */
    public function rule1355($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSqwtsConsentForms($ZYH);
    ...
    /**
     * 规则1356：授权委托书代理人信息填写不全
     */
    public function rule1356($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSqwtsConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            ...
            foreach ($this->getSqwtsAgentSlots($form) as $index => $slot) {
                ...
                if ($this->consentFieldEmpty($slot['name'])) {
                    $basis[] = '代理人' . $index . '姓名未填写';
                }

                if ($this->consentFieldEmpty($slot['age'])) {
                    $basis[] = '代理人' . $index . '年龄未填写';
                }

                if ($this->consentFieldEmpty($slot['relation'])) {
                    $basis[] = '代理人' . $index . '与委托人（患者）关系未填写';
                }

                if ($this->consentFieldEmpty($slot['phone'])) {
                    $basis[] = '代理人' . $index . '电话未填写';
                }
            }

            if (!empty($basis)) {
                array_unshift($basis, '【' . $blmc . '】有代理人签字');
                $basisList[] = $basis;
            }
        }
    ...
    /**
     * 规则1357：授权委托书无委托人（患者）签名
     */
    public function rule1357($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSqwtsConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            ...
            if ((int)($form['SFHZQM'] ?? 0) !== 1) {
                $basisList[] = ['【' . $blmc . '】无委托人（患者）签名'];
            }
        }
    ...
    /**
     * 规则1358：授权委托书委托人（患者）签字时间未填写
     */
    public function rule1358($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSqwtsConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            ...
            if ($this->consentFieldEmpty($form['HZQMSJ'] ?? '')) {
                $basisList[] = ['【' . $blmc . '】委托人（患者）签字时间未填写'];
            }
        }
     ...
     * 规则1359：授权委托书无代理人签字
     */
    public function rule1359($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSqwtsConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            $blmc = $form['BLMC'] ?? '';
            $hasAgentSign = (int)($form['SFDLRQM'] ?? 0) === 1 || (int)($form['SFDLRQM2'] ?? 0) === 1;
    ...
    /**
     * 规则1360：授权委托书中代理人签字时间未填写
     */
    public function rule1360($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSqwtsConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            ...
            if ((int)($form['SFDLRQM'] ?? 0) === 1 && $this->consentFieldEmpty($form['DLRQMSJ'] ?? '')) {
                $basis[] = '代理人1签字时间未填写';
            }

            if ((int)($form['SFDLRQM2'] ?? 0) === 1 && $this->consentFieldEmpty($form['DLRQMSJ2'] ?? '')) {
                $basis[] = '代理人2签字时间未填写';
            }
    ...
    /**
     * 规则1361：拒绝检查/治疗告知书无医师签字
     */
    public function rule1361($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getJjjctysConsentForms($ZYH);
    ...
    /**
     * 规则1362：拒绝检查/治疗告知书无患方签字
     */
    public function rule1362($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getJjjctysConsentForms($ZYH);
    ...
    /**
     * 规则1363：拒绝检查/治疗告知书内容填写不全，有空项
     */
    public function rule1363($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getJjjctysConsentForms($ZYH);
    ...
    /**
     * 规则1364：拒绝检查/治疗告知书中“拒绝检查/治疗项目名称”未填写
     */
    public function rule1364($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getJjjctysConsentForms($ZYH);
    ...
    /**
     * 规则1365：拒绝检查/治疗告知书中医师签字时间未填写
     */
    public function rule1365($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getJjjctysConsentForms($ZYH);
    ...
    /**
     * 规则1366：拒绝检查/治疗告知书中患方签字时间未填写
     */
    public function rule1366($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getJjjctysConsentForms($ZYH);
    ...
    /**
     * 规则1367：拒绝检查/治疗告知书中医师签字时间晚于患方签字时间，未体现先告后知
     */
    public function rule1367($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getJjjctysConsentForms($ZYH);
    ...
    /**
     * 规则1368：拒绝检查/治疗告知书中患方意见未填写
     */
    public function rule1368($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $consentForms = $this->getJjjctysConsentForms($ZYH);
    ...
    /**
     * 规则1369：拒绝检查/治疗告知书中代理人与患者关系未填
     */
    public function rule1369($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getJjjctysConsentForms($ZYH);
    ...
    /**
     * 规则1370：自动出院/转院风险告知书无医师签字
     */
    public function rule1370($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZdcygzsConsentForms($ZYH);
    ...
    /**
     * 规则1371：自动出院/转院风险告知书无患方签字
     */
    public function rule1371($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZdcygzsConsentForms($ZYH);
    ...
    /**
     * 规则1372：自动出院/转院风险告知书内容填写不全，有空项
     */
    public function rule1372($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZdcygzsConsentForms($ZYH);
    ...
    /**
     * 规则1373：自动出院/转院风险告知书中医师签字时间未填写
     */
    public function rule1373($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZdcygzsConsentForms($ZYH);
    ...
    /**
     * 规则1374：自动出院/转院风险告知书中患方签字时间未填写
     */
    public function rule1374($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZdcygzsConsentForms($ZYH);
    ...
    /**
     * 规则1375：自动出院/转院风险告知书中医师签字时间晚于患方签字时间，未体现先告后知
     */
    public function rule1375($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZdcygzsConsentForms($ZYH);
    ...
    /**
     * 规则1376：自动出院/转院风险告知书中患方意见未填写
     */
    public function rule1376($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $consentForms = $this->getZdcygzsConsentForms($ZYH);
    ...
    /**
     * 规则1377：自动出院/转院风险告知书中代理人与患者关系未填
     */
    public function rule1377($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZdcygzsConsentForms($ZYH);
     ...
     * 规则1378：尸检告知书无医师签字
     */
    public function rule1378($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSjgzsConsentForms($ZYH);
     ...
     * 规则1379：尸检告知书无代理人签字
     */
    public function rule1379($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSjgzsConsentForms($ZYH);
    ...
    /**
     * 规则1380：尸检告知书内容填写不全，有空项
     */
    public function rule1380($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSjgzsConsentForms($ZYH);
    ...
    /**
     * 规则1381：尸检告知书中医师签字时间未填写
     */
    public function rule1381($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSjgzsConsentForms($ZYH);
    ...
    /**
     * 规则1382：尸检告知书中患方签字时间未填写
     */
    public function rule1382($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSjgzsConsentForms($ZYH);
    ...
    /**
     * 规则1383：尸检告知书中医师签字时间晚于患方签字时间，未体现先告后知
     */
    public function rule1383($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSjgzsConsentForms($ZYH);
    ...
    /**
     * 规则1384：尸检告知书中患方意见未填写
     */
    public function rule1384($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $consentForms = $this->getSjgzsConsentForms($ZYH);
    ...
    /**
     * 规则1385：尸检告知书中代理人与患者关系未填
     */
    public function rule1385($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getSjgzsConsentForms($ZYH);
    ...
    /**
     * 规则1386：医保外付费项目知情同意书无医师签字
     */
    public function rule1386($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYbwffgzsConsentForms($ZYH);
    ...
    /**
     * 规则1387：医保外付费项目知情同意书无患方签字
     */
    public function rule1387($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYbwffgzsConsentForms($ZYH);
    ...
    /**
     * 规则1388：医保外付费项目知情同意书内容填写不全，有空项
     */
    public function rule1388($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYbwffgzsConsentForms($ZYH);
    ...
    /**
     * 规则1389：医保外付费项目知情同意书中签字时间未填写
     */
    public function rule1389($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYbwffgzsConsentForms($ZYH);
        ...
        foreach ($consentForms as $form) {
            ...
            $blmc = $form['BLMC'] ?? '';
            $rows = $this->getYbwffgzsRows($form);
            $basis = [];

            foreach ($rows as $row) {
                $itemName = $row['item_name'] ?? '';
                if ($this->consentFieldEmpty($itemName)) {
                    continue;
                }

                if ($this->consentFieldEmpty($row['sign_time'] ?? '')) {
                    $basis[] = '【' . $itemName . '】签字时间未填写';
                }
            }
    ...
    /**
     * 规则1390：医保外付费项目知情同意书中患方意见未填写
     */
    public function rule1390($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $consentForms = $this->getYbwffgzsConsentForms($ZYH);
    ...
    /**
     * 规则1391：植入类或Ⅲ级医用耗材知情同意书无医师签字
     */
    public function rule1391($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZrltysConsentForms($ZYH);
    ...
    /**
     * 规则1392：植入类或Ⅲ级医用耗材知情同意书无患方签字
     */
    public function rule1392($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZrltysConsentForms($ZYH);
    ...
    /**
     * 规则1393：植入类或Ⅲ级医用耗材知情同意书内容填写不全，有空项
     */
    public function rule1393($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZrltysConsentForms($ZYH);
    ...
    /**
     * 规则1394：植入类或Ⅲ级医用耗材知情同意书中“拟用耗材名称”等未填写
     */
    public function rule1394($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZrltysConsentForms($ZYH);
    ...
    /**
     * 规则1395：植入类或Ⅲ级医用耗材知情同意书中替代耗材名称未填写
     */
    public function rule1395($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZrltysConsentForms($ZYH);
    ...
    /**
     * 规则1396：植入类或Ⅲ级医用耗材知情同意书中医师签字时间未填写
     */
    public function rule1396($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZrltysConsentForms($ZYH);
    ...
    /**
     * 规则1397：植入类或Ⅲ级医用耗材知情同意书中患方签字时间未填写
     */
    public function rule1397($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZrltysConsentForms($ZYH);
    ...
    /**
     * 规则1398：植入类或Ⅲ级医用耗材知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
     */
    public function rule1398($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZrltysConsentForms($ZYH);
    ...
    /**
     * 规则1399：植入类或Ⅲ级医用耗材知情同意书中患方意见未填写
     */
    public function rule1399($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $consentForms = $this->getZrltysConsentForms($ZYH);
    ...
    /**
     * 规则1400：植入类或Ⅲ级医用耗材知情同意书中代理人与患者关系未填
     */
    public function rule1400($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZrltysConsentForms($ZYH);
    ...
    /**
     * 规则1401：缺特殊检查/特殊治疗知情同意书
     */
    public function rule1401($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = $this->getTszlRequiredSurgeryMatches($ZYH);

        if (empty($basisList)) {
            return [];
        }

        $consentForms = $this->getTszlOrSszqtysConsentForms($ZYH);
        if (!empty($consentForms)) {
            return [];
        }

        foreach ($basisList as &$basis) {
            $basis[] = '缺少特殊检查/特殊治疗知情同意书';
        }
        unset($basis);
    ...
    /**
     * 规则1402：未在操作开始前签署特殊检查/特殊治疗知情同意书
     */
    public function rule1402($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $operationRecords = $this->getTszlOperationStartRecords($ZYH);
        if (empty($operationRecords)) {
            return [];
        }

        usort($operationRecords, function ($a, $b) {
            return strtotime($a['operation_start_time'] ?? '') <=> strtotime($b['operation_start_time'] ?? '');
        });

        $tszlConsentForms = $this->appendBl01TitleTimeToConsentForms($this->getTszlConsentForms($ZYH));
        $sszqtysConsentForms = $this->appendBl01TitleTimeToConsentForms($this->getSszqtysConsentForms($ZYH));
        $yzccstysConsentForms = $this->appendBl01TitleTimeToConsentForms($this->getYzccstysConsentForms($ZYH));
        $basisList = [];
        $previousOperationTimestamp = null;
        $previousOperationTime = '';

        foreach ($operationRecords as $record) {
            $operationStartTime = $record['operation_start_time'] ?? '';
            if ($operationStartTime === '') {
                continue;
            }

            $operationTimestamp = strtotime($operationStartTime);
            $recordBasis = [
                '操作记录：【' . ($record['BLMC'] ?? '') . '】',
                '操作开始时间【' . $operationStartTime . '】',
            ];

            if (empty($tszlConsentForms) && empty($sszqtysConsentForms) && empty($yzccstysConsentForms)) {
                $recordBasis[] = '缺少特殊检查/特殊治疗知情同意书';
                $basisList[] = $recordBasis;
                $previousOperationTimestamp = $operationTimestamp;
                $previousOperationTime = $operationStartTime;
                continue;
            }

            $matchedConsentForms = $this->getRule1402CandidateConsentForms(
                $tszlConsentForms,
                $sszqtysConsentForms,
                $yzccstysConsentForms,
                $operationTimestamp,
                $previousOperationTimestamp
            );

            if (empty($matchedConsentForms)) {
                $recordBasis[] = '缺少特殊检查/特殊治疗知情同意书';
                $basisList[] = $recordBasis;
                $previousOperationTimestamp = $operationTimestamp;
                $previousOperationTime = $operationStartTime;
            ...
            $hasTimelyConsent = false;
            $operationBasis = $recordBasis;

            foreach ($matchedConsentForms as $form) {
                $formBasis = ['同意书：【' . ($form['BLMC'] ?? '') . '】'];
                $doctorTime = $this->normalizeConsentComparableTime($form['YSQMSJ'] ?? '');
                $patientSignInfo = $this->getConsentPatientSignTimeInfo($form);
                ...
                if (
                    strtotime($doctorTime) <= $operationTimestamp &&
                    $patientSignInfo['timestamp'] <= $operationTimestamp
                ) {
                    $hasTimelyConsent = true;
                    break;
                }

                if (strtotime($doctorTime) > $operationTimestamp) {
                    $formBasis[] = '医师签字时间【' . $doctorTime . '】晚于操作开始时间';
                }

                if ($patientSignInfo['timestamp'] > $operationTimestamp) {
                    $formBasis[] = $patientSignInfo['label'] . '【' . $patientSignInfo['time'] . '】晚于操作开始时间';
                }

                if (count($formBasis) > 1) {
                    foreach ($formBasis as $basis) {
                        $operationBasis[] = $basis;
                    }
                }
            }

            if (!$hasTimelyConsent && count($operationBasis) > count($recordBasis)) {
                $basisList[] = $operationBasis;
            }
    ...
    /**
     * 获取规则1402当前操作记录对应的候选同意书。
     *
     * @param array $tszlConsentForms 特殊检查/特殊治疗知情同意书列表
     * @param array $sszqtysConsentForms 手术知情同意书列表
     * @param array $yzccstysConsentForms 腰椎穿刺术知情同意书列表
     * @param int $operationTimestamp 当前操作时间戳
     * @param int|null $previousOperationTimestamp 上一条操作时间戳
     * @return array
     */
    protected function getRule1402CandidateConsentForms(
        $tszlConsentForms = [],
        $sszqtysConsentForms = [],
        $yzccstysConsentForms = [],
        ...
        $matchedConsentForms = $this->getPreferredConsentFormsByTitleTimeRange(
            $tszlConsentForms,
            $sszqtysConsentForms,
            $yzccstysConsentForms,
            $operationTimestamp,
        ...
        if (!empty($matchedConsentForms) || $previousOperationTimestamp === null) {
            return $matchedConsentForms;
        }

        return $this->getPreferredConsentFormsByTitleTimeRange(
            $tszlConsentForms,
            $sszqtysConsentForms,
            $yzccstysConsentForms,
            $operationTimestamp,
     ...
     * @return array
     */
    protected function getPreferredConsentFormsByTitleTimeRange(
        $tszlConsentForms = [],
        $sszqtysConsentForms = [],
        $yzccstysConsentForms = [],
        $operationTimestamp = 0,
        ...
        $matchedConsentForms = $this->filterConsentFormsByTitleTimeRange(
            $tszlConsentForms,
            $operationTimestamp,
            $previousOperationTimestamp
        );
        ...
        return $this->filterConsentFormsByTitleTimeRange(
            $yzccstysConsentForms,
            $operationTimestamp,
            $previousOperationTimestamp
        );
    ...
    /**
     * 按同意书标题时间筛选当前操作记录对应的候选同意书。
     *
     * @param array $consentForms 同意书列表
     * @param int $operationTimestamp 当前操作时间戳
     * @param int|null $previousOperationTimestamp 上一条操作时间戳
     * @return array
     */
    protected function filterConsentFormsByTitleTimeRange($consentForms = [], $operationTimestamp = 0, $previousOperationTimestamp = null)
    {
        $matchedConsentForms = [];
        $operationMinuteTimestamp = $this->normalizeTimestampToMinute($operationTimestamp);
        $previousOperationMinuteTimestamp = $previousOperationTimestamp === null
            ? null
            : $this->normalizeTimestampToMinute($previousOperationTimestamp);

        foreach ($consentForms as $form) {
            $consentTitleTime = $form['title_time'] ?? '';
            if ($consentTitleTime === '') {
                continue;
            }

            $consentTitleTimestamp = $this->normalizeTimestampToMinute(strtotime($consentTitleTime));
            $isBeforeCurrentOperation = $consentTitleTimestamp <= $operationMinuteTimestamp;
            $isAfterPreviousOperation = $previousOperationMinuteTimestamp === null
                || $consentTitleTimestamp >= $previousOperationMinuteTimestamp;

            if ($isAfterPreviousOperation && $isBeforeCurrentOperation) {
                $matchedConsentForms[] = $form;
            }
        }
    ...
    /**
     * 将时间戳截断到分钟级，避免秒数影响标题时间和操作时间比较。
     *
     * @param int $timestamp 时间戳
     * @return int
     */
    protected function normalizeTimestampToMinute($timestamp = 0)
    {
        return (int)floor(((int)$timestamp) / 60) * 60;
    }
     ...
     * @return array
     */
    protected function appendBl01TitleTimeToConsentForms($consentForms = [])
    {
        if (empty($consentForms)) {
            return [];
        }

        $blbhList = array_values(array_unique(array_filter(array_column($consentForms, 'BLBH'))));
        if (empty($blbhList)) {
            return $consentForms;
        }

        $titleTimeMap = EMR_BL_BL01::query()
        ...
        foreach ($consentForms as &$form) {
            $blbh = $form['BLBH'] ?? '';
            $form['title_time'] = $this->normalizeConsentComparableTime($titleTimeMap[$blbh] ?? '');
        }
        unset($form);
    ...
    /**
     * 规则1403：特殊检查/特殊治疗知情同意书中为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
     */
    public function rule1403($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getTszlConsentForms($ZYH);
    ...
    /**
     * 规则1404：缺术中冰冻切片检查知情同意书
     */
    public function rule1404($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $surgeryRecords = $this->getBdqpRequiredSurgeryRecords($ZYH);
        if (empty($surgeryRecords)) {
            return [];
        }
        ...
        $basisList = [];
        foreach ($surgeryRecords as $record) {
            $surgeryName = trim((string)($record['SSMC'] ?? ''));
            $basis = [];
            ...
            $basis[] = '手术记录【' . ($record['BLMC'] ?? '') . '】';
            foreach (($record['matched_keywords'] ?? []) as $keyword) {
                $basis[] = '手术经过命中关键词【' . $keyword . '】';
            }
            $basis[] = '缺少术中冰冻切片检查知情同意书';
    ...
    /**
     * 规则1405：未在检查前签署术中冰冻切片检查知情同意书
     */
    public function rule1405($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $surgeryRecords = $this->getBdqpRequiredSurgeryRecords($ZYH);
        if (empty($surgeryRecords)) {
            return [];
        ...
        foreach ($surgeryRecords as $record) {
            $surgeryStartTime = $this->normalizeConsentComparableTime($record['SSKSSJ'] ?? '');
            if ($surgeryStartTime === '') {
                continue;
            }
            ...
            $recordBasis[] = '手术记录【' . ($record['BLMC'] ?? '') . '】';
            $recordBasis[] = '手术开始时间【' . $surgeryStartTime . '】';

            foreach (($record['matched_keywords'] ?? []) as $keyword) {
                $recordBasis[] = '手术经过命中关键词【' . $keyword . '】';
            }

            if (empty($consentForms)) {
                $recordBasis[] = '缺少术中冰冻切片检查知情同意书';
                $basisList[] = $recordBasis;
                continue;
            }
            ...
            foreach ($consentForms as $form) {
                ...
                if (
                    $doctorTime !== '' && strtotime($doctorTime) <= $surgeryStartTimestamp &&
                    !empty($patientSignInfo) && $patientSignInfo['timestamp'] <= $surgeryStartTimestamp
                ) {
                    $hasTimelyConsent = true;
                    break;
                }
                ...
                if ($doctorTime === '') {
                    $formBasis[] = '医师签字时间未填写';
                } elseif (strtotime($doctorTime) > $surgeryStartTimestamp) {
                    $formBasis[] = '医师签字时间【' . $doctorTime . '】晚于手术开始时间';
                }

                if (empty($patientSignInfo)) {
                    $formBasis[] = '患方签字时间未填写';
                } elseif ($patientSignInfo['timestamp'] > $surgeryStartTimestamp) {
                    $formBasis[] = $patientSignInfo['label'] . '【' . $patientSignInfo['time'] . '】晚于手术开始时间';
                }

                if (count($formBasis) > 1) {
                    $lateBasisList[] = $formBasis;
                }
            }
            ...
            $basisList[] = $recordBasis;
            foreach ($lateBasisList as $lateBasis) {
                $basisList[] = $lateBasis;
            }
        }
    ...
    /**
     * 规则1406：术中冰冻切片检查知情同意书为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
     */
    public function rule1406($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getBdqpConsentForms($ZYH);
    ...
    /**
     * 规则1407：未在化疗开始前签署化疗知情同意书
     */
    public function rule1407($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $orderInfo = $this->getFirstChemoOrderInfo($ZYH);
        if (empty($orderInfo)) {
            return [];
        ...
        foreach ($consentForms as $form) {
            ...
            if ($doctorTime !== '' && strtotime($doctorTime) > $orderTimestamp) {
                $formBasis[] = '【' . $blmc . '】医师签名时间【' . $doctorTime . '】未在化疗开始之前';
            }

            if ($patientTime !== '' && strtotime($patientTime) > $orderTimestamp) {
                $formBasis[] = '【' . $blmc . '】患者签名时间【' . $patientTime . '】未在化疗开始之前';
            }

            if ($agentTime !== '' && strtotime($agentTime) > $orderTimestamp) {
                $formBasis[] = '【' . $blmc . '】代理人签名时间【' . $agentTime . '】未在化疗开始之前';
            }

            $hasDoctorBefore = $doctorTime !== '' && strtotime($doctorTime) <= $orderTimestamp;
            $hasPatientBefore = $patientTime !== '' && strtotime($patientTime) <= $orderTimestamp;
            $hasAgentBefore = $agentTime !== '' && strtotime($agentTime) <= $orderTimestamp;

            if (empty($formBasis) && $hasDoctorBefore && ($hasPatientBefore || $hasAgentBefore)) {
                $hasTimelyConsent = true;
                break;
            }
    ...
    /**
     * 规则1408：化疗知情同意书中为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
     */
    public function rule1408($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getHltysConsentForms($ZYH);
     ...
     * 规则1409：缺CT增强检查知情同意书
     */
    public function rule1409($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $orderInfo = $this->getFirstCtzqOrderInfo($ZYH);
        if (empty($orderInfo)) {
            return [];
    ...
    /**
     * 规则1410：拒绝检查/治疗告知书中为代理人签字，无授权委托书
     */
    public function rule1410($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getJjjctysConsentForms($ZYH);
    ...
    /**
     * 规则1411：自动出院/转院风险告知书中为代理人签字，无授权委托书
     */
    public function rule1411($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZdcygzsConsentForms($ZYH);
    ...
    /**
     * 规则1412：植入类或Ⅲ级医用耗材知情同意书中为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
     */
    public function rule1412($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getZrltysConsentForms($ZYH);
     ...
     * 规则1413：缺腰椎穿刺术知情同意书
     */
    public function rule1413($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $orderInfo = $this->getFirstYzccstysOrderInfo($ZYH);
        if (empty($orderInfo)) {
            return [];
        }

        $consentForms = $this->getYzccstysConsentForms($ZYH);
        if (!empty($consentForms)) {
            return [];
        }

        $surgeryConsentForms = $this->getSszqtysConsentForms($ZYH);
        foreach ($surgeryConsentForms as $form) {
            if ($this->containsNormalizedKeyword($form['BLMC'] ?? '', '腰椎穿刺')) {
                return [];
            }
        }
    ...
    /**
     * 规则1414：腰椎穿刺术知情同意书无医师签字
     */
    public function rule1414($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYzccstysConsentForms($ZYH);
    ...
    /**
     * 规则1415：腰椎穿刺术知情同意书无患方签字
     */
    public function rule1415($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYzccstysConsentForms($ZYH);
    ...
    /**
     * 规则1416：未在操作开始前签署腰椎穿刺术知情同意书
     */
    public function rule1416($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $operationRecords = $this->getYzccstysOperationStartRecords($ZYH);
        if (empty($operationRecords)) {
            return [];
        }
        ...
        foreach ($operationRecords as $record) {
            ...
            foreach ($consentForms as $form) {
                ...
                if (
                    $doctorTime !== '' && strtotime($doctorTime) <= $operationTimestamp &&
                    !empty($patientSignInfo) && $patientSignInfo['timestamp'] <= $operationTimestamp
                ) {
                    $hasTimelyConsent = true;
                    break;
                }
                ...
                if ($doctorTime === '') {
                    $formBasis[] = '医师签字时间未填写';
                } elseif (strtotime($doctorTime) > $operationTimestamp) {
                    $formBasis[] = '医师签字时间【' . $doctorTime . '】晚于操作开始时间';
                }

                if (empty($patientSignInfo)) {
                    $formBasis[] = '患方签字时间未填写';
                } elseif ($patientSignInfo['timestamp'] > $operationTimestamp) {
                    $formBasis[] = $patientSignInfo['label'] . '【' . $patientSignInfo['time'] . '】晚于操作开始时间';
                }
    ...
    /**
     * 规则1417：腰椎穿刺术知情同意书内容填写不全，有空项
     */
    public function rule1417($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYzccstysConsentForms($ZYH);
    ...
    /**
     * 规则1418：腰椎穿刺术知情同意书中“拟施特殊检查/特殊治疗项目目的”未填写
     */
    public function rule1418($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYzccstysConsentForms($ZYH);
    ...
    /**
     * 规则1419：腰椎穿刺术知情同意书中替代方案未填写
     */
    public function rule1419($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYzccstysConsentForms($ZYH);
    ...
    /**
     * 规则1420：腰椎穿刺术知情同意书中医师签字时间未填写
     */
    public function rule1420($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYzccstysConsentForms($ZYH);
    ...
    /**
     * 规则1421：腰椎穿刺术知情同意书中患方签字时间未填写
     */
    public function rule1421($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYzccstysConsentForms($ZYH);
    ...
    /**
     * 规则1422：腰椎穿刺术知情同意书中医师签字时间晚于患方签字时间，未体现先告后知
     */
    public function rule1422($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYzccstysConsentForms($ZYH);
    ...
    /**
     * 规则1423：腰椎穿刺术知情同意书中患方意见未填写
     */
    public function rule1423($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $consentForms = $this->getYzccstysConsentForms($ZYH);
    ...
    /**
     * 规则1424：腰椎穿刺术知情同意书中为代理人签字，无授权委托书，或授权委托书中无代理人姓名及签字
     */
    public function rule1424($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYzccstysConsentForms($ZYH);
    ...
    /**
     * 规则1425：腰椎穿刺术知情同意书中代理人与患者关系未填
     */
    public function rule1425($caseRule = [], $ruleId = 0, $ZYH = "")
    {
        $basisList = [];
        $consentForms = $this->getYzccstysConsentForms($ZYH);
