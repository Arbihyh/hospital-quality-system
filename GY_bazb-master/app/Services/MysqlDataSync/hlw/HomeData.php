...
use App\Console\Commands\ShizhongWarningSync;
...
class HomeData
{
    ...
    //mysql数据库连接
    public function getConnect3()
    {
        if (self::$con3) {
            return self::$con3;
        }
        $connection = '115.191.26.134';
        $port = '4000';
        $username = 'root';
        $password = 'Jmjk@886';
        $database = 'quality';
        // 服务器未安装mysqli扩展，改用PDO(pdo_mysql)连接
        $dsn = "mysql:host={$connection};port={$port};dbname={$database};charset=utf8";
        $con3 = new \PDO($dsn, $username, $password, [
    ...
    public function OMRBL01($startTime = "")
    {
        ...
        while (true) {
            ...
            $startTime += 86400;
            $sql = "SELECT ID_MEDRECDOC as BLBH,ID_DS as BLLB,NA_MED as BLMC,ID_DS as MBLB,CD_CREATE as SXYS,CD_DEPT as SXKS,FG_ACTIVE as BLZT,HTML_TEXT as BLNR_TXT,ID_ARCHIVES as mzh,NAME as xm,ID_PATIENT as BRID,to_char(CREATE_TIME,'yyyy-mm-dd hh24:mi:ss') AS JLSJ,to_char(UPDATE_TIME,'yyyy-mm-dd hh24:mi:ss') AS WCSJ from WHIS_EMR.HI_VIEW_REC_MZ_HTML WHERE UPDATE_TIME BETWEEN TO_DATE('" . $startTimeStr . "000000', 'yyyy-MM-dd HH24:mi:ss') AND TO_DATE('" . $startTimeStr . "235959', 'yyyy-MM-dd HH24:mi:ss')";
    ...
    public function getBldata($zyh = "", $type = [], $blbh = "", $startTime = "", $cfjd = '', $qmys = '', $qmrq = '')
    {
        ...
        if ($zyh) {
            /* $szds = new ShizhongDataSync();
            $szds->ZY_BRRY($zyh, 0, 0, 0); */
            $szds = new ShizhongWarningSync();
            $szds->updateBrryByZyh($zyh);
        }
        ...
        // 医嘱
        if (empty($type) || in_array('yzb', $type)) {
            Log::info("getBldata 医嘱：" . $zyh);
            $yzbData = $this->getYzb($zyh);
            if (!empty($yzbData)) {
                Log::info("getBldata 医嘱数量：" . count($yzbData));
                $this->addYzb($yzbData);
            }
        }
        ...
        // 费用明细
        if (empty($type) || in_array('fee_detailed', $type)) {
            Log::info("getBldata 费用明细：" . $zyh);
            $feeDetailedData = $this->getFeeDetailed($zyh);
            if (!empty($feeDetailedData)) {
                Log::info("getBldata 费用明细数量：" . count($feeDetailedData));
                $this->addFeeDetailed($feeDetailedData);
            }
        }
        ...
        // 医生签名
        if (empty($type) || in_array('bl01', $type)) {
            Log::info("getBldata 医生签名：" . $zyh);
            $this->addBLSY($zyh, $blbh, 1, $cfjd, $qmys, $qmrq);
            Log::info("getBldata 医生签名完成");
        }
    ...
    public function getYzb($ZYH)
    {
        $sql = "SELECT 
            ...
            PSBZ AS PSBZ,
            PSJG AS PSJG
        FROM quality.TEST_yzb WHERE ZYH = :zyh";

        $data = self::$con3->prepare($sql);
        $data->bindValue(':zyh', trim((string)$ZYH), \PDO::PARAM_STR);
        $data->execute();
        //dd($data->fetchAll(\PDO::FETCH_ASSOC));
        $result = [];
    ...
    public function addYzb($data)
    {
        ...
        $yzb = Yzb::query()->where('ZYH', '=', (string)$zyh)->get()->toArray();
        //Log::info('addYzb yzb', ['yzb' => $yzb]);
        $idArr = array_column($yzb, 'id');
        //Log::info('addYzb idArr', ['idArr' => $idArr]);
    ...
    public function addBLSY($zyh = "", $blbh3 = "", $isRecordLog = 1, $cfjd = '', $qmys = '', $qmrq = '')
    {
        ...
        $this->getConnect3();
        $sql = "SELECT a.BLBH, a.JZHM, a.AAB01, a.AAC01, a.BLLB, a.BLMC, a.MBLB, a.ZXSJ, a.CJSJ, a.WCSJ, a.SXYS, b.BRKS, a.BLZT, a.JLSJ, a.HTML_PRINT FROM TEST_EMR_BL_BL01 a LEFT JOIN TEST_ZY_BRRY b ON a.JZHM = b.ZYH WHERE a.JZHM = '{$zyh}'";
        if (!empty($blbh1)) {
            $sql .= " and a.BLBH='{$blbh1}'";
        }
        $queryResult = self::$con3->query($sql);
        $data = [];
        ...
        foreach ($data as $item) {
            ...
            // con3中HTML_PRINT已是字符串，取bl01的HTML_PRINT
            $HTML_PRINT = $item['HTML_PRINT'] ?? '';
            ...
            $bl01Data = $result;
            $str = '';
            EMR_BL_BL01::query()->updateOrInsert(['BLBH3' => $item['BLBH']], $result); //插入或更新病历数据
            Log::info("getBldata 病历数据：" . $zyh);
            //BLXG数据从con3的TEST_EMR_BL_BLXG表读取，HJNR直接使用表值
            $blxgSql = "SELECT JLXH, BLBH, XGGH, XGSJ, HJNR FROM TEST_EMR_BL_BLXG WHERE BLBH = '" . $item['BLBH'] . "'";
            $blxgResultSet = self::$con3->query($blxgSql);
            foreach ($blxgResultSet->fetchAll() as $blxgRow) {
                $temp = [
                    'JZHM' => $item['JZHM'] ?? '',
                    'JLXH' => $blxgRow['JLXH'] ?? '',
                    'XGGH' => $blxgRow['XGGH'] ?? '',
                ...
                $str = $blxgRow['HJNR'] ?? '';
                EMR_BL_BLXG::query()->updateOrInsert(['BLBH' => $temp['BLBH']], $temp); //插入或更新BLXG数据(唯一索引在BLBH)
                Log::info("getBldata BLXG数据：" . $zyh);
            }

            $sql = "SELECT JLXH, BLBH, SYYS, SYSJ, JLSJ, SYQX, QMLX, QMMC, QMYS, FG_ACTIVE FROM TEST_EMR_BL_BLSY WHERE BLBH = '" . $item['BLBH'] . "'";
            $resultSet = self::$con3->query($sql);
            $data = [];
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $v) {
                    ...
                    EMR_BL_BLSY::query()->updateOrInsert(['JLXH' => $insertData['JLXH'], 'BLBH' => $insertData['BLBH']], $insertData);
                }
            ...
            } elseif ($bl01Data["BLLB"] == 294 || $bl01Data["BLLB"] == 43 || $bl01Data["BLLB"] == 329) {
    ...
    public function addFeeDetailed($result)
    {
        foreach ($result as $val) {
            ...
            FeeDetailed::query()->updateOrInsert(['AAA28' => $item['AAA28'], 'FYXH' => $item['FYXH']], $item);
        }
        DataSyncLog::addData(['zyh' => $val['AAA28'], 'content' => '费用明细同步结束']);
    }
}
