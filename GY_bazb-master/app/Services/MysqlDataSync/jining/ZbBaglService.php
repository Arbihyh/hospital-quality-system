                ...
                foreach ($surgeries as $surgery) {
                    ...
                    if (!empty($allResults)) {
                        ...
                        if (!empty($validRecords)) {
                            ...
                            // 术者签名
                            $hjnr = $bestRecord['HJNR'];
                            $hjnr = str_replace([' ', "\t", "\n", "\r\n", '　'], '', $hjnr);
                            preg_match("/手术者(.*)助手/", $hjnr, $timeMatches);
                            $surgeonName = !empty($timeMatches[1]) ? $timeMatches[1] : '';
                            if(!empty($surgeonName)){
                                $surgeryGroup['content'][] = [
                                    'status' => 1,
                                    'content' => "手术者【{$surgeonName}】"
                                ];
                                
                                // 2. 检查签名中是否包含术者姓名
                                $blsy = EMR_BL_BLSY::query()
                                    ->where('BLBH', $bestRecord['BLBH'])
                                    ->where('FG_ACTIVE', 1)
                                    ->get()
                                    ->toArray();
                                ...
                                if (!empty($blsy)) {
                                    foreach ($blsy as $blsyItem) {
                                        ...
                                        if (!empty($ysname) && (strpos($ysname, $surgeonName) !== false || strpos($surgeonName, $ysname) !== false)) {
                                            $surgeryCriteriaMet = true;
                                            $surgeryGroup['content'][] = [
                                                'status' => 1,
                                                'content' => "手术者签名【{$ysname}】"
                                            ];
                                            break;
                                        }
                                    }
