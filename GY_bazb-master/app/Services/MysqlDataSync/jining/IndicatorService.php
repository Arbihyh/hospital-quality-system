...
class IndicatorService
{
    ...
    public function handleRjssblsqtl($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $ssapData = $indicatorCalcService->getOperationAll($item['MED_REC_ID']);
            if (empty($ssapData)) {
                continue; // 没有手术记录，跳过
            }
            ...
            foreach ($ssapData as $surgery) {
                $surgeryStartTime = $surgery['START_TIME'] ?? '';
                $surgeryEndTime = $surgery['END_TIME'] ?? '';
                $surgeryName = $surgery['ICD9_SSCZMC'] ?? '';
                $surgeon = $surgery['SZ'] ?? ''; // 术者
                echo '手术开始时间:' . $surgeryStartTime . PHP_EOL;
                echo '手术结束时间:' . $surgeryEndTime . PHP_EOL;
                echo '手术名称:' . $surgeryName . PHP_EOL;
                echo '术者:' . $surgeon . PHP_EOL;
        ...
        foreach ($data as $item) {
            ...
            foreach ($ssapData as $surgery) {
                ...
                $recordsQuery = EMR_BL_BL01::query()
                    ->where('JZHM', $item['MED_REC_ID'])
                    ->where(function($query)use($recordTypes){
                        $query->whereIn('MBLB', $recordTypes)->orWhere('BLMC',"like","%术前小结及术前讨论%")->orWhere('BLMC',"like","%术前评估%");
                    });
    ...
    public function handleKzrzcswtll($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $order = $orders[0];
            $swYzbName = $order['YZMC'] ?? '';
            $error = $content = [];
            $content[] = ["status" => 1, "content" => "医嘱名称【" . $swYzbName . "】"];
            $content[] = ["status" => 1, "content" => "死亡时间【" . $swsj . "】"];
            ...
            if (!empty($bl01Data[0])) {
                ...
                $content[] = ["status" => 0, "content" => "死亡病例讨论记录【无】"];
            }
            if (!empty($content)) {
                $status2 = array_unique(array_column($content, "status"));
                if (!in_array(0, $status2)) {
    ...
    public function handleHzry8xsncf($data, $startTime)
    {
        ...
        foreach ($data as $item) {
            ...
            echo $insert['zyh'] . '-' . data_get($item, 'AAB06C') . '-' . data_get($item, 'AAD01C') . "\r\n";

            // 排除标准去除，急诊入院患者和患者转入/转出重症医学科的情况。
            if (data_get($item, 'AAB06C') == 1 || strripos(data_get($item, 'AAD01C'), '重症医学科') !== false) {
                continue;
            }
    ...
    public function handleKjywcfqlshg($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $ryqxData = [];
            if (!empty($kzysList)) {
                $ryqxData = GY_KSSQX::query()->whereIn('YGDM', $kzysList)->get()->toArray();
            }
            ...
            foreach ($yzbData as $yzb) {
                ...
                if (!empty($kzys) && !empty($kjjbValue) && isset($kjjb[$kjjbValue])) {
                    // 查找该医师是否有对应级别的权限
                    foreach ($ryqxData as $ryqx) {
                        if ($ryqx['YGDM'] == $kzys && $ryqx['KJJBDM'] == $kjjbValue) {
                            $hasPermission = true;
                            break;
                        }
                    }
