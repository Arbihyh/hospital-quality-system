...
namespace App\Services\MysqlDataSync\jining;
...
use App\Services\MysqlDataSync\jining\IndicatorService;
...
class IndicatorCalcService
{
     ...
     * 获取指定住院号的所有手术信息
     */
    public function getOperationAll($ZYH = '')
    {

        // 查询主要手术
        $mainOperationData = MainOperation::query()
            ->where('AAA28', '=', $ZYH)
            ->get([DB::RAW('concat(id, "_main") as id'), 'AAA28', 'OPE_DATE', 'OPE_MAN_NAME', 'ICD9_NAME', 'SSPB', 'START_TIME', 'END_TIME', 'SSLX'])
            ->toArray();

        // 查询次要手术
        $secondaryOperationData = SecondaryOperation::query()
            ->where('AAA28', '=', $ZYH)
            ->get([DB::RAW('concat(id, "_other") as id'), 'AAA28', 'OPE_DATE', 'OPE_MAN_NAME', 'ICD9_NAME', 'SSPB', 'START_TIME', 'END_TIME', 'SSLX'])
            ->toArray();

        // 合并数据并进行字段映射
        $ssapData = [];
        foreach ($mainOperationData as $item) {
            // 【手术类型】为【手术+介入治疗】的手术台次。
            if (!in_array($item['SSPB'], [1, 4])) {
                continue;
            }
            $opeDate = $item['OPE_DATE'] ?? '';
            // 将 OPE_DATE 转换为 Y-m-d 23:59:59 格式
            $ssrq = '';
            if (!empty($opeDate)) {
                $timestamp = strtotime($opeDate);
                if ($timestamp !== false) {
                    // 提取日期部分（Y-m-d），然后拼接 23:59:59
                    $datePart = date('Y-m-d', $timestamp);
                    $ssrq = $datePart . ' 23:59:59';
                }
            }
            $ssapData[] = [
                'id' => $item['id'],
                'SSRQ' => $ssrq,
                'SZ' => $item['OPE_MAN_NAME'] ?? '',
                'ICD9_SSCZMC' => $item['ICD9_NAME'] ?? '',
        ...
        foreach ($secondaryOperationData as $item) {
            // 【手术类型】为【手术+介入治疗】的手术台次。
            if (!in_array($item['SSPB'], [1, 4])) {
                continue;
            }
    ...
    public function calculateSzsqcf($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            try {
                // 获取患者的所有手术信息
                $ssapData = $this->getOperationAll($ZYH);

                // 按手术日期排序
                usort($ssapData, function ($a, $b) {
                    $timeA = !empty($a['SSRQ']) ? strtotime($a['SSRQ']) : 0;
                    $timeB = !empty($b['SSRQ']) ? strtotime($b['SSRQ']) : 0;
                    return $timeA - $timeB;
                });
            ...
            $mblbRaw = RuleWordMap::query()->where('id', '=', 8016)->value('keyword') ?? '82,304';
            $mblbList = [];
            if (strpos($mblbRaw, ',') !== false) {
                $mblbList = explode(',', $mblbRaw);
            } else {
                $mblbList = [$mblbRaw];
            }
            ...
            for ($i = 0; $i < count($ssapData); $i++) {
                ...
                // $surgery['SSLX'] == 2 紧急手术跳过
                if (
                    empty($surgery['SSRQ']) || empty($surgery['ICD9_SSCZMC']) || $surgery['ICD9_SSCZMC'] == 'NULL' || $surgery['SSLX'] == 2
                ) {
                    continue;
                }
                ...
                $records = EMR_BL_BL01::query()
                    ->where('JZHM', $ZYH)
                    ->where(function($query) use ($mblbList) {
                        $query->whereIn('MBLB', $mblbList)
                            ->orWhere('BLMC', 'like', '%术前小结及术前讨论%');
                    })
                ...
                $surgeryGroup['content'][] = [
                    'status' => 1,
                    'content' => "手术名称【{$surgeryName}】"
                ];
                $surgeryGroup['content'][] = [
                    'status' => 1,
                    'content' => "手术日期【{$surgeryStartTime}】"
                ];
                $surgeryGroup['content'][] = [
                    'status' => 1,
                    'content' => "术者【{$surgeon}】"
                ...
                if (empty($records)) {
                    ...
                    if ($hasSurgeonSignature) {
                        ...
                        $surgeryGroup['content'][] = [
                            'status' => 1,
                            'content' => "术前小结/术前讨论记录【{$records[0]['BLMC']}】"
                        ];
                    } /* else {
                        // 没有有效的术者签名
    ...
    public function calculatePthzjswcl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            foreach ($consultations as $consultation) {
                ...
                if ($consultationOpinion) {
                    $writeTime = Carbon::parse($consultationOpinion->QMSJ);
                    $requestTime = Carbon::parse($consultation->SQSJ);
    ...
    public function calculateSzshcf24($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            foreach ($ssapData as $surgery) {
                // 检查基本字段有效性
                if (
                    empty($surgery['SSRQ']) || empty($surgery['ICD9_SSCZMC']) || $surgery['ICD9_SSCZMC'] == 'NULL'
                ) {
                    continue;
                }

                $surgeryEndTime = $surgery['SSRQ'];
                $surgeon = $surgery['SZ'];
                $surgeon1 = $surgery['SZ'];
                //$surgeonCode = $surgery['SZDM'];
                $surgeryName = $surgery['ICD9_SSCZMC'];
                ...
                if (
                    ...
                    $surgeryEndTime == '0000-00-00 00:00:00' || empty($surgeon) || empty($surgeon1)
                ) {
                    continue;
                }
    ...
    public function calculateSqtlwcl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            try {
                // 获取患者的所有手术信息
                $ssapList = $this->getOperationAll($ZYH);

                // 按手术日期排序
                usort($ssapList, function ($a, $b) {
                    $timeA = !empty($a['SSRQ']) ? strtotime($a['SSRQ']) : 0;
                    $timeB = !empty($b['SSRQ']) ? strtotime($b['SSRQ']) : 0;
                    return $timeA - $timeB;
                });
                ...
                Log::error("calculateSqtlwcl-$ZYH-error: " . $e->getMessage());
                continue;
            }
            ...
            for ($i = 0; $i < count($ssapList); $i++) {
                ...
                $surgeon = $surgeonName;
                if (
                    empty($surgeryStart) || empty($surgeryName) || $surgeryName === 'NULL'
                ) {
                    continue;
                }
                ...
                $group['content'][] = ['status' => 1, 'content' => "术者【{$surgeon}】"];
                $group['content'][] = ['status' => 1, 'content' => "手术日期【{$surgeryStart}】"];
    ...
    public function calculateSqtlrygfcyl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            try {
                ...
                Log::error("calculateSqtlrygfcyl-$ZYH-error: " . $e->getMessage());
                continue;
            }
            ...
            for ($i = 0; $i < count($ssapList); $i++) {
                ...
                // 手术开始前24小时
                $startCheckTime = date('Y-m-d H:i:s', strtotime($surgeryStart) - 24 * 3600);
                ...
                $group['content'][] = ['status' => 1, 'content' => "手术名称【{$surgeryName}】"];
                $group['content'][] = ['status' => 1, 'content' => "术者【{$surgeonDisp}】"];
                $group['content'][] = ['status' => 1, 'content' => "手术日期【{$surgeryStart}】"];
            ...
            foreach ($records as $record) {
                ...
                try {
                    ...
                    if (!empty($codes)) {
                        $group['status'] = 1;
                        $insert['qjjlsh_fz'] += 1;
                        $group['content'][] = ['status' => 1, 'content' => "签名【有】"];
                    } else {
                        // 错误提示
                        $group['content'][] = ['status' => 0, 'content' => "签名【无】"];
                    }
                } catch (\Exception $e) {
    ...
    public function calculateSsyzgfkjl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            // 分母：查询MainOperation和SecondaryOperation，有数据就算分母（只算一个）
            try {
                // 获取患者的所有手术信息
                $ssapList = $this->getOperationAll($ZYH);
                ...
                // 按 SSRQ 排序
                usort($ssapList, function ($a, $b) {
                    return strcmp($a['SSRQ'] ?? '', $b['SSRQ'] ?? '');
                });
            } catch (\Exception $e) {
                Log::error("calculateSsyzgfkjl-$ZYH-error: " . $e->getMessage());
                continue;
            }
            ...
            // 参考宁夏版本：遍历每台手术，每台手术算一个分母，满足条件算一个分子
            for ($i = 0; $i < count($ssapList); $i++) {
                $op = $ssapList[$i];
                ...
                // 过滤条件
                if (empty($surgeryStart) || empty($surgeryName) || $surgeryName === 'NULL') {
                    continue;
                }
                ...
                $group['content'][] = ['status' => 1, 'content' => "手术名称【{$surgeryName}】"];
                $group['content'][] = ['status' => 1, 'content' => "术者【{$surgeonName}】"];
                $group['content'][] = ['status' => 1, 'content' => "手术开始时间【{$surgeryStart}】"];
                ...
                if (!empty($surgeryNameOrders)) {
                    // 找到包含手术名称的医嘱，取第一个（开嘱时间最接近手术开始时间的）
    ...
    public function calculateSwtljswcl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            // 死亡医嘱下达后五个工作日内完成【死亡讨论记录】,因为用的工作日，所以需要考虑到
            $endTime = $this->getEndTime($swsj, 5);
            ...
            $records = EMR_BL_BL01::query()
                ...
                ->where(function($query) {
                    $query->where('EMR_BL_BL01.BLMC', 'like', '%死亡病例讨论%')
                        ->orWhere('EMR_BL_BLXG.HJNR', 'like', '%死亡病例讨论%');
                })
                ->where('EMR_BL_BL01.ZXSJ', '>=', $swsj)
                ->where('EMR_BL_BL01.ZXSJ', '<=', $endTime)
                ->select('EMR_BL_BL01.BLBH', 'EMR_BL_BL01.BLMC', 'EMR_BL_BL01.ZXSJ', 'EMR_BL_BLXG.HJNR')
                ->get()
                ->toArray();
    ...
    public function calculateSzfhsqmlyzl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            for ($i = 0; $i < count($ssapList); $i++) {
                ...
                // 获取 $surgeryStart 手术的年月日
                $surgeryDateOnly = '';
                if (!empty($surgeryStart)) {
                    $surgeryDateOnly = date('Y-m-d', strtotime($surgeryStart));
                }

                // 改为从病案首页提取  术者
                $mainOpRecord = MainOperation::query()
                    ->where('AAA28', $ZYH)
                    ->where('ICD9_ID1', $surgeryCode)
                    ->whereDate('OPE_DATE', $surgeryDateOnly)
                    ->first();
                if (!empty($mainOpRecord)) {
                    $surgeonName = $mainOpRecord['OPE_MAN_NAME'] ?? '';
                    $surgeonCode = $mainOpRecord['OPE_MAN_CODE'] ?? '';
                } else {
                    $otherOpRecord = SecondaryOperation::query()
                        ->where('AAA28', $ZYH)
                        ->where('ICD9_ID1', $surgeryCode)
                        ->whereDate('OPE_DATE', $surgeryDateOnly)
                        ->first();
                    if (!empty($otherOpRecord)) {
                        $surgeonName = $otherOpRecord['OPE_MAN_NAME'] ?? '';
                        $surgeonCode = $otherOpRecord['OPE_MAN_CODE'] ?? '';
                    }
                }
    ...
    public function calculateSsyssssjch($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            // 遍历每个手术
            foreach ($ssapData as $surgery) {
                $currentId = $surgery['id'] ?? '';
                $surgeryName = $surgery['ICD9_SSCZMC'] ?? '';
                $surgeonName = $surgery['SZ'] ?? '';
                $surgeryStart = $surgery['START_TIME'] ?? '';
                $surgeryEnd = $surgery['END_TIME'] ?? '';
                $surgeryType = $surgery['SSLX'] ?? '';
                $surgeryPB = $surgery['SSPB'] ?? '';
                $surgeryPB = $surgeryPB == 1 ? '有对应权限' : '无对应权限';
                ...
                foreach ($ssapData as $ssap) {
                    if ($ssap['id'] == $currentId) {
                        continue;
                    }
                    // 检查当前 $ssap['START_TIME'] 到 $ssap['END_TIME'] 和 $surgeryStart 到 $surgeryEnd 的时间是否重叠
                    $ssapStart = strtotime($ssap['START_TIME']);
                    $ssapEnd = strtotime($ssap['END_TIME']);
                    $currentStart = strtotime($surgeryStart);
                    $currentEnd = strtotime($surgeryEnd);

                    // 时间重叠判断
                    if ($ssapStart < $currentEnd && $currentStart < $ssapEnd) {
                        $overlappingSurgeries[] = $ssap;
                    }
                }
                ...
                if (!empty($overlappingSurgeries)) {
                    ...
                    foreach ($overlappingSurgeries as $overlap) {
                        $overlapName = $overlap['ICD9_NAME'] ?? '';
                        $overlapStart = $overlap['START_TIME'] ?? '';
                        $overlapEnd = $overlap['END_TIME'] ?? '';
    ...
    /**
     * 根据开始时间和工作日天数，获取结束时间，自动跳过节假日和调休日
     * @param string $startTime 开始时间
     * @param int $days 工作日天数
     * @return string 结束时间
     */
    public function getEndTime($startTime = '', $days = 0)
    {
        // 获取配置
        // 节假日日期列表
        $ruleMap8092 = RuleWordMap::query()->where('id', '=', 8092)->value('keyword');
        ...
        $realDays = $days;
        for ($i = 0; $i < $days; $i++) {
            $endTime = date("Y-m-d", strtotime($startTime . ' +' . $i . ' days'));
            // 如果是周六日但是不是调休日则跳过
            $dayOfWeek = date('w', strtotime($endTime));
            if (in_array($endTime, $holidays) || (($dayOfWeek == 0 || $dayOfWeek == 6) && !in_array($endTime, $workdays))) {
                $realDays++;
                continue;
            }
        }

        return date("Y-m-d H:i:s", strtotime($startTime . ' +' . $realDays . ' days'));
    }
}
