...
class BanhzkgzService
{
    ...
    public function zkgzHandle($caseRule, $info, $isSz = 0)
    {
        ...
        $methodList = [
            ...
            8000 => 'rule8000', // 首次病程记录中姓名与入院记录的患者姓名不一致
            8001 => 'rule8001', // 首次病程记录中年龄与入院记录的患者年龄不一致
        ];
    ...
    public function rule1039($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        if (empty($ZYTS) || empty($AAC04)) {
            return [];
        }

        // 如果两个字段都不为空且不一致，则记录
        $zytsDiff = null;
        if (!empty($ZYTS) && !empty($AAC04) && is_numeric($ZYTS) && is_numeric($AAC04)) {
            $zytsDiff = abs($ZYTS - $AAC04);
        }
        if ($zytsDiff > 1) {
    ...
    public function rule1034($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $newData = $bl01esService->getDataByEs($restful);
    ...
    public function rule1028($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        if (strtotime($newData["CYRQ"]) === false) {
            return [];
        }
    ...
    public function rule1027($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        if (strtotime($newData["RYRQ"]) === false) {
            return [];
        }
     ...
     * @param string $ZYH
     * @return array
     * 首次病程记录中姓名与入院记录的患者姓名不一致
     */
    public function rule8000($caseRule = [], $ruleId, $ZYH = "")
    {
        $ryjl = Bllb292::query()->where("ZYH", $ZYH)->first();
        if (empty($ryjl)) {
            return [];
        }
        $ryjl = $ryjl->toArray();
        ...
        $keyword = RuleWordMap::query()->where('id', '=', 20051)->value('keyword');
        $keyword = !empty($keyword) ? trim($keyword) : '患者';
        $keywords = strpos($keyword, ',') !== false ? explode(',', $keyword) : [$keyword];

        $scbc = Bllb294_295::query()->where("ZYH", $ZYH)->first();
        if (empty($scbc)) {
            return [];
        }
        $scbc = $scbc->toArray();

        $fullText = ($scbc['HJNR'] ?? '') . ($scbc['BLTD'] ?? '');
        if (empty($fullText)) {
            return [];
        }

        $normalizedText = preg_replace('/\s+/u', '', $fullText);
        $targets = [];
        foreach ($keywords as $item) {
            ...
            if (strpos($normalizedText, $normalizedTarget) !== false) {
                return [];
            }
        }
        ...
        $basis = [];
        $basis[] = '入院记录/患者姓名【' . $xm . '】';
        $basis[] = '首次病程记录全文未包含【' . implode('、', $targets) . '】中的任意一项';
        $basis['BLBH'] = $scbc['BLBH'] ?? '';
     ...
     * @param string $ZYH
     * @return array
     * 首次病程记录中年龄与入院记录的患者年龄不一致
     */
    public function rule8001($caseRule = [], $ruleId, $ZYH = "")
    {
        $ryjl = Bllb292::query()->where("ZYH", $ZYH)->first();
        if (empty($ryjl)) {
            return [];
        ...
        $nl = trim($ryjl['NL'] ?? '');
        if (empty($nl) || !preg_match('/(\d+)/', $nl, $matches)) {
            return [];
        }
        $admissionAge = (int)$matches[1];
        ...
        $fullText = ($scbc['HJNR'] ?? '') . ($scbc['BLTD'] ?? '');
        if (empty($fullText) || mb_strpos($fullText, '岁') === false) {
            return [];
        }

        $replaceCount = 0;
        $fullText = preg_replace('/鉴别诊断(.*?)诊疗计划/su', '', $fullText, 1, $replaceCount);
        if (!preg_match('/(\d+)\s*岁/u', $fullText, $matches)) {
            return [];
        }
        $courseAge = (int)$matches[1];

        if ($courseAge === $admissionAge) {
            return [];
        }

        $basis = [];
        $basis[] = '入院记录/患者年龄【' . $nl . '】';
        $basis[] = '首次病程记录中年龄为【' . $courseAge . '岁】，与入院记录不一致';
        $basis['BLBH'] = $scbc['BLBH'] ?? '';
    ...
    public function rule261($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($yzb as $y) {
            ...
            $params = $bl01esService->clearMust()
                ...
                ->source(['BLBH', 'CJSJ', 'BLMC', 'BLLB', 'first_blsy_time'])
                ->getParams();
            ...
            $must = [
                ['term' => ['JZHM' => $ZYH]],
                ['term' => ['MBLB' => 82]],
                ['range' => ['operation_time' => ['lt' => strtotime($y['KZSJ'])]]]
            ];
            $mustNot = [
                ['term' => ['BLZT' => 9]],
            ];
    ...
    public function rule102($caseRule, $ruleId, $ZYH, $isYj = 0)
    {
        ...
        foreach ($pacs as $p) {
            ...
            if (!$hasYzb) {
                ...
                $must[] = ['terms' => ['BLLB' => $map9009]];
                $must[] = ['range' => ['ZXSJ' => ['gt' => $p['BGSJ'], 'lt' => $bgsj]]];
                $shouldList = [];
                foreach ($ctFirst as $item) {
                ...
                //$shouldNotList['should'] = [];
                $mustNot = [];
                foreach ($octFirst as $item) {
                    //$mustNot['should'][] = ['match_phrase' => ['HJNR' => $item]];
                ...
                $params = $bl01esService->clearMust()
                    ...
                    ->source(['BLBH', 'CJSJ', 'BLMC', 'BLLB', 'first_blsy_time', 'HJNR'])
                    ->getParams();
                $res = app('es')->search($params);
                $resData = $bl01esService->getDataByEs($res);
        ...
        foreach ($pacs as $p) {
            ...
            if (!$hasYzb) {
                ...
                $must = [
                    ['term' => ['JZHM' => $ZYH]],
                    ['terms' => ['BLLB' => $map9009]],
                    ['range' => ['ZXSJ' => ['gte' => $p['BGSJ'], 'lte' => $bgsj]]]
                ];
                /* $mustNot = [
                    ['term' => ['BLZT' => 9]],
                    ['match_phrase' => ['BLMC' => $map9015['keyword']]]
    ...
    public function rule109($caseRule, $ruleId, $ZYH, $isYj = 0): array
    {
        ...
        $map1010 = RuleWordMap::query()->where('id', 1010)->value('keyword');
        $map9009 = RuleWordMap::query()->where('id', 8068)->value('keyword');
        if (strpos($map9009, ',') !== false) {
            $map9009 = explode(',', $map9009);
        } else {
        ...
        if (!empty($resData)) {
            ...
            foreach ($resData as $v) {
                ...
                $endTime = date("Y-m-d H:i:s", strtotime($v['KZSJ']) + intval($map1010) * 3600);
                ...
                $must = [
                    ['term' => ['JZHM' => $ZYH]],
                    ['terms' => ['BLLB' => $map9009]],
                    //['match_phrase_prefix' => ['HJNR' => $kjywName]],
                ...
                $params = $bl01esService->clearMust()
                    ->queryByMustBatch($must)
                    //->queryByMustNotBatch($shouldListNot)
                    ->queryByShouldBatch($shouldList)
                    ->minimumShouldMatch(1)
                    ->getParams();
                $res = app('es')->search($params);
    ...
    public function rule110($caseRule, $ruleId, $ZYH, $isYj = 0): array
    {
        ...
        if (!empty($resData)) {
            foreach ($resData as $v) {
                ...
                $startTime = date("Y-m-d H:i:s", strtotime($v['KZSJ']) - 24 * 3600);
                $endTime = date("Y-m-d H:i:s", strtotime($v['KZSJ']) + $map9010['keyword'] * 3600);
                //获取当前时间
                $currentDate = Carbon::now()->toDateTimeString();
                //如果当前时间小于开嘱时间+72小时就跳过
                if (strtotime($currentDate) < strtotime($endTime) && $isYj == 0) {
                ...
                $must = [
                    ['term' => ['JZHM' => $ZYH]],
                    ['terms' => ['BLLB' => $map9009]],
                    //['match_phrase_prefix' => ['HJNR' => $hlywName]],
                    ['range' => ["ZXSJ" => ['from' => $startTime, 'to' => $endTime]]]
                ];
                /* $mustNot = [
                    ['match_phrase' => ['BLMC' => $map9015['keyword']]]
                ...
                $shouldList[] = ['match_phrase_prefix' => ['HJNR' => $hlywName]];
    ...
    public function rule95($caseRule, $ruleId, $ZYH, $isYj = 0)
    {
        ...
        $params = $yzbEs->clearMust()
            ->queryByMust(['term' => ['ZYH' => $ZYH]])
            ->queryByShouldBatch($should)
            ->queryByMustNotBatch($shouldNot)
            ->getParams();
        $res = app('es')->search($params);
    ...
    public function rule278($caseRule, $ruleId, $ZYH)
    {
        ...
        //使用nested查询
        foreach ($exitKeywords as $keyword) {
            $should[] = ['nested' => ['path' => 'yzb', 'query' => ['match_phrase' => ['yzb.YZMC' => $keyword]]]];
        }
    ...
    public function rule97($caseRule, $ruleId, $ZYH)
    {
        ...
        // 5. 查询医嘱是否包含出院/离院关键字
        $yzbEs = new ElasticsearchService('yzb_2023');

        $shouldNot = [];
        //排除死亡医嘱
        $shouldNot[] = ['match_phrase' => ['YZMC' => '死亡']];

        $params = $yzbEs->clearMust()
            ->queryByMust(['term' => ['ZYH' => $ZYH]])
            ->queryByShouldBatch($shouldNot)
            ->minimumShouldMatch()
            ->getParams();
        $res = app('es')->search($params);
        $yzbData = $yzbEs->getDataByEs($res);
    ...
    public function rule101($caseRule, $ruleId, $ZYH)
    {
        ...
        if (strtotime($firstBlsyTime) < strtotime($enterTime)) {
            // 签名时间早于入院时间，提前创建
            $basis[] = "首次病程记录首次签名时间【" . $firstBlsyTime . "（提前创建）】";
            $basis["BLBH"] = $bl01Data[0]['BLBH']; //加入被质控病程BLBH
            $this->insertData[] = [
                'JZHM' => $ZYH,
                'rule_id' => $ruleId,
    ...
    public function rule133($caseRule, $ruleId, $ZYH)
    {
        ...
        // 2. 从ES查询医嘱名称含"病危"的医嘱
        $esService = new ElasticsearchService('yzb_2023');
        //获取病危字段
        ...
        foreach ($yzbData[0] as $order) {
            ...
            for ($i = 0; $i < $totalRemainingCycles; $i++) {
                ...
                $must = [
                    ...
                    ['match_phrase' => ['ZYH' => $ZYH]],
                    ['range' => ['KZSJ' => ['gte' => $starttime, 'lte' => $endtime]]]
                ];
    ...
    public function rule134($caseRule, $ruleId, $ZYH)
    {
        ...
        // 构建查询条件：医嘱名称含"病重"且住院号为$ZYH
        $must = [
            ['match_phrase' => ['YZMC' => $ruleMap8014]],
            ['match_phrase' => ['ZYH' => $ZYH]]
        ];
    ...
    public function rule1001($caseRule, $ruleId, $ZYH)
    {
        ...
        try {
            ...
            //查询是否有手术记录mblb=306
            $ssjl = EMR_BL_BL01::query()->where('JZHM', '=', $ZYH)->where('MBLB', 306)->get()->toArray();
            if (empty($ssjl)) {
                return true;
            }
        } catch (\Exception $e) {
    ...
    public function rule1002($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($ssapData as $surgery) {
            ...
            // 两种条件：
            // 1. MBLB符合术后首次病程记录类型
            // 2. BLLB为病程记录且BLMC包含"术后首次病程记录"
            // 前面两项是或的关系，就是mblb满足或者blmc满足
            $should = [
                ['terms' => ['MBLB' => $recordTypes]],
                [
                    'bool' => [
                        'must' => [
                            ['term' => ['BLLB' => $ruleMap8010]],
                            ['match_phrase' => ['BLMC' => $ruleMap8018]]
                        ]
                    ]
                ]
            ];

            // 时间范围：手术结束时间 到 手术结束后6小时
            $time_range = [
                'range' => [
                    $ruleMap8011 => [
                        'gte' => $surgeryEndTime,
                        'lte' => $sixHoursAfterSurgery
                    ]
                ]
            ];
            ...
            $res = app('es')->search($params);
            $records = $bl01Service->getDataByEs($res);
        ...
        foreach ($ssapData as $surgery) {
            ...
            $should = [
                ...
                [
                    'bool' => [
                        'must' => [
                            ['term' => ['BLLB' => $ruleMap8021]],
                            ['match_phrase' => ['BLMC' => $ruleMap8020]]
                        ]
                    ]
                ]
            ];
                    ...
                    if ($jjbz == 1) {
                        ...
                        $must[] = [
                            'range' => [
                                $ruleMap8011 => [
                                    'gte' => $startCheckTime,
                                    'lte' => $surgeryStartTime
                                ]
                            ]
                        ];
                        $must[] = [
                            'terms' => [
                                'MBLB' => $recordTypes8016
                            ]
                        ];
                        $must[] = [
                            'term' => [
                        ...
                        $params = $bl01esService->clearMust()
                            ...
                            ->getParams();
                        $res = app('es')->search($params);
                        $records8016 = $bl01esService->getDataByEs($res);
                        if (!empty($records8016[0])) {
                            $records = $records8016[0];
                        } else {
                            // 无病程记录情况
    ...
    public function rule1006($caseRule, $ruleId, $ZYH)
    {
        ...
        // 构建查询条件：医嘱名称含"抢救"且住院号为$ZYH
        $must = [
            ['match_phrase' => ['YZMC' => $ruleMap8026]],
            ['match_phrase' => ['ZYH' => $ZYH]]
        ];
    ...
    public function rule1014($caseRule, $ruleId, $ZYH)
    {
        ...
        if (strpos($ruleMap8035, ',') !== false) {
            ...
            foreach ($zcKeywords as $keyword) {
                if (!empty(trim($keyword))) {
                    $should[] = [
                        'match_phrase' => ['YZMC' => trim($keyword)]
                    ];
                }
            }
            if (!empty($should)) {
            ...
            $must[] = ['match_phrase' => ['YZMC' => $ruleMap8035]];
        }

        //排除YZMC中包含‘好转’的医嘱
        $mustnot[] = ['match_phrase' => ['YZMC' => '好转']];
        ...
        foreach ($yzbData[0] as $order) {
            ...
            $esExcludeService = new ElasticsearchService('yzb_2023');
            $excludeMust = [
                ['match_phrase' => ['ZYH' => $ZYH]],
                ['range' => ['KZSJ' => [
                    'gte' => $beforeStartTime,
            ...
            if (!empty($excludeShould)) {
                $excludeMust[] = ['bool' => ['should' => $excludeShould]];
            }

            try {
                $excludeParams = $esExcludeService->clearMust()
                    ->queryByMustBatch($excludeMust)
                    ->getParams();
                $excludeRes = app('es')->search($excludeParams);
                $excludeData = $esExcludeService->getDataByEs($excludeRes);
 ...
 */
    ...
    public function rule1063($caseRule, $ruleId, $ZYH)
    {
        ...
        foreach ($cultivationResults as $result) {
            ...
            // 计算72小时后的时间
            $deadline = date('Y-m-d H:i:s', strtotime($reportTime) + $map9010['keyword'] * 3600);
            ...
            $must = [
                ['term' => ['JZHM' => $ZYH]],
                ['terms' => ['MBLB' => $bcRecordTypes]], // 上级医师查房记录，日常病程记录
                ['range' => [$rule8011 => ['gte' => $reportTime, 'lte' => $deadline]]]
            ];

            // 构建搜索条件：病程文本中需包含"培养"或细菌名称
            $shouldList = [];
            foreach ($rule8080 as $v) {
                $shouldList[] = ['match_phrase' => ['HJNR' => $v]];
            }
            if (!empty($xjmc)) {
                $shouldList[] = ['match_phrase' => ['HJNR' => $xjmc]];
            }
    ...
    public function rule1064($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        $rule8011 = RuleWordMap::query()->where('id', 8011)->value('keyword');
        $rule8011 = !empty($rule8011) ? $rule8011 : 'ZXSJ';
        // es查询符合条件的病程记录：BLLB=294且BLMC含"操作记录"
        $bl01esService = new ElasticsearchService('bl01_202303');
        $must = [];
        $must[] = ['term' => ['JZHM' => $ZYH]];
        $must[] = ['terms' => ['BLLB' => $rule8010]];
        $shouldList = [];
        if (!empty($rule8081)) {
            foreach ($rule8081 as $item) {
                $shouldList[] = ['match_phrase' => ['BLMC' => $item]];
            }
        }
    ...
    public function rule1247($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        // 使用Elasticsearch查询死亡病例讨论结论记录
        $bl01esService = new ElasticsearchService('bl01_202303');
        $must = [];
        $must[] = ['term' => ['JZHM' => $ZYH]];
        $must[] = ['terms' => ['MBLB' => $mblbTypes]];
    ...
    public function rule1251($caseRule = [], $ruleId, $ZYH = "")
    {
        ...
        // 2. ES查询符合条件的操作记录：BLLB=294且BLMC含"操作记录"
        $bl01esService = new ElasticsearchService('bl01_202303');
        $must = [];
        $must[] = ['term' => ['JZHM' => $ZYH]];
        $must[] = ['terms' => ['BLLB' => $rule8010]];
