...
use App\Services\MysqlDataSync\sanyuan\IndicatorService;
...
class IndicatorCalcService
{
    ...
    public function calculateAll($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        $this->calculateSjsshzssdrbcjjbzb($zyh, $start_time, $end_time);
        // 计算高额异常费用患者进行疑难病例讨论的占比
        $this->calculateGeycfyhfzjxynbltldzb($zyh, $start_time, $end_time);
        // 计算术前讨论计划手术一致率
        $this->calculateSqtljhssyzl($zyh, $start_time, $end_time);
        // 计算医务部门组织讨论的死亡病例与发生纠纷的死亡病例比值（医务科组织死亡病例病例讨论率）
        $this->calculateSwblyfsjfdswblbz($zyh, $start_time, $end_time);
        // 计算术中自体血回输率
        $this->calculateSzztxhsl($zyh, $start_time, $end_time);
    }
        ...
        foreach ($patientData as $patient) {
            ...
            for ($i = 0; $i < count($ssapData); $i++) {
                ...
                if (!empty($ruleMap8047)) {
                    ...
                    if (!empty($ssCZ)) {
                        //更新到ssap的ICD9_SSLB字段
                        SM_SSAP::query()->where('id', $surgery['id'])->update(['ICD9_SSLB' => $ssCZ]);
    ...
    public function calculateSsyzgfkjl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            for ($i = 0; $i < count($ssapList); $i++) {
                ...
                // 查找该手术对应的"拟...术"医嘱（手术开始时间之前开嘱的最后一个医嘱）
                $matchedOrder = null;
                $endCheckTime = $surgeryStart; // 手术开始时间作为结束时间
                
                // 查找手术开始时间之前开嘱的最后一个"拟...术"医嘱
                foreach ($orders as $order) {
    ...
    public function calculatePthzyxl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        // 获取规则词映射：会诊医嘱关键词
        $ruleMap20000 = RuleWordMap::query()->where('id', '=', 20000)->value('keyword'); // 邀请
        $ruleMap20001 = RuleWordMap::query()->where('id', '=', 20001)->value('keyword'); // 会诊

        if (empty($ruleMap20000) || empty($ruleMap20001)) {
        ...
        // 处理多个关键词（用英文逗号分隔）
        $inviteKeywords = strpos($ruleMap20000, ',') !== false ? explode(',', $ruleMap20000) : [$ruleMap20000];
        $consultKeywords = strpos($ruleMap20001, ',') !== false ? explode(',', $ruleMap20001) : [$ruleMap20001];
    ...
    public function calculateZyhzfjhssl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            foreach ($ssapList as $ssap) {
                ...
                if (!empty($ruleMap8047)) {
                    // 如果SSLB在8047包含列表中，则保留
                    if (!empty($ssLB) && in_array($ssLB, $excludeKeywords8047)) {
                        $isValidSurgery = true;
                    } else {
                        // 如果SSLB不在8047中，检查8048例外列表
                        if (!empty($ruleMap8048)) {
                ...
                if ($isValidSurgery) {
                    $validSurgeries[] = $surgeryName;
                }
            }

            $validSurgeryCount = count($validSurgeries);

            // 只显示过滤后符合条件的手术
            if ($validSurgeryCount == 0) {
                $patientGroup['content'][] = [
                    'status' => 0,
                    'content' => "无手术"
                ];
                $patientGroup['status'] = 0;
            } elseif ($validSurgeryCount == 1) {
                // 如果只有1台手术，展示这台手术，status=0
                $patientGroup['content'][] = [
                    'status' => 0,
                    'content' => "（手麻）手术名称1：【{$validSurgeries[0]}】"
                ];
                $patientGroup['status'] = 0;
            } else {
                // 如果有2台及以上手术，全部展示，每个status=1
                $surgeryIndex = 1;
                foreach ($validSurgeries as $surgeryName) {
                    $patientGroup['content'][] = [
                        'status' => 1,
                        'content' => "（手麻）手术名称{$surgeryIndex}：【{$surgeryName}】"
                    ];
                    $surgeryIndex++;
                }
                // 分子+1，总的status=1
    ...
    public function calculateSjsshzssdrbcjjbzb($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        $isSanyuan = (strpos($hospitalName, 'sanyuan') !== false);
        ...
        foreach ($patientData as $patient) {
            ...
            foreach ($validOperations as $op) {
                ...
                if ($isSanyuan) {
                    // 三院：对应ICD9_NAME手术名称 + 术者姓名/工号在GY_SSML中匹配
                    // YY_SSCZMC like '%手术名称%' and code like '%工号%'
                    if (!empty($surgeonCode)) {
                        $ssmlList = GY_SSML::query()
                            ->where('YY_SSCZMC', 'like', '%' . $surgeryName . '%')
                            ->where('code', 'like', '%' . $surgeonCode . '%')
                            ->get()
                            ->toArray();
                        ...
                        if (!empty($ssmlList)) {
                            ...
                            $group['content'][] = ['status' => 0, 'content' => "术者【{$surgeonDisplayName}】，无手术资质"];
                        }
                    } else {
                        $group['content'][] = ['status' => 0, 'content' => "术者【{$surgeonDisplayName}】，无手术资质（缺少术者工号）"];
                    }
                } else {
        ...
        foreach ($patientData as $patient) {
            ...
            foreach ($ssapList as $surgery) {
                ...
                if (!empty($SQDH)) {
                    $sssqData = SSSQ::query()->where('SQDH', $SQDH)->first();
                    if (!empty($sssqData)) {
                        $sssqSurgeryName = $sssqData['NSSMC'] ?? '';
                        
                        if (!empty($sssqSurgeryName)) {
                            // 比较手术名称是否一致
                            if ($surgeryName === $sssqSurgeryName) {
                                $isConsistent = true;
                                $insert['sqtljhssyzl_fz'] += 1;
                                $group['status'] = 1;
                                $group['content'][] = ['status' => 1, 'content' => "（手术申请）手术名称：【{$sssqSurgeryName}】，一致"];
                            } else {
                                $group['content'][] = ['status' => 0, 'content' => "（手术申请）手术名称：【{$sssqSurgeryName}】，不一致"];
                            }
                        } else {
                            $group['content'][] = ['status' => 0, 'content' => "（手术申请）手术名称：【无】，不一致"];
                        }
                    } else {
                        $group['content'][] = ['status' => 0, 'content' => "（手术申请）手术名称：【无申请单】，不一致"];
                    }
                } else {
                    $group['content'][] = ['status' => 0, 'content' => "（手术申请）手术名称：【无SQDH】，不一致"];
                }
    ...
    public function calculateSwblyfsjfdswblbz($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            if (empty($bl01Records)) {
                // 没有死亡病例讨论记录
                $group['content'][] = ['status' => 0, 'content' => "【blmc】，医务科未组织（无死亡病例讨论记录）"];
                $errorContent[] = $group;
    ...
    public function calculateSzztxhsl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        // 自体血回收收费项目关键字（fee_detailed.FYMC，id=8104）
        $ruleMap8104 = RuleWordMap::query()->where('id', '=', 8104)->value('keyword');
        $autoBloodFeeKeywords = !empty($ruleMap8104)
            ? (strpos($ruleMap8104, ',') !== false ? explode(',', $ruleMap8104) : [$ruleMap8104])
            : ['自体血回收'];
        ...
        foreach ($patientData as $patient) {
            ...
            foreach ($sxList as $sx) {
                ...
                foreach ($autoBloodKeywords as $kw) {
                    $kw = trim($kw);
                    if ($kw !== '' && strpos($cfx, $kw) !== false) {
                        $hasAutoBloodTransfusion = true;
                        break 2;
                    }
                }
            }

            if (!$hasAutoBloodTransfusion) {
                // 没有自体血输注记录，不计入分母
                continue;
            }

            // 分母：按患者计算，有自体血输注记录就算 1 个分母
            ...
            if (!empty($cfxValues)) {
                $group['content'][] = [
                    'status' => 1,
                    'content' => "输【" . implode('、', array_unique($cfxValues)) . "】"
                ];
            }

            // 2. 检查是否存在自体血回收收费项目（fee_detailed）
            $hasAutoBloodFee = false;
            $autoBloodFeeNames = [];
            try {
                $feeQuery = DB::table('fee_detailed')->where('AAA28', '=', $ZYH);
                $feeQuery->where(function ($query) use ($autoBloodFeeKeywords) {
                    foreach ($autoBloodFeeKeywords as $kw) {
                        $kw = trim($kw);
                        if ($kw !== '') {
                            $query->orWhere('FYMC', 'like', '%' . $kw . '%');
                        }
                    }
                });
                $feeData = $feeQuery->get(['FYMC'])->toArray();

                if (!empty($feeData)) {
                    $hasAutoBloodFee = true;
                    foreach ($feeData as $fee) {
                        $name = $fee->FYMC ?? '';
                        if ($name !== '') {
                            $autoBloodFeeNames[] = $name;
                        }
                    }
                }
            } catch (\Exception $e) {
                Log::error("calculateSzztxhsl-$ZYH-fee_detailed-error: " . $e->getMessage());
            }

            if ($hasAutoBloodFee && !empty($autoBloodFeeNames)) {
                $group['content'][] = [
                    'status' => 1,
                    'content' => "自体血收费项目【" . implode('、', array_unique($autoBloodFeeNames)) . "】"
                ];
            } else {
                $group['content'][] = [
                    'status' => 0,
            ...
            // 3. 检查手术记录（MBLB=306）中是否含“自体血”
            $hasAutoBloodInNote = false;
            $matchedBlmc = '';
            try {
                $ssRecords = EMR_BL_BL01::query()
                    ->where('JZHM', '=', $ZYH)
                    ->where('MBLB', '=', 306)
                    ->get(['BLBH', 'BLMC'])
                    ->toArray();
                ...
                if (!empty($ssRecords)) {
                    foreach ($ssRecords as $record) {
                        ...
                        foreach ($autoBloodKeywords as $kw) {
                            $kw = trim($kw);
                            if ($kw !== '' && strpos($hjnr, $kw) !== false) {
                                $hasAutoBloodInNote = true;
                                $matchedBlmc = $blmc;
                                break 2;
                            }
                        }
                ...
                Log::error("calculateSzztxhsl-$ZYH-ss-record-error: " . $e->getMessage());
            }

            if ($hasAutoBloodInNote) {
                $group['content'][] = [
                    'status' => 1,
                    'content' => "手术记录【{$matchedBlmc}】含自体血"
                ];
            } else {
                $group['content'][] = [
                    'status' => 0,
            ...
            // 分子：同时满足收费项目和手术记录条件
            if ($hasAutoBloodFee && $hasAutoBloodInNote) {
