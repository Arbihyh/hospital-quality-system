...
class IndicatorService
{
    ...
    public function getPatientInfoData($zyh, $startTime, $endTime)
    {
        ...
        $filed = [
            ...
            'patient_info.AAA28',
            'patient_info.AEM01C',
            'patient_info.AAC02C_MC AS AAC11N',
            'patient_doctor_info.AEE03'
        ];
        $query = PatientInfo::query()
            ->join('patient_doctor_info', 'patient_info.MED_REC_ID', '=', 'patient_doctor_info.AAA28')
    ...
    public function handleSjyscf($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['sjyscf_fm' => 0, 'sjyscf_fz' => 0, 'sjyscf_error' => null]);
            $error = [];
            //如果出院时间和入院时间在24小时内的排除
            /**
             * 日间病历排除
             */
            $aac01Date = date('Ymd', strtotime($item['AAC01']));
            $aab01Date = date('Ymd', strtotime($item['AAB01']));
            ...
            if ($insert['zyh']) {
                ...
                $insert['AAA28'] = $item['AAA28'];
                $insert['AAA28'] = $item['AAA28'];
                $insert['AAA01'] = $item['AAA01'];
                $insert['AAC11N'] = $item['AAC11N'];
    ...
    public function handleEjyscf($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['ejyscf_fm' => 0, 'ejyscf_fz' => 0, 'ejyscf_error' => null]);
            // 如果出院时间和入院时间在24小时内的排除
            /**
             * 日间病历排除：日期相差小于等于1天的跳过
             */
            $aac01Date = date('Ymd', strtotime($item['AAC01']));
            $aab01Date = date('Ymd', strtotime($item['AAB01']));
            $diff = $aac01Date - $aab01Date;
            ...
            if (empty($endTime)) {
                ...
                while (true) {
                    ...
                    $bl01Data = EMR_BL_BL01::where('JZHM', $item['MED_REC_ID'])
                        ->where('BLLB', 294)
                        ->where('BLZT', '<>', $this->ruleMap2036)
                        ->whereBetween($ruleMap1058, [$startTime, $startTimeEnd])
                        ...
                        foreach ($bl01Data as $val) {
                            ...
                            if (
                                ...
                                if ($hasValidDoctor) {
                                    ...
                                    $strContent5 = ["status" => 0, "content" => '无二级医师签名'];
                                }
    ...
    {
        $esService = new ElasticsearchService('indicator');
        $ruleMap8110 = RuleWordMap::query()->where('id', '=', 8110)->value('keyword');
        $ruleMap8110 = !empty($ruleMap8110) ? (strpos($ruleMap8110, ',') !== false ? array_filter(explode(',', $ruleMap8110)) : [$ruleMap8110]) : ['三级医师首次查房', '主任', '副主任', '三级'];
        $ruleMap8111 = RuleWordMap::query()->where('id', '=', 8111)->value('keyword');
        $ruleMap8111 = !empty($ruleMap8111) ? (strpos($ruleMap8111, ',') !== false ? array_filter(explode(',', $ruleMap8111)) : [$ruleMap8111]) : ['目前治疗措施疗效分析及下一步诊疗计划'];
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['zlfajcqyssh_fz' => 0, 'zlfajcqyssh_fm' => 0, 'zlfajcqyssh_error' => null]);
            /**
             * 日间病历排除
             */
            $aac01Date = date('Ymd', strtotime($item['AAC01']));
            ...
            $bl294295 = Bllb294_295::query()->where('ZYH', $item['MED_REC_ID'])->first(['BLMC', 'ZLJH']);
            $firstCoursePlan = '';
            $firstCourseBlmc = '';
            if (!empty($bl294295)) {
                $firstCourseBlmc = $bl294295->BLMC ?? '';
            }
            if (!empty($bl294295) && !empty($bl294295->ZLJH)) {
                $zljh = json_decode($bl294295->ZLJH, true);
                if (is_array($zljh)) {
                    $firstCoursePlan = trim(implode('', array_filter($zljh, function ($val) {
                        return $val !== null && $val !== '';
                    })));
                } elseif (is_string($bl294295->ZLJH)) {
                    $firstCoursePlan = trim($bl294295->ZLJH);
                }
            }
            $content[] = ["status" => 1, "content" => "入院时间【" . $item['AAB01'] . "】"];
            $content[] = ["status" => 1, "content" => "出院时间【" . $item['AAC01'] . "】"];
            $content[] = ["status" => 1, "content" => "首次病程【" . ($firstCourseBlmc ?: '首次病程记录') . "】"];
            if (empty($firstCoursePlan)) {
                $insert['zlfajcqyssh_fz'] += 1;
                $content[] = ["status" => 1, "content" => "首次病程诊疗计划【无】"];
                $content[] = ["status" => 1, "content" => "无诊疗方案，无法判断"];
                $error[] = ["status" => 1, "content" => $content];
                $insert['zlfajcqyssh_error'] = json_encode($error, 256);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
            ...
            $must = [
                ...
                [
                    'bool' => [
                        'should' => array_values(array_filter(array_map(function ($keyword) {
                            $keyword = trim($keyword);
                            return $keyword === '' ? null : ['match_phrase' => ['BLMC' => $keyword]];
                        }, $ruleMap8110))),
                ...
                [
                    'bool' => [
                        'should' => array_values(array_filter(array_map(function ($keyword) {
                            $keyword = trim($keyword);
                            return $keyword === '' ? null : ['match_phrase' => ['HJNR' => $keyword]];
                        }, $ruleMap8111))),
                        'minimum_should_match' => 1
                    ]
                ]
            ...
            $mustNot = ['term' => ['BLZT' => $this->ruleMap2036]];
            $params = (new ElasticsearchService('bl01_202303'))
                ->clearMust()
                ->queryByMustBatchSafe($must)
                ->queryByMustNot($mustNot)
                ->source(['BLBH', 'BLMC', 'HJNR', 'ZXSJ'])
                ->paginate(1, 1)
                ->orderBy('ZXSJ', 'asc')
                ->getParams();
            $res = app('es')->search($params);
            $bl01Data = (new ElasticsearchService('bl01_202303'))->getDataByEs($res);
            if (empty($bl01Data[0][0])) {
                $insert['zlfajcqyssh_fz'] += 1;
                $content[] = ["status" => 1, "content" => "三级医师首次查房【无】"];
                $content[] = ["status" => 1, "content" => "无诊疗方案，无法判断"];
                $error[] = ["status" => 1, "content" => $content];
                $insert['zlfajcqyssh_error'] = json_encode($error, 256);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
            ...
            $matchedRecord = $bl01Data[0][0];
            $hjnr = $matchedRecord['HJNR'] ?? '';
            $doctorPlan = '';
            $matched8111 = '';
            $startPos = false;
            foreach ($ruleMap8111 as $keyword) {
                $keyword = trim($keyword);
                if ($keyword === '') {
                    continue;
                }
                $startPos = mb_strpos($hjnr, $keyword);
                if ($startPos !== false) {
                    $matched8111 = $keyword;
                    break;
                }
            ...
            if ($startPos === false) {
                $content[] = ["status" => 1, "content" => "三级医师首次查房【" . $matchedRecord['BLMC'] . "】"];
                $content[] = ["status" => 1, "content" => "三级医师首次查房诊疗计划【无】"];
                $content[] = ["status" => 1, "content" => "无诊疗方案，无法判断"];
                $insert['zlfajcqyssh_fz'] += 1;
                $error[] = ["status" => 1, "content" => $content];
                $insert['zlfajcqyssh_error'] = json_encode($error, 256);
            ...
            $segment = mb_substr($hjnr, $startPos);
            $endPos = mb_strpos($segment, '五、');
            if ($endPos !== false) {
                $segment = mb_substr($segment, 0, $endPos);
            }
            ...
            $normalizedFirstCoursePlan = preg_replace('/[^\p{L}\p{N}]/u', '', $firstCoursePlan);
            $normalizedDoctorPlan = preg_replace('/[^\p{L}\p{N}]/u', '', $doctorPlan);
            if (!empty($normalizedFirstCoursePlan) && !empty($normalizedDoctorPlan) && $normalizedFirstCoursePlan === $normalizedDoctorPlan) {
                $insert['zlfajcqyssh_fz'] += 1;
                $content[] = ["status" => 1, "content" => "首次病程与三级医师首次查房诊疗计划【一致】"];
            } else {
                $content[] = ["status" => 0, "content" => "首次病程与三级医师首次查房诊疗计划【不一致】"];
            }
    ...
    public function handleSqfjhzssbltl($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['sqfjhzssbltlsq_fz' => 0, 'sqfjhzssbltlsq_fm' => 0, 'sqfjhzssbltlsq_error' => null]);
            ...
            $sssqData = SM_SSAP::query()->where('ZYH', $item['MED_REC_ID'])->where('FJHSS', 'Y')->get()->toArray();
            ...
            foreach ($sssqData as $value) {
                ...
                //分子
                $ssmc = $value['ICD9_SSCZMC'] ?? "";
                if (empty($ssmc)) {
                    continue;
                }
                $content[] = ["status" => 1, "content" => "(手麻)手术名称【" . $ssmc . "】(非计划再次手术)"];
                //$ssap = $value;
                $SSRQ = $value['SSRQ'] ?? "";
                $JSRQ = $value['JSRQ'] ?? "";
     ...
     * @return void
     */
    public function handleFjhzcsshzynblwzl($data)
    {
        $esService = new ElasticsearchService('indicator');
        $indicatorCode = 'fjhzcsshzynblwzl';
        $this->UpdateIndexCataLog($indicatorCode, 1, time());

        foreach ($data as $item) {
            $zyh = $item['MED_REC_ID'];
            Indicator::query()->updateOrInsert(['zyh' => $zyh], [
                'fjhzcsshzynblwzl_fz' => 0,
                'fjhzcsshzynblwzl_fm' => 0,
                'fjhzcsshzynblwzl_error' => null
            ]);
            $esService->esSaveIndicator($zyh, [
                'fjhzcsshzynblwzl_fz' => 0,
                'fjhzcsshzynblwzl_fm' => 0,
                'fjhzcsshzynblwzl_error' => null
            ]);
            ...
            /* $sssqData = SSSQ::query()
                ...
                ->where(function ($query) {
                    $query->where('ZFBZ', '0')->orWhere('ZFBZ', '否');
                }) //=0或者否

                ->get()
                ->toArray(); */
            $sssqData = SM_SSAP::query()->where('ZYH', $zyh)->where('FJHSS', 'Y')->get()->toArray();
            if (empty($sssqData)) {
                continue;
            }

            foreach ($sssqData as $value) {
                $insert['fjhzcsshzynblwzl_fm'] += 1;
                $content = [
                    ['status' => 1, 'content' => '(手麻)非计划再手术【是】']
                ];

                $ssmc = $value['ICD9_SSCZMC'] ?? '';
                if (!empty($ssmc)) {
                    $content[] = ['status' => 1, 'content' => '(手麻)手术名称【' . $ssmc . '】'];
                }

                $ssrq = $value['SSRQ'] ?? '';
                if (empty($ssrq)) {
                    $content[] = ['status' => 0, 'content' => '手术开始时间【无】'];
                    $error[] = ['status' => 0, 'content' => $content];
                    continue;
                }
                ...
                $kzsjStart = date('Y-m-d H:i:s', strtotime($ssrq) - 48 * 3600);
                $kzsjEnd = date('Y-m-d H:i:s', strtotime($ssrq));
                $bl01Data = $this->getDifficultCaseRecords($zyh);
                if (empty($bl01Data)) {
                    $content[] = ['status' => 0, 'content' => '无疑难病例讨论记录'];
                    $error[] = ['status' => 0, 'content' => $content];
                    continue;
                }
                ...
                foreach ($bl01Data as $record) {
                    $recordGroup = $this->buildDifficultCaseCompletenessGroup($record, $content);
                    if ($recordGroup['status'] == 1) {
                ...
                if (!empty($selectedGroup) && $selectedGroup['status'] == 1) {
                    $insert['fjhzcsshzynblwzl_fz'] += 1;
                }
                if (!empty($selectedGroup)) {
                    $error[] = $selectedGroup;
                }
            ...
            if (!empty($error)) {
                $insert['fjhzcsshzynblwzl_error'] = json_encode($error, JSON_UNESCAPED_UNICODE);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);

                if ($insert['zyh']) {
                    $insert['AAA28'] = $item['AAA28'] ?? $zyh;
                    $insert['AAA01'] = $item['AAA01'] ?? '';
                    $insert['AAC11N'] = $item['AAC11N'];
                    $esService->esSaveIndicator($insert['zyh'], $insert);
                }
        ...
        $this->UpdateIndexCataLog($indicatorCode, 2, time());
    }
     ...
     * @return void
     */
    public function handleFjhzcryynblwzl($data)
    {
        $esService = new ElasticsearchService('indicator');
        $indicatorCode = 'fjhzcryynblwzl';
        $this->UpdateIndexCataLog($indicatorCode, 1, time());
        ...
        foreach ($data as $item) {
            ...
            $denominator = $this->getUnplannedReadmissionDenominatorContent($item);
            if (empty($denominator)) {
                continue;
            }

            $insert['fjhzcryynblwzl_fm'] = 1;
            $content = $denominator;
            $bl01Data = $this->getDifficultCaseRecords($zyh);
            if (empty($bl01Data)) {
                $content[] = ['status' => 0, 'content' => '无疑难病例讨论记录'];
                ...
                if (!empty($selectedGroup) && $selectedGroup['status'] == 1) {
                    $insert['fjhzcryynblwzl_fz'] = 1;
                }
                $error = !empty($selectedGroup) ? [$selectedGroup] : [];
            }

            if (!empty($error)) {
                $insert['fjhzcryynblwzl_error'] = json_encode($error, JSON_UNESCAPED_UNICODE);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
    ...
    public function handleShfjhzssbltl($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['shfjhzssbltlsh_fz' => 0, 'shfjhzssbltlsh_fm' => 0, 'shfjhzssbltlsh_error' => null]);
            /**
             * 日间病历排除
             */
            $aac01Date = date('Ymd', strtotime($item['AAC01']));
    ...
    public function handleLcyxhpg($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['lcyxhpg_fz' => 0, 'lcyxhpg_fm' => 0, 'lcyxhpg_error' => null]);
            $insert = [
                'zyh' => $item['MED_REC_ID'],
                'lcyxhpg_fz' => 0,
                'lcyxhpg_fm' => 0,
            ...
            foreach ($sxList as $sx) {
                ...
                if (empty($jssj) || $jssj == '1970-01-01 00:00:00' || $jssj == '0000-00-00 00:00:00') {
                    // 无效结束时间，记录并跳过分子判断
                    $content[] = ["status" => 0, "content" => "输血结束时间【无】"];
                } else {
                    $start = date('Y-m-d H:i:00', strtotime($kssj) - 24 * 3600);
                    ...
                    $must3[] = ['term' => ['BLLB' => 294]];
                    $must3[] = ['range' => ['ZXSJ' => ['from' => $start, 'to' => $end]]];
    ...
    public function handleRjssblsqtl($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['rjssblsqtl_fz' => 0, 'rjssblsqtl_fm' => 0, 'rjssblsqtl_error' => null]);
    ...
    public function handleRjssblsqpg($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['rjssblsqpg_fz' => 0, 'rjssblsqpg_fm' => 0, 'rjssblsqpg_error' => null]);
    ...
    public function handleRjssblrcyjl24($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['rjssblrcyjl24_fz' => 0, 'rjssblrcyjl24_fm' => 0, 'rjssblrcyjl24_error' => null]);
    ...
    public function handleYnbltlgf($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['ynbltlgf_fz' => 0, 'ynbltlgf_fm' => 0, 'ynbltlgf_error' => null]);
            $must = [
                ['term' => ['JZHM' => $item['MED_REC_ID']]],
                ['term' => ['MBLB' => $this->ruleMap1045]]
            ];
    ...
    public function handleKjywsyjl($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['kjywsyjl_fz' => 0, 'kjywsyjl_fm' => 0, 'kjywsyjl_error' => null]);
    ...
    public function handleKzrzcswtll($data)
    {
        ...
        //获取科主任数据
        $kzrData = KZR::query()->get(['KZRMC', 'KZRZC'])->toArray();
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['kzrzcswtll_fz' => 0, 'kzrzcswtll_fm' => 0, 'kzrzcswtll_error' => null]);
            ...
            $bl01Data = EMR_BL_BL01::query()
                ...
                ->where('EMR_BL_BL01.JZHM', '=', $item['MED_REC_ID'])
                ->where('EMR_BL_BL01.MBLB', '=', $ruleMap1051)
                ->where('EMR_BL_BL01.BLZT', '<>', $this->ruleMap2036)
            ...
            if (!empty($bl01Data)) {
                ...
                if ($hasSwYzb) {
                    if (!empty($bl01Data[0])) {
                        ...
                        // 模式2: 在...科主任XXX主持下（限制"科"字前的字符数，避免匹配到"医务科"）
                        if (empty($matches[1]) && preg_match("/在[^科]{0,6}(科|病区)主任([^\s，、。；]{2,20}?)(主持下|的主持下)/u", $HJNR, $temp)) {
                            $matches = $temp;
                        }

                        // 模式3: 匹配顿号或逗号后的科主任
                        if (empty($matches[1]) && preg_match("/[、，](科|病区)主任([^\s，、。；]{2,20}?)(主持下|的主持下)/u", $HJNR, $temp)) {
                            $matches = $temp;
                        }

                        // 模式3: 匹配顿号或逗号后的科主任
                        if (empty($matches[1]) && preg_match('/主持人姓名:(.*?)\s/u', $HJNR, $temp)) {
                            $matches = $temp;
                        }
                        var_dump($matches);
                        // 确定要检查的科主任信息
    ...
    public function handleWjzjlfhl($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['wjzjlfhl_fz' => 0, 'wjzjlfhl_fm' => 0, 'wjzjlfhl_error' => null]);
            ...
            try {
                $wjzData = WJZ::query()->where('ZYH', '=', $item['MED_REC_ID'])->groupBy('WJZSJ')->orderBy('WJZSJ', 'asc')->get(['WJZSJ', 'WJZNR'])->toArray();
                if (empty($wjzData)) {
                    continue; // 没有危急值数据，跳过
                }
            } catch (\Exception $e) {
            ...
            // 处理每条危急值记录
            foreach ($wjzData as $wjzIndex => $wjz) {
                ...
                $matchedWindow = 'forward';

                // 第一条危急值仅查后6小时，后续危急值在后6小时无记录时再回查前30分钟。
                try {
                    $bl01Data = EMR_BL_BL01::query()
                        ->where('JZHM', $item['MED_REC_ID'])
                    ...
                    if ($wjzIndex > 0 && empty($bl01Data)) {
                        $matchedWindow = 'backward';
                        $backwardLimit = date('Y-m-d H:i:s', strtotime($wjzsj) - 30 * 60);
                        $bl01Data = EMR_BL_BL01::query()
                            ->where('JZHM', $item['MED_REC_ID'])
                            ->whereIn('BLLB', $blTypes)
                            ->where('BLMC', 'like', '%' . $ruleMap8030 . '%')
                            ->where($ruleMap8011, '>=', $backwardLimit)
                            ->where($ruleMap8011, '<', $wjzsj)
                            ->orderBy($ruleMap8011, 'desc')
                            ->get()
                            ->toArray();
                    }
                } catch (\Exception $e) {
                ...
                if (!empty($bl01Data)) {
                    ...
                    $zxsj = $record[$ruleMap8011] ?? ''; // 标题时间
                    $firstBlsyTime = $record[$firstBlsyTimeField] ?? ''; // 首次签名时间
                    $isBackwardMatched = $matchedWindow === 'backward';
                    $signTimeValid = !empty($firstBlsyTime)
                        && strtotime($firstBlsyTime) <= strtotime($sixHoursLimit)
                        && ($isBackwardMatched || strtotime($firstBlsyTime) >= strtotime($wjzsj));

                    $wjzGroup['content'][] = [
                        'status' => 1,
                        'content' => "危急值记录【" . $blmc . "】" . ($isBackwardMatched ? '（前30分钟匹配）' : '')
                    ];
                    ...
                    } else if (!$isBackwardMatched && strtotime($firstBlsyTime) < strtotime($wjzsj)) {
                        $wjzGroup['content'][] = [
                            'status' => 0,
    ...
    public function handleRyjl24wcl($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['ryjlxswcl24_fz' => 0, 'ryjlxswcl24_fm' => 0, 'ryjlxswcl24_error' => null]);
            ...
            if (empty($bl01)) {
                ...
                if (empty($tmpTime) || $tmpTime == '1970-01-01 00:00:00' || $tmpTime == '0000-00-00 00:00:00') {
                    $errorContent[] = ["status" => 0, "content" => "首次签名时间【无上级医师审签】"];
    ...
    public function handlescbc8wcl($data)
    {
        ...
        foreach ($data as $item) {
            ...
            //删除
            Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], ['scbcjlwc8_fm' => 0, 'scbcjlwc8_fz' => 0, 'scbcjlwc8_error' => null]);
            $esService->esSaveIndicator($insert['zyh'], ['scbcjlwc8_fm' => 0, 'scbcjlwc8_fz' => 0, 'scbcjlwc8_error' => null]);

            /**
             * 日间病历排除（与handleHzry48xsnzk保持一致）
             */
            $aac01Date = date('Ymd', strtotime($item['AAC01']));
            $aab01Date = date('Ymd', strtotime($item['AAB01']));
            $diff = $aac01Date - $aab01Date;
            ...
            // 分母：住院天数大于等于24小时的患者（与handleHzry48xsnzk保持一致）
    ...
    public function handleSsjlwc24($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['ssjlwc24_fz' => 0, 'ssjlwc24_fm' => 0, 'ssjlwc24_error' => null]);
            ...
            $errorMsg = [];
            $allSurgeriesCompliant = true; // 用于跟踪所有手术是否都符合条件
            $hasSurgery = false; // 是否有手术
                ...
                if (!empty($ruleMap8047)) {
                    ...
                    if (!empty($ssCZ)) {
                        if (!in_array($ssCZ, $excludeKeywords8047)) {
                            if (!empty($ruleMap8048)) {
                                if (!in_array($surgeryName, $excludeKeywords8048)) {
                                    echo '手术名称【' . $surgeryName . '】符合排除条件，跳过' . PHP_EOL;
                                    continue;
                                }
                            } else {
                                echo '手术类别【' . $ssCZ . '】符合排除条件，跳过' . PHP_EOL;
                                continue;
                            }
                        }
                    }
            ...
            // 只有当所有手术都符合条件时，分子才为1
            if ($allSurgeriesCompliant && $hasSurgery) {
    ...
    public function handleXjshxxmssr($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['xjshxxmssr_fz' => 0, 'xjshxxmssr_fm' => 0, 'xjshxxmssr_error' => null]);
                ...
                while (true) {
                    ...
                    if (empty($data) && (int) $ycfNum <= 0) {
                        ...
                        foreach ($data as $val) {
                            ...
                            //查询签名
                            $blsy = '';
                            $blsydata = EMR_BL_BLSY::query()->where('BLBH', $val['BLBH'])->where('FG_ACTIVE', 1)->groupBy('SYYS')->get()->toArray(); //SYYS分组
                            if (!empty($blsydata)) {
    ...
    public function handleHzry48xsnzk($data, $startTime)
    {
        ...
        foreach ($data as $item) {
            ...
            //删除
            Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], ['hzry48xsnzk_fm' => 0, 'hzry48xsnzk_fz' => 0, 'hzry48xsnzk_error' => null]);
            $esService->esSaveIndicator($insert['zyh'], ['hzry48xsnzk_fm' => 0, 'hzry48xsnzk_fz' => 0, 'hzry48xsnzk_error' => null]);
            $errorMsg = [
                [
                    'status' => 0,
                    'content' => []
    ...
    public function handleHzry8xsncf($data, $startTime)
    {
        ...
        foreach ($data as $item) {
            ...
            if (!empty($BL01)) {
                $BLMC = $BL01[0]['BLMC'];
                $ZXSJ = $BL01[0]['ZXSJ'];
                $firstBlsyTime = $BL01[0]['first_blsy_time'];
                var_dump($ZXSJ);
                var_dump($firstBlsyTime);
                var_dump($endTime);
                //zxsj和firstBlsyTime都在8小时范围内才记作分子(大于等于入院时间小于等于endTime)
                if (strtotime($firstBlsyTime) <= $endTime) {
                    ...
                    $errorMsg[0]['content'][0]['content'] = "入院时间【" . $item['AAB01'] . "】";
                    $errorMsg[0]['content'][1]['status'] = 1;
                    $errorMsg[0]['content'][1]['content'] = "首次病程记录中首次签名时间【" . $firstBlsyTime . "】";
                } //ZXSJ在8小时范围内，firstBlsyTime不在8小时范围内
                elseif (strtotime($firstBlsyTime) > $endTime) {
                    $errorMsg[0]['content'][1]['status'] = 0;
                    $errorMsg[0]['content'][1]['content'] = "首次病程记录中首次签名时间【" . $firstBlsyTime . "】（超时）";
                }
            } else {
                $errorMsg[0]['content'][2]['status'] = 0;
                $errorMsg[0]['content'][2]['content'] = "首次病程记录【无】";
            }
    ...
    public function handleEjhlsjhlcy($data, $startTime)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator(data_get($item, 'MED_REC_ID'), ['ejhlsjhlcy_fm' => 0, 'ejhlsjhlcy_fz' => 0, 'ejhlsjhlcy_error' => null]);
            ...
            $yzb = Yzb::query()->where('ZYH', data_get($item, 'MED_REC_ID'))
                ->where('YZQX', 1)
                ->whereBetween('TZSJ', [$startTime, $endTime])
                ->get(['YZMC', 'TZSJ'])->toArray();
            if (empty($yzb)) {
                continue;
            }


            $should = ['三级护理', '特级护理', 'Ⅲ级护理', 'III级护理'];
            $hlyzb = [];
            // 匹配一级护理、特级护理、Ⅰ级护理关键字
            if (!empty($yzb)) {
                foreach ($yzb as $item) {
                    /* if (in_array($item['YZMC'], $should)) {
                        $hlyzb = $item;
                    } */
                    foreach ($should as $val) {
                        if (strpos($item['YZMC'], $val) !== false || strpos($val, $item['YZMC']) !== false) {
                            $hlyzb = $item;
                            break;
                        }
                    }
            ...
            if ($hlyzb) {
                ...
                $errorMsg[0]['content'][0]['status'] = 1;
                $errorMsg[0]['content'][1]['status'] = 1;
                $errorMsg[0]['content'][1]['content'] = "医嘱名称【" . $hlyzb['YZMC'] . "】";
                $errorMsg[0]['content'][2]['status'] = 1;
                $errorMsg[0]['content'][2]['content'] = "停嘱时间【" . $hlyzb['TZSJ'] . "】";
            }
    ...
    public function handleCqyzdrzz($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator(data_get($item, 'MED_REC_ID'), ['cqyzdrzz_fm' => 0, 'cqyzdrzz_fz' => 0, 'cqyzdrzz_error' => null]);
            $insert = [
                'zyh' => data_get($item, 'MED_REC_ID'),
                'cqyzdrzz_fz' => 0,
                'cqyzdrzz_fm' => 0,
    ...
    public function handleSsyssssjch($data)
    {
        ...
        foreach ($data as $item) {
            //删除之前的数据
            Indicator::query()->updateOrInsert(['zyh' => data_get($item, 'MED_REC_ID')], ['ssyssssjch_fm' => 0, 'ssyssssjch_fz' => 0, 'ssyssssjch_error' => null]);
            $esService->esSaveIndicator(data_get($item, 'MED_REC_ID'), ['ssyssssjch_fm' => 0, 'ssyssssjch_fz' => 0, 'ssyssssjch_error' => null]);
            $insert = [
                'zyh' => data_get($item, 'MED_REC_ID'),
                'ssyssssjch_fz' => 0,
                'ssyssssjch_fm' => 0,
     ...
     * 麻醉医师手术时间重合率
     */
    public function handleMzysmzsjch($zyh = "", $start_time = "", $end_time = "")
    {

        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("mzysmzsjch", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);

        foreach ($patientData as $patient) {
            // 删除旧结果
            Indicator::updateOrInsert(['zyh' => $patient['MED_REC_ID']], ['mzysmzsjch_fz' => 0, 'mzysmzsjch_fm' => 0, 'mzysmzsjch_error' => null]);
            $ZYH = $patient['MED_REC_ID'];
            $esService->esSaveIndicator($ZYH, ['mzysmzsjch_fz' => 0, 'mzysmzsjch_fm' => 0, 'mzysmzsjch_error' => null]);
            echo 'zyh----：' . $ZYH . PHP_EOL;
            $insert = [
                'zyh' => $ZYH,
                'mzysmzsjch_fz' => 0,
                'mzysmzsjch_fm' => 0,
                'mzysmzsjch_error' => null,
            ...
            for ($i = 0; $i < count($ssapList); $i++) {
                ...
                if (
                    empty($mzkssj) || empty($mzjsj) || empty($surgeryName) ||
                    strpos($mzjsj, '1970-01-01') !== false || $surgeryName === 'NULL'
                ) {
                    continue;
                }

                // 检查必要字段（id 和 SZDM 用于后续业务逻辑）
                if (empty($currentId) || empty($mzysName)) {
                    continue;
                }
                ...
                $group['content'][] = ['status' => 1, 'content' => "（手麻）麻醉医师【{$mzysName}】"];
                $group['content'][] = ['status' => 1, 'content' => "（手麻）麻醉开始时间【{$mzkssj}】"];
                $group['content'][] = ['status' => 1, 'content' => "（手麻）麻醉结束时间【{$mzjsj}】"];

                // 统计分母
                $insert['mzysmzsjch_fm'] += 1;
                ...
                $overlappingSurgeries = SM_SSAP::query()
                    ...
                    ->where('SSRQ', '<>', '')
                    ->where('MZJSSJ', '<>', '')
                    ->where('MZKSSJ', 'not like', '1970-01-01%')
                    ->where('MZJSSJ', 'not like', '1970-01-01%')
                    ->get()
                    ->toArray();
                ...
                if (!empty($overlappingSurgeries)) {
                    // 有重合，算分子
                    $group['status'] = 1;
                    $insert['mzysmzsjch_fz'] += 1;

                    // 输出所有重合时间段的手术
                    foreach ($overlappingSurgeries as $overlap) {
                        $overlapName = $overlap['ICD9_SSCZMC'] ?? '';
                        $overlapStart = $overlap['MZKSSJ'] ?? '';
                        $overlapEnd = $overlap['MZJSSJ'] ?? '';
                        //住院号
                        ...
                        //术者
                        $overlapMzysName = $overlap['MZYS_MC'] ?? '';
                        //$overlapSurgeonCode = $overlap['SZDM'] ?? '';
                        $group['content'][] = ['status' => 1, 'content' => "同时间段(手麻)麻醉医师【{$overlapMzysName}】"];
                        ...
                        $group['content'][] = ['status' => 1, 'content' => "同时间段(手麻)麻醉开始时间【{$overlapStart}】"];
                        $group['content'][] = ['status' => 1, 'content' => "同时间段(手麻)麻醉结束时间【{$overlapEnd}】"];
                    }
                } else {
                    $group['content'][] = ['status' => 0, 'content' => "同时间段麻醉【无】"];
                }
            ...
            if (!empty($errorContent)) {
                $insert['mzysmzsjch_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
        ...
        IndexCatalog::query()->where("index_name", "=", "mzysmzsjch")->update(["status" => 2, 'quality_time' => time()]);
    }

    /* public function handleMzysmzsjch($data)
    {

        $this->UpdateIndexCataLog('mzysmzsjch', 1, time());
        $esService = new ElasticsearchService('indicator');
        $carbon = new Carbon();

        foreach ($data as $item) {
            Indicator::query()->updateOrInsert(['zyh' => data_get($item, 'MED_REC_ID')], ['mzysmzsjch_fm' => 0, 'mzysmzsjch_fz' => 0, 'mzysmzsjch_error' => null]);
            $esService->esSaveIndicator(data_get($item, 'MED_REC_ID'), ['mzysmzsjch_fm' => 0, 'mzysmzsjch_fz' => 0, 'mzysmzsjch_error' => null]);
            $insert = [
                'zyh' => data_get($item, 'MED_REC_ID'),
                'mzysmzsjch_fz' => 0,
                'mzysmzsjch_fm' => 0,
            ...
            echo $insert['zyh'] . PHP_EOL;


            
            $mainOperation = mainOperation::query()
                ->where('AAA28', '=', data_get($item, 'MED_REC_ID'))
                ->get(['HOCUS_MAN_CODE', 'ICD9_ID1', 'ICD9_NAME', 'START_TIME', 'END_TIME'])->toArray();
            ...
            $insert['mzysmzsjch_fm'] = 1;
            $mzysmzsjchError = [];
            foreach ($allOperation as $val) {
                if (empty($val['START_TIME']) || empty($val['END_TIME'])) {
                    continue;
                }


                // 判断是否存在ICD9_ID1不同但时间有重合的数据
                foreach ($allOperation as $other) {
                    if (empty($other['START_TIME']) || empty($other['END_TIME'])) {
                        continue;
                    }
                    if ($other['ICD9_ID1'] != $val['ICD9_ID1'] && $other['HOCUS_MAN_CODE'] == $val['HOCUS_MAN_CODE']) {
                        // 时间区间重叠判断：[val_start, val_end] 与 [other_start, other_end] 有重叠
                        if (strtotime($other['START_TIME']) < strtotime($val['END_TIME']) && strtotime($val['END_TIME']) > strtotime($other['START_TIME'])) {
                            $errorMsg = [
                                'status' => 1,
                                'content' => []
                            ];
                            ...
                            $errorMsg['content'][] = [
                                'status' => 1,
                                'content' => "手术名称：{$val['ICD9_NAME']}，手术时间：{$val['START_TIME']}至{$val['END_TIME']}"
                            ];
                            $errorMsg['content'][] = [
                                'status' => 1,
                                'content' => "手术名称：{$other['ICD9_NAME']}，手术时间：{$other['START_TIME']}至{$other['END_TIME']}"
                            ];
                            $mzysmzsjchError[] = $errorMsg;
                            break;
                        }
                    }
                }
            }
            if ($insert['mzysmzsjch_fz'] == 0 && empty($mzysmzsjchError)) {
                foreach ($allOperation as $val) {
                    if (empty($val['START_TIME']) || empty($val['END_TIME'])) {
                        continue;
                    }
            ...
            $insert['mzysmzsjch_error'] = !empty($mzysmzsjchError) ? json_encode($mzysmzsjchError, 256) : '';
            Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
    ...
    public function handleSjssysjssbfzfsl($data)
    {
        ...
        $this->UpdateIndexCataLog('sjssysjssbfzfs', 1, time());
        $esService = new ElasticsearchService('indicator');
        $carbon = new Carbon();
    ...
    public function handleSjssbfzfsl($data)
    {
        ...
        foreach ($data as $key => $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['sjssbfz_fz' => 0, 'sjssbfz_fm' => 0, 'sjssbfz_error' => null]);
            $insert = [
                'zyh' => data_get($item, 'MED_REC_ID'),
                'sjssbfz_fz' => 0,
                'sjssbfz_fm' => 0,
            ...
            // 并发症编码通过 rule_word_map 配置，按编码长度截取诊断编码前缀匹配
            $ruleMap20071 = RuleWordMap::query()->where('id', '=', 20071)->value('keyword');
            $bfz = array_values(array_unique(array_filter(array_map('trim', explode(',', (string)$ruleMap20071)))));
            ...
            //查询所有四级手术，如果有就跳过
            $mainOperation4 = MainOperation::query()->where('AAA28', '=', data_get($item, 'MED_REC_ID'))->where('ope_level', 4)->get(['ope_level', 'ICD9_NAME', 'ICD9_ID1'])->toArray();
            $secondaryOperation4 = SecondaryOperation::query()->where('AAA28', '=', data_get($item, 'MED_REC_ID'))->where('ope_level', 4)->get(['ope_level', 'ICD9_NAME', 'ICD9_ID1'])->toArray();
            $operation4 = array_merge($mainOperation4, $secondaryOperation4);
            if (!empty($operation4)) {
                echo '有四级手术，跳过' . PHP_EOL;
                continue; // 有四级手术，跳过
            }
            echo '没有四级手术' . PHP_EOL;
            //是否有满足条件的手术
            ...
            // 获取主诊断和其他诊断
            $mainDiagnosis = MainDiagnosis::query()->where('AAA28', '=', data_get($item, 'MED_REC_ID'))->get(['ICD10_ID1', 'ICD10_NAME', 'RYQK'])->toArray();
            $otherDiagnosis = OtherDiagnosis::query()->where('AAA28', '=', data_get($item, 'MED_REC_ID'))->get(['ICD10_ID1', 'ICD10_NAME', 'RYQK'])->toArray();
            ...
            foreach ($allDiagnosis as $diagnosis) {
                ...
                if ($diagnosis['RYQK'] == 1) {
                    $ryqk = '有';
                } elseif ($diagnosis['RYQK'] == 2) {
                    $ryqk = '临床未确定';
                } elseif ($diagnosis['RYQK'] == 3) {
                    $ryqk = '情况不明';
                } elseif ($diagnosis['RYQK'] == 4) {
                    $ryqk = '无';
                } else {
                    $ryqk = $diagnosis['RYQK'];
                }
                $diagnosisCode = trim($diagnosis['ICD10_ID1'] ?? '');
                $diagnosisName = $diagnosis['ICD10_NAME'] ?? '';
                $isComplication = false;
                $matchedComplicationCode = '';

                // 判断是否为并发症编码：按规则编码位数截取诊断编码前缀匹配
                if ($ryqk == '无') {
                    foreach ($bfz as $bfzCode) {
                        if (substr(strtoupper($diagnosisCode), 0, strlen($bfzCode)) === strtoupper($bfzCode)) {
                            $isComplication = true;
                            $hasComplication = true;
                            $matchedComplicationCode = $bfzCode;
                            break;
                        }
                    }
                }
                ...
                if ($isComplication) {
                    $complicationDiagnosis[] = [
                        'code' => $diagnosisCode,
                        'name' => $diagnosisName,
                        'rule_code' => $matchedComplicationCode,
                        'ryqk' => $ryqk
                    ];
                } else {
                    $nonComplicationDiagnosis[] = [
            ...
            if (!empty($complicationDiagnosis)) {
                foreach ($complicationDiagnosis as $comp) {
                    ...
                    $surgeryGroup['content'][] = [
                        'status' => 1,
                        'content' => "诊断编码【" . $comp['code'] . "(" . $comp['ryqk'] . ")】（属于术后并发症）"
                    ];
                }
            }
            ...
            if (!empty($nonComplicationDiagnosis) && empty($complicationDiagnosis)) {
                foreach ($nonComplicationDiagnosis as $nonComp) {
                    $surgeryGroup['content'][] = [
                        'status' => 0,
                        'content' => "诊断编码【" . $nonComp['code'] . "(" . $nonComp['ryqk'] . ")】（不是术后并发症）"
                    ];
                }
            }
    ...
    public function handleSijssbfzfsl($data)
    {
        ...
        foreach ($data as $key => $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['sijssbfz_fz' => 0, 'sijssbfz_fm' => 0, 'sijssbfz_error' => null]);
            $insert = [
                'zyh' => data_get($item, 'MED_REC_ID'),
                'sijssbfz_fz' => 0,
                'sijssbfz_fm' => 0,
    ...
    public function handleSjsssw($data)
    {
        ...
        foreach ($data as $key => $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['sjsssw_fz' => 0, 'sjsssw_fm' => 0, 'sjsssw_error' => null]);
            $insert = [
                'zyh' => data_get($item, 'MED_REC_ID'),
                'sjsssw_fz' => 0,
                'sjsssw_fm' => 0,
    ...
    public function handleSijsssw($data)
    {
        ...
        foreach ($data as $key => $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['sijsssw_fz' => 0, 'sijsssw_fm' => 0, 'sijsssw_error' => null]);
            $insert = [
                'zyh' => data_get($item, 'MED_REC_ID'),
                'sijsssw_fz' => 0,
                'sijsssw_fm' => 0,
            ...
            /* //查询是否有三级手术，如果有，跳过
            $mainOperations = MainOperation::query()->where('AAA28', '=', data_get($item, 'MED_REC_ID'))->where('ope_level', 3)->get(['ope_level', 'ICD9_NAME', 'ICD9_ID1'])->toArray();
            $secondaryOperations = SecondaryOperation::query()->where('AAA28', '=', data_get($item, 'MED_REC_ID'))->where('ope_level', 3)->get(['ope_level', 'ICD9_NAME', 'ICD9_ID1'])->toArray();
            $operations = array_merge($mainOperations, $secondaryOperations);
            if (!empty($operations)) {
                continue; // 有三级手术，跳过
            }
            echo '有三级手术，跳过' . PHP_EOL; */
    ...
    /* public function handleNewSqfjhzssbltl($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['sqfjhzssbltl_fz' => 0, 'sqfjhzssbltl_fm' => 0, 'sqfjhzssbltl_error' => null]);
            ...
            if (empty($ssapData) || count($ssapData) == 0) {
                echo '无手术记录：' . $item['MED_REC_ID'] . PHP_EOL;
                continue;
            }
            ...
            if (count($surgeryGroups) <= 1) {
                echo '无多台非重合手术：' . $item['MED_REC_ID'] . PHP_EOL;
                continue;
            }
            ...
            if (empty($SSRQ) || empty($JSRQ)) {
                echo '手术时间为空：' . $item['MED_REC_ID'] . PHP_EOL;
                continue;
            }

            $content[] = ["status" => 1, "content" => "(手麻)手术名称【" . $ssmc . "】(非计划)"];
            $content[] = ["status" => 1, "content" => "(手麻)手术开始时间【" . $SSRQ . "】"];
            $content[] = ["status" => 1, "content" => "(手麻)手术结束时间【" . $JSRQ . "】"];
            ...
            $bl01Data = EMR_BL_BL01::query()
                ->where('JZHM', $item['MED_REC_ID'])
                ->where('BLLB', 43)
                ->where('MBLB', $this->ruleMap1045)
                ->where('ZXSJ', '>=', $kzsjStart)
                ->where('ZXSJ', '<=', $kzsjEnd)
                ->where('BLZT', '<>', $this->ruleMap2036)
                ->get()->toArray();

            if (!empty($bl01Data)) {
                $firstTime = $bl01Data[0]['first_blsy_time'] ?? "";
                if (!empty($firstTime)) {
                    if (strtotime($firstTime) >= strtotime($kzsjStart) && strtotime($firstTime) <= strtotime($kzsjEnd)) {
                        $insert['sqfjhzssbltl_fz'] += 1;
                        $content[] = ["status" => 1, "content" => "疑难病历讨论记录【" . $bl01Data[0]['BLMC'] . "】"];
                        $content[] = ["status" => 1, "content" => "首次签名时间【" . $firstTime . "】"];
                    } else if (strtotime($firstTime) < strtotime($kzsjStart)) {
                        $content[] = ["status" => 0, "content" => "疑难病历讨论记录【" . $bl01Data[0]['BLMC'] . "】"];
                        $content[] = ["status" => 0, "content" => "首次签名时间【" . $firstTime . "】(提前)"];
                    } else if (strtotime($firstTime) > strtotime($kzsjEnd)) {
                        $content[] = ["status" => 0, "content" => "疑难病历讨论记录【" . $bl01Data[0]['BLMC'] . "】"];
                        $content[] = ["status" => 0, "content" => "首次签名时间【" . $firstTime . "】(超时)"];
    ...
    public function handleNewSqfjhzssbltl($data)
    {
        ...
        foreach ($data as $item) {
            ...
            foreach ($sssqData as $value) {
                ...
                if (!empty($bl01Data[0])) {
                    $insert['sqfjhzssbltl_fz'] += 1;
                    foreach ($bl01Data[0] as $bl) {
                        $content[] = ["status" => 1, "content" => "疑难病历讨论记录【" . $bl['BLMC'] . "】"];
                    }
                } else {
                    $content[] = ['status' => 0, 'content' => "疑难病历讨论记录【无】"];
                }
    ...
    public function handleNewShfjhzssbltl($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['shfjhzssbltl_fz' => 0, 'shfjhzssbltl_fm' => 0, 'shfjhzssbltl_error' => null]);
            $insert = [
                'zyh' => $item['MED_REC_ID'],
                'shfjhzssbltl_fz' => 0,
                'shfjhzssbltl_fm' => 0,
            ...
            if (empty($diagnosis)) {
                echo '无诊断：' . $item['MED_REC_ID'] . PHP_EOL;
                continue;
            }
            $iszd = false;
            foreach ($diagnosis as $value) {
                ...
                if ($diagnosisName == 'Z51.0' || $diagnosisName == 'Z51.1' || $diagnosisName == 'Z51.2' || $diagnosisName == 'Z51.8') {
                    $iszd = true;
                    break;
                }
            }
            if ($iszd) {
                echo '有需要排除的诊断：' . $item['MED_REC_ID'] . PHP_EOL;
                continue;
            }
            //住院次数
            $zycs = data_get($item, 'AAA29', 0);
            if ($zycs == 1 || empty($zycs)) {
                echo '住院次数为1或为空：' . $zycs . PHP_EOL;
                continue;
            }
            ...
            if ($sczycs <= 0) {
                echo '上次住院次数为0：' . $sczycs . PHP_EOL;
                continue;
            }
            $AAA28 = data_get($item, 'AAA28', '');
            if (empty($AAA28)) {
                echo '无住院记录：' . $item['MED_REC_ID'] . PHP_EOL;
                continue;
            }
            //查询上次住院记录
            $sczyData = PatientInfo::query()->where('AAA28', $AAA28)->where('AAA29', $sczycs)->get()->toArray();
            $scaac01 = $sczyData[0]['AAC01'] ?? '';//上次出院时间
            $zcaab01 = $item['AAB01'] ?? '';//本次入院时间
            //如果本次入院时间-上次出院时间超过31天就跳过（按天计算）
            if (floor((strtotime($zcaab01) - strtotime($scaac01)) / 86400) > 31) {
                echo '本次入院时间-上次出院时间超过31天：' . $zcaab01 . ' - ' . $scaac01 . PHP_EOL;
                continue;
            }
            if (empty($sczyData)) {
                echo '无上次住院记录：' . $sczycs . PHP_EOL;
                continue;
            }
            $sczyh = $sczyData[0]['MED_REC_ID'] ?? '';
            if (empty($sczyh)) {
                echo '无上次住院记录：' . $item['MED_REC_ID'] . PHP_EOL;
                continue;
            }
            $hosdata = PatientHospitalInfo::query()->where('AAA28', $sczyh)->get()->toArray();
            if (empty($hosdata)) {
                echo '上次住院信息：' . $item['MED_REC_ID'] . PHP_EOL;
                var_dump($hosdata);
                continue;
            }
            ...
            if ($zzyjh == '2') {
                echo '上次住院31天再住院计划：' . $zzyjh . PHP_EOL;
                continue;
            }
            ...
            if (empty($nowDiagnosisName)) {
                echo '无主要诊断：' . $item['MED_REC_ID'] . PHP_EOL;
                continue;
            }
            ...
            if (empty($sczyDiagnosis)) {
                echo '无上次主要诊断：' . $item['MED_REC_ID'] . PHP_EOL;
                continue;
            }
            ...
            if ($nowDiagnosisName != $sczyDiagnosisName) {
                echo '主要诊断不一致：' . $nowDiagnosisName . ' != ' . $sczyDiagnosisName . PHP_EOL;
                continue;
            }
            ...
            $content = [];
            //病案号
            $content[] = ["status" => 1, "content" => "病案号【" . $item['AAA28'] . "】"];
            //当前住院次数
            $content[] = ["status" => 1, "content" => "当前住院次数【" . $zycs . "】"];
            //本次入院时间
            $content[] = ["status" => 1, "content" => "当前入院时间【" . $item['AAB01'] . "】"];
            //上次出院时间
            $content[] = ["status" => 1, "content" => "(前一次)出院时间【" . $scaac01 . "】（相差" . floor((strtotime($item['AAB01']) - strtotime($scaac01)) / 86400) . "天）"];
            ...
            echo '----------------分子' . $insert['shfjhzssbltl_fz'] . '--------------分母----------------' . $insert['shfjhzssbltl_fm'] . PHP_EOL;
            if (!empty($error)) {
    ...
    public function handleKjywcfqlshg($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['kjywcfqlshg_fz' => 0, 'kjywcfqlshg_fm' => 0, 'kjywcfqlshg_error' => null]);
            ...
            foreach ($yzbData as $yzb) {
                ...
                $kzys1 = Staff::query()->where('code', $kzys)->value('base_code') ?? '';
                $yzbGroup['content'][] = [
                    'status' => 1,
                    'content' => "开嘱医师【" . ($staffName ?: '未知') . "（" . $kzys1 . "）】"
                ];
    ...
    public function handleTsjkjywhz($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['tsjkjywhz_fz' => 0, 'tsjkjywhz_fm' => 0, 'tsjkjywhz_error' => null]);
            ...
            foreach ($yzbList as $idx => $y) {
                ...
                // 查询会诊申请，通过 zyh 关联
                $hzsqQuery = YS_ZY_HZSQ::query()->where('JZHM', '=', $item['MED_REC_ID'])->where('OA', '=', 1);
                ...
                $hzsqList = $hzsqQuery->get(['SQXH', 'SQSJ'])->toArray();
                ...
                if (!empty($hzsqList)) {
                    foreach ($hzsqList as $hzsq) {
                        ...
                        //申请时间
                        $sqsj = $hzsq['SQSJ'] ?? '';
                        $grp['content'][] = ['status' => 1, 'content' => '会诊申请时间【' . $sqsj . '】'];
                        if (empty($sqxh)) {
                            continue;
                        }
                        ...
                        if (!empty($hzyj) && !empty($hzyj['QMSJ'])) {
                            // 命中会诊意见
                            //QMSJ需要在KZSJ之前
                            if (strtotime($hzyj['QMSJ']) <= strtotime($kzsj)) {
                                ...
                                $chaoshi[] = ['status' => 0, 'content' => '审批医师签名时间【' . $hzyj['QMSJ'] . '】（在开嘱时间之后）'];
                            }
                        }
                    }
                } else {
                    $grp['content'][] = ['status' => 0, 'content' => 'HIS审批【无】'];
                }

                if (!empty($hzsqList) && !$foundConsultOpinion && empty($chaoshi)) {
                    $grp['content'][] = ['status' => 0, 'content' => 'HIS审批医师未签名'];
                } else {
                    $grp['content'] = array_merge($grp['content'], $chaoshi);
                }
    ...
    public function handleShscbcjkwc($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['shscbcjkwc_fz' => 0, 'shscbcjkwc_fm' => 0, 'shscbcjkwc_error' => null]);
    ...
    public function handleLcyxqpg($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['lcyxqpgjl_fz' => 0, 'lcyxqpgjl_fm' => 0, 'lcyxqpgjl_error' => null]);
    ...
    public function handleCyqxdkjywy($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $esService->esSaveIndicator($item['MED_REC_ID'], ['cyqxdkjywy_fz' => 0, 'cyqxdkjywy_fm' => 0, 'cyqxdkjywy_error' => null]);
    ...
    /**
     * 比较编码员与医生的所有其他诊断编码前4位是否一致
     * - 编码员其他诊断：表other_diagnosis全部记录
     * - 医生其他诊断：patient_info_diagnosis_v2，type=2
     * 只要有一个编码员的前4位没有在医生的前4位中，就算错
     *
     * @param string $zyh 住院号
     * @return array ['match' => true|false, 'details' => [...]]
     */
    public function handleQtzdzql4($data = [])
    {

        $esService = new ElasticsearchService('indicator');

        $this->UpdateIndexCataLog('qtzdzql4', 1, time());
        ...
        foreach ($data as $item) {
            ...
            // 获取编码员其他诊断
            $coderDiagnoses = OtherDiagnosis::query()
                ->where('AAA28', $zyh)
                ->get(['ICD10_ID1', 'ICD10_NAME']) // 诊断编码字段名假设为ZDBM
                ->toArray();

            if (empty($coderDiagnoses)) {
                continue;
            }

            // 获取医生其他诊断（type=2）
            $doctorDiagnoses = PatientInfoDiagnosisV2::query()
                ->where('ZYH', $zyh)
                ->where('type', '=', 2)
                ->get(['ICD10_ID1', 'ICD10_NAME']) // 假设诊断编码字段名为diagnosis_code
                ->toArray();

            // 先检查编码员和医生的差别
            foreach ($coderDiagnoses as $key => $coderDiagnosis) {
                $coderDiagnosisCode = $coderDiagnosis['ICD10_ID1'] ?? '';
                $coderDiagnosisName = $coderDiagnosis['ICD10_NAME'] ?? '';
                $coderDiagnosisPrefix = substr($coderDiagnosisCode, 0, 4);
                $flag = false;
                foreach ($doctorDiagnoses as $doctorDiagnosis) {
                    $doctorDiagnosisCode = $doctorDiagnosis['ICD10_ID1'] ?? '';
                    $doctorDiagnosisPrefix = substr($doctorDiagnosisCode, 0, 4);
                    if ($coderDiagnosisPrefix == $doctorDiagnosisPrefix) {
                        $flag = true;
                        break;
                    }
                }
                if ($flag == true) {
                    $keyNumber = $key + 1;
                    $group['content'][] = ['status' => 1, 'content' => "编码员其他诊断{$keyNumber}【{$coderDiagnosisName} {$coderDiagnosisCode}】正确"];
                } else {
                    $group['status'] = 0;
                    $group['content'][] = ['status' => 0, 'content' => "编码员其他诊断{$keyNumber}【{$coderDiagnosisName} {$coderDiagnosis['ICD10_ID1']}】不一致"];
                }
            }

            // 在检查医生和编码员的差别
            foreach ($doctorDiagnoses as $key => $doctorDiagnosis) {
                $doctorDiagnosisCode = $doctorDiagnosis['ICD10_ID1'] ?? '';
                $doctorDiagnosisName = $doctorDiagnosis['ICD10_NAME'] ?? '';
                $doctorDiagnosisPrefix = substr($doctorDiagnosisCode, 0, 4);
                $flag = false;
                foreach ($coderDiagnoses as $coderDiagnosis) {
                    $coderDiagnosisCode = $coderDiagnosis['ICD10_ID1'] ?? '';
                    $coderDiagnosisPrefix = substr($coderDiagnosisCode, 0, 4);
                    if ($coderDiagnosisPrefix == $doctorDiagnosisPrefix) {
                        $flag = true;
                ...
                if ($flag == false) {
                    $group['status'] = 0;
                    $group['content'][] = ['status' => 0, 'content' => "医生其他诊断{$keyNumber}【{$doctorDiagnosisName} {$doctorDiagnosisCode}】不一致"];
                }
            }

            if ($group['status'] == 1) {
                $insert['qtzdzql4_fz'] = 1;
            }
            $insert['qtzdzql4_error'] = json_encode([$group], JSON_UNESCAPED_UNICODE);
            Indicator::query()->updateOrInsert(['zyh' => $zyh], $insert);
            if ($insert['zyh']) {
                $insert['AAA28'] = $item['AAA28'];
                $insert['AAA28'] = $item['AAA28'];
                $insert['AAA01'] = $item['AAA01'];
        ...
        $this->UpdateIndexCataLog('qtzdzql4', 2, time());
    }

    /**
     * 编码员的全部其他手术编码前3位，与医生其他手术编码前3位（含小数点）对比，有一个不对就算错"
     * - 编码员其他手术：表secondary_operation 中所有的都是其他手术
     * - 医生其他手术：表patient_info_operation_v2 【type=2】是全部其他手术，包含主刀手术    
     *
     * @param string $zyh 住院号
     * @return array ['match' => true|false, 'details' => [...]]
     */
    public function handleQtsszql($data = [])
    {

        $esService = new ElasticsearchService('indicator');

        $this->UpdateIndexCataLog('qtsszql', 1, time());
        ...
        foreach ($data as $item) {
            ...
            // 获取编码员其他手术
            $coderOtherSurgeries = SecondaryOperation::query()
                ->where('AAA28', $zyh)
                ->get(['ICD9_ID1', 'ICD9_NAME']) // 手术编码字段名假设为SSBM
                ->toArray();

            if (empty($coderOtherSurgeries)) {
                continue;
            }

            // 获取医生其他手术（type=2）
            $doctorOtherSurgeries = PatientInfoOperationV2::query()
                ->where('ZYH', $zyh)
                ->where('type', '=', 2)
                ->get(['ICD9_ID1', 'ICD9_NAME']) // 假设手术编码字段名为operation_code
                ->toArray();

            // 先检查编码员和医生的差别
            foreach ($coderOtherSurgeries as $key => $coderOtherSurgery) {
                $keyNumber = $key + 1;
                $coderOtherSurgeryCode = $coderOtherSurgery['ICD9_ID1'] ?? '';
                $coderOtherSurgeryName = $coderOtherSurgery['ICD9_NAME'] ?? '';
                $coderOtherSurgeryPrefix = substr($coderOtherSurgeryCode, 0, 3);
                $flag = false;
                foreach ($doctorOtherSurgeries as $doctorOtherSurgery) {
                    $doctorOtherSurgeryCode = $doctorOtherSurgery['ICD9_ID1'] ?? '';
                    $doctorOtherSurgeryPrefix = substr($doctorOtherSurgeryCode, 0, 3);
                    if ($coderOtherSurgeryPrefix == $doctorOtherSurgeryPrefix) {
                        $flag = true;
                        break;
                    }
                }
                if ($flag == true) {
                    $group['content'][] = ['status' => 1, 'content' => "编码员其他手术{$keyNumber}【{$coderOtherSurgeryName} {$coderOtherSurgeryCode}】正确"];
                } else {
                    $group['status'] = 0;
                    $group['content'][] = ['status' => 0, 'content' => "编码员其他手术{$keyNumber}【{$coderOtherSurgeryName} {$coderOtherSurgeryCode}】不一致"];
                }
            }

            // 在检查医生和编码员的差别
            foreach ($doctorOtherSurgeries as $key => $doctorOtherSurgery) {
                $doctorOtherSurgeryCode = $doctorOtherSurgery['ICD9_ID1'] ?? '';
                $doctorOtherSurgeryName = $doctorOtherSurgery['ICD9_NAME'] ?? '';
                $doctorOtherSurgeryPrefix = substr($doctorOtherSurgeryCode, 0, 3);
                $flag = false;
                foreach ($coderOtherSurgeries as $coderOtherSurgery) {
                    $coderOtherSurgeryCode = $coderOtherSurgery['ICD9_ID1'] ?? '';
                    $coderOtherSurgeryPrefix = substr($coderOtherSurgeryCode, 0, 3);
                    if ($coderOtherSurgeryPrefix == $doctorOtherSurgeryPrefix) {
                        $flag = true;
                ...
                if ($flag == false) {
                    $group['status'] = 0;
                    $group['content'][] = ['status' => 0, 'content' => "医生其他手术{$keyNumber}【{$doctorOtherSurgeryName} {$doctorOtherSurgeryCode}】不一致"];
                }
            }
            ...
            $insert['qtsszql_error'] = json_encode([$group], JSON_UNESCAPED_UNICODE);
            Indicator::query()->updateOrInsert(['zyh' => $zyh], $insert);
            //如果zyh不为空，根据zyh更新或插入es
            if ($insert['zyh']) {
                $insert['AAA28'] = $item['AAA28'];
        ...
        $this->UpdateIndexCataLog('qtsszql', 2, time());
    }
     ...
     * @return array
     */
    private function getUnplannedReadmissionDenominatorContent($item)
    {
        $zyh = $item['MED_REC_ID'];
        $mainDiagnosis = MainDiagnosis::query()->where('AAA28', $zyh)->get()->toArray();
        ...
        foreach ($diagnosis as $value) {
            $diagnosisCode = substr($value['ICD10_ID1'] ?? '', 0, 5);
            if (in_array($diagnosisCode, ['Z51.0', 'Z51.1', 'Z51.2', 'Z51.8'])) {
                return [];
            }
        }
        ...
        $bah = data_get($item, 'AAA28', '');
        if (empty($bah)) {
            return [];
        }

        $sczyData = PatientInfo::query()->where('AAA28', $bah)->where('AAA29', $sczycs)->get()->toArray();
        if (empty($sczyData)) {
            return [];
        }
        $scaac01 = $sczyData[0]['AAC01'] ?? '';//上次出院时间
        ...
        $sczyDiagnosisName = $sczyDiagnosis[0]['ICD10_ID1'] ?? '';
        if (substr($nowDiagnosisName, 0, 5) != substr($sczyDiagnosisName, 0, 5)) {
            return [];
        }
        ...
        return [
            ['status' => 1, 'content' => '非计划再次入院【是】'],
            ['status' => 1, 'content' => '病案号【' . $bah . '】'],
            ['status' => 1, 'content' => '当前住院次数【' . $zycs . '】'],
            ['status' => 1, 'content' => '当前入院时间【' . $item['AAB01'] . '】'],
            ['status' => 1, 'content' => '(前一次)出院时间【' . $scaac01 . '】（相差' . floor((strtotime($item['AAB01']) - strtotime($scaac01)) / 86400) . "天）"],
            ['status' => 1, 'content' => '(前一次)住院31天再住院计划【' . $zzyjhText . '】'],
            ['status' => 1, 'content' => '主要诊断编码【' . ($mainDiagnosis[0]['ICD10_ID1'] ?? '') . '】'],
            ['status' => 1, 'content' => '主要诊断名称【' . ($mainDiagnosis[0]['ICD10_NAME'] ?? '') . '】'],
            ['status' => 1, 'content' => '(前一次)主要诊断编码【' . ($sczyDiagnosis[0]['ICD10_ID1'] ?? '') . '】'],
            ['status' => 1, 'content' => '(前一次)主要诊断名称【' . ($sczyDiagnosis[0]['ICD10_NAME'] ?? '') . '】']
        ];
    }
     ...
     * @return array
     */
    private function getDifficultCaseRecords($zyh, $startTime = '', $endTime = '')
    {
        $query = EMR_BL_BL01::query()
            ->leftJoin('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
            ->where('EMR_BL_BL01.JZHM', $zyh)
            ->where('EMR_BL_BL01.BLLB', 43)
            ->where('EMR_BL_BL01.MBLB', $this->ruleMap1045)
            ->where(function ($query) {
                $query->where('EMR_BL_BL01.BLZT', '<>', $this->ruleMap2036)
                    ->orWhereNull('EMR_BL_BL01.BLZT')
                    ->orWhere('EMR_BL_BL01.BLZT', '');
            });

        if (!empty($startTime) && !empty($endTime)) {
            $query = $query->whereBetween('EMR_BL_BL01.ZXSJ', [$startTime, $endTime]);
        }

        return $query->orderBy('EMR_BL_BL01.ZXSJ', 'asc')
            ->select([
                'EMR_BL_BL01.BLBH',
                'EMR_BL_BL01.BLMC',
                'EMR_BL_BL01.ZXSJ',
                DB::raw('EMR_BL_BLXG.HJNR as HJNR')
            ])
            ->get()
            ->toArray();
    }
     ...
     * @return array
     */
    private function buildDifficultCaseCompletenessGroup($record, $baseContent)
    {
        $content = $baseContent;
        $content[] = ['status' => 1, 'content' => '【' . ($record['BLMC'] ?? '') . '】'];

        $hjnr = $this->normalizeDifficultCaseContent($record['HJNR'] ?? '');
        $fields = [
            '讨论时间' => $this->extractDifficultCaseField($hjnr, '讨论时间'),
            '讨论地点' => $this->extractDifficultCaseField($hjnr, '讨论地点'),
            '主持人' => $this->extractDifficultCaseField($hjnr, '主持人'),
            '参加讨论人员' => $this->extractDifficultCaseField($hjnr, '参加讨论人员'),
            '具体讨论意见' => $this->extractDifficultCaseField($hjnr, '具体讨论意见'),
            '讨论结论' => $this->extractDifficultCaseField($hjnr, '讨论结论')
        ];
        $fields['主持人'] = $this->normalizeDifficultCaseHost($fields['主持人']);

        foreach (['讨论时间', '讨论地点', '主持人'] as $field) {
            $status = $fields[$field] !== '' ? 1 : 0;
            $content[] = [
                'status' => $status,
                'content' => $field . '【' . ($status ? '符合' : '不符合') . '】' . ($status ? '【' . $fields[$field] . '】' : '')
            ];
        }

        foreach (['具体讨论意见', '讨论结论'] as $field) {
            $status = $fields[$field] !== '' ? 1 : 0;
            $content[] = ['status' => $status, 'content' => $field . '【' . ($status ? '符合' : '不符合') . '】'];
        }

        $participants = $this->parseDifficultCaseParticipants($fields['参加讨论人员']);
        $participantsText = $this->formatDifficultCaseParticipants($participants);
        $participantsStatus = $this->checkDifficultCaseParticipants($participants) ? 1 : 0;
        $content[] = [
            'status' => $participantsStatus,
            'content' => '参加讨论人员【' . ($participantsStatus ? '符合' : '不符合') . '】' . ($participantsText !== '' ? '【' . $participantsText . '】' : '')
        ];

        $signatureNames = $this->getDifficultCaseSignatureNames($record['BLBH'] ?? '');
        $hostSignStatus = $this->checkDifficultCaseHostSignature($fields['主持人'], $signatureNames) ? 1 : 0;
        $content[] = [
            'status' => $hostSignStatus,
            'content' => '主持人签字【' . ($hostSignStatus ? '符合' : '不符合') . '】 ' . (!empty($signatureNames) ? implode('、', $signatureNames) : '无')
        ];

        $statusList = array_unique(array_column($content, 'status'));
        return [
            'status' => in_array(0, $statusList) ? 0 : 1,
            'content' => $content
        ];
    }
     ...
     * @return string
     */
    private function normalizeDifficultCaseContent($content)
    {
     ...
     * @return string
     */
    private function extractDifficultCaseField($content, $field)
    {
        $fields = ['讨论时间', '讨论地点', '主持人', '参加讨论人员', '具体讨论意见', '讨论结论'];
        $otherFields = array_values(array_diff($fields, [$field]));
        $endPattern = implode('|', array_map(function ($item) {
    ...
    /**
     * 主持人字段可能粘连后续正文，按职称截取到“姓名+职称”。
     *
     * @param string $host
     * @return string
     */
    private function normalizeDifficultCaseHost($host)
    {
        ...
        $normalizedHost = preg_replace('/[\s（）()：:]+/u', '', $firstLine);
        $normalizedHost = str_replace(['姓名', '职称'], '', $normalizedHost);
        $titlePattern = '(副主任(?:医师|护师)?|主任(?:医师|护师)?|主治(?:医师)?|医师|护士|护师)';

        if (preg_match('/^(.+?)' . $titlePattern . '/u', $normalizedHost, $matches)) {
    ...
    /**
     * 解析参加讨论人员，姓名和职称按常见职称关键字拆分。
     *
     * @param string $participants
     * @return array
     */
    private function parseDifficultCaseParticipants($participants)
    {
        $result = [];
        $participants = str_replace([',', ';', '；', '，'], '、', (string)$participants);
        $parts = preg_split('/、+/u', $participants, -1, PREG_SPLIT_NO_EMPTY);
        $titleKeywords = ['副主任', '主任', '主治', '医师', '护士', '护师'];
        ...
        foreach ($parts as $part) {
            ...
            $name = '';
            $title = '';
            $normalizedPart = preg_replace('/[\s（）()：:]+/u', '', $part);
            $normalizedPart = str_replace(['姓名', '职称'], '', $normalizedPart);
            $matchedPosition = false;
            foreach ($titleKeywords as $titleKeyword) {
                $position = mb_strpos($normalizedPart, $titleKeyword, 0, 'UTF-8');
                if ($position === false) {
                    continue;
                }
                if ($matchedPosition === false || $position < $matchedPosition) {
                    $matchedPosition = $position;
                }
            }

            if ($matchedPosition !== false) {
                $name = mb_substr($normalizedPart, 0, $matchedPosition, 'UTF-8');
                $title = mb_substr($normalizedPart, $matchedPosition, null, 'UTF-8');
            }
     ...
     * @return bool
     */
    private function checkDifficultCaseParticipants($participants)
    {
        ...
        foreach ($participants as $participant) {
            $name = preg_replace('/\s+/u', '', $participant['name'] ?? '');
            if (mb_strlen($name, 'UTF-8') < 2 || empty($participant['title'])) {
                return false;
            }
        }
     ...
     * @return string
     */
    private function formatDifficultCaseParticipants($participants)
    {
        $items = [];
        foreach ($participants as $participant) {
            $name = $participant['name'] ?? '';
            $title = $participant['title'] ?? '';
            $items[] = $title !== '' ? $name . '（' . $title . '）' : ($participant['raw'] ?? $name);
        }
     ...
     * @return array
     */
    private function getDifficultCaseSignatureNames($blbh)
    {
        if (empty($blbh)) {
            return [];
        }

        $syysList = EMR_BL_BLSY::query()
            ->where('BLBH', $blbh)
            ->whereNotNull('SYYS')
            ->where('SYYS', '<>', '')
            ->pluck('SYYS')
     ...
     *
     * @param string $host
     * @param array $signatureNames
     * @return bool
     */
    private function checkDifficultCaseHostSignature($host, $signatureNames)
    {
        $host = preg_replace('/\s+/u', '', (string)$host);
        if ($host === '' || empty($signatureNames)) {
            return false;
        }

        foreach ($signatureNames as $name) {
            $name = preg_replace('/\s+/u', '', (string)$name);
            if ($name !== '' && strpos($host, $name) !== false) {
                return true;
            }
        }
