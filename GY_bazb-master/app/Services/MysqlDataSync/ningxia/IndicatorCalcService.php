 ...
 * - 普通会诊有效率 calculatePyhzyxl()
 * - 手术患者特级护理/一级护理出院率 calculateSshztjhlyjhlcyl()
 * - 住院患者非计划手术率 calculateZyhzfjhssl()
...
class IndicatorCalcService
{
    ...
    public function __construct()
    {
        ...
        $this->esService = new EsSaveService();
    }

    /**
     * 计算危急值报告时间。(住院)
     *
     * 该指标不按住院号计算，只按开始时间和结束时间拆分月份后统计每月危急值报告耗时中位数。
     *
     * @param string $start_time 开始时间
     * @param string $end_time 结束时间
     * @return void
     */
    public function calculateWjzbgsj($start_time = "", $end_time = "")
    {
        ...
        try {
            IndexCatalogRg::editStatus('wjzbgsj', 1);

            $monthRanges = $this->getWjzbgsjMonthRanges($start_time, $end_time);
            $customData = [];

            foreach ($monthRanges as $monthRange) {
                $monthlyResult = $this->calculateWjzbgsjMonthlyResult(
                    $monthRange['start_time'],
                    $monthRange['end_time'],
                    2
                );
                $customData[] = [
                    'month' => $monthRange['month'],
                    'year' => (string)$monthRange['year'],
                    'fenzi' => 0,
                    'fenmu' => $monthlyResult['median_report_minutes'],
                ];
            }

            $this->updateIndexCatalogRgCustomData('wjzbgsj', $customData);
        } catch (\Exception $exception) {
            Log::error('calculateWjzbgsj-error: ' . $exception->getMessage());
        }
    }
     ...
     * @return void
     */
    public function calculateWjzbgsjMz($start_time = "", $end_time = "")
    {
        if (empty($start_time) || empty($end_time)) {
            return;
        }
        ...
        try {
            IndexCatalogRg::editStatus('wjzbgsjmz', 1);
            ...
            $this->updateIndexCatalogRgCustomData('wjzbgsjmz', $customData);
        } catch (\Exception $exception) {
            Log::error('calculateWjzbgsjmz-error: ' . $exception->getMessage());
        }
    }
     ...
     * @return void
     */
    public function calculateWjzbgsjJz($start_time = "", $end_time = "")
    {
        if (empty($start_time) || empty($end_time)) {
            return;
        }
        ...
        try {
            IndexCatalogRg::editStatus('wjzbgsjjz', 1);
            ...
            $this->updateIndexCatalogRgCustomData('wjzbgsjjz', $customData);
        } catch (\Exception $exception) {
            Log::error('calculateWjzbgsjjz-error: ' . $exception->getMessage());
        }
    }
     ...
     * @return array
     */
    private function getWjzbgsjMonthRanges($start_time, $end_time)
    {
        $startMonth = Carbon::parse($start_time)->startOfMonth();
        $endMonth = Carbon::parse($end_time)->startOfMonth();
        $monthRanges = [];
        ...
        while ($startMonth->lte($endMonth)) {
            $monthRanges[] = [
                ...
                'start_time' => $startMonth->copy()->startOfMonth()->format('Y-m-d H:i:s'),
                'end_time' => $startMonth->copy()->endOfMonth()->format('Y-m-d H:i:s'),
            ];
     ...
     * 计算单月危急值报告时间结果。
     *
     * @param string $monthStartTime 月开始时间
     * @param string $monthEndTime 月结束时间
     * @return array
     */
    private function calculateWjzbgsjMonthlyResult($monthStartTime, $monthEndTime,$wjzLx = 2)
    {
        $criticalValueData = WJZ::query()
            ->whereBetween('WJZSJ', [$monthStartTime, $monthEndTime])
            ->where('LX', $wjzLx)
            ->orderBy('WJZSJ', 'asc')
            ->get(['WJZSJ', 'WJZTSSJ'])
            ->toArray();
        ...
        foreach ($criticalValueData as $criticalValueItem) {
            if (empty($criticalValueItem['WJZSJ']) || empty($criticalValueItem['WJZTSSJ'])) {
                continue;
            }

            $reportStartTimestamp = strtotime($criticalValueItem['WJZSJ']);
            $reportEndTimestamp = strtotime($criticalValueItem['WJZTSSJ']);

            if (!$reportStartTimestamp || !$reportEndTimestamp || $reportEndTimestamp < $reportStartTimestamp) {
                continue;
            }

            $reportMinute = (int)floor(($reportEndTimestamp - $reportStartTimestamp) / 60);
            $reportMinutes[] = $reportMinute;
            $totalReportMinutes += $reportMinute;
        }

        return [
            'critical_value_count' => count($criticalValueData),
            'total_report_minutes' => $totalReportMinutes,
            'median_report_minutes' => $this->calculateWjzbgsjMedian($reportMinutes),
        ];
    }
     ...
     * 计算危急值报告耗时中位数。
     *
     * @param array $reportMinutes 报告耗时分钟数组
     * @return float|int
     */
    private function calculateWjzbgsjMedian(array $reportMinutes)
    {
        $validReportCount = count($reportMinutes);
        if ($validReportCount === 0) {
            return 0;
        }

        sort($reportMinutes, SORT_NUMERIC);

        if ($validReportCount % 2 === 1) {
            return $reportMinutes[(int)(($validReportCount - 1) / 2)];
        }

        $leftMiddleIndex = (int)($validReportCount / 2) - 1;
        $rightMiddleIndex = (int)($validReportCount / 2);

        return ($reportMinutes[$leftMiddleIndex] + $reportMinutes[$rightMiddleIndex]) / 2;
    }
     ...
     * @throws \Exception
     */
    private function updateIndexCatalogRgCustomData($indexName, array $customData)
    {
        ...
        if (!$indexCatalog) {
            throw new \Exception('未找到对应的人工指标：' . $indexName);
        }
        ...
        foreach ($existingData as $existingItem) {
            if (isset($existingItem['year']) && isset($existingItem['month'])) {
                $dataMap[$existingItem['year'] . '-' . $existingItem['month']] = $existingItem;
            }
        }

        foreach ($customData as $newItem) {
            if (isset($newItem['year']) && isset($newItem['month'])) {
                $dataMap[$newItem['year'] . '-' . $newItem['month']] = $newItem;
            }
        }

        IndexCatalogRg::query()
            ->where('index_name', $indexName)
            ->update([
                'custom_data' => json_encode(array_values($dataMap), JSON_UNESCAPED_UNICODE),
                'status' => 2,
                'quality_time' => time(),
            ]);
    }
    ...
    public function calculateAll($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        // 计算危急值报告时间
        $this->calculateWjzbgsj($start_time, $end_time);
        // 计算术者术后查房完成率
        ...
        // 计算普通会诊有效率
        $this->calculatePthzyxl($zyh, $start_time, $end_time);
        // 计算手术患者特级护理/一级护理出院率
        $this->calculateSshztjhlyjhlcyl($zyh, $start_time, $end_time);
        // 计算住院患者非计划手术率
        $this->calculateZyhzfjhssl($zyh, $start_time, $end_time);
    }
    ...
    public function calculateSzsqcf($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            $this->esService->esSaveIndicator($patient['MED_REC_ID'], ['szsqcf_fz' => 0, 'szsqcf_fm' => 0, 'szsqcf_error' => null]);
            $ZYH = $patient['MED_REC_ID'];

            /**
             * 日间病历排除
             */
            $aac01Date = date('Ymd', strtotime($patient['AAC01']));
            $aab01Date = date('Ymd', strtotime($patient['AAB01']));
            $diff = $aac01Date - $aab01Date;
            if ($diff <= 1) {
                continue;
            }
            ...
            $excludeKeywords8048 = RuleWordMap::getArrayById(8048);
            ...
            for ($i = 0; $i < count($ssapData); $i++) {
                ...
                if (!in_array($ssLB, $excludeKeywords8047) && !in_array($surgeryName, $excludeKeywords8048)) {
                    continue;
                }

                // 从staff表获取术者工号
                $surgeon = $surgeon . "(" . $surgeonCode . ")";
            ...
            if (!empty($errorContent)) {
                ...
                $insert['AAA28'] = $patient['AAA28'];
                $insert['AAA01'] = $patient['AAA01'];
                $insert['AAC11N'] = $patient['AAC11N'];
                $esService->esSaveIndicator($insert['zyh'], $insert);
            }
        }
    ...
    public function calculateJhzjsdwl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            $zyh = $patient['MED_REC_ID'];
            //先删除之前的数据
            Indicator::query()->updateOrInsert(['zyh' => $zyh], ['jhzjsdwl_fm' => 0, 'jhzjsdwl_fz' => 0, 'jhzjsdwl_error' => null]);
            $this->esService->esSaveIndicator($zyh, ['jhzjsdwl_fz' => 0, 'jhzjsdwl_fm' => 0, 'jhzjsdwl_error' => null]);
            /**
             * 日间病历排除
             */
            ...
            foreach ($consultations as $consultation) {
                ...
                $consultationGroup['content'][] = [
                    'status' => 1,
                    'content' => "申请科室【{$consultation->SQKS_MC}】"
                ];
                $consultationGroup['content'][] = [
                    'status' => 1,
                    'content' => "申请医师【{$consultation->SQYS_MC}】"
                ];
                ...
                if ($consultationOpinion) {
                    $consultationGroup['content'][] = [
                        'status' => 1,
                        'content' => "会诊科室【{$consultationOpinion->KSDM_MC}】"
                    ];
                    ...
                    // 查询医生信息
                    $writingDoctor = Staff::query()->where('code', $consultationOpinion->SXYS)->value('name');
                    $DoctorGH = Staff::query()->where('code', $consultationOpinion->SXYS)->value('YGBH');
                    $arrivalStatus = $isOnTime ? '正常' : "超{$timeLimit}分钟";
                    $consultationGroup['content'][] = [
                        'status' => 1,
                        'content' => "会诊医师【{$consultationOpinion->SSYS_MC}（{$DoctorGH}）】"
                    ];
                    $consultationGroup['content'][] = [
                        'status' => 1,
    ...
    public function calculatePthzjswcl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            $zyh = $patient['MED_REC_ID'];
            Indicator::query()->updateOrInsert(['zyh' => $zyh], ['pthzjswcl_fm' => 0, 'pthzjswcl_fz' => 0, 'pthzjswcl_error' => null]);
            $this->esService->esSaveIndicator($zyh, ['pthzjswcl_fz' => 0, 'pthzjswcl_fm' => 0, 'pthzjswcl_error' => null]);
            ...
            foreach ($consultations as $consultation) {
                ...
                if ($consultationOpinion) {
                    ...
                    try {
                        ...
                        $consultationGroup['content'][] = [
                            'status' => 1,
                            'content' => "会诊医师【{$consultationOpinion->SSYS_MC}（{$consultationOpinion->SSYS}）】"
                        ];
                        $consultationGroup['content'][] = [
                            'status' => 1,
    ...
    public function calculateZbqjzlcz($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);
        $ruleMap8112 = RuleWordMap::query()->where('id', '=', 8112)->value('keyword');
        $rescueKeywords8112 = [];
        if (!empty($ruleMap8112)) {
            $rescueKeywords8112 = strpos($ruleMap8112, ',') !== false ? explode(',', $ruleMap8112) : [$ruleMap8112];
            $rescueKeywords8112 = array_values(array_filter(array_map('trim', $rescueKeywords8112)));
        }
        ...
        foreach ($patientData as $patient) {
            ...
            $errorContent = [];
            try {
                $orders = Yzb::query()->where('ZYH', $ZYH)->orderBy('KZSJ', 'asc')->get()->toArray();
            } catch (\Exception $e) {
            ...
            foreach ($orders as $order) {
                ...
                foreach ($rescueKeywords8112 as $keyword) {
                    if ($keyword !== '' && strpos($orderName, $keyword) !== false) {
                        $isSpecialOrder = true;
                        break;
                    }
                }

                $recordEnd = date('Y-m-d H:i:s', strtotime($orderTime) + ($isSpecialOrder ? 24 : 72) * 3600);
                $insert['zbqjzlcz_fm'] += 1;

                $group = [
                    'status' => 0,
                    'content' => [
                        ['status' => 1, 'content' => "医嘱名称【{$orderName}】"],
                        ['status' => 1, 'content' => "开嘱时间【{$orderTime}】"],
                        ['status' => 1, 'content' => '时限要求【' . ($isSpecialOrder ? '24小时内' : '72小时内') . '】'],
                    ]
                ];
                ...
                try {
                    ...
                    if ($record) {
                        ...
                        $group['status'] = 1;
                        $group['content'][] = ['status' => 1, 'content' => "病历文书【{$record['BLMC']}】"];
                        $group['content'][] = ['status' => 1, 'content' => "标题时间【{$record['ZXSJ']}】"];
                    } else {
                        $group['content'][] = ['status' => 0, 'content' => '病历文书【无】'];
                    }
                } catch (\Exception $e) {
     ...
     * @param string $end_time 结束时间，可选
     */
    public function calculateJzzzhzqjcgl($zyh = "", $start_time = "", $end_time = "")
    {

        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("jzzzhzqjcgl", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);
        ...
        foreach ($patientData as $patient) {
            ...
            //设置数据
            $esService->esSaveIndicator($insert['zyh'], ['jzzzhzqjcgl_fz' => 0, 'jzzzhzqjcgl_fm' => 0, 'jzzzhzqjcgl_error' => null]);
            ...
            $yzbInfo1 = Yzb::query()
                ...
                ->where(function ($query) {
                    $query->where('YZMC', 'LIKE', '%病危%')
                        ->orWhere('YZMC', 'LIKE', '%病重%');
                })
                ->orderBy('KZSJ', 'asc')
                ->get()
                ->toArray();
            //echo 'zyh:'.$insert['zyh'].'---yzbInfo1:'.json_encode($yzbInfo1).'<br>';
            if (empty($yzbInfo1)) {
                continue;
            }
            ...
            $yzbInfo = Yzb::query()
                ...
                ->get()
                ->toArray();
            //echo 'zyh:'.$insert['zyh'].'---yzbInfo:'.json_encode($yzbInfo).'<br>';
            if (empty($yzbInfo)) {
                continue;
            }


            /* $insert['jzzzhzqjcgl_fm'] += 1;

            // 依据添加医嘱名称【YZMC】（取第一条）
            $firstYzmc = '';
            if (!empty($yzbInfo) && isset($yzbInfo[0]['YZMC'])) {
                $firstYzmc = $yzbInfo[0]['YZMC'];
            }

            $errorMsg[0]['content'][0]['status'] = 1;
            $errorMsg[0]['content'][0]['content'] = "医嘱名称：【" . $firstYzmc . "】"; */
            ...
            // 查询主要诊断和其他诊断的CYQK，如果所有的CYQK都不等于4就是fz=1
            $cyqkList = [];
            // 只查询CYQK=4的诊断
            $cyqk4List = false;

            // 查询主要诊断（只取CYQK=4）
            /* $mainDiag = MainDiagnosis::query()
                ->where('AAA28', '=', $insert['zyh'])
                ->where('CYQK', '=', 4)
                ->first();
            if ($mainDiag) {
                $cyqk4List[] = [
                    'CYQK' => $mainDiag->CYQK,
                    'ICD10_NAME' => $mainDiag->ICD10_NAME ?? '',
                ];
            }

            // 查询其他诊断（只取CYQK=4）
            $otherDiags = OtherDiagnosis::query()
                ->where('AAA28', '=', $insert['zyh'])
                ->where('CYQK', '=', 4)
                ->get();
            ...
            //查询离院方式patient_info_v2表的AEM01C
            $aem01c = PatientInfoV2::query()
                ->where('ZYH', '=', $insert['zyh'])
                ->value('AEM01C');
            //echo 'zyh:'.$insert['zyh'].'---aem01c:'.$aem01c.'<br>';
            //如果不等于5
            if ($aem01c != 5) {
                $cyqk4List = true;
            }
            if(!empty($aem01c)){
                //离院方式ID，1.医嘱离院2.医嘱转院3.医嘱转社区卫生服务机构/乡镇卫生院4.非医嘱离院5.死亡9.其他
                if ($aem01c == 1) {
                    $aem01c = '医嘱离院';
                }elseif ($aem01c == 2) {
                    $aem01c = '医嘱转院';
                }elseif ($aem01c == 3) {
                    $aem01c = '医嘱转社区卫生服务机构/乡镇卫生院';
                }elseif ($aem01c == 4) {
                    $aem01c = '非医嘱离院';
                }elseif ($aem01c == 5) {
                    $aem01c = '死亡';
            ...
            // 如果没有CYQK=4的诊断，说明“出院情况无死亡”
            //离院方式非死亡
            if ($cyqk4List) {
                /* $insert['jzzzhzqjcgl_fz'] = 1;
                $errorMsg[0]['status'] = 1;
                $errorMsg[0]['content'][1]['status'] = 1;
                $errorMsg[0]['content'][1]['content'] = "【出院情况无死亡】"; */
                //$errorMsg[0]['status'] = 1;
                foreach ($yzbInfo as $item) {
                    ...
                    $yzberror['content'][] = [
                        'status' => 1,
                        'content' => "医嘱名称：【" . $item['YZMC'] . "】"
                    ];
                    $yzberror['content'][] = [
                        'status' => 1,
                        'content' => "开嘱时间：【" . $item['KZSJ'] . "】"
                    ];
                    $yzberror['content'][] = [
                        'status' => 1,
                        'content' => "离院方式：【" . $aem01c . "】"
                ...
                // 有死亡的情况，补充诊断名称
                /* $icd10Names = array_filter(array_column($cyqk4List, 'ICD10_NAME'));
                $icd10Text = !empty($icd10Names) ? '诊断名称：【' . implode('、', $icd10Names) . '】' : '';
                $insert['jzzzhzqjcgl_fz'] = 0;
                $errorMsg[0]['status'] = 0;
                $errorMsg[0]['content'][0]['status'] = 0;
                $errorMsg[0]['content'][1]['status'] = 0;
                $errorMsg[0]['content'][1]['content'] = "出院情况：【4.死亡】" . $icd10Text; */
                //离院方式为死亡，使用i++循环，最后一条医嘱分子不加1
                for ($i = 1; $i <= count($yzbInfo); $i++) {
                    $yzberror = [
                        'status' => 1,
                        'content' => []
                    ];
                    $yzberror['content'][] = [
                        'status' => 1,
                        'content' => "医嘱名称：【" . $yzbInfo[$i-1]['YZMC'] . "】"
                    ];
                    $yzberror['content'][] = [
                        'status' => 1,
                        'content' => "开嘱时间：【" . $yzbInfo[$i-1]['KZSJ'] . "】"
                    ];
                    $insert['jzzzhzqjcgl_fm'] += 1;
                    if ($i != count($yzbInfo)) {                      
                        //分子+1
            ...
            //echo 'zyh:'.$insert['zyh'].'---errorMsg:'.json_encode($errorMsg).'<br>';

            $insert['jzzzhzqjcgl_fz'] = $insert['jzzzhzqjcgl_fz'] > $insert['jzzzhzqjcgl_fm'] ? $insert['jzzzhzqjcgl_fm'] : $insert['jzzzhzqjcgl_fz'];
            $insert['jzzzhzqjcgl_error'] = !empty($errorMsg) ? json_encode($errorMsg, 256) : '';
            Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
        ...
        IndexCatalog::query()->where("index_name", "=", "jzzzhzqjcgl")->update(["status" => 2, 'quality_time' => time()]);
    }
    ...
    public function calculateSqtlwcl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            $ZYH = $patient['MED_REC_ID'];
            $esService->esSaveIndicator($ZYH, ['sqtlwcl_fz' => 0, 'sqtlwcl_fm' => 0, 'sqtlwcl_error' => null]);
            ...
            for ($i = 0; $i < count($ssapList); $i++) {
                ...
                $sslx = $op['SSLX'] ?? '';
                if (strpos($sslx, '日间手术') !== false || strpos($sslx, '急诊') !== false) {
                    continue;
                }
                ...
                $startCheckTime = date('Y-m-d 00:00:00', strtotime($surgeryStart));
                /* if ($i > 0) {
                ...
                foreach ($records as $rec) {
                    ...
                    $firstTime = $blDataFormatService->formatTime($firstTime);
                    if (empty($firstTime)) {
                        continue;
                    }
                    ...
                    if (strtotime($surgeryStart) - strtotime($firstTime) >= 24 * 3600) {
                        $found = true;
                        $bl01 = EMR_BL_BL01::query()->where('BLBH', $rec['BLBH'])->get()->toArray();
                        break; // 找到符合条件的即可
                    }
                }
                ...
                if (!$found || empty($bl01)) {
                    ...
                    $group['status'] = 1;
                    $insert['sqtlwcl_fz'] += 1;
                    $group['content'][] = ['status' => 1, 'content' => "术前讨论【{$bl01[0]['BLMC']}】"];
                    $group['content'][] = ['status' => 1, 'content' => "讨论时间【{$firstTime}】"];
                }
    ...
    public function calculateSqtlrygfcyl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        $mblbList = RuleWordMap::getArrayById(8016);

        foreach ($patientData as $patient) {
            // 删除旧结果
            Indicator::updateOrInsert(['zyh' => $patient['MED_REC_ID']], ['sqtlrygfcyl_fz' => 0, 'sqtlrygfcyl_fm' => 0, 'sqtlrygfcyl_error' => null]);
            $ZYH = $patient['MED_REC_ID'];
            $esService->esSaveIndicator($ZYH, ['sqtlrygfcyl_fz' => 0, 'sqtlrygfcyl_fm' => 0, 'sqtlrygfcyl_error' => null]);
            ...
            for ($i = 0; $i < count($ssapList); $i++) {
                ...
                if ($i === 0) {
                    ...
                    //如果上一台手术的手术开始时间=当前手术的手术开始时间，则判断i=几，
                    //如果i=1，则开始时间从1970-01-01 00:00:00开始，
                    //否则看上上台开始时间，直到找到不同的开始时间，startCheckTime等于那台的jsrq
                    $prevSurgeryStartTime = $ssapList[$i - 1]['SSRQ'] ?? '';
                    if ($prevSurgeryStartTime == $surgeryStart) {
                        $j = $i - 1;
                        while ($j >= 0 && ($ssapList[$j]['SSRQ'] ?? '') === $surgeryStart) {
                            $j--;
                        }
                        if ($j < 0) {
                            $startCheckTime = date('Y-m-d H:i:s', strtotime('1970-01-01 00:00:00'));
                        } else {
                            $prevSurgeryEndTime = $ssapList[$j]['JSRQ'] ?? '';
                    ...
                    //如果上一台手术的手术开始时间=当前手术的手术开始时间，则判断i=几，如果i=1，则开始时间从1970-01-01 00:00:00开始，否则看上上台开始时间，直到找到不同的开始时间，startCheckTime等于那台的jsrq
                ...
                //echo '（手麻）术前讨论记录数【' . count($records) . '】';
                if (empty($records)) {
                    $group['content'][] = ['status' => 0, 'content' => "术前小结及术前讨论结论记录1【无】"];
                    $errorContent[] = $group;
                    continue;
                }
                ...
                foreach ($records as $rec) {
                    ...
                    $firstTime = $blData->formatTime($firstTime);
    ...
    public function calculateQjcg($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            $ZYH = $patient['MED_REC_ID'];
            $esService->esSaveIndicator($ZYH, ['qjcg_fz' => 0, 'qjcg_fm' => 0, 'qjcg_error' => null]);
    ...
    public function calculateQjjlsh($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            $ZYH = $patient['MED_REC_ID'];
            $esService->esSaveIndicator($ZYH, ['qjjlsh_fz' => 0, 'qjjlsh_fm' => 0, 'qjjlsh_error' => null]);
    ...
    public function calculateSsyzgfkjl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            $ZYH = $patient['MED_REC_ID'];
            $esService->esSaveIndicator($ZYH, ['ssyzgfkjl_fz' => 0, 'ssyzgfkjl_fm' => 0, 'ssyzgfkjl_error' => null]);
            ...
            // 获取该患者的所有手术（参考calculateSqtlwcl，按开始时间排序）
            $ssapList = SM_SSAP::query()
                ->where('ZYH', $ZYH)
                ->orderBy('SSRQ', 'asc')
                ->get()
            ...
            // 参考calculateSqtlwcl：遍历每台手术，每台手术算一个分母，满足条件算一个分子
            for ($i = 0; $i < count($ssapList); $i++) {
                $op = $ssapList[$i];
                ...
                // 查找该手术对应的医嘱（优先级：1. YZMC包含手术名称的医嘱 2. "拟...术"医嘱）
                $matchedOrder = null;
                $endCheckTime = $surgeryStart; // 手术开始时间作为结束时间

                // 第一步：查找 YZMC 包含手术名称的医嘱，且开嘱时间在手术开始时间之前
                $surgeryNameOrders = Yzb::query()
                    ->where('ZYH', $ZYH)
                    ->where('YZMC', 'like', '%' . $surgeryName . '%')
                    ->where('KZSJ', '<=', $endCheckTime)
                    ->orderBy('KZSJ', 'desc')
                    ->get()
                    ->toArray();

                if (!empty($surgeryNameOrders)) {
                    // 找到包含手术名称的医嘱，取最后一个（开嘱时间最接近手术开始时间的）
                    $matchedOrder = $surgeryNameOrders[0];
                } else {
                    // 第二步：如果没有找到，再查找"拟...术"医嘱（手术开始时间之前开嘱的最后一个）
                    foreach ($orders as $order) {
                        $orderKzsj = $order['KZSJ'] ?? '';
                        if (!empty($orderKzsj) && strtotime($orderKzsj) <= strtotime($endCheckTime)) {
                            $matchedOrder = $order;
                        } else {
                            break; // 由于已按KZSJ排序，后续的医嘱时间更晚，直接跳出
                        }
                    }
                }

                if (empty($matchedOrder)) {
                    // 没有找到对应的医嘱
                ...
                $orderName = $matchedOrder['YZMC'] ?? '';
                $orderKzsj = $matchedOrder['KZSJ'] ?? '';
    ...
    /**
     * 计算四级手术患者手术当日床旁交接班占比（四级手术术者手术资质符合率）
     * - 分母：同期进行四级手术的患者总数量
     *   1、编目首页手术级别为"四级手术"（ope_level=4）
     *   2、表main_operation或表secondary_operation中
     *   3、ICD9_NAME关联表SSCZ，SSMC一致，且SSLB=手术或介入治疗
     *   4、只要有一个MED_REC_ID符合，分母+1
     * - 分子：四级手术患者当日进行床旁交接班的患者数量（术者有手术资质）
     *   1、满足分母条件
     *   2、术者有四级手术资质
     *   3、三院：对应ICD9_NAME手术名称 + 术者姓名/工号在GY_SSML中匹配（YY_SSCZMC like '%手术名称%' and code like '%工号%'）
     *   4、其他医院：手术级别符合就行
     */
    public function calculateSjsshzssdrbcjjbzb($zyh = "", $start_time = "", $end_time = "")
    {

        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("sjsshzssdrbcjjbzb", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);
        $hospitalName = env('APP_NAME');
        $isSanyuan = (strpos($hospitalName, 'binyi') !== false);

        foreach ($patientData as $patient) {
            // 删除旧结果
            Indicator::updateOrInsert(['zyh' => $patient['MED_REC_ID']], ['sjsshzssdrbcjjbzb_fz' => 0, 'sjsshzssdrbcjjbzb_fm' => 0, 'sjsshzssdrbcjjbzb_error' => null]);
            $ZYH = $patient['MED_REC_ID'];
            echo $ZYH . PHP_EOL;
            $esService->esSaveIndicator($ZYH, ['sjsshzssdrbcjjbzb_fz' => 0, 'sjsshzssdrbcjjbzb_fm' => 0, 'sjsshzssdrbcjjbzb_error' => null]);

            $insert = [
                'zyh' => $ZYH,
                'sjsshzssdrbcjjbzb_fz' => 0,
                'sjsshzssdrbcjjbzb_fm' => 0,
                'sjsshzssdrbcjjbzb_error' => null,
                'AAC11N' => $patient['AAC11N'],
                'AEE03' => !empty($patient['AEE03']) ? $patient['AEE03'] : '',
                'AAC01_YEAR' => date('Y', strtotime($patient['AAC01'])),
                'AAC01_MONTH' => date('m', strtotime($patient['AAC01'])),
            ...
            // 获取四级手术（从main_operation和secondary_operation）
            ...
            echo $ZYH . ' 四级手术数量' . count($allOperations) . PHP_EOL;
            // 用于记录符合分母条件的手术
            $validOperations = [];
            $hasValidOperation = false;
            var_dump($allOperations);
            ...
            var_dump('满足要求的手术数量' . count($validOperations));
            // 如果没有符合分母条件的手术，跳过
            if (!$hasValidOperation) {
                continue;
            }

            // 分母：按患者计算，有符合条件的四级手术就算1个分母
            $insert['sjsshzssdrbcjjbzb_fm'] = 1;

            // 检查是否有术者有手术资质（分子条件）
            $hasQualifiedSurgeon = false;
            $surgeryGroups = [];

            foreach ($validOperations as $op) {
                $surgeryName = $op['surgeryName'];
                $surgeonName = $op['surgeonName'];
                $surgeonCode = $op['surgeonCode'];
                ...
                $group['content'][] = ['status' => 1, 'content' => "手术名称【{$surgeryName}】"];

                // 判断术者是否有手术资质
                $isQualified = false;
                $surgeonDisplayName = !empty($surgeonName) ? $surgeonName : '无';

                // 其他医院：手术级别符合就行（已经是四级手术，所以符合）
                $isQualified = true;
                $group['content'][] = ['status' => 1, 'content' => "术者【{$surgeonDisplayName}】，有手术资质"];

                if ($isQualified) {
                    $hasQualifiedSurgeon = true;
                    $group['status'] = 1;
                }
            ...
            // 分子：按患者计算，有符合条件的手术且术者有资质就算1个分子
            if ($hasQualifiedSurgeon) {
                $insert['sjsshzssdrbcjjbzb_fz'] = 1;
            }

            // 构建错误信息
            $errorContent = array_merge($errorContent, $surgeryGroups);

            if (!empty($errorContent)) {
                $insert['sjsshzssdrbcjjbzb_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
            }
        ...
        IndexCatalog::query()->where("index_name", "=", "sjsshzssdrbcjjbzb")->update(["status" => 2, 'quality_time' => time()]);
    }


    /**
     * 计算高额异常费用患者进行疑难病例讨论的占比
     * - 分母：同期高额异常费用患者数量
     *   1、住院总费用 ≥20万
     *   2、表patient_info【ADA01 总费用】≥20万
     * - 分子：对产生高额异常费用患者进行疑难病例讨论的数量
     *   1、满足分母条件
     *   2、有疑难病例讨论
     *   3、表bl01中【blmc 含id8100（疑难病例讨论）】
     */
    public function calculateGeycfyhfzjxynbltldzb($zyh = "", $start_time = "", $end_time = "")
    {
        IndexCatalog::editStatus("geycfyhzjxynbltldzb", 1);
        $esService = new ElasticsearchService('indicator');
        ...
        // 获取疑难病例讨论的BLMC关键字（id=8100）
        $ruleMap8100 = RuleWordMap::query()->where('id', '=', 8100)->value('keyword');
        $discussionBLMC = !empty($ruleMap8100) ? $ruleMap8100 : '疑难病例讨论';

        // 如果配置中包含逗号，拆分为数组
        $discussionBLMCList = strpos($discussionBLMC, ',') !== false ? explode(',', $discussionBLMC) : [$discussionBLMC];

        foreach ($patientData as $patient) {
            $ZYH = $patient['MED_REC_ID'];
            // 删除旧结果
            Indicator::updateOrInsert(['zyh' => $ZYH], ['geycfyhzjxynbltldzb_fz' => 0, 'geycfyhzjxynbltldzb_fm' => 0, 'geycfyhzjxynbltldzb_error' => null]);
            $esService->esSaveIndicator($ZYH, ['sjsshzssdrbcjjbzb_fz' => 0, 'sjsshzssdrbcjjbzb_fm' => 0, 'sjsshzssdrbcjjbzb_error' => null]);
            ...
            $patientInfo = PatientInfo::query()
                ...
                ->value('ADA01');
            $totalCost = !empty($patientInfo) ? floatval($patientInfo) : 0;
            ...
            echo $ZYH . ' 住院总费用' . $patientInfo . PHP_EOL;
            $insert = [
                'zyh' => $ZYH,
                'geycfyhzjxynbltldzb_fz' => 0,
                'geycfyhzjxynbltldzb_fm' => 1, // 符合分母条件
                'geycfyhzjxynbltldzb_error' => null,
                'AAC11N' => $patient['AAC11N'],
                'AEE03' => !empty($patient['AEE03']) ? $patient['AEE03'] : '',
                'AAC01_YEAR' => date('Y', strtotime($patient['AAC01'])),
                'AAC01_MONTH' => date('m', strtotime($patient['AAC01'])),
            ...
            // 使用match_phrase查询BLMC包含配置的关键字
            $should = [];
            foreach ($discussionBLMCList as $blmcKeyword) {
                $should[] = ['match_phrase' => ['BLMC' => trim($blmcKeyword)]];
            }
            ...
            try {
                ...
                // 住院总费用
                $group['content'][] = ['status' => 1, 'content' => "住院总费用【{$totalCost}】"];

                if (!empty($data)) {
                    // 有疑难病例讨论记录
                    $discussionRecord = $data[0];
                    $insert['geycfyhzjxynbltldzb_fz'] = 1;
                    $group['status'] = 1;
                    $group['content'][] = ['status' => 1, 'content' => "【疑难病例讨论blmc】，有"];
                    $group['content'][] = ['status' => 1, 'content' => "疑难病例讨论记录【{$discussionRecord['BLMC']}】"];
                } else {
                    // 无疑难病例讨论记录
                    $group['content'][] = ['status' => 0, 'content' => "【疑难病例讨论blmc】，无"];
                }

                $errorContent[] = $group;
                $insert['geycfyhzjxynbltldzb_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
                ...
                Log::error("calculateGeycfyhzjxynbltldzb-$ZYH-error: " . $e->getMessage());
                continue;
            }
        }

        IndexCatalog::query()->where("index_name", "=", "geycfyhzjxynbltldzb")->update(["status" => 2, 'quality_time' => time()]);
    }
     ...
     * - 分母：同期手术总例数
     *   1、手麻系统中手术+介入的手术
     *   2、表SM_SSAP中【icd9_ssczmc】关联表SSCZ为手术+介入（8047，8048过滤）
     *   3、手术台次（按手术台次计算）
     * - 分子：实际开展手术与术前讨论计划手术一致的手术例数
     *   1、满足分母条件
     *   2、手麻系统中手术+介入内手术申请单和手术记录的名称一致的手术例数
     *   3、表SM_SSAP【手术名称】表SSSQ一致（SQDH关联）
     */
    public function calculateSqtljhssyzl($zyh = "", $start_time = "", $end_time = "")
    {
        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("sqtljhssyzl", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);
        ...
        foreach ($patientData as $patient) {
            ...
            // 删除旧结果
            Indicator::updateOrInsert(['zyh' => $ZYH], ['sqtljhssyzl_fz' => 0, 'sqtljhssyzl_fm' => 0, 'sqtljhssyzl_error' => null]);
            $esService->esSaveIndicator($ZYH, ['sqtljhssyzl_fz' => 0, 'sqtljhssyzl_fm' => 0, 'sqtljhssyzl_error' => null]);
            $insert = [
                'zyh' => $ZYH,
                'sqtljhssyzl_fz' => 0,
                'sqtljhssyzl_fm' => 0,
                'sqtljhssyzl_error' => null,
                'AAC11N' => $patient['AAC11N'],
                'AEE03' => !empty($patient['AEE03']) ? $patient['AEE03'] : '',
                'AAC01_YEAR' => date('Y', strtotime($patient['AAC01'])),
                'AAC01_MONTH' => date('m', strtotime($patient['AAC01'])),
            ...
            // 按手术台次循环处理
            foreach ($ssapList as $surgery) {
                $surgeryStart = $surgery['SSRQ'] ?? '';
                $surgeryEnd = $surgery['JSRQ'] ?? '';
                $surgeryName = $surgery['ICD9_SSCZMC'] ?? '';
                $SQDH = $surgery['SQDH'] ?? '';
                //SSLX
                $sslx = $surgery['SSLX'] ?? '';
                ...
                // （手麻）手术名称
                $group['content'][] = ['status' => 1, 'content' => "（手麻）手术名称：【{$surgeryName}】"];
                $sssqData = SSSQ::query()->where('ZYH', $ZYH)->where('NSSMC', $surgeryName)->first();
                if (!empty($sssqData)) {
                    $insert['sqtljhssyzl_fz'] += 1;
                    $group['status'] = 1;
                    $group['content'][] = ['status' => 1, 'content' => "（手术申请）手术名称：【{$surgeryName}】，一致"];
                } else {
                    $group['content'][] = ['status' => 0, 'content' => "（手术申请）手术名称：【无】，不一致"];
                }
            ...
            if (!empty($errorContent)) {
                $insert['sqtljhssyzl_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
            }
        ...
        IndexCatalog::query()->where("index_name", "=", "sqtljhssyzl")->update(["status" => 2, 'quality_time' => time()]);
    }

    /**
     * 实际手术术者与计划手术术者一致率
     */
    public function calculateSjssszyjhssszyzl($zyh = "", $start_time = "", $end_time = "")
    {
        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("sjssszyjhssszyzl", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);
        ...
        foreach ($patientData as $patient) {
            ...
            // 删除旧结果
            Indicator::updateOrInsert(['zyh' => $ZYH], ['sjssszyjhssszyzl_fz' => 0, 'sjssszyjhssszyzl_fm' => 0, 'sjssszyjhssszyzl_error' => null]);
            $esService->esSaveIndicator($ZYH, ['sjssszyjhssszyzl_fz' => 0, 'sjssszyjhssszyzl_fm' => 0, 'sjssszyjhssszyzl_error' => null]);
            $insert = [
                'zyh' => $ZYH,
                'sjssszyjhssszyzl_fz' => 0,
                'sjssszyjhssszyzl_fm' => 0,
                'sjssszyjhssszyzl_error' => null,
                'AAC11N' => $patient['AAC11N'],
                'AEE03' => !empty($patient['AEE03']) ? $patient['AEE03'] : '',
                'AAC01_YEAR' => date('Y', strtotime($patient['AAC01'])),
                'AAC01_MONTH' => date('m', strtotime($patient['AAC01'])),
            ...
            foreach ($ssapList as $surgery) {
                ...
                // （手麻）手术名称
                $group['content'][] = ['status' => 1, 'content' => "（手麻）手术名称：【{$surgeryName}】"];
                $group['content'][] = ['status' => 1, 'content' => "（手麻）手术者：【{$sz}】"];
                $sssqData = SSSQ::query()->where('SQDH', $SQDH)->first();
                if (!empty($sssqData)) {
                    $staff = Staff::query()->where('base_code', $sssqData->SSYS)->first();
                    if (!empty($staff)) {
                        if ($staff->name === $sz) {
                            $insert['sjssszyjhssszyzl_fz'] += 1;
                            $group['status'] = 1;
                            $group['content'][] = ['status' => 1, 'content' => "（手术申请）手术者：【{$sz}】，一致"];
                        } else {
                            $group['content'][] = ['status' => 0, 'content' => "（手术申请）手术者：【{$staff->name}】，不一致"];
                        }
                    } else {
                        $group['content'][] = ['status' => 0, 'content' => "（手术申请）手术者：【无】，不一致"];
                    }
                } else {
                    $group['content'][] = ['status' => 0, 'content' => "（手术申请）手术者：【无】，不一致"];
                }
            ...
            if (!empty($errorContent)) {
                $insert['sjssszyjhssszyzl_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
            }
        ...
        IndexCatalog::query()->where("index_name", "=", "sjssszyjhssszyzl")->update(["status" => 2, 'quality_time' => time()]);
    }
    ...
    public function calculateSwtljswcl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            $ZYH = $patient['MED_REC_ID'];
            $esService->esSaveIndicator($ZYH, ['swtljswcl_fz' => 0, 'swtljswcl_fm' => 0, 'swtljswcl_error' => null]);
            ...
            $swsj = '';
            $isFromNotice = false;
            $deathNotice = EMR_BL_BL01::query()
                ->join('EMR_BL_BLXG', 'EMR_BL_BL01.BLBH', '=', 'EMR_BL_BLXG.BLBH')
                ->where('EMR_BL_BL01.JZHM', '=', $ZYH)
                ->where('EMR_BL_BL01.BLMC', '=', '死亡通知书')
                ->select('EMR_BL_BLXG.HJNR')
                ->first();

            if (!empty($deathNotice) && !empty($deathNotice->HJNR)) {
                // 从内容中提取死亡时间，格式：与2025-10-10 23:59去世
                $hjnr = $deathNotice->HJNR;
                // 匹配格式：与YYYY-MM-DD HH:MM去世 或 与YYYY-MM-DD HH:MM:SS去世
                // 支持多种格式：23:59、23:59:59、2:59等
                if (preg_match('/于(\d{4}-\d{2}-\d{2}\s+\d{1,2}:\d{2}(?::\d{2})?)\s*去世/', $hjnr, $matches)) {
                    $swsj = trim($matches[1]);
                    // 如果只有日期和时间（没有秒数），补充秒数
                    if (preg_match('/^\d{4}-\d{2}-\d{2}\s+\d{1,2}:\d{2}$/', $swsj)) {
                        $swsj .= ':00';
                    }
                    // 转换为标准格式 Y-m-d H:i:s
                    $swsjTimestamp = strtotime($swsj);
                    if ($swsjTimestamp !== false) {
                        $swsj = date('Y-m-d H:i:s', $swsjTimestamp);
                        $isFromNotice = true;
                    } else {
                        $swsj = '';
                    }
            ...
            $group['content'][] = ['status' => 1, 'content' => "医嘱名称【{$orderName}】"];
            // 根据是否从死亡通知书提取，显示不同的标签
            if ($isFromNotice) {
                $group['content'][] = ['status' => 1, 'content' => "死亡时间【{$swsj}】"];
            } else {
                $group['content'][] = ['status' => 1, 'content' => "出院时间【{$swsj}】"];
            }
            ...
            foreach ($records as $record) {
                $tmpTime = $record[$timeField] ?: $record['ZXSJ'];
                if (empty($tmpTime)) {
                    continue;
                }
                ...
                $XZJDSJ_END = $publicService->getWorkDays(date('Y-m-d H:i:s', $swsjTimestamp), 5);
                $XZJDSJ_END = date('Y-m-d 23:59:59', strtotime($XZJDSJ_END));
                $group['content'][] = ['status' => 1, 'content' => "时间范围【{$swsj}至{$XZJDSJ_END}】"];

                if ($tmpTimeTimestamp <= strtotime($XZJDSJ_END)) {
                    // 在5天内
                    $matched = true;
                    $matchedTime = $tmpTime;
                    $matchedStatus = '（5天内）';
                    break;
                } elseif ($tmpTimeTimestamp > strtotime($XZJDSJ_END)) {
                    // 超5天
                    if (!$matched) {
                        $matchedTime = $tmpTime;
                        $matchedStatus = '（超5天）';
    ...
    public function calculateSzfhsqmlyzl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            $ZYH = $patient['MED_REC_ID'];
            $esService->esSaveIndicator($ZYH, ['szfhsqmlyzl_fz' => 0, 'szfhsqmlyzl_fm' => 0, 'szfhsqmlyzl_error' => null]);
            ...
            // 获取该患者的所有手术（与calculateSqtlwcl保持一致，按开始时间排序）
            $ssapList = SM_SSAP::query()
                ->where('ZYH', $ZYH)
                ->orderBy('SSRQ', 'asc')
                ->get()
            ...
            // 与calculateSqtlwcl保持一致，使用基于索引的循环
            for ($i = 0; $i < count($ssapList); $i++) {
                $op = $ssapList[$i];
                ...
                // 检查必要字段（ICD9_SSCZBM 和 SZDM 用于后续业务逻辑）
                if (empty($surgeryCode) || empty($surgeonCode)) {
                    continue;
                }
                ...
                $ssmlList = GY_SSML::query()
                    ->where('YY_SSCZMC', $surgeryCode)
                    ->orWhere('YY_SSCZBM', $surgeryName)
                    ->get()
                    ->toArray();
                ...
                $ysqxList = GY_YSQX::query()->where('YGDM', $surgeonCode)->get()->toArray();
                foreach ($ysqxList as $ysqx) {
                    $ysqx = $ysqx['SSQX_SSJBMC'] ?? '';
                    $ysqxcode = 0;
                    if (!empty($ysqxcode)) {
                        if ($ysqx == '一级手术') {
                            $ysqxcode = 1;
                ...
                if ($matched) {
                    ...
                    $permission = $ssmlList[0]['SSQX_SSJBMC'] ?? '';
    ...
    public function calculateSsyssssjch($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            $ZYH = $patient['MED_REC_ID'];
            $esService->esSaveIndicator($ZYH, ['ssyssssjch_fz' => 0, 'ssyssssjch_fm' => 0, 'ssyssssjch_error' => null]);
            ...
            for ($i = 0; $i < count($ssapList); $i++) {
                ...
                // 检查必要字段（id 和 SZDM 用于后续业务逻辑）
                if (empty($currentId) || empty($surgeonCode)) {
                    continue;
                }
                ...
                if (!empty($overlappingSurgeries)) {
                    ...
                    foreach ($overlappingSurgeries as $overlap) {
                        ...
                        $group['content'][] = ['status' => 1, 'content' => "同时间段(手麻)手术名称【{$overlapName}】"];
                        //术者
                        $overlapSurgeonName = $overlap['SZ'] ?? '';
                        //$overlapSurgeonCode = $overlap['SZDM'] ?? '';
                        $group['content'][] = ['status' => 1, 'content' => "同时间段(手麻)术者【{$overlapSurgeonName}】"];
                        //$group['content'][] = ['status' => 1, 'content' => "同时间段(手麻)术者代码【{$overlapSurgeonCode}】"];
                        if (!empty($brry)) {
                        ...
                        $group['content'][] = ['status' => 1, 'content' => "同时间段(手麻)手术开始时间【{$overlapStart}】"];
                        $group['content'][] = ['status' => 1, 'content' => "同时间段(手麻)手术结束时间【{$overlapEnd}】"];
                    }
                } else {
    ...
    public function calculateSjsssqdxktl($zyh = "", $start_time = "", $end_time = "")
    {
        ...
        foreach ($patientData as $patient) {
            ...
            $ZYH = $patient['MED_REC_ID'];
            $esService->esSaveIndicator($ZYH, ['sjsssqdxktl_fz' => 0, 'sjsssqdxktl_fm' => 0, 'sjsssqdxktl_error' => null]);
            ...
            // 查询四级手术（使用病案首页数据）
            $mainoperation = MainOperation::query()->where('AAA28', '=', $ZYH)->where('OPE_LEVEL', '=', '4')->where('SSLX_MC', '!=', '急诊')->get()->toArray();
            $secondaryoperation = SecondaryOperation::query()->where('AAA28', '=', $ZYH)->where('OPE_LEVEL', '=', '4')->where('SSLX_MC', '!=', '急诊')->get()->toArray();
            ...
            foreach ($results as $surgeryGroup) {
                ...
                $bl01Res = EMR_BL_BL01::query()
                    ...
                    ->where('ZXSJ', '>=', $secsj)
                    ->where('ZXSJ', '<=', $sskssj)
                    ->where('BLMC', 'like', '%多学科讨论%')->get()->toArray();


                if (count($bl01Res) > 0) {
                    $surgeryGroupContent['content'][] = ["status" => 1, "content" => "手术术前多学科讨论【" . ($bl01Res[0]['BLMC'] ?? '') . "】"];
                    //签名时间
                    $firstTime = $bl01Res[0]['first_blsy_time'] ?? '';
                    if ($m == 1) {
     ...
     * 分母：同期普通会诊患者总次数
     * - 会诊申请，普通会诊（未作废）
     * - 表YS_ZY_HZSQ【JJBZ≠2 普通会诊】且【ZFBZ≠1 代表未作废】取【SQXH 申请序号】
     * - 表YS_ZY_HZSQ.JZHM=patient_info.MED_REC_ID
     * - 表patient_info【IS_CATA=1  已编目】的【MED_REC_ID 就诊号码】
     * - 注：一个SQXH 分母+1
     *
     * 分子：普通会诊结束后开具相关医嘱的次数
     * - 满足分母条件
     * - 普通会诊时间（会诊意见签名时间），12小时内开嘱医嘱
     * - 取表YS_ZY_HZYJ 【QMSJ 签名时间  如果为空则属于错误，跳过即可】
     * - 【QMSJ 签名时间】前12小时内（含）开具会诊医嘱
     * - 会诊医嘱：表yzb中【yzmc 医嘱名称】包含id20000（邀请）且包含id20001（会诊）
     * - 开嘱时间在【QMSJ 签名时间】前12小时内（含），分子+1
     *
     * @param string $zyh 住院号，可选
     * @param string $start_time 开始时间，可选
     * @param string $end_time 结束时间，可选
     */
    public function calculatePthzyxl($zyh = "", $start_time = "", $end_time = "")
    {

        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("pthzyxl", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);

        // 获取规则词映射：会诊医嘱关键词
        $inviteKeywords = RuleWordMap::getArrayById(20000); // 邀请
        $consultKeywords = RuleWordMap::getArrayById(20001); // 会诊

        if (empty($inviteKeywords) || empty($consultKeywords)) {
            Log::error("calculatePthzyxl: 规则词映射配置缺失，id20000或id20001未配置");
            IndexCatalog::query()->where("index_name", "=", "pthzyxl")->update(["status" => 2, 'quality_time' => time()]);
            return;
        }
        foreach ($patientData as $patient) {
            $zyh = $patient['MED_REC_ID'];
            Indicator::query()->updateOrInsert(['zyh' => $zyh], ['pthzyxl_fm' => 0, 'pthzyxl_fz' => 0, 'pthzyxl_error' => null]);
            $esService->esSaveIndicator($zyh, ['pthzyxl_fz' => 0, 'pthzyxl_fm' => 0, 'pthzyxl_error' => null]);


            // 查询普通会诊申请（JJBZ≠2 表示普通会诊，ZFBZ≠1 表示未作废）
            $consultations = DB::table('YS_ZY_HZSQ')
                ->where('JZHM', $zyh)
                ->where('JJBZ', '!=', 2)
                ->where('ZFBZ', '!=', 1)
            ...
            foreach ($consultations as $consultation) {
                ...
                $consultationGroup['content'][] = [
                    'status' => 1,
                    'content' => "会诊申请时间【{$consultation->SQSJ}】"
                ];
                ...
                if (!$consultationOpinion || empty($consultationOpinion->QMSJ)) {
                    // QMSJ为空则跳过
                    $consultationGroup['content'][] = [
                        'status' => 0,
                        'content' => "会诊意见签名时间【无】"
                ...
                $qmsj = $consultationOpinion->QMSJ; // 签名时间
                $consultationGroup['content'][] = [
                    'status' => 1,
                    'content' => "会诊意见签名时间【{$qmsj}】"
                ];

                // 计算时间范围：QMSJ前12小时内（含）
                $qmsjTime = Carbon::parse($qmsj);
                $startTime = $qmsjTime->copy()->subHours(12)->format('Y-m-d H:i:s');
                $endTime = $qmsjTime->format('Y-m-d H:i:s');

                // 查询该患者在该时间范围内的会诊医嘱
                // 会诊医嘱：YZMC包含"邀请"且包含"会诊"
                $orders = Yzb::query()
                    ->where('ZYH', $zyh)
                ...
                foreach ($orders as $order) {
                    ...
                    // 检查是否同时包含"邀请"和"会诊"关键词
                    $hasInvite = false;
                    $hasConsult = false;

                    foreach ($inviteKeywords as $inviteKeyword) {
                        if (strpos($yzmc, trim($inviteKeyword)) !== false) {
                            $hasInvite = true;
                            break;
                        }
                    }

                    foreach ($consultKeywords as $consultKeyword) {
                        if (strpos($yzmc, trim($consultKeyword)) !== false) {
                            $hasConsult = true;
                            break;
                        }
                    }

                    if ($hasInvite && $hasConsult) {
                        // 找到符合条件的会诊医嘱
                        $matchedOrder = $order;
                        break; // 找到一条即可
                    }
                ...
                if ($matchedOrder) {
                    ...
                    $consultationGroup['content'][] = [
                        'status' => 1,
                        'content' => "会诊医嘱名称【{$matchedOrder['YZMC']}】"
                    ];
                    $consultationGroup['content'][] = [
                        'status' => 1,
                        'content' => "开嘱时间【{$matchedOrder['KZSJ']}】（12小时内）"
                    ];
                } else {
                    $consultationGroup['content'][] = [
                        'status' => 0,
                        'content' => "会诊医嘱【无】（12小时内未开具）"
                    ];
                }
            ...
            if (!empty($errorContent) && count($errorContent) === $insert['pthzyxl_fm']) {
                $insert['pthzyxl_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
    ...
    /**
     * 计算医务部门组织讨论的死亡病例与发生纠纷的死亡病例比值（医务科组织死亡病例病例讨论率）
     * - 分母：同期发生医疗纠纷的死亡病例数量（同期死亡病例总数）
     *   1、医嘱中含"死亡"
     * - 分子：医务部门组织进行死亡病例讨论的病例数量
     *   1、满足分母条件
     *   2、死亡病例讨论结论记录全文中含"医务科组织"或"医务处组织"或"医务部组织"
     *   3、bl01中blmc含id8101（死亡病例讨论结论记录）含id8102（医务科组织,医务部组织）
     */
    public function calculateSwblyfsjfdswblbz($zyh = "", $start_time = "", $end_time = "")
    {
        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("swblyfsjfdswblbz", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);

        // 获取死亡医嘱关键字（id=8003）
        $ruleMap8003 = RuleWordMap::query()->where('id', '=', 8003)->value('keyword');
        $deathKeywords = !empty($ruleMap8003) ? (strpos($ruleMap8003, ',') !== false ? explode(',', $ruleMap8003) : [$ruleMap8003]) : ['死亡'];

        // 获取死亡病例讨论结论记录的BLMC关键字（id=8101）
        $ruleMap8101 = RuleWordMap::query()->where('id', '=', 8101)->value('keyword');
        $discussionBLMC = !empty($ruleMap8101) ? $ruleMap8101 : '死亡病例讨论结论记录';
        $discussionBLMCList = strpos($discussionBLMC, ',') !== false ? explode(',', $discussionBLMC) : [$discussionBLMC];

        // 获取医务部门组织关键字（id=8102）
        $ruleMap8102 = RuleWordMap::query()->where('id', '=', 8102)->value('keyword');
        $organizationKeywords = !empty($ruleMap8102) ? (strpos($ruleMap8102, ',') !== false ? explode(',', $ruleMap8102) : [$ruleMap8102]) : ['医务科组织', '医务处组织', '医务部组织'];

        foreach ($patientData as $patient) {
            $ZYH = $patient['MED_REC_ID'];

            // 删除旧结果
            Indicator::updateOrInsert(['zyh' => $ZYH], ['swblyfsjfdswblbz_fz' => 0, 'swblyfsjfdswblbz_fm' => 0, 'swblyfsjfdswblbz_error' => null]);
            $esService->esSaveIndicator($ZYH, ['swblyfsjfdswblbz_fz' => 0, 'swblyfsjfdswblbz_fm' => 0, 'swblyfsjfdswblbz_error' => null]);
            // 查询医嘱中是否包含死亡关键字
            $yzbQuery = Yzb::query()->where('ZYH', '=', $ZYH);
            ...
            echo $ZYH . PHP_EOL;
            // 如果没有死亡医嘱，跳过
            echo '死亡医嘱数量：' . count($yzbData) . PHP_EOL;

            $insert = [
                'zyh' => $ZYH,
                'swblyfsjfdswblbz_fz' => 0,
                'swblyfsjfdswblbz_fm' => 1, // 符合分母条件
                'swblyfsjfdswblbz_error' => null,
                'AAC11N' => $patient['AAC11N'],
                'AEE03' => !empty($patient['AEE03']) ? $patient['AEE03'] : '',
                'AAC01_YEAR' => date('Y', strtotime($patient['AAC01'])),
                'AAC01_MONTH' => date('m', strtotime($patient['AAC01'])),
            ...
            // 医嘱名称
            $orderName = $yzbData[0]['YZMC'] ?? '';
            $group['content'][] = ['status' => 1, 'content' => "医嘱名称：【{$orderName}】"];

            // 查询死亡病例讨论结论记录（BLMC包含配置的关键字）
            $bl01Query = EMR_BL_BL01::query()->where('JZHM', $ZYH);
            $bl01Query->where(function ($query) use ($discussionBLMCList) {
                foreach ($discussionBLMCList as $blmcKeyword) {
                    $query->orWhere('BLMC', 'like', '%' . trim($blmcKeyword) . '%');
                }
            });
            $bl01Records = $bl01Query->get(['BLBH', 'BLMC'])->toArray();
            echo '死亡病例讨论记录数量：' . count($bl01Records) . PHP_EOL;

            echo '死亡病例讨论记录：' . count($bl01Records) . PHP_EOL;
            if (empty($bl01Records)) {
                // 没有死亡病例讨论记录
                $group['content'][] = ['status' => 0, 'content' => "医务科未组织（无死亡病例讨论记录）"];
                $errorContent[] = $group;
                $insert['swblyfsjfdswblbz_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
                continue;
            }
            ...
            foreach ($bl01Records as $record) {
                ...
                try {
                    $hjnr = EMR_BL_BLXG::query()->where('BLBH', $blbh)->value('HJNR');
                    $hjnr = !empty($hjnr) ? $hjnr : '';

                    // 检查全文是否包含医务部门组织关键字
                    foreach ($organizationKeywords as $orgKeyword) {
                        $orgKeyword = trim($orgKeyword);
                        if (!empty($orgKeyword) && strpos($hjnr, $orgKeyword) !== false) {
                    ...
                    Log::error("calculateSwblyfsjfdswblbz-$ZYH-查询HJNR error: " . $e->getMessage());
                    continue;
                }
            }
            ...
            if ($hasOrganization) {
                // 有医务部门组织
                $insert['swblyfsjfdswblbz_fz'] = 1;
                $group['status'] = 1;
                $group['content'][] = ['status' => 1, 'content' => "【{$matchedBLMC}】，医务科组织"];
            } else {
                // 没有医务部门组织
                $blmcList = array_column($bl01Records, 'BLMC');
                $blmcStr = !empty($blmcList) ? implode('、', array_unique($blmcList)) : '';
                $group['content'][] = ['status' => 0, 'content' => "【{$blmcStr}】，医务科未组织"];
            }
        ...
        IndexCatalog::query()->where("index_name", "=", "swblyfsjfdswblbz")->update(["status" => 2, 'quality_time' => time()]);
    }
     ...
     * 分母：同期手术患者总数量
     * - 手麻系统中手术+介入的手术
     * - 手术：表SM_SSAP【ICD9_SSCZMC 手术名称】关联表SSCZ【SSMC 手术名称】取其【SSLB 手术类型=1(手术)或 4(介入治疗)】
     * - 同一个MED_REC_ID，只要有手术，分母只+1
     * - 同一个MED_REC_ID，有多台手术，分母只+1
     * - 表patient_info【MED_REC_ID】=表SM_SSAP【ZYH】
     *
     * 分子：手术患者出院时为特级护理/一级护理级别的患者数量
     * - 满足分母条件
     * - 出院当天，最晚的一次护理级别为"特级护理"或一级护理的手术患者数量
     * - 出院当天：出院时间表patient_info【AAC01】截取到日
     * - 表yzb中【yzmc 医嘱名称】包含id20002（rule_word_map【id】）：特级护理,一级护理,Ⅰ级护理（rule_word_map【keyword】）
     * - 取对应医嘱的【KZSJ 开嘱时间】最晚的一条
     * - 最后一条护理医嘱为id20002中任意一个，是正确
     *
     * @param string $zyh 住院号，可选
     * @param string $start_time 开始时间，可选
     * @param string $end_time 结束时间，可选
     */
    public function calculateSshztjhlyjhlcyl($zyh = "", $start_time = "", $end_time = "")
    {

        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("sshztjhlyjhlcyl", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);

        // 获取规则词映射：护理级别关键词（特级护理、一级护理、Ⅰ级护理等）
        $ruleMap20002 = RuleWordMap::query()->where('id', '=', 20002)->value('keyword');

        if (empty($ruleMap20002)) {
            Log::error("calculateSshztjhlyjhlcyl: 规则词映射配置缺失，id20002未配置");
            IndexCatalog::query()->where("index_name", "=", "sshztjhlyjhlcyl")->update(["status" => 2, 'quality_time' => time()]);
            return;
        }

        //获取20070
        $ruleMap20070 = RuleWordMap::query()->where('id', '=', 20070)->value('keyword');
        $ruleMap20070 = !empty($ruleMap20070) ? $ruleMap20070 : '特级护理,一级护理,Ⅰ级护理,二级护理,Ⅱ级护理,三级护理,Ⅲ级护理';
        //所有护理级别关键词
        $allNursingKeywords = strpos($ruleMap20070, ',') !== false ? explode(',', $ruleMap20070) : [$ruleMap20070];
        // 去除空格
        $allNursingKeywords = array_map('trim', $allNursingKeywords);

        // 处理多个关键词（用英文逗号分隔）
        $nursingKeywords = strpos($ruleMap20002, ',') !== false ? explode(',', $ruleMap20002) : [$ruleMap20002];
        // 去除空格
        $nursingKeywords = array_map('trim', $nursingKeywords);
        echo '患者总数：' . count($patientData) . PHP_EOL;
        foreach ($patientData as $patient) {
            $zyh = $patient['MED_REC_ID'];
            Indicator::query()->updateOrInsert(['zyh' => $zyh], ['sshztjhlyjhlcyl_fm' => 0, 'sshztjhlyjhlcyl_fz' => 0, 'sshztjhlyjhlcyl_error' => null]);
            $esService->esSaveIndicator($zyh, ['sshztjhlyjhlcyl_fz' => 0, 'sshztjhlyjhlcyl_fm' => 0, 'sshztjhlyjhlcyl_error' => null]);
            /**
             * 日间病历排除
             */
            $aac01Date = date('Ymd', strtotime($patient['AAC01']));
            ...
            // 检查是否有符合条件的手术
            $hasValidSurgery = false;
            $firstSurgery = null; // 记录第一台手术（用于显示）
            ...
            foreach ($ssapList as $ssap) {
                ...
                // 使用8047/8048规则过滤手术
                $isValidSurgery = false;
                if (!empty($ruleMap8047)) {
                    // 如果SSLB在8047包含列表中，则保留
                    if (in_array($ssLB, $excludeKeywords8047)) {
                        $isValidSurgery = true;
                    } else {
                        // 如果SSLB不在8047中，检查8048例外列表
                        if (!empty($ruleMap8048)) {
                            if (in_array($surgeryName, $excludeKeywords8048)) {
                                $isValidSurgery = true;
                            }
                ...
                if ($isValidSurgery) {
                    $hasValidSurgery = true;
                    if ($firstSurgery === null) {
                        $firstSurgery = [
                            'name' => $surgeryName,
                            'start_time' => $ssap['SSRQ'] ?? ''
                        ];
                    }
                    break; // 找到符合条件的手术即可
                }
            ...
            if (!$hasValidSurgery) {
                continue; // 没有符合条件的手术，跳过
            }

            // 分母：同一个MED_REC_ID，只要有手术，分母只+1
            $insert = [
                'zyh' => $zyh,
                'sshztjhlyjhlcyl_fz' => 0,
                'sshztjhlyjhlcyl_fm' => 1,
                'sshztjhlyjhlcyl_error' => '',
                'AAC11N' => $patient['AAC11N'],
                'AEE03' => !empty($patient['AEE03']) ? $patient['AEE03'] : '',
                'AAC01_YEAR' => date('Y', strtotime($patient['AAC01'])),
                'AAC01_MONTH' => date('m', strtotime($patient['AAC01'])),
            ...
            // 显示手术信息（如果有多台手术，显示手术开始时间最早的一台手术名称即可）
            if ($firstSurgery) {
                $surgeryGroup['content'][] = [
                    'status' => 1,
                    'content' => "（手麻）手术名称【{$firstSurgery['name']}】"
                ];
                if (!empty($firstSurgery['start_time'])) {
                    $surgeryGroup['content'][] = [
                        'status' => 1,
                        'content' => "（手麻）手术开始时间【{$firstSurgery['start_time']}】"
                    ];
                }
            }

            // 获取出院时间，截取到日
            $exitDate = date('Y-m-d', strtotime($patient['AAC01']));
            $exitDateStart = $exitDate . ' 00:00:00';
            $exitDateEnd = $exitDate . ' 23:59:59';

            // 查询出院当天的护理级别医嘱
            $nursingOrders = Yzb::query()
                ->where('ZYH', $zyh)
                ->where('KZSJ', '>=', $exitDateStart)
                ->where('KZSJ', '<=', $exitDateEnd)
                ->orderBy('KZSJ', 'desc')
                ->get()
                ->toArray();

            // 筛选出包含护理级别关键词的医嘱
            $matchedNursingOrders = [];
            foreach ($nursingOrders as $order) {
                $yzmc = $order['YZMC'] ?? '';
                if (empty($yzmc)) {
                    continue;
                }
                ...
                // 检查是否包含护理级别关键词
                foreach ($allNursingKeywords as $keyword) {
                    if (strpos($yzmc, $keyword) !== false) {
                        $matchedNursingOrders[] = $order;
                        break; // 找到匹配的关键词即可
                    }
                }
            }

            if (empty($matchedNursingOrders)) {
                // 没有找到护理级别医嘱
                $surgeryGroup['content'][] = [
                    'status' => 0,
                    'content' => "医嘱名称【无】（出院当天未找到护理级别医嘱）"
                ];
                $errorContent[] = $surgeryGroup;
            } else {
                // 取最晚的一条护理医嘱（已按KZSJ倒序排列，第一条就是最晚的）
                $lastNursingOrder = $matchedNursingOrders[0];
                $lastYzmc = $lastNursingOrder['YZMC'] ?? '';
                $lastKzsj = $lastNursingOrder['KZSJ'] ?? '';

                $surgeryGroup['content'][] = [
                    'status' => 1,
                    'content' => "医嘱名称【{$lastYzmc}】"
                ];
                $surgeryGroup['content'][] = [
                    'status' => 1,
                    'content' => "开嘱时间【{$lastKzsj}】"
                ];

                // 判断最后一条护理医嘱是否为特级护理或一级护理
                $isValidNursing = false;
                foreach ($nursingKeywords as $keyword) {
                    if (strpos($lastYzmc, $keyword) !== false) {
                        $isValidNursing = true;
                        break;
                    }
                }
                ...
                if ($isValidNursing) {
                    // 分子+1
                    $insert['sshztjhlyjhlcyl_fz'] += 1;
                    $surgeryGroup['status'] = 1;
                } else {
                    $surgeryGroup['content'][] = [
                        'status' => 0,
                        'content' => "护理级别【不符合】（最后一条护理医嘱不是特级护理/一级护理）"
                    ];
                }
                $errorContent[] = $surgeryGroup;
            }
            ...
            if (!empty($errorContent)) {
                $insert['sshztjhlyjhlcyl_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
     ...
     * 分母：同期住院患者总人次数
     * - 已编目病案首页总数
     * - 表patient_info【IS_CATA=1  已编目】的【MED_REC_ID 就诊号码，每次住院的唯一标识】
     * - 注：一个MED_REC_ID 分母+1
     *
     * 分子：行非计划手术的住院患者人次数
     * - 满足分母条件
     * - 同一次住院 2次手术（手麻系统中手术+介入）
     * - 手术：表SM_SSAP【ICD9_SSCZMC 手术名称】关联表SSCZ【SSMC 手术名称】取其【SSLB】
     * - SSLB在8047或者手术名称在8048
     * - 同一个MED_REC_ID，有2次或以上符合条件的手术，分子+1
     *
     * @param string $zyh 住院号，可选
     * @param string $start_time 开始时间，可选
     * @param string $end_time 结束时间，可选
     */
    public function calculateZyhzfjhssl($zyh = "", $start_time = "", $end_time = "")
    {

        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("zyhzfjhssl", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);
        ...
        foreach ($patientData as $patient) {
            $zyh = $patient['MED_REC_ID'];
            echo $zyh . PHP_EOL;
            Indicator::query()->updateOrInsert(['zyh' => $zyh], ['zyhzfjhssl_fm' => 0, 'zyhzfjhssl_fz' => 0, 'zyhzfjhssl_error' => null]);
            $esService->esSaveIndicator($zyh, ['zyhzfjhssl_fz' => 0, 'zyhzfjhssl_fm' => 0, 'zyhzfjhssl_error' => null]);
            /**
             * 日间病历排除
             */
            /* $aac01Date = date('Ymd', strtotime($patient['AAC01']));
            ...
            $patientGroup['content'][] = [
                'status' => 1,
                'content' => "入院时间【{$patient['AAB01']}】"
            ];

            // 先查询所有手术，然后过滤掉不满足8047/8048规则的
            $validSurgeriesY = [];
            $validSurgeriesN = [];
            ...
            foreach ($ssapList as $ssap) {
                ...
                if ($fjhss == 'Y') {
                    $validSurgeriesY[] = $surgeryName;
                } else {
                    $validSurgeriesN[] = $surgeryName;
                }
            }

            $validSurgeryCountY = count($validSurgeriesY);
            $validSurgeryCountN = count($validSurgeriesN);

            // 只显示过滤后符合条件的手术
            if ($validSurgeryCountY == 0 && $validSurgeryCountN == 0) {
                $patientGroup['content'][] = [
                    'status' => 0,
                    'content' => "无手术"
                ];
                $patientGroup['status'] = 0;
            } elseif ($validSurgeryCountY >= 1) {
                //正确

                $insert['zyhzfjhssl_fz'] += 1;
                $patientGroup['status'] = 1;
                foreach ($validSurgeriesY as $surgeryName) {
                    $patientGroup['content'][] = [
                        'status' => 1,
                        'content' => "（手麻）手术名称：【{$surgeryName}(非计划)】"
                    ];
                }
                if ($validSurgeryCountN >= 1) {
                    foreach ($validSurgeriesN as $surgeryName) {
                        $patientGroup['content'][] = [
                            'status' => 1,
                            'content' => "（手麻）手术名称：【{$surgeryName}】"
                        ];
            ...
            $errorContent[] = $patientGroup;

            if (!empty($errorContent)) {
                $insert['zyhzfjhssl_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
        ...
        IndexCatalog::query()->where("index_name", "=", "zyhzfjhssl")->update(["status" => 2, 'quality_time' => time()]);
    }
    /**
     * 计算住院患者危急值当日及时处置率指标
     *
     ...
     * @param string $end_time 结束时间，可选
     */
    public function calculateZyhzwjZzZdrJsczl($zyh = "", $start_time = "", $end_time = "")
    {

        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("zyhzwjzdrjsczl", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);

        $ruleMap94 = RuleWordMap::getArrayById(94);

        foreach ($patientData as $patient) {
            $zyh = $patient['MED_REC_ID'];
            echo $zyh . PHP_EOL;
            Indicator::query()->updateOrInsert(['zyh' => $zyh], ['zyhzwjzdrjsczl_fm' => 0, 'zyhzwjzdrjsczl_fz' => 0, 'zyhzwjzdrjsczl_error' => null]);
            $esService->esSaveIndicator($zyh, ['zyhzwjzdrjsczl_fz' => 0, 'zyhzwjzdrjsczl_fm' => 0, 'zyhzwjzdrjsczl_error' => null]);

            $wjzList = WJZ::query()->where('zyh', $zyh)->get()->toArray();
            if (empty($wjzList)) {
                continue;
            }
            echo '危急值：' . count($wjzList) . PHP_EOL;
            // 分母：一个MED_REC_ID 分母+1
            $insert = [
                'zyh' => $zyh,
                'zyhzwjzdrjsczl_fz' => 0,
                'zyhzwjzdrjsczl_fm' => 1,
                'zyhzwjzdrjsczl_error' => '',
                'AAC11N' => $patient['AAC11N'],
                'AEE03' => !empty($patient['AEE03']) ? $patient['AEE03'] : '',
                'AAC01_YEAR' => date('Y', strtotime($patient['AAC01'])),
                'AAC01_MONTH' => date('m', strtotime($patient['AAC01'])),
            ...
            // 获取危急值病程记录
            $emrBlBl01List = EMR_BL_BL01::query()->where('JZHM', $zyh)->get(['BLMC', 'ZXSJ'])->toArray();
            if (empty($emrBlBl01List)) {
                continue;
            }
            $wjzBl01 = [];
            foreach ($emrBlBl01List as $emrBlBl01) {
                foreach ($ruleMap94 as $rule) {
                    if (strpos($emrBlBl01['BLMC'], $rule) !== false) {
                        $wjzBl01[] = $emrBlBl01;
                    }
                }
            }
            ...
            $errorContent = [];
            $allRight = true;
            foreach ($wjzList as $wjz) {
                //如果危急值时间》出院时间，则跳过
                if (date("Y-m-d H:i:s", strtotime($wjz['WJZSJ'])) > date("Y-m-d H:i:s", strtotime($patient['AAC01']))) {
                    continue;
                }
                $patientGroup = [
                    'status' => 0,
                ...
                $isExist = false;
                $blmc = '';
                foreach ($wjzBl01 as $bl) {
                    if (date("Y-m-d", strtotime($wjz['WJZSJ'])) == date("Y-m-d", strtotime($bl['ZXSJ']))) {
                        $isExist = true;
                        $blmc = $bl['BLMC'];
                        break;
                    }
                ...
                if ($isExist) {
                    ...
                    $patientGroup['content'][] = [
                        'status' => 1,
                        'content' => "推送时间【{$wjz['WJZSJ']}】"
                    ];
                    $patientGroup['content'][] = [
                        'status' => 1,
                        'content' => "危急值【{$wjz['WJZNR']}】"
                    ];
                    $patientGroup['content'][] = [
                        'status' => 1,
                        'content' => "病程记录【{$blmc}】"
            ...
            if ($allRight) {
                $insert['zyhzwjzdrjsczl_fz'] += 1;
            }

            if (!empty($errorContent)) {
                $insert['zyhzwjzdrjsczl_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
                $insert['AAA28'] = $patient['AAA28'];
                $insert['AAA01'] = $patient['AAA01'];
                $insert['AAC11N'] = $patient['AAC11N'];
        ...
        IndexCatalog::query()->where("index_name", "=", "zyhzwjzdrjsczl")->update(["status" => 2, 'quality_time' => time()]);
    }
     ...
     * 分子：急会诊结束后开具相关医嘱的次数
     * - 满足分母条件
     * - 急会诊时间（会诊意见签名时间），40分钟内开嘱医嘱
     * - 取表YS_ZY_HZYJ 【QMSJ 签名时间  如果为空则属于错误，跳过即可】
     * - 【QMSJ 签名时间】前40分钟内（含）开具会诊医嘱
     * - 会诊医嘱：表yzb中【yzmc 医嘱名称】包含id20000（邀请）且包含id20001（会诊）
     * - 开嘱时间在【QMSJ 签名时间】前40分钟内（含），分子+1
     *
     * @param string $zyh 住院号，可选
     * @param string $start_time 开始时间，可选
     * @param string $end_time 结束时间，可选
     */
    public function calculateJhzyxl($zyh = "", $start_time = "", $end_time = "")
    {

        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("jhzyxl", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);
        ...
        if (empty($inviteKeywords) || empty($consultKeywords)) {
            Log::error("calculatePyhzyxl: 规则词映射配置缺失，id20000或id20001未配置");
            IndexCatalog::query()->where("index_name", "=", "jhzyxl")->update(["status" => 2, 'quality_time' => time()]);
            return;
        }

        foreach ($patientData as $patient) {
            $zyh = $patient['MED_REC_ID'];
            Indicator::query()->updateOrInsert(['zyh' => $zyh], ['jhzyxl_fm' => 0, 'jhzyxl_fz' => 0, 'jhzyxl_error' => null]);
            $esService->esSaveIndicator($zyh, ['jhzyxl_fz' => 0, 'jhzyxl_fm' => 0, 'jhzyxl_error' => null]);

            /**
             * 日间病历排除
             */
            /* if (strtotime($patient['AAC01']) - strtotime($patient['AAB01']) <= 86400) {
                continue;
            } */
            ...
            if (!empty($errorContent) && count($errorContent) === $insert['jhzyxl_fm']) {
                $insert['jhzyxl_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
                Indicator::query()->updateOrInsert(['zyh' => $insert['zyh']], $insert);
     ...
     * 计算术中自体血回输率（szztxhsl）
     * - 分母：同期术中进行自体血输血患者总数量
     *   1、输血系统 ZY_SS 中 CFX 含 id8103【自体血】（可配置）
     * - 分子：术中使用自体血回输的患者数量
     *   1、满足分母条件
     *   2、收费项目自体血回收（fee_detailed.FYMC 含 id8104【自体血回收】）
     *   3、手术记录（MBLB=306）HJNR 含 id8103【自体血】
     */
    public function calculateSzztxhsl($zyh = "", $start_time = "", $end_time = "")
    {
        $esService = new ElasticsearchService('indicator');
        IndexCatalog::editStatus("szztxhsl", 1);
        $indicatorService = new IndicatorService();
        $patientData = $indicatorService->getPatientInfoData($zyh, $start_time, $end_time);

        // 自体血关键字（输注内容 SM_SSAP.ZTX，id=8103）
        $ruleMap8103 = RuleWordMap::query()->where('id', '=', 8103)->value('keyword');
        $autoBloodKeywords = !empty($ruleMap8103)
            ? (strpos($ruleMap8103, ',') !== false ? explode(',', $ruleMap8103) : [$ruleMap8103])
            : ['自体血'];
        ...
        foreach ($patientData as $patient) {
            ...
            // 删除旧结果
            Indicator::updateOrInsert(
                ['zyh' => $ZYH],
                ['szztxhsl_fz' => 0, 'szztxhsl_fm' => 0, 'szztxhsl_error' => null]
            );
            $esService->esSaveIndicator($ZYH, ['szztxhsl_fz' => 0, 'szztxhsl_fm' => 0, 'szztxhsl_error' => null]);
            ...
            // 1. 分母：查询 SM_SSAP 中该患者输注内容（ZTX）不为空的手术记录
            $ssapList = SM_SSAP::query()
                ->where('ZYH', '=', $ZYH)
                ->whereNotNull('ZTX')
                ->where('ZTX', '<>', '')
                ->orderBy('SSRQ', 'asc')
                ->get(['ICD9_SSCZMC', 'SSRQ', 'ZTX'])
                ->toArray();
            echo 'ZYH：' . $ZYH . '，含输注内容手术记录数：' . count($ssapList) . PHP_EOL;
            if (empty($ssapList)) {
                // 没有输注内容的手术记录，不计入分母
                continue;
            }

            // 有输注内容的手术记录，按患者算 1 个分母
            $insert['szztxhsl_fm'] = 1;
            ...
            // 2. 分子：任意一台手术的输注内容（ZTX）命中自体血关键字即满足
            // 先判断每台手术是否命中，并得出整体是否满足
            $matchedFlags = [];
            $hasAutoBlood = false;
            foreach ($ssapList as $index => $ssap) {
                $ztx = $ssap['ZTX'] ?? '';
                $currentMatched = false;
                foreach ($autoBloodKeywords as $kw) {
                    $kw = trim($kw);
                    if ($kw !== '' && $ztx !== '' && strpos($ztx, $kw) !== false) {
                        $currentMatched = true;
                        break;
                    }
                }
                $matchedFlags[$index] = $currentMatched;
                if ($currentMatched) {
                    $hasAutoBlood = true;
                }
            }
            ...
            // 整体状态：任意一台满足即为满足
            $groupStatus = $hasAutoBlood ? 1 : 0;

            // 再构建依据：手术名称、手术开始时间、输注内容（多台手术依次展示）
            foreach ($ssapList as $index => $ssap) {
                $ztx = $ssap['ZTX'] ?? '';
                $currentMatched = $matchedFlags[$index];
                $lineStatus = $currentMatched ? 1 : 0;

                $group['content'][] = [
                    'status' => $lineStatus,
                    'content' => "手术名称【" . ($ssap['ICD9_SSCZMC'] ?? '') . "】"
                ];
                $group['content'][] = [
                    'status' => $lineStatus,
                    'content' => "手术开始时间【" . ($ssap['SSRQ'] ?? '') . "】"
                ];
                $group['content'][] = [
                    'status' => $lineStatus,
                    'content' => "输注内容【" . $ztx . "】"
                ...
                // 当前手术无自体血时展示“无自体血”。
                // 若整体已满足（存在其它手术有自体血），该行 status 跟随整体为 1，无需标红为 0
                if (!$currentMatched) {
                    $group['content'][] = [
                        'status' => $groupStatus,
                        'content' => "无自体血"
            ...
            // 分子：任意一台手术满足即算 1
            if ($hasAutoBlood) {
                $insert['szztxhsl_fz'] = 1;
                $group['status'] = 1;
            }

            $errorContent[] = $group;
            $insert['szztxhsl_error'] = json_encode($errorContent, JSON_UNESCAPED_UNICODE);
            Indicator::updateOrInsert(['zyh' => $insert['zyh']], $insert);
            $insert['AAA28'] = $patient['AAA28'];
            $insert['AAA01'] = $patient['AAA01'];
            $insert['AAC11N'] = $patient['AAC11N'];
            $esService->esSaveIndicator($insert['zyh'], $insert);
        ...
        IndexCatalog::query()
            ->where("index_name", "=", "szztxhsl")
            ->update(["status" => 2, 'quality_time' => time()]);
    }
}
