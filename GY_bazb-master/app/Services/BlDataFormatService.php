...
class BlDataFormatService
{
    ...
    protected function insertSanyuanConsentData($blData = [], $baseData = [])
    {
        ...
        switch ($templateKey) {
            case 'tszl':
                Bllb329Tszl::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'yzccstys':
                Bllb329Yzccstys::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'bdqp':
                Bllb329Bdqp::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'hltys':
                Bllb329Hltys::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'sszqtys':
                Bllb329Sszqtys::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'sxzqtys':
                Bllb329Sxzqtys::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'ctzq':
                Bllb329Ctzq::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'bwbztys':
                Bllb329Bwbztys::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'sqwts':
                Bllb329Sqwts::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'zrltys':
                Bllb329Zrltys::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'jjjctys':
                Bllb34Jjjctys::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'zdcygzs':
                Bllb34Zdcygzs::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'sjgzs':
                Bllb34Sjgzs::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            case 'ybwffgzs':
                Bllb34Ybwffgzs::query()->updateOrInsert([
                    'BLBH' => (string)($blData['BLBH'] ?? ''),
                ], $data);
                break;
            default:
    ...
    protected function extractSanyuanConsentTemplateData($templateKey = '', $htmlPrint = '')
    {
        ...
        if ($templateKey === 'yzccstys') {
            return array_merge($data, [
                'XMMC' => $this->getHtmlFieldTextByXPath($xpath, '特殊治疗项目名称', ['项目名称']),
                'XMMD' => $this->extractYzccstysPurposeByXPath($xpath),
    ...
    /**
     * 提取腰椎穿刺术知情同意书的"拟施特殊检查/特殊治疗项目目的"。
     *
     * 兼容两种模板：
     * 1. 旧模板：目的写在文本字段"检查及治疗目的文本"里，直接取文本值；
     * 2. 新模板：目的为复选框字段"检查及治疗目的"（data-type="checkboxs"），
     *    需要读取所有被勾选的复选框对应的 checkbox-value 标签文本并拼接。
     *
     * @param \DOMXPath|null $xpath
     * @return string
     */
    protected function extractYzccstysPurposeByXPath($xpath = null)
    {
        if (empty($xpath)) {
            return '';
        }
        ...
        // 优先读取文本字段模板
        $textValue = $this->getHtmlFieldTextByXPath($xpath, '检查及治疗目的文本', ['检查及治疗目的文本']);
        if ($textValue !== '') {
            return $textValue;
        }

        // 兼容复选框字段模板：读取被勾选项的标签文本
        return $this->extractCheckedCheckboxLabelsByXPath($xpath, '检查及治疗目的');
    }

    /**
     * 读取指定复选框字段（data-id）下所有被勾选项对应的 checkbox-value 标签文本，
     * 并以顿号拼接返回。用于兼容以复选框方式记录选项的知情同意书模板。
     *
     * @param \DOMXPath|null $xpath
     * @param string $dataId
     * @return string
     */
    protected function extractCheckedCheckboxLabelsByXPath($xpath = null, $dataId = '')
    {
        ...
        foreach ($checkboxes as $checkbox) {
            ...
            // 勾选项对应的文本位于紧邻的 checkbox-value 节点中
            $labelNodes = $xpath->query(
                'following-sibling::*[contains(concat(" ", normalize-space(@class), " "), " checkbox-value ")][1]',
                $checkbox
            );

            if (empty($labelNodes) || $labelNodes->length === 0) {
                continue;
            }

            $label = $this->normalizeHtmlFieldValue($labelNodes->item(0)->textContent ?? '', []);
            if ($label !== '') {
                $labels[] = $label;
            }
        }
