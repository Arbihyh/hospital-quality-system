...
class IndicatorService
{
    ...
    public function getPatientInfoData($zyh, $startTime, $endTime)
    {
        $filed = [
            ...
            DB::raw('COALESCE(department.dep_name, patient_info.AAC11N) as AAC11N')
        ];
        $query = PatientInfo::query()
            ->leftJoin('ZY_BRRY', 'patient_info.MED_REC_ID', '=', 'ZY_BRRY.ZYH')
            ->leftJoin('department', 'ZY_BRRY.BRKS', '=', 'department.dep_id')
    ...
    public function handleEjyscf($data)
    {
        ...
        foreach ($data as $item) {
            ...
            // 如果出院时间和入院时间在24小时内的排除
            if ($carbon::parse($item['AAC01'])->lt($carbon::parse($item['AAB01'])->addDays(1))) {
                continue;
            }
            ...
            if (empty($endTime)) {
                ...
                // 计算周期数
                $startTimeOne = $startTime;
                $zqNum = 0;
                while (strtotime($startTimeOne) < strtotime($endTime)) {
                    $zqNum++;
                    $startTimeOne = date('Y-m-d H:i:s', strtotime($startTimeOne) + ($ejyscfPeriod - 1) * 24 * 3600 + 1);
                }
                $insert['ejyscf_fm'] += $zqNum; // 分母累加实际周期数
    ...
    public function newHandleSjyscf($data_content)
    {
        ...
        foreach ($data_content as $item) {
            ...
            if (empty($endTime)) {
                ...
                while (strtotime($startTimeOne) < strtotime($endTime)) {
                    $zqNum++;
                    $startTimeOne = date('Y-m-d H:i:s', strtotime($startTimeOne) + ($weekTimeNum - 1) * 24 * 3600 + 1);
                }
                $insert['sjyscf_fm'] += $zqNum;
    ...
    public function handleShscbcjkwc($data)
    {
        ...
        foreach ($data as $item) {
            ...
            foreach ($ssapData as $surgery) {
                ...
                // 获取手术类型（通过手术名称查询SSCZ表的SSLB）
                $surgeryType = '';
                try {
                    $ssCZ = SSCZ::query()->where('SSMC', $surgeryName)->value('SSLB');
                    $surgeryType = !empty($ssCZ) ? $ssCZ : '';
                    if (!empty($ruleMap8047)) {
                        //根据手术名称查询SSCZ的SSLB
                        if (!empty($ssCZ)) {
