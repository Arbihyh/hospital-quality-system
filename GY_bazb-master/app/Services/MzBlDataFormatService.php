 ...
 * 门急诊病历数据清洗
 */
class MzBlDataFormatService
{
    public function formatBlData($blbh)
    {
        /**
         * 获取门诊病历数据
         */
        $mzblData = $this->getMzBlData($blbh);
        foreach ($mzblData as $item) {
     ...
     * 格式化数据
     */
    public function insertData($mzblData)
    {
        $hjnr = $mzblData['BLNR_TXT'];

        // $data['BLBH'] = $mzblData['BLBH'];//病历编号
        // $data['JZXH'] = $mzblData['JZXH'];//就诊序号
        // $data['BRID'] = $mzblData['BRID'];

        /**
         * 表可能不存在需新增字段
         */
        // $data['BLMC'] = $mzblData['BLMC'];//病历名称
        ...
        $hjnr = str_replace('：', ':', $hjnr);
        $hjnr = str_replace(["{", "}"], '', $hjnr);
        //$data['BLNR'] = $hjnr;//病历痕迹内容(取时间最晚的一条）

        $data = [];

        $resData = $this->cleanDataFilter("clean_omr_bl01", $hjnr);
        $data = array_merge($data, $resData);
        if (isset($data['xys'])) {
            unset($data['xys']);
        }
        if (isset($data['jzycbs'])) {
            unset($data['jzycbs']);
        }
        /**
         * 时间在格式的时候去掉了空格，但是数据表中的字段时间格式，所以需要把去掉空格的时间格式化成正确的时间格式
         */
        //$data['CJSJ'] = substr($data['CJSJ'], 0, 10) . ' ' . substr($data['CJSJ'], 10);
        //$data['WCSJ'] = substr($data['WCSJ'], 0, 10) . ' ' . substr($data['WCSJ'], 10);

        OMR_BL01::query()->updateOrInsert([
            'BLBH' => $mzblData['BLBH'],
        ], $data);
     ...
     * 门诊病历数据
     */
    public function getMzBlData($blbh)
    {
        $mzblData = OMR_BL01::where('BLBH', '=', $blbh)->get()->toArray();
        return $mzblData ?? [];
    }
     ...
     * 根据配置信息完成数据清洗
     */
    function cleanDataFilter($hjnr = "", $filter = 0)
    {

        // 去除所有空格并统一冒号格式（使用更强的正则）
        $hjnr = preg_replace("/[\s\x{3000}\x{00A0}]+/u", "", $hjnr);  // 包括全角空格和不间断空格
        $hjnr = str_replace("：", ":", $hjnr);
     ...
     * @return true
     */
    public function omrBl01Format($blbh = '')
    {
        // 查询指定ID范围内的所有记录
        ...
        foreach ($data as $key => $value) {
            ...
            //使用JZXH管理YS_MZ_JZLS表取KSSJ赋值给jzsj
            if (empty($saveData1['jzsj'])) {
                $ysMzJzlsData = YS_MZ_JZLS::query()->where('JZXH', '=', $value['JZXH'])->first();
                if (!empty($ysMzJzlsData)) {
                    if (!empty($ysMzJzlsData->KSSJ)) {
                        $saveData1['jzsj'] = $ysMzJzlsData->KSSJ;
                    } else {
                        $saveData1['jzsj'] = '';
                    }
                } else {
            ...
            if ($saveData) {
                ...
                if ($result) {
                    // 临时跳过ES写入，避免磁盘空间不足错误
                    // 先合并value，再合并saveData，这样saveData的非空值会覆盖value的空值
                    $this->esSave(array_merge($value, $saveData));
                }
            }
        }
        return true;
    ...
    function getContent($ruleMapArr, $str, $field, $filter = 1)
    {
        ...
        // 去除所有空格（包括全角空格和不间断空格）
        $str = preg_replace("/[\s\x{3000}\x{00A0}]+/u", "", $str);
        $field = preg_replace("/[\s\x{3000}\x{00A0}]+/u", "", $field);
        ...
        // 四诊字段名可能存在多种别名，统一作为潜在分隔符，避免前序段落“吃到”四诊内容。
        $fourDiagFieldLabels = ['四诊资料:', '中医望闻切诊:', '望闻切诊:', '中医四诊资料:', '中医四诊:'];
        foreach ($fourDiagFieldLabels as $lbl) {
            if (!in_array($lbl, $specialFields, true)) {
                $specialFields[] = $lbl;
            }
        }

        // 特殊处理：如果当前字段是"四诊资料:"，则"望诊"、"闻诊"、"问诊"、"切诊"（无冒号）不应该作为分隔符
        if (!in_array($field, $fourDiagFieldLabels, true)) {
            $specialFields[] = '望诊';
            $specialFields[] = '闻诊';
            $specialFields[] = '问诊';
            $specialFields[] = '切诊';
        ...
        foreach ($ruleMapArr as $k) {
            ...
            if (in_array($field, $fourDiagFieldLabels, true) && in_array($keyword, ['望诊', '闻诊', '问诊', '切诊'])) {
                continue;
            }
