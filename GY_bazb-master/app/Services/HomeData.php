...
use App\Console\Commands\Bl01Cleaning;
use App\Console\Commands\Clean_bl01_first_blsy_time;
...
class HomeData
{
     ...
     * @return int
     */
    public function getData($zyh, $type = [])
    {
        $this->getConnect();
        ...
        if(empty($type) || in_array('yzb', $type)){
            ...
            if (!empty($yzbData)) {
//                echo '医嘱本' . ' - ' . date('Y-m-d H:i:s', time()) . PHP_EOL;
                $this->addYzb($yzbData);
            }
        }

        // 医生签名
//            echo '医生签名' . ' - ' . date('Y-m-d H:i:s', time()) . PHP_EOL;
    ...
    public function pacs($zyh)
    {
        ...
        $sql = "SELECT DCID,VISITID as JZLSH,EXAMINATIONITEM as JCMC,REPORTDATETIME as BGSJ,REQUESTDATETIME as KDSJ FROM RIS.V_CDR_5411 WHERE HospizationId='{$AAA28}'";
        ...
        foreach ($insertData as $k => $arr) {
            \App\Model\PACS::query()->where('JZLSH', '=', $k)->delete();
            \App\Model\PACS::query()->insert($arr);
        }
    }
...
//        $sql = "SELECT A.*,to_char(ZKRQ,'yyyy-mm-dd hh24:mi:ss') as ZKRQ1 FROM PORTAL_HIS.V_JMGS_BASY_FY A WHERE A.ZYH=".$ZYH;
    ...
    public function addBLSY($zyh)
    {
        ...
        // 清洗病历里面的首次签名时间
        $blf = new Clean_bl01_first_blsy_time();
        foreach ($data as $item) {
            $blf->editFirstBlsyTime($item);
        }
...
//        }
    ...
    {
        $sql = "SELECT ZYH,to_char(HCRQ,'yyyy-mm-dd hh24:mi:ss') as HCRQ,to_char(ZZRQ,'yyyy-mm-dd hh24:mi:ss') as ZZRQ,HCLX,HQCH,HHCH,HQKS,HHKS,HQBQ,HHBQ,JSCS,CZGH,JGID FROM PORTAL_HIS.ZY_HCMX WHERE ZYH={$zyh}";
        $result = oci_parse(self::$con, $sql);
        oci_execute($result, OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
    ...
    public function YS_ZY_HZYJ($zyh)
    {
        ...
        $sql = "SELECT SQXH, JZHM, SQKS, SQYS, to_char(SQSJ,'yyyy-mm-dd hh24:mi:ss') as SQSJ, HZMD, HZMD2, HZSJ, YQDX, JJBZ, TJBZ, TJYS, TJSJ, ZFBZ, JSBZ, JSSJ, TXRY, BQZL, HZLX, BLBH, SQZD, JGID, JSYS, JZBZ, HZLB FROM PORTAL_HIS.YS_ZY_HZSQ  WHERE JZHM={$zyh}";
        $result = oci_parse(self::$con, $sql);
        oci_execute($result, OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
        ...
        $sql = "SELECT JLXH,SQXH,HZYJ,HZYJ2,KSDM,SSYS,SXYS,to_char(SXSJ,'yyyy-mm-dd hh24:mi:ss') as SXSJ,to_char(QMSJ,'yyyy-mm-dd hh24:mi:ss') as QMSJ FROM PORTAL_HIS.YS_ZY_HZYJ WHERE SQXH in ($SQXHStr)";
        $result = oci_parse(self::$con, $sql);
        oci_execute($result, OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
    ...
    {
        $con = oci_connect('jmba', 'jmba', '172.16.2.29:1521/bslis', "UTF8");
        $sql = "SELECT ZYH,TXM,NO,XM,XB,NL,CH,YBLX,YBZT,AAA28,BQ,LCZD,PYJG,XJMC,XJJL,YMMC,YMJG,YMBW,SJYS,JYY,SHY,CJSJ,JSSJ,BGSJ,EXAMINAIM,STAYHOSPITALMODE FROM BSLIS52_Y12.V_JMGS_YMRESULT WHERE ZYH={$zyh}";
        $result = oci_parse($con, $sql);
        oci_execute($result, OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
