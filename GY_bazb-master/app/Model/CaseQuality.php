...
class CaseQuality extends Model
{
    ...
    public static function getByJZHM($id = 0, $select = ['*'])
    {
        return self::query()->where('JZHM', '=', $id)->get($select)->toArray();
    }
    ...
    public static function addData(array $data = [])
    {
        ...
        return self::query()->updateOrInsert(
            ...
            [
                ...
                'appeal_id' => (empty($data['appeal_id']) ? '' : $data['appeal_id']),
                'is_appeal' => (empty($data['is_appeal']) ? '' : $data['is_appeal']),
            ]
        );
    }
}
