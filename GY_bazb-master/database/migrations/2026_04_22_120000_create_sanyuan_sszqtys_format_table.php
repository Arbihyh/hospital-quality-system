...
class CreateSanyuanSszqtysFormatTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        if (Schema::hasTable('bllb329_sszqtys')) {
            return;
        }

        Schema::create('bllb329_sszqtys', function (Blueprint $table) {
            $table->increments('id');
            ...
            $table->string('YSQM', 255)->nullable()->comment('经治医师签名');
            $table->string('SSZQM', 255)->nullable()->comment('术者签名');
            ...
            $table->text('SQZD')->nullable()->comment('术前诊断');
            $table->text('NSSSMC')->nullable()->comment('拟施手术名称');
            $table->text('TDFA')->nullable()->comment('替代医疗方案');
    ...
    {
        Schema::dropIfExists('bllb329_sszqtys');
    }
}
