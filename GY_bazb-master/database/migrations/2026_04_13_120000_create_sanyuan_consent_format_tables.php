...
class CreateSanyuanConsentFormatTables extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        if (!Schema::hasTable('bllb329_tszl')) {
            Schema::create('bllb329_tszl', function (Blueprint $table) {
                $table->increments('id');
                $this->addCommonConsentColumns($table);
        ...
        if (!Schema::hasTable('bllb329_bdqp')) {
            Schema::create('bllb329_bdqp', function (Blueprint $table) {
                $table->increments('id');
                $this->addCommonConsentColumns($table);
                $table->text('JCMD')->nullable()->comment('手术中冰冻切片检查目的');
            });
        }

        if (!Schema::hasTable('bllb329_hltys')) {
            Schema::create('bllb329_hltys', function (Blueprint $table) {
                $table->increments('id');
                $this->addCommonConsentColumns($table);
                $table->text('TDFA')->nullable()->comment('替代方案');
            });
    ...
    {
        Schema::dropIfExists('bllb329_hltys');
        Schema::dropIfExists('bllb329_bdqp');
        Schema::dropIfExists('bllb329_tszl');
    }

    private function addCommonConsentColumns(Blueprint $table)
    {
