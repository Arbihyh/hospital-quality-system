...
class HomeDataService
{
    ...
    public function index($startTime = '', $endTime = '', $day = 1)
    {
        ...
        */
        $con = oci_connect('VS_BAZK', 'vs_bazk#123', '10.32.82.8:1521/ods',"UTF8");
        if (!$con) {
            $e = oci_error();
...
//        $timeIndex = $day == 1 ? 1685548800 : time() - 5 * 24 * 3600;
        //第一次从2023年6月1日 0点0分0秒到现在
//        $timeIndex = $day == 1 ? strtotime(date('2024-09-01 00:00:00',time())) : 1685548800;
...
//                $startTime = $timeIndex;
//                $startTime = strtotime(date('Y-09-01 00:00:00',time()));
                $startTime = Carbon::now()->subDays(15)->startOfDay()->timestamp;

//                file_put_contents(storage_path() . '/homedata1.log', $str. "\r\n", FILE_APPEND);
                echo $str;
            }
            // 获取用户信息
            ...
            if (!empty($patientData)) {
                ...
                foreach ($patientData as $patientInfo) {
                    ...
                    echo $ZYH . ' - ' .$patientInfo['AAB01']." - ". date('Y-m-d',$startTime). " - " .date('Y-m-d H:i:s', time()) . PHP_EOL;
...
//                    Log::error("住院号{$ZYH}医嘱信息::",[empty($yzbData)]);
                    if (!empty($yzbData)) {
                        $this->addYzb($yzbData);
                    }
...
//                    $this->EMR_BL_BASYSJ($ZYH);
    ...
    public function newIndex($startTime = '', $endTime = '', $day = 1)
    {
        $username = env('ORACLE_USERNAME', 'zdyh');
        $password = env('ORACLE_PASSWORD', 'zdyh');
        $connection = env('ORACLE_HOST', '172.16.9.8');
        ...
        while (true) {
            ...
            if (!empty($patientData)) {
                ...
                foreach ($patientData as $patientInfo) {
                    ...
                    echo $ZYH . ' - ' .$patientInfo['AAB01']." - ". date('Y-m-d',$startTime). " - " .date('Y-m-d H:i:s', time()) . PHP_EOL;
                    // 主信息
                    $this->addPatientInfoNew($patientInfo);
                }
            }
        }
    }

    public function addPatientInfoNew($data)
    {
        // 主信息
...
//                if(!$res) Log::error("主表信息插入更新失败::{$insertData['ZYH']}",$insertData);
