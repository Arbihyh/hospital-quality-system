...
{
    public static function getUserGroupList($page, $len)
    {
        $data = UserGroup::getUserGroupPage($page, $len);
...
//            $v["admin_name"] = $adminIdData[$v["admin_id"]]["name"];
    ...
    {
        return UserGroup::add($groupName,$desc,json_encode($role),$admin_id);
    }

    // 添加管理员组
    public static function edit($id,$groupName,$desc,$role)
    {
        $state = UserGroup::edit($id,$groupName,$desc,json_encode($role));
        if($state){
            return true;
        }else{
            return false;
