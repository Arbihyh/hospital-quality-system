...
class MedicalRecordService
{
    ...
    public static function getData($id)
    {
        ...
        //患者信息
        $field = ['ABC01N', 'AAA28', 'ADA0101', 'AAB01', 'MED_REC_ID', 'ADA01', 'ABG01N', 'ABG01C', 'AEM01C', 'AAC04', 'AAB06C', 'AAA26C', 'AAA01', 'AAA02C', 'AAA03', 'AAA29', 'AAA04', 'AAA05C', 'AAA40', 'AEN01', 'AAA42', 'AAA26C', 'AAA06C', 'AAA07', 'AAC01', 'AAA08C', 'ATTEND_GRP_CODE', 'ATTEND_GRP_NAME', 'source', 'AAC11N', 'hospital_name', 'CWH'];
        ...
        //患者联系人信息
        $patient_field = ['AAA22', 'AAA24', 'AAA25', 'AAA23C', 'GX_MC'];
        ...
        if ($patient_contacts_info) {
            ...
            $patient_contacts_info['AAA23C'] = $patient_contacts_info['GX_MC'] ?? "无";
        } else {
            $patient_contacts_info = [];
        }
        //患者住院信息
        $hospital_info_field = ['ABC01C', 'AAA27', 'AAB02C', 'AAB03', 'AAC02C', 'AAC03', 'AAD01C', 'AEI01C', 'AEM03C', 'AEM04', 'AAC11C', 'AAB11C', 'AAB11N', 'ABC01C', 'AEM02'];
        ...
        if ($patient_hospital_info) {
            ...
            //入院科别,出院科别,转院科别改查科别主表
            if (!empty($patient_hospital_info['AAB02C'])) {
                $patient_hospital_info['AAB02C'] = $department[$patient_hospital_info['AAB02C']] ?? "";
            }
            if (!empty($patient_hospital_info['AAC02C'])) {
                $patient_hospital_info['AAC02C'] = $department[$patient_hospital_info['AAC02C']] ?? "";
            }
            if (!empty($patient_hospital_info['AAD01C'])) {
                $patient_hospital_info['AAD01C'] = $department[$patient_hospital_info['AAD01C']] ?? "";
            }
        } else {
        ...
        //患者医疗信息
        $medical_info_field = ['SFRJSS', 'ABC03C', 'ABF01C', 'ABF04', 'ABF01N', 'AEL01', 'ABA01N', 'ABA01C', 'AEB02C', 'AEB01', 'AEG01C', 'AEG02C', 'AED01C', 'AEJ01', 'AEJ02', 'AEJ03', 'AEJ04', 'AEJ05', 'AEJ06', 'ABF02C'];
        ...
        if ($patient_medical_info) {
            ...
            $patient_medical_info['RJSS'] = $patient_medical_info['SFRJSS'] == 'N' ? '否' : '是';//是否为日间手术
        } else {
        ...
        $diagnosis = TargetService::arrayUnique($diagnosis, 'ICD10_NAME');
        ...
        if ($patient_doctor_info) {
            ...
            if (!empty($patient_doctor_info['AEE08']))
                $patient_doctor_info['BMY'] = $patient_doctor_info['AEE08'];
        } else {
            $patient_doctor_info = [];
        }
        //患者其他补充信息
        $other_field = ['SFZJLX', 'TYSHXYDM', 'JKKH', 'BFRY', 'ZKKB', 'SJHL', 'EJHL', 'YJHL', 'TJHL', 'ZRHSBM', 'ZKRQ', 'SSLX1', 'SSLX2', 'SSLX3', 'SSLX4', 'SSLX5', 'SSLX6', 'SSLX7', 'SSLX8', 'SSLX9', 'SSLX10', 'SSLX11', 'SSLX12', 'SSLX13', 'SSLX14', 'SSLX15', 'LCLJ', 'WCQK', 'SSPB'];
        ...
        //患者主要手术信息
        $main_operation_field = ['SSLX_MC', 'SSPB_MC', 'QKDJ', 'YHDJ', 'HOCUS_MAN_CODE', 'OPE_ORDER', 'RJSS', 'SSPB', 'id', 'ICD9_ID1', 'OPE_DATE', 'OPE_LEVEL', 'ICD9_NAME', 'OPE_MAN_NAME', 'FRIST_ASSISTANT_NAME', 'SECOND_ASSISTANT_NAME', 'INCISION_GRADE_MC', 'INCISION_GRADE_ID', 'HOCUS_WAY_ID', 'HOCUS_WAY_MC', 'HOCUS_MAN_NAME'];
        ...
        if ($main_operation) {
            ...
            foreach ($main_operation as $k => $v) {
                ...
                $main_operation[$k]['SSPB'] = $v['SSPB_MC'] ?? '';
                $main_operation[$k]['HOCUS_WAY_ID'] = $main_operation[$k]['HOCUS_WAY_MC'] ?? '无';
                $main_operation[$k]['SSLX'] = $v['SSLX_MC'];
        ...
        //患者次要手术信息
        $secondary_operation_field = ['SSPB_MC', 'SSLX_MC', 'HOCUS_MAN_CODE', 'OPE_ORDER', 'RJSS', 'SSPB', 'id', 'ICD9_ID1', 'OPE_DATE', 'OPE_LEVEL', 'ICD9_NAME', 'OPE_MAN_NAME', 'FRIST_ASSISTANT_NAME', 'SECOND_ASSISTANT_NAME', 'INCISION_GRADE_ID', 'INCISION_GRADE_MC', 'HOCUS_WAY_ID', 'HOCUS_WAY_MC', 'HOCUS_MAN_NAME'];
        ...
        if ($secondary_operation) {
            ...
            foreach ($secondary_operation as $k => $v) {
                ...
                $secondary_operation[$k]['SSPB'] = $v['SSPB_MC'] ?? '';
                $secondary_operation[$k]['HOCUS_WAY_ID'] = $secondary_operation[$k]['HOCUS_WAY_MC'] ?? '无';
                $secondary_operation[$k]['SSLX'] = $v['SSLX_MC'] ?? '';
        ...
        //$operation = TargetService::arrayUnique($operation,'ICD9_NAME');

        //患者费用信息
        $cost_field = ['D11', 'D12', 'D13', 'D14', 'D15', 'D16', 'D17', 'D18', 'D19', 'D19X01', 'D20', 'D20X01', 'D20X02', 'D21', 'D22', 'D23', 'D23X01', 'D24', 'D25', 'D26', 'D27', 'D28', 'D29', 'D30', 'D31', 'D32', 'D33', 'D34'];
        $patient_cost_info = PatientCostInfo::query()->where('AAA28', $patient_info['MED_REC_ID'])->first($cost_field);
        ...
        //error评分
        $error = Error::query()
            ->where(['ZYH' => $patient_info['MED_REC_ID']])
            ->where('status', 0)
            ->select('down', 'desc', 'error_field', 'error_name', 'category', 'error_rule', 'level', 'id')
            ->orderBy('category')->get()->toArray();
        if (!empty($error)) {
            $errorRuleData = ErrorRule::query()->where('status', '=', 0)->get()->toArray();
            $errorRuleData = array_column($errorRuleData, null, 'id');

            $id = array_column($error, 'id');
            $appeal = Appeal::query()->whereIn("error_id", $id)->where('quality_type', 1)->where('type', 2)->get()->toArray();
            $appeal = array_column($appeal, null, 'error_id');
            foreach ($error as $k => &$errorInfo) {
                ...
                $appealStatus = $appeal[$errorInfo['id']]['status'] ?? 0;
                if ($appealStatus == 1) {
                    unset($error[$k]);
                    continue;
                }
        ...
        // 费用明细
        $feeDetail = FeeDetailed::query()->where('AAA28', '=', $patient_info['MED_REC_ID'])->get()->toArray();
        ...
        $datas['CWH'] = $patient_info['CWH'];
        $datas['feeDetail'] = $feeDetail;
