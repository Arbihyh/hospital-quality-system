...
//    protected $fillable = ["id","name","account","password","salt","group_id"];
    protected static $field = ["id", "name", "account", "phone", "realname", "password", "salt", "login_at", "login_ip", "group_id", "token", 'desc', 'KSDM', 'KSMC'];
    ...
    // 添加
    public static function add($name, $account, $pwd, $salt, $groupId, $desc, $phone = '', $realname = '', $KSDM = "", $KSMC = "")
    {
    ...
    {
        $data = self::query()->where(["account" => $account, "name" => $name])->first(self::$field);
        if ($data) {
            return $data->toArray();
        } else {
            return false;
