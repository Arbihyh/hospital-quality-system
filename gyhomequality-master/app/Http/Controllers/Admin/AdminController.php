...
class AdminController extends Controller
{
    public function adminList(Request $request)
    {
        ...
        $data = AdminService::adminList($group_id, $account, $page, $length, $name, $realname);
    ...
    public function addAdmin(Request $request)
    {
        ...
        $realname = $request->post("realname", "");
        $KSDM = $request->post("KSDM", "");
        $dep = Department::query()->where("dep_id", $KSDM)->get()->toArray();
        $KSMC = $dep[0]['dep_name'] ?? "";
        ...
        $state = AdminService::addAdmin($name, $account, $pwd, $group_id, $desc, $phone, $realname, $KSDM, $KSMC);
        if ($state) {
    ...
    public function editAdmin(Request $request)
    {
        ...
        $dep = Department::query()->where("dep_id", $KSDM)->get()->toArray();
        $KSMC = $dep[0]['dep_name'] ?? "";
        $state = AdminService::editAdmin($id, $name, $account, $pwd, $group_id, $desc, $phone, $realname, $KSDM, $KSMC);
        if ($state) {
            return ToolsService::returnAdmin(0, '', '操作成功');
        } else {
            return ToolsService::returnAdmin(0, '', '操作成功');
    ...
    public function adminInfo(Request $request){
        ...
        $adminInfo = Admin::query()->where('id', $admin['id'])->get()->toArray();
        $data = [
...
//                $value['admin_name'] = Admin::query()->where('id','=',$value['admin_id'])->value('name');
//                $logList[] = $value;
//            }
//        }
