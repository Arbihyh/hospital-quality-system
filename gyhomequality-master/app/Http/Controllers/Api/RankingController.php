...
$item['department_name'] = Department::getDepartmentName($item['department_id']) ?? '';
...
//                DB::raw("sum(complete_error_medical) as complete_error_medical"),
        ...
        if ($result) {
            ...
            $department = config('dictionaries.ABAS02');
            foreach ($data as &$v) {
                $v['name'] = $department[$v['department_id']] ?? '';//科室
...
//                DB::raw('sum(average_score) as average_score_qa'),
        ...
        $countData = Count::query()
            ->where('hospital_name','=',$hospital_name)
            ->whereRaw("(year >= $sy and month >= $sm) and (year <= $ey and month <= $em)")
            ->first($countField);
        if ($countData){
        ...
        $averageError = 0;
        if (!empty($countData['cumulative_medical']) && !empty($countData['total_medical'])) {
            $averageError = sprintf('%.2f', ($countData['cumulative_medical'] / $countData['total_medical']) * 100);
        }
        $total_medical = 0;
        if (!empty($countData['total_score_qa']) && !empty($countData['total_medical'])) {
            $total_medical = sprintf('%.2f',$countData['total_score_qa']/$countData['total_medical']);
        }

        $data = [
            'total' => $countData['total_medical'] ?? 0,//总数量
            'averageError' => $averageError,//缺陷占比
            'errorMedical' => $countData['cumulative_medical'] ?? 0,//总缺陷
            'averageScore' => $total_medical,
        ...
        // 日均病历数
        $dayCount = (strtotime($end_time)-strtotime($start_time))/(3600*24);
        $data['day_avg'] = 0;
        if (!empty($data['total']) && !empty($dayCount)) {
            $data['day_avg'] = (int)ceil($data['total']/$dayCount);
        }
        ...
        $errorInfo = Error::query()
            ...
            ->groupBy('ZYH')
            ->orderByDesc('down')
            ->first(['ZYH',DB::raw('sum(down) as down')]);
        $data['min_score'] = !empty($errorInfo) ? 100-$errorInfo['down'] : 0;

        $data['you_ratio'] = 0;     // 优占比
        $data['liang_ratio'] = 0;   // 良占比
        $data['zhong_ratio'] = 0;   // 中占比
        $data['cha_ratio'] = 0;    // 差占比
        if ($data['total'] > 0) {
            $data['you_ratio'] = sprintf('%.2f', ($data['highest_score'] / $data['total']) * 100);
            $data['liang_ratio'] = sprintf('%.2f', ($data['good'] / $data['total']) * 100);
            $data['zhong_ratio'] = sprintf('%.2f', ($data['middle'] / $data['total']) * 100);
            $data['cha_ratio'] = sprintf('%.2f', ($data['minimum_score'] / $data['total']) * 100);
        }
        return ToolsService::returnData(200, $data, '成功');
    }
