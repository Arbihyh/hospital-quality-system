...
class Supplement extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'laravel:supplement {start?} {end?}';
    ...
    public function handle()
    {
        ...
        while (true){
            ...
            echo $start."\n";
            $idList = self::index($start);
            if (empty($idList)){
                $start = $end;
                continue;
            }
            $strList = implode(',',$idList);
            echo $strList."\n";
            $info = self::selectInsertData($strList);
            //费用查询 返回查询数据
            ...
            //患者信息入库
            self::insertDataMain($info['data'],$info['inData'],$ABC03C,$config);
            //费用信息查询
            $feeInfo = self::selectFeeInfo($strList);
            if (!empty($feeInfo)){
                $ZF = array_column($info['data'],'ADA0101','MED_REC_ID');
                //费用信息入库
...
//            self::putBeanstalkd($list);
    ...
    //按天查询唯一数据
    public static function index($start){
        $sql = "SELECT MED_REC_ID FROM PORTAL_HIS.INIT_MED_REC_MAIN WHERE AAC01 BETWEEN to_date(".$start."000000,'yyyy-MM-dd HH24:mi:ss') AND to_date(".$start."235959,'yyyy-MM-dd HH24:mi:ss')";
        ...
        $medID = array_column($data,'MED_REC_ID');
        $bMedId = PatientInfo::query()
            ->whereBetween('AAC01',[date('Y-m-d H:i:s',strtotime($start.'000000')),date('Y-m-d H:i:s',strtotime($start.'235959'))])
            ->pluck('MED_REC_ID');
        if ($bMedId){
            $bMedId = $bMedId->toArray();
        }else{
            $bMedId = [];
        }
        return array_diff($medID,$bMedId);
    }
    //查询患者信息
    public static function selectInsertData($medList){
        $sql = "SELECT * FROM PORTAL_HIS.INIT_MED_REC_MAIN WHERE MED_REC_ID IN ($medList)";
        $result = oci_parse(self::$con, $sql);
        oci_execute($result,OCI_DEFAULT);
        $data = [];
        while ($row = oci_fetch_assoc($result)) {
    ...
    {
        $beanstalkd1 = Pheanstalk::create('beanstalkd')->useTube('validate');
//        $beanstalkd2 = Pheanstalk::create(env('BEANSTALKD') ?? 'localhost')->useTube('patient_follow');
        foreach ($list as $item){
            $beanstalkd1->put(json_encode(['AAA28' => $item]));
//            $beanstalkd2->put(json_encode(['AAA28' => $item]));
        }
    }
}
