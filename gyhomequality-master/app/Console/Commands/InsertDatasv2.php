...
class InsertDatasv2 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'laravel:insertv2 {start} {end}';
    ...
    public function handle()
    {
        ...
        // 其他视图的账号
        $con1 = oci_connect("jm_chen", "jm_chen",  $connection.':'.$port.'/'.$tns, 'UTF8');
        if (!$con1) {
            $e = oci_error();
            print_r(htmlentities($e['message'])) . "\n";
            die;
        ...
        while (true) {
            ...
            $med = array_column($Id, 'MED_REC_ID');

            // 将住院号按照100个一组拆分
            $idsChunk = array_chunk($med, 100);
            foreach ($idsChunk as $ids) {
                $info = self::selectInsertData($ids);
                if (empty($info['data'])) {
                    break;
                }
    ...
    public static function selectList($start)
    {
        ...
        $result = oci_parse(self::$con, $sql1);
