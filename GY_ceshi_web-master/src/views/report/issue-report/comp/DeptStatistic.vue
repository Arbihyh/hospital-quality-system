  ...
  <div class="medical-quality-content">
    <div class="table-card">
      ...
      >
        ...
        <el-table-column label="科室名称" prop="department_name" align="left" min-width="220" sortable>
        ...
        <el-table-column label="锁定次数" align="left" min-width="120" prop="lock_count" sortable>
          <!-- <template slot-scope="scope">{{ scope.row.lock_count || '0' }}</template> -->
          <template slot-scope="scope">
            <span
              class="value-text-blue value-underline"
              @click="handleClick( scope.row)"
            >{{ scope.row.lock_count || '0' }}</span>
          </template>
        </el-table-column>
      </el-table>
...
<script>
import { shizhong_quality_department_statistics } from '@/api/excel';
export default {
  name: 'RuleStatistic',
  data() {
    return {
  ...
  methods: {
    ...
    exportCaseList() {
      ...
      shizhong_quality_department_statistics(params).then(res => {
    ...
    handleClick(row) {
      this.$emit('update:tab', row, this.queryParams, 'dept');
    },
