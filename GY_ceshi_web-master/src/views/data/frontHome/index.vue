<template>
  <div class="conter-dev" style="padding: 0 16px 16px 16px;">
    ...
    <div class="cardBox">
      <div style="margin-bottom: 13px;">
        <el-button-group>
          ...
          <el-button @click="toPage2">缺陷问题列表</el-button>
        </el-button-group>
      </div>
    ...
    <div class="cardBox" style="padding: 1px 15px 0 15px">
      <div class="tableBox2">
        <div class="top">
          <el-button-group>
            <el-button style="background-color: #409EFF;color:#fff;">科室最佳（前十）</el-button>
            <el-button @click="funTotalRanking(rankingFleg, '1')">科室总排名</el-button>
          </el-button-group>
          <!-- <el-radio-group v-model="rankingFleg" @change="funTotalRanking(rankingFleg, '1')">
        ...
        <el-table :data="ranking" style="width: 100%">
          ...
          <el-table-column prop="total_error_medical" align="center" label="总缺陷">
            <template slot-scope="scope">
              <span class="blue" @click="getBlankT(scope.row, 2)">{{ scope.row.total_error_medical }}</span>
            </template>
          </el-table-column>
    ...
    <div class="cardBox" style="padding: 1px 15px 0 15px">
      <div class="tableBox2">
        <div class="top">
          <el-button-group>
            <el-button style="background-color: #409EFF;color:#fff;">主诊组（前十）</el-button>
            <el-button @click="funTotalRanking(diagnosisRankingFleg, '2')">主诊组总排名</el-button>
          </el-button-group>
          <!-- <el-radio-group v-model="diagnosisRankingFleg" @change="funTotalRanking(diagnosisRankingFleg, '2')">
    ...
    <div class="cardBox" style="padding: 1px 15px 0 15px">
      <div class="tableBox2">
        <div class="top">
          <el-button-group>
            <el-button style="background-color: #409EFF;color:#fff;">主治医师（前十）</el-button>
            <el-button @click="funTotalRanking(indicationsFleg, '3')">主治医师总排名</el-button>
          </el-button-group>
          <!-- <el-radio-group v-model="indicationsFleg" @change="funTotalRanking(indicationsFleg, '3')">
        ...
        <el-table :data="indications" style="width: 100%">
          ...
          <el-table-column prop="total_error_medical" align="center" label="缺陷病案数"></el-table-column>
          <!-- <el-table-column prop="complete_error_medical" align="center" label="缺陷病案数">
            <template slot-scope="scope">
              <span class="blue" @click="funIndications(scope.row, 1)">{{ scope.row.total_error_medical }}</span>
            </template>
          </el-table-column> -->
          <el-table-column prop="total_error_proportion" align="center" label="问题比例"></el-table-column>
          <!-- <el-table-column align="center" label="完整性问题">
    ...
    <div class="cardBox" style="padding: 1px 15px 0 15px">
      <div class="tableBox2">
        <div class="top">
          <el-button-group>
            <el-button style="background-color: #409EFF;color:#fff;">住院医师（前十）</el-button>
            <el-button @click="funTotalRanking(hospitalDataFleg, '4')">住院医师总排名</el-button>
          </el-button-group>
          <!-- <el-radio-group v-model="hospitalDataFleg" @change="funTotalRanking(hospitalDataFleg, '4')">
    ...
    <div class="cardBox" style="padding: 1px 15px 0 15px">
      <div class="tableBox2">
        <div class="top">
          <el-button-group>
            <el-button style="background-color: #409EFF;color:#fff;">编码员（前十）</el-button>
            <el-button @click="funTotalRanking(coderDataFleg, '5')">编码员总排名</el-button>
          </el-button-group>
          <!-- <el-radio-group v-model="coderDataFleg" @change="funTotalRanking(coderDataFleg, '5')">
        ...
        <el-table :data="coderData" style="width: 100%">
          ...
          <el-table-column prop="total_medical" align="center" label="处理病案数">
            <!-- <template v-slot="{ row }">
                <span class="blue" @click="jump('/codeList', { AAC11C: row.department_id })">{{ row.total_error }}</span>
              </template> -->
            <template v-slot="{ row }">
              <span class="blue" @click="jump('/defectList?coder_name=' + row.name)">{{ row.total_medical }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  data() {
    return {
      value1: '',
      testMsg: '优：≥97分；</br>' + '良：90~96.9分且不出现A类错误；</br>' + '中：75~89.9分且不出现A类错误；</br>' + '差：＜75分。',
      tableData: [],
      formData: {
        problem: 'all',
        defectFelg: 'all',
      ...
      yearList: [],
      rankingFleg: false,
      diagnosisRankingFleg: false,
      indicationsFleg: false,
      hospitalDataFleg: false,
      coderDataFleg: false,
      nextListFelg: 0,
      page: 1,
      chart: null,
  ...
  methods: {
    ...
    toPage2() {
      this.$router.push({ path: '/defectProblemList', query: { start: this.formData.startTime, end: this.formData.endTime} })
    },
    // 点击缺陷病案
    funGototo(){
    ...
    // 获取默认当前日期
    getdaTe(){
      this.formData.endTime = dateFormat(this.getPreviousThreeDaysStart(1), 'YYYYMMDD');
      this.formData.startTime = dateFormat(this.getPreviousThreeDaysStart(3), 'YYYYMMDD');
      this.storageSet('endTime', this.formData.endTime || '');
      this.storageSet('startTime', this.formData.startTime || '');
    },
    getPreviousThreeDaysStart(n) {  
        // 获取当前日期  
        ...
        // 重置时间为当天的开始（00:00:00）  
        currentDate.setHours(0, 0, 0, 0); // 参数分别为小时、分钟、秒、毫秒  
    ...
    funTotalRanking(val, num) {
      // console.log(val);
      // if (val) {
      //   this.storageSet('endTime', this.formData.endTime || '');
      //   this.storageSet('startTime', this.formData.startTime || '');
      //   this.storageSet('jumpFleg', num);
      //   this.goto('/TotalRankingList');
      // }
        this.storageSet('endTime', this.formData.endTime || '');
        this.storageSet('startTime', this.formData.startTime || '');
        this.storageSet('jumpFleg', num);
    ...
    funSeleterQuarter(val) {
      ...
      console.error("this.formData.year.name",this.formData.year);
    ...
    funBlank(row, type, num) {
      if (type == 1) {
        this.$router.push(`/defectList?hospital_name=${row.hospital_name}`);
      } else {
        this.$router.push(`/defectList?hospital_name=${row.hospital_name}&error_type=${num}`);
      }
    },
    /**
     * 主治医生根据条件查询
     */
    funIndications(row, type, num) {
      if (type == 1) {
        this.$router.push(`/defectList?department_name=${row.department_name}`);
      } else {
        this.$router.push(`/defectList?department_name=${row.department_name}&error_type=${num}`);
      }
    },
    /**
     * 携带缺陷id跳转缺陷列表页面条件查询
    ...
    getBlankT(depart, type) {
      ...
      this.storageSet('startTime', this.formData.startTime || '');
      if (type == 1) {
        this.$router.push(`/errorList?AAC11C=${depart.department_id?depart.department_id:''}`);
      } else {
        this.$router.push(`/department?AAC11C=${depart.department_id?depart.department_id:''}`);
      }
    },
    selectInfo() {
      // let pramse = {};
    ...
    funQueryIndications() {
      ...
      this.$axios.post('/ranking_indications', pramse).then(res => {
        this.indications = res.data.list;
      });
    },
    funQueryRanking() {
      //主诊排名
      // ranking_attending_group
      let type_id = '';
      if (this.formData.type == '1') {
        type_id = this.formData.year.id || '';
      } else if (this.formData.type == '2') {
      ...
      this.$axios.post('/ranking_attending_group', pramse).then(res => {
        this.diagnosisRanking = res.data;
      });
    },
    ...
    initCharts2() {
      ...
      this.$axios.post('/ranking_department', pramse).then(res => {
        ...
        let myChartNum = [];
        let myChartname = [];
        for (let item in this.ranking) {
          myChartNum.push(this.ranking[item].average_error);
          myChartname.push(this.ranking[item].name);
        }
    ...
    initCharts3() {
      ...
      this.$axios.post('/ranking_coder', pramse).then(res => {
        this.coderData = res.data.list;
        console.error('编码员', res);
        let tabledata = [];
        for (let item in this.coderData) {
          tabledata.push({
            value: this.coderData[item].total_error,
            name: this.coderData[item].name,
          });
        }
