<template>
  <div id="MyDiv" style="margin: 16px" :class="{ nocopy: $route.query.status }">
    <div v-if="topMtnShow" class="header zbjg-box" style="background: #fff; margin: 10px 0; padding: 6px 20px">
      <!-- <span v-if="$route.query.pageType != 'LIST-KS' && $route.query.pageType != 'DETAIL-KS' && $route.query.from != 'review'">
      ...
      <span v-if="$route.query.from != 'review'">
        <el-button type="primary" v-if="!($route.query.from == 'majorIndexDetail')" style="background-color: #328240" @click="onControll">重新质控</el-button>
        <el-button
          type="primary"
          v-if="!($route.query.from == 'majorIndexDetail') && $route.query.pageType != 'LIST-KS' && $route.query.pageType != 'DETAIL-KS'"
          style="background-color: #328240"
          @click="onModuleControl"
        >
          模型质控
        </el-button>
        <el-button
          type="primary"
          v-if="$route.query.pageType === 'middleCaseControl' || $route.query.pageType === 'terminally'"
          style="background-color: #328240"
          @click="dataUpdata"
        >
          数据更新
      ...
      <div v-else></div>
      <div v-if="$route.query.pageType != 'LIST-KS' && $route.query.pageType != 'DETAIL-KS' && $route.query.from != 'review'">
        ...
        <el-button type="primary" v-if="!($route.query.from == 'majorIndexDetail')" @click="generateMedicalRecord">病历生成</el-button>
      </div>
      <div style="flex: 1" v-else>
        <div style="display: flex; justify-content: space-between; width: 100%; flex: 1; min-width: 0" v-if="$route.query.from == 'review'">
          <el-button type="primary" style="background-color: #328240" @click="onControll">重新质控</el-button>
          <el-button type="primary" @click="isControl = !isControl">{{$route.query.isNotSource === 'OK'? '解锁审核':'申诉问题'}}</el-button>
        </div>
        <div v-else>
          <el-button type="primary" @click="toExamine" :disabled="review_status == 2">审核通过</el-button>
          <el-button type="primary" @click="isControl = !isControl">病历智审结果</el-button>
        </div>
      </div>
    </div>
    <div class="cont_container">
      ...
      <div class="cont-left-tiem">
        ...
        <ul class="el-menu-vertical-demo el-menu">
          <!-- 预警信息不展示病案首页 -->
          <li class="li-left-item" :class="is_active == 0 ? 'is-active' : ''">
          ...
          <li
            ...
            :class="[
              ...
              item.bllb == 49 || [2000002, 294, 303, 329, 288, 87, 43, 2000185, 34, 'hzxx'].includes(item.bllb) || item.blbh === 'hzxx' ? 'li-left-itemyz' : 'li-left-item',
            ]"
            :key="index"
          >
            <el-row type="flex" justify="space-between" align="middle">
              <span class="treeTitle" @click="clickTree(item.bllb, item.name, item, false, item.name)">
                ...
                <span v-if="item.bllb == 1 || item.bllb == 292">（1）</span>
            ...
            <!-- 病程记录、手术记录 -->
            <div v-if="([294, 303, 329, 288, 87, 43, 2000185, 34].includes(item.bllb) || item.blbh === 'hzxx') && !item.collapse">
              <div
                v-for="(jitem, jindex) in item.list"
      ...
      <div class="cont-left-file" @mouseup.prevent="handleMouseupHandle" @contextmenu.prevent="openMenu($event)">
        ...
        <div v-else-if="is_active == 288 || is_active == 18">
          <!-- 死亡记录 或 24小时内入院记录 -->
          <DeathText :dataObjArr="dataObj" :key="JSON.stringify(dataObj)"></DeathText>
        </div>
        <div v-else>
          <div v-if="update">
      ...
      <template v-if="!$route.query.status && isControl && evaluationIndexShow && $route.query.pageType !== 'search'">
        <!-- 科室质控 || 申诉列表 -->
        <div class="case-content" v-if="$route.query.pageType === 'LIST-KS' || $route.query.pageType === 'DETAIL-KS' || $route.query.from === 'review'">
          <DeportmentQualityBox :linkType="$route.query.from" ref="deportmentQualityBoxRef" @changeTab="e => (currentTab = e)" />
        </div>

        <!-- 病案首页质控 -->
        <CaseQualityBox v-else ref="CaseQualityBoxRef" :mainHomeData="mainHomeData" :MED_REC_ID="valData" type="type_v" :width="340" :height="820" @hightRight="hightRight" />
      </template>
      <!-- 添加质控结果 -->
...
import DeportmentQualityBox from '@/views/recordsRoom/qc/components/CaseQualityBox2.vue';
import DeathText from './components/DeathText.vue';
import VueDragResize from 'vue-drag-resize';
import Axios from 'axios';
import { Loading } from 'element-ui';
import { getToken, removeToken } from '@/utils/auth';
import { getHomeData, applyForReview } from '@/api/qc';
...
export default {
  ...
  created() {
    ...
    if (this.$route.query.topBtn == 'top' || this.$route.query.pageType == 'search') {
  ...
  mounted() {
    // 页面挂载后，修改父组件的height样式
    this.setKeepAliveContainerFullHeight();
    this.getInitData();
  },
  beforeDestroy() {
    // 页面销毁前，还原原样式
    this.restoreKeepAliveContainerStyle();
  },
  activated() { },
  methods: {
    generateMedicalRecord() {
      ...
      this.$nextTick(() => {
        this.$refs.CaseQualityBoxRef.hideTabShowMedicalRecord();
      });
    },
    setKeepAliveContainerFullHeight() {
      const keepAliveContainer = document.querySelector('.keepAliveContainer');
      if (keepAliveContainer) {
        this.originalHeight = keepAliveContainer.style.height;
        keepAliveContainer.style.height = '100%';
      }
    },
    // 还原原样式
    restoreKeepAliveContainerStyle() {
      const keepAliveContainer = document.querySelector('.keepAliveContainer');
      if (keepAliveContainer && this.originalHeight) {
        keepAliveContainer.style.height = this.originalHeight;
      }
    },
    /**
     * 获取审核状态
    ...
    handelRefreshResults() {
      console.log('刷新质控结果');
      if (this.$refs.CaseQualityBoxRef || this.getIsPageType()) {
        if (this.getIsPageType()) {
          this.$refs.deportmentQualityBoxRef.getTabsData(true);
        } else {
    ...
    handleMouseupHandle() {
      ...
      console.log("text", this.blbh);
      if (text.trim().length) {
        this.dialogData.blbh = this.blbh;
    ...
    getIsPageType() {
      return this.$route.query.pageType === 'LIST-KS' || this.$route.query.pageType === 'DETAIL-KS' || this.$route.query.from === 'review';
    },
    clickzbjg() {
      // this.is_VueDragResize = true;
    ...
    onControll() {
      //科室质控
      // if (this.$route.query.pageType === 'LIST-KS' || this.$route.query.pageType === 'DETAIL-KS') {
      //   this.$axios2
      //     .post(`/quality_handle_v2?zyh=${this.valData}`, {
      //       timeout: 180000,
      //     })
      //     .then(res => {
      //       if (res.code == 200) {
      ...
      //只有终末质控才是quality_handle_v2接口，其他都是quality_handle接口
      if (this.$route.query.pageType === 'terminally') {
        this.$axios2
          .get(`/quality_handle?zyh=${this.valData}`, {
            timeout: 180000,
          })
          .then(res => {
            if (res.code == 200) {
              ...
              if (this.$refs.deportmentQualityBoxRef) {
                this.$refs.deportmentQualityBoxRef.getTabsData(true);
              }
    ...
    clickTree(b, n, item, isLeaf = false, catalogName = '') {
      ...
      if (item) {
        if (isLeaf) {
          if (n == '长期医嘱' || n == '临时医嘱') {
            ...
            // 报告单
            console.log('报告单', item.type);
            this.is_active = item.type;
          } else if (catalogName == '会诊记录单') {
            // 会诊记录单
            this.is_active = '会诊记录单';
          } else {
      ...
      if (b == 303 && isLeaf) {
        ...
        that.$axios.post('/get_surgery_data', parm).then(res => {
          console.log('get_surgery_data', res);
          ...
          if (res && res.data && res.data[0] && res.data[0].type) {
            that.surgeryData.type = res.data[0].type;
          }
          this.currentTreeItem.blbh = res?.p?.[0]?.blbh ?? '';
          console.log('this.surgeryData', this.surgeryData);
        });
        // }
      }
    },
