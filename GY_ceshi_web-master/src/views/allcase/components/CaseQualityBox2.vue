<template>
  <div
    class="CaseQualityBox2"
    :style="{
      width: $route.path == '/whitelist-caseControl' ? '100%' : tabsActive == 3 ? '400px' : currentTaskName ? '840px' : '400px',
    }"
  >
    <el-row style="padding: 10px; line-height: 20px" type="flex" justify="space-between">
      ...
      <span>
        <span class="text-bold">患者：</span>
        {{ baseInfo.BRXM }}
      </span>
    </el-row>
    <!-- <el-tabs v-model="tabsActive" style="padding: 0 10px; background-color: #fff">
      <el-tab-pane v-for="(item, index) in tabsArray" :key="index" :label="item.label" :name="item.name"></el-tab-pane>
    </el-tabs> -->
            ...
            <el-submenu :index="`${item.id}`" v-if="item.child">
              ...
              <div v-for="element in item.child">
                <el-submenu :index="`${element.parent_id}-${element.id}`" v-if="element.child">
                  ...
                  <el-menu-item
                    v-for="elementItem in element.child"
    ...
    <div v-else-if="tabsActive == 3" class="tabContent">
      <div ref="box" class="box22" :style="{ width: '100%' }" :class="{ nocopy: $route.meta.nocopy }">
        <div class="score-box" :class="scoreLevel == '甲' ? 'scoreLevel_1' : scoreLevel == '乙' ? 'scoreLevel_2' : scoreLevel == '丙' ? 'scoreLevel_3' : ''">
          <div>
            病案评分
            <span class="score-f">{{ data.score }}分</span>
          </div>
          <!-- <h2 class="score-dj">{{ scoreLevel }}</h2> -->
        </div>

        <el-row type="flex" justify="space-between" align="middle" class="card-box card-box-noStyle" v-if="data.is_case != -1">
        ...
        <el-row class="filterBox" v-if="!filterCollapse">
          <el-col :span="8" v-for="(value, key) in filterList" :class="`${filterFormData.type == key ? 'filterRow selected' : 'filterRow'}`">
              ...
              <div class="list-box box-card" :key="index">
                <div class="list-score-tips-box">
                  <el-row type="flex" align="middle" justify="space-between" style="margin-bottom: 15px">
                    ...
                    <div v-if="item.level != -1" style="color: #da1515" class="text-bold">-{{ item.score }}分</div>
                  </el-row>
                  <el-row type="flex" justify="space-between" align="top">
                    ...
                    <el-image class="list-item-image" v-if="item.is_ai == 1 || item.is_ai == 3" :src="require('../../../assets/images/zsicon.png')" fit="contain"></el-image>
                    <el-image v-if="item.is_ai == 2" class="list-item-image" :src="require('../../../assets/images/ysicon.png')" fit="contain"></el-image>
                  </el-row>
                  <div v-if="item.basis && item.basis.length > 0">
                    <div class="list-item-title" @click="item.basis && clickListItem(index)">
                      质控依据
                      <i :class="`el-icon-arrow-${!item.show ? 'up' : 'down'}`" style="cursor: pointer; font-weight: bold"></i>
                    </div>
                  </div>
                  <div class="list-item-basis-box">
                    <div class="list-basis-text">
                      <div class="list-basis-text-t" style="white-space: pre-line" :class="!item.show ? 'show' : ''">
                      ...
                      <div class="list-basis-bottom-box" v-if="$route.path == '/whitelist-caseControl'">
                        <div class="list-basis-bottom-tips">
                          ...
                          <div class="appeal_progress" style="cursor: pointer" v-if="item.appeal_type == 2 && item.appeal_status == 0" @click="clickAppeal('appeal_ing', item, 2)">
                            申诉中
                          </div>
...
import SpanWrap from '@/components/text-wrapping/span-wrap.vue';
import NoCircle from './no-circle.vue';
...
export default {
  ...
  methods: {
    ...
    simulateModelResponse(userMessage) {
      ...
      return fetch(this.bigModuleParams.url, options)
        .then(response => {
    ...
    getTableData(isRender = false) {
      ...
      let url = '/get_case_quality_v2';
      if (pageType == 'terminally') {
        url = '/get_case_quality';
      }
      console.log('', this.$props.MED_REC_ID);
      const params = {
