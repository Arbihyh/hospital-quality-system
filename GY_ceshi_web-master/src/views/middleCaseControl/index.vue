    ...
    <div class="bg-card" style="margin-bottom: 16px">
      <el-form :model="formData" ref="filterFormRef">
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label-width="80px" label="出院时间" prop="cysj_start">
              <div style="width: 94%; display: flex; gap: 5px">
                <el-form-item prop="cysj_start">
                  <el-date-picker
                    style="width: 100%"
                    v-model="formData.cysj_start"
                    type="date"
                    placeholder="出院开始日期"
...
import ProblemTableBoxVue from '@/views/allcase/components/ProblemTableBox.vue';
import MedicalRecordTableBoxVue from '@/views/allcase/components/MedicalRecordTableBox.vue';
import { getDepartmentRankExport, medicalRecordDoctorExport, exportDefectIssues } from '@/api/excel';
import DoctorRankVue from '@/views/allcase/components/DoctorRank.vue';
...
export default {
    ...
    return {
      ...
      departmentOptions: {
        tableField: [
          ...
          { prop: 'bl_num', label: '质控病历' },
          { prop: 'qx_num', label: '问题病历' },
        ...
        shortcuts: [
          {
            ...
            onClick(picker) {
              picker.$emit('pick', moment().format('YYYYMMDD'));
              that.formData.cysj_end = moment().format('YYYYMMDD');
            },
          },
          {
            text: '近7天',
          ...
          },
          {
            ...
            onClick(picker) {
              picker.$emit('pick', moment().startOf('year').format('YYYYMMDD'));
              that.formData.cysj_end = moment().startOf('year').add(3, 'M').subtract(1, 'days').format('YYYYMMDD');
            },
          },
          {
            text: '二季度',
            onClick(picker) {
              picker.$emit('pick', moment().startOf('year').add(3, 'M').format('YYYYMMDD'));
              that.formData.cysj_end = moment().startOf('year').add(6, 'M').subtract(1, 'days').format('YYYYMMDD');
            },
          },
          {
            text: '三季度',
            onClick(picker) {
              picker.$emit('pick', moment().startOf('year').add(6, 'M').format('YYYYMMDD'));
              that.formData.cysj_end = moment().startOf('year').add(9, 'M').subtract(1, 'days').format('YYYYMMDD');
            },
          },
          {
            text: '四季度',
            onClick(picker) {
              picker.$emit('pick', moment().startOf('year').add(9, 'M').format('YYYYMMDD'));
              that.formData.cysj_end = moment().startOf('year').add(12, 'M').subtract(1, 'days').format('YYYYMMDD');
            },
          },
          {
            text: moment().add(-2, 'Y').format('YYYY'),
            onClick(picker) {
              picker.$emit('pick', moment().add(-2, 'Y').startOf('year').format('YYYYMMDD'));
              that.formData.cysj_end = moment().add(-2, 'Y').endOf('year').format('YYYYMMDD');
            },
          },
          {
            text: moment().add(-1, 'Y').format('YYYY'),
            onClick(picker) {
              picker.$emit('pick', moment().add(-1, 'Y').startOf('year').format('YYYYMMDD'));
              that.formData.cysj_end = moment().add(-1, 'Y').endOf('year').format('YYYYMMDD');
            },
          },
          {
            text: moment().format('YYYY'),
            onClick(picker) {
              picker.$emit('pick', moment().startOf('year').format('YYYYMMDD'));
              that.formData.cysj_end = moment().endOf('year').format('YYYYMMDD');
            },
          },
        ],
      },
  ...
  methods: {
    ...
    urlGoto(url, value) {
      ...
      urlString = urlString + '&' + value + '&pageType=' + this.pageType();
      this.goto(urlString);
    },
    ...
    getDepartmentTableList() {
      let params = Object.assign({}, this.departmentSearch, this.formData);
      this.$axios.post('/CaseHistory/Sz/getDepartmentTableList', params).then(res => {
    ...
    getCaseList() {
      ...
      this.$axios.post('/CaseHistory/Sz/defectIssues', pramse).then(res => {
    ...
    getDoctorRank() {
      ...
      this.$axios.post('/CaseHistory/Sz/doctorRanking', params).then(res => {
    ...
    getSummaryData() {
      ...
      this.$axios.post('/CaseHistory/Sz/getTotalList', params).then(res => {
        if (res.code == 200) {
    ...
    handleExport() {
      ...
      const params = {
        ...
        start_time: this.formData.startTime,
        end_time: this.formData.endTime,
        cysj_start: this.formData.cysj_start,
        cysj_end: this.formData.cysj_end,
        is_export: 1,
      };
