  ...
  <div class="dashboard-container">
    <div class="tableBox">
      ...
      <el-form :inline="true" :model="formData" class="demo-form-inline">
        ...
        <el-form-item label="">
          <!-- <el-input v-model="formData.status" placeholder="病历状态" style="width: 180px"></el-input> -->
          <el-select v-model="formData.status" class="selects" filterable clearable placeholder="病历状态" style="width: 180px">
...
import { outHospitalShouldExport } from '@/api/excel';
...
export default {
  ...
  watch: {
    // $route(to, from) {
    //   if (to.path === '/outpatientMedicalShouldDefectNumber' && from.path === '/outpatientControl') {
    //     this.formData = {
    //       dep_id: '',
    //       startTime: '',
    //       endTime: '',
  ...
  methods: {
    // 导出
    onExport() {
      const { dep_id, startTime, endTime, status, doctor_id, mzh } = this.formData;
      const params = {
        dep_id,
      ...
      outHospitalShouldExport(params).then(res => {
        const content = res.data; // 后台返回二进制数据
        const blob = new Blob([content]);
        const fileName = `门诊应有病例.csv`;
    ...
    funGoto(blbh) {
      ...
      this.$router.push({ path: '/outpatientMedicalRecordDetail', query: { blbh: blbh, pageType: 'outpatient' } });
    },
    // 获取部门和医生select
    selectInfo() {
    ...
    funQuery() {
      ...
      if (this.formData.status) {
        pramse.status = this.formData.status;
      }
      ...
      this.$axios.post('/omr_zk/get_should_be_bl_list', pramse).then(res => {
