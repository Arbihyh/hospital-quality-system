  ...
  methods: {
    async funQuery() {
      try {
        ...
        const res = await this.$axios.get('/xxxx', { params });
        if (res.code === 200) {
          this.tableData = res.data.list || [];
          this.pageParams.total = res.data.total || 0;
        } else {
          // this.$message.warning(res.msg || '查询接口配置失败');
          this.tableData = [];
          this.pageParams.total = 0;
        }
      } catch (error) {
        // this.$message.error('网络异常，查询接口配置失败！');
        console.error('接口配置查询报错：', error);
        this.tableData = [];
        this.pageParams.total = 0;
      } finally {
    ...
    async handleExport() {
      try {
        ...
        a.href = url;
        a.download = `接口配置_${new Date().getTime()}.xlsx`;
        a.click();
        window.URL.revokeObjectURL(url);
        this.$message.success('导出成功');
      } catch (error) {
        // this.$message.error('导出失败，请重试');
        console.error('接口配置导出报错：', error);
      }
    },
  },
};
