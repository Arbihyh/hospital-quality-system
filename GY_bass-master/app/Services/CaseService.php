...
class CaseService
{
    ...
    public static function getCasePlatform($blbh = 0, $bllb = 1)
    {
...
//                        $xbsData = str_replace(['.', '(', ')', '（', '）', "“", "”", "，", "。", "；", "、", ' '], "", $xbsData);
    ...
    public function getBcData($blbh, $bllb)
    {
        $dataList = EMR_BL_BL01::query()
            ...
            ->where('BLLB','=',$bllb)
            ->where('BLZT','!=',9)
            ->get(['EMR_BL_BL01.BLBH','BRXM','EMR_BL_BL01.JZHM','BLMC','BRBH','BRXM','HJNR','bingcheng_content'])
            ->toArray();

        $returnData = [];
        foreach ($dataList as $value) {
            if (!empty($value['bingcheng_content'])) {
                $returnData[] = [
                    'blbh' => $value["BLBH"],
                    'is_format' => 1,
                    'bc_data' => json_decode($value['bingcheng_content'], true),
                ];
            } else {
                $hjnr = $value['HJNR'];
                $surgeryType = '';
