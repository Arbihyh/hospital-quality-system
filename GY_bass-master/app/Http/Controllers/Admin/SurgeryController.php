...
class SurgeryController extends Controller
{
     ...
     * @bodyParam level string required 手术操作分级
     * @bodyParam type string required 类别
     * @bodyParam option string required 录入选项
     * @response {
     *  "code":0,
     * 	"msg":"结果"
     * }
    ...
    public function addSurgery(Request $request)
    {
        ...
        $res = Db::table('surgery_class')->insert($data);
        if(!empty($res)){
    ...
    public function editSurgery(Request $request)
    {
        ...
        $res = Db::table('surgery_class')->update($data);
        if(!empty($res)){
    ...
    public function delSurgery(Request $request)
    {
        ...
        $result = Db::table('surgery_class')->where(['id'=>$id])->first();
        ...
        $res = Db::table('surgery_class')->where(['id'=>$id])->delete();
