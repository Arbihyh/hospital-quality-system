...
class QualityController extends Controller
{
    ...
    public function qualityList(Request $request)
    {
        ...
        if (!empty($AAA28) && !static::isNull($AAA28)) {
            $patientInfo[] = ['patient_info.AAA28', '=', $AAA28];
        }
        //出院科室
        $AAC11C = $request->post('AAC11C', 'all');
        if ($AAC11C != 'all' && !static::isNull($AAC11C)) {
            //$patientHospitalInfo['phi.AAC11C'] = $AAC11C;
            $patientHospitalInfo[] = ['phi.AAC11C', '=', $AAC11C, 'and'];
        }
        //问题属性
        $level = $request->post('level', 'all');
        $source = $request->post('source', 0);
        ...
        if ($ORG_STATE != 'all' && !static::isNull($ORG_STATE)) {
            $patientInfo[] = ['ORG_STATE', '=', $ORG_STATE];
        }
        //出院时间
        $startDate = $request->post('AAC01_start_date', null);
        $endDate = $request->post('AAC01_end_date', null);
        ...
        if (!empty($startDate)) {
            $startDate = date('Y-m-d', strtotime($startDate)) . ' 00:00:00';
            $patientInfo[] = ['patient_info.AAC01', '>=', $startDate];
        }
        if (!empty($endDate)) {
            $endDate = date('Y-m-d', strtotime($endDate)) . ' 23:59:59';
            $patientInfo[] = ['patient_info.AAC01', '<=', $endDate];
        }
        //编码员ID
        $coder_id = $request->post('coder_id', 'all');
        ...
        if ($field != '' && !self::isNull($field)) {
            ...
            foreach ($field as $item) {
                ...
                if (empty($item['select_type'])) {
                    $item['select_type'] = "and";
                }
                switch ($item['key']) {
                    case 'ABC01N';
                        $mainDiagnosis[] = ['md.ICD10_NAME', $operator, $value, $item['select_type']];
                        break;
                    case 'ABC01C';
                        $mainDiagnosis[] = ['md.ICD10_ID1', $operator, $value, $item['select_type']];
                        break;
                    case 'ICD10_ID1_first';
                        $otherDiagnosis[] = ['ICD10_ID1', $operator, $value, $item['select_type']];
                        $otherDiagnosis[] = ['DIA_ORDER', '=', 1, $item['select_type']];
                        break;
                    case 'ICD10_NAME_first';
                        $otherDiagnosis[] = ['ICD10_NAME', $operator, $value, $item['select_type']];
                        $otherDiagnosis[] = ['DIA_ORDER', '=', 1, $item['select_type']];
                        break;
                    case 'ICD10_ID1';
                        $otherDiagnosis[] = ['ICD10_ID1', $operator, $value, $item['select_type']];
                    ...
                    case 'ICD10_NAME';
                        ...
                        break;
                    case 'ICD9_ID1';
                        $mainOperation[] = ['ICD9_ID1', $operator, $value, $item['select_type']];
                        $secondaryOperation[] = ['ICD9_ID1', $operator, $value, $item['select_type']];
                        break;
                    case 'ICD9_NAME';
                        $mainOperation[] = ['ICD9_NAME', $operator, $value, $item['select_type']];
                        $secondaryOperation[] = ['ICD9_NAME', $operator, $value, $item['select_type']];
                        break;
                    case 'ICD8_ID1';
                        $secondaryOperation[] = ['ICD9_ID1', $operator, $value, $item['select_type']];
                        break;
                    ...
                    case 'ABC03C';//主要诊断入院病情
                        if ($item['value'] > 0) {
                            $patientMedicalInfo[] = ["pmi.ABC03C", " = ", $item['value'], $item['select_type']];
                        }
                        break;
                    case 'RYQK';//其他诊断入院病情
                        if ($item['value'] > 0) {
                            $otherDiagnosis[] = ["RYQK", "=", $item['value'], $item['select_type']];
                        }
                        break;
                    case 'OPE_LEVEL';//手术级别
                        if ($item['value'] != 0) {
                            $mainOperation[] = ['OPE_LEVEL', '=', $item['value'], $item['select_type']];
                            $secondaryOperation[] = ['OPE_LEVEL', '=', $item['value'], $item['select_type']];
                        }
                        break;
                    case 'IS_MAIN_WAY';//重症监护室名称
                        $icu[] = ['icu.IS_MAIN_WAY', $operator, $value, $item['select_type']];
                        break;
                    case 'AEM01C';//离院方式
                        if ($item['value'] != 0) {
                            $patientInfo[] = ['patient_info.AEM01C', '=', $item['value'], $item['select_type']];
                        }
                        break;
                    case 'ABA01N';//门急诊诊断
                        $patientMedicalInfo[] = ['pmi.ABA01N', $operator, $value, $item['select_type']];
                        break;
                    case 'ABA01C';//门急诊疾病编码
                        $patientMedicalInfo[] = ['pmi.ABA01C', $operator, $value, $item['select_type']];
                        break;
                    case 'AEL01';//呼吸机
                        if ($item['value'] == 1) {
                            $patientMedicalInfo[] = ['pmi.AEL01', '>', 0, $item['select_type']];
                        } elseif ($item['value'] == 2) {
                            $patientMedicalInfo[] = ['pmi.AEL01', '<=', 0, $item['select_type']];
                        }
                        break;
                    case 'RJSS';//日间手术
                        if ($item['value'] != 0) {
                            $sSType = ['', '是', '否'];
                            $mainOperation[] = ['SFWRJSS', '=', $sSType[$item['value']], $item['select_type']];
                            $secondaryOperation[] = ['SFWRJSS', '=', $sSType[$item['value']], $item['select_type']];
                        }
                        break;
                    case 'LNSSQ';//颅脑损伤前昏迷
                        if ($item['value'] == 1) {
                    ...
                    case 'LNSSH';//颅脑损伤后昏迷
                        ...
                        break;
                    case 'AAA28';//病案号
                        $patientInfo[] = ['patient_info.AAA28', $operator, $value, $item['select_type']];
                        break;
                    case 'AAA01';//姓名
                        $patientInfo[] = ['patient_info.AAA01', $operator, $value, $item['select_type']];
                        break;
                    case 'AAA02C';//性别
                        $patientInfo[] = ['patient_info.AAA02C', $operator, $value, $item['select_type']];
                        break;
                    case 'AAA04';//年龄
                        $patientInfo[] = ['patient_info.AAA04', '=', $item['value'], $item['select_type']];
                        break;
                    case 'AAA29';//住院次数
                        $patientInfo[] = ['patient_info.AAA29', '=', $item['value'], $item['select_type']];
                        break;
                    case 'SSPB': // 手术判别
                        if ($value < 5) {
                            $mainOperation[] = ['SSPB', '=', $item['value'], $item['select_type']];
                            $secondaryOperation[] = ['SSPB', '=', $item['value'], $item['select_type']];
                        }
                        break;
                    case 'AAC11N': // 出院科室
                        $patientHospitalInfo[] = ['phi.AAC11C', '=', $item['value'], $item['select_type']];
                        break;
                    default;
                }
            }
        ...
        if ($age_start_type == $age_end_type) {
            if ($age_start_type == 1) {//天
                if ($age_start > 0 && $age_end > 0) {
                    $patientInfo[] = ['patient_info.AAA40', '>=', $age_start];
                    $patientInfo[] = ['patient_info.AAA40', '<=', $age_end];
            ...
            if ($age_start_type == 2) {//年
                if ($age_start > 0 && $age_end > 0) {
                    $patientInfo[] = ['patient_info.AAA04', '>=', $age_start];
                    $patientInfo[] = ['patient_info.AAA04', '<=', $age_end];
                } elseif ($age_start > 0 && $age_end <= 0) {
                    $patientInfo[] = ['patient_info.AAA04', '>=', $age_start];
                } elseif ($age_start <= 0 && $age_end > 0) {
                    $patientInfo[] = ['patient_info.AAA04', '<=', $age_end];
        ...
        if ($AAC0401 > 0 && $AAC0402 > 0) {
            $patientInfo[] = ['patient_info.AAC04', [$AAC0401, $AAC0402]];
        } elseif ($AAC0401 > 0 && $AAC0402 <= 0) {
            $patientInfo[] = ['patient_info.AAC04', '>=', $AAC0401];
        } elseif ($AAC0401 <= 0 && $AAC0402 > 0) {
            $patientInfo[] = ['patient_info.AAC04', '<=', $AAC0402];
        }
        //关键词搜索
    ...
    {
        $coder = array_merge([['id' => 'all', 'name' => '全部']], CoderService::getCoderList());
        ...
        $department = array_merge([['id' => 'all', 'name' => '全部']], DepartmentService::getDepartmentArray());
...
//                    ['id'=>'AAA04','name'=>'年龄'],
    ...
    public function errorList(Request $request)
    {
        ...
        $data = $query
            ...
            ->limit($limit)
            ->orderBy('created_at', 'desc')
            ->get(['AAA28', 'error_name', 'desc']);
        if ($data) {
            $data = $data->toArray();
        } else {
            $data = [];
        }
        $bahList = array_column($data, 'AAA28');
        $qtData = PatientInfo::query()
            ...
            ->get(['patient_info.MED_REC_ID', 'AAA01', 'AAC01', 'AAC11N', 'AAB01']);
        if ($qtData) {
            $qtData = $qtData->toArray();
        } else {
            $qtData = [];
        }
        $qtData = array_column($qtData, null, 'MED_REC_ID');
        $bah = PatientInfo::query()
            ->whereIn('MED_REC_ID', $bahList)
            ->get(['MED_REC_ID', 'AAA28'])
            ->toArray();
        $bah = array_column($bah, 'AAA28', 'MED_REC_ID');
        foreach ($data as &$item) {
            $item['AAA28'] = $bah[$item['AAA28']] ?? '';
            $item['AAC01'] = $qtData[$item['AAA28']]['AAC01'] ?? '';
            $item['AAC11N'] = $qtData[$item['AAA28']]['AAC11N'] ?? '';
            $item['AAB01'] = $qtData[$item['AAA28']]['AAB01'] ?? '';
            $item['MED_REC_ID'] = $qtData[$item['AAA28']]['MED_REC_ID'] ?? '';
        }
    ...
    public function errorData(Request $request)
    {
        ...
        $listQuery = Error::query();
        if ($level != 'all') {
            $query->where('er.level', $level);
            $listQuery->where('level', $level);
        }
        $start_time = date('Y-m-d 00:00:00', strtotime($start_time));
        $end_time = date('Y-m-d 23:59:59', strtotime($end_time));
        $query->whereBetween('error_data.created_at', [$start_time, $end_time]);
        //$listQuery->whereBetween('created_at',[$start_time,$end_time]);
        $SYear = date('Y', strtotime($start_time));
        $SMonth = date('m', strtotime($start_time));
        $EYear = date('Y', strtotime($end_time));
        $EMonth = date('m', strtotime($end_time));
        if ($AAC11C != 'all') {
            $listQuery->where('AAC11C', $AAC11C);
            /*$error = Error::query()
                ->where('AAC11C',$AAC11C)
        ...
        $list = $listQuery
            ...
            ->offset($offset)
            ->limit(10)
            ->get([DB::raw('DISTINCT AAA28'), 'error_rule', 'month', 'year', DB::raw('count(DISTINCT `AAA28`) as count'), DB::raw('error_name as field'), 'desc', 'level']);
        if ($list) {
            $list = $list->toArray();
        } else {
            $list = [];
...
//        var_dump($AAC11C);exit;
        if ($AAC11C != 'all' && !empty($AAC11C)) {
            $AAC11C = DepartmentService::getDepartment($AAC11C);
    ...
    public function errorDataList(Request $request)
    {
        ...
        $query = EMR_BL_BL01::query()
            ->rightJoin('patient_info as pi', 'EMR_BL_BL01.JZHM', '=', 'pi.MED_REC_ID')
        ...
        if ($start_time && $end_time) {
            $whereCase = 'pi.AAC01 > "' . $start_time . '" and pi.AAC01 < "' . $end_time . '"';
        ...
        $list = $query
            ->offset($offset)
            ->limit($limit)
            ->get(['pi.AAA28', 'EMR_BL_BL01.JZHM as MED_REC_ID', 'AAA01', 'AAC11N', 'AAC01', 'AAC03', 'EMR_BL_BL01.BRXM as AEE04'])->toArray();
        if ($isExport == 1) {
            return $this->errorDataListExportData($list);
        }
...
//
    ...
    public function getTree(Request $request)
    {
        ...
        $bllb = EMR_BL_BL01::query()
            ->whereRaw("(BRBH='{$id}' or JZHM='{$id}')")
            ->distinct()
            ->pluck('BLLB')->toArray();
        ...
        foreach ($config as $k => $v) {
            ...
            if (in_array($v['bllb'], [294, 303])) {
                // 获取病程记录、手术二级菜单
                $bllbList = $targetService->getSurgeryMenu($id, $v['bllb']);
                if (!empty($bllbList)) {
                    $config[$k]['list'] = $bllbList;
                }
            } elseif ($v['bllb'] == 2000002) {
                // 获取报告单二级菜单
                $pacsMenuList = $targetService->getPacsMenu($id);
    ...
    public function getAllCase(Request $request)
    {
        ...
        $text = implode("\r\n\r\n\r\n", $data);
        return ToolsService::returnData(200, ['text' => $text ?? '']);
    }
    ...
    public function exportNormalSearch(Request $request)
    {
        $excel_data[] = ['序号', '住院号', '年龄', '出院科室', '入院时间', '出院时间'];
        $limit = 1000;
        $page = 0;
        $no = 1;
        while (true) {
            $data = $this->normalSearchPublic($request, $page, $limit);

            if(empty($data['list']) || $page >= 100){
                break;
            }
            foreach ($data['list'] as $item) {
                if (empty($item['MED_REC_ID'])) {
                    continue;
                }
                $excel_data[] = [
                    $no,
        ...
        $csv = new CsvService();
        $csv->filename = $csv->charset('医院大数据病例', 'UTF-8');
        return $csv->export($excel_data, false);
    }


    //普通搜索
    public function normalSearchPublic(Request $request, $page = 1, $limit = 20)
    {

        $keyword = $request->post("keyword", "");
        $detail = $request->post("detail", 1);// 1为详情，0为列表
        $AAC01_start = $request->post("AAC01_start", "");
        $AAC01_end = $request->post("AAC01_end", "");
        ...
        if ($keyword) {
            ...
            $params = QualityService::normalSearchByKeyword($params, $keyword, $detail);

            // 排序
            if (!empty($sort)) {
                $params['body']['sort'] = [[$sort['sort']['field'] => $sort['sort']['sort']]];
            }

            // 出院时间
            if ($AAC01_start && $AAC01_end) {
                $startTime = date("Y-m-d", strtotime($AAC01_start)) . ' 00:00:00';
                $endTime = date("Y-m-d", strtotime($AAC01_end)) . ' 23:59:59';
            ...
            $fee_ret = app('es')->search($params);
            $fee_list = [];
            if (!empty($fee_ret['hits']['hits'])) {
                foreach ($fee_ret['hits']['hits'] as $fee) {
                    $fee['_source']['FYMC'] = $fee['highlight']['FYMC'];
                    $fee_list[$fee['_source']['MED_REC_ID']][] = $fee['_source'];
                }
            }
            $params = [
                'index' => 'yzb_2023',
            ...
            $yzb_ret = app('es')->search($params);
            $yzb_list = [];
            if (!empty($yzb_ret['hits']['hits'])) {
                foreach ($yzb_ret['hits']['hits'] as $yzb) {
                    $yzb['_source']['YZMC'] = $yzb['highlight']['YZMC'];
                    $yzb_list[$yzb['_source']['MED_REC_ID']][] = $yzb['_source'];
                }
            }
...
//            ];
            ...
            // 排序
            if (!empty($sort)) {
                $params['body']['sort'] = [[$sort[0] => $sort[1]]];
            } else {
        ...
        $data['total'] = $total;
        if ($total > 0) {
            $medIds = array_column($ret['hits']['hits'], '_source.MED_REC_ID');
            $patientInfo = PatientInfo::query()->whereIn('MED_REC_ID', $medIds)->get(['MED_REC_ID', 'AAA01', 'AAA04', 'AAC11N', 'AAB01', 'AAC01'])->toArray();
            $patientInfoMap = array_column($patientInfo, null, 'MED_REC_ID');
            foreach ($ret['hits']['hits'] as $value) {
                if (!isset($value['_source']['AAA28'])) {

                    $patientInfo = $patientInfoMap[$value['_source']['MED_REC_ID']];
                    if ($patientInfo) {
                        $value['_source'] = [
                ...
                $emr_bl_01 = [];
                if (!empty($value['inner_hits']['EMR_BL_BL01']['hits']['hits'][0]['highlight'])) {
                    foreach ($value['inner_hits']['EMR_BL_BL01']['hits']['hits'][0]['highlight'] as $key => $emr) {
                        $bllb = str_replace('EMR_BL_BL01.', '', $key);
                        $emr_bl_01[] = [
                            'BLLB' => $bllb,
                            'HJNR' => $emr
                        ];
                ...
                $fee_detailed = [];
                if (!empty($fee_list[$value['_source']['MED_REC_ID']])) {
                    $fee_detailed = $fee_list[$value['_source']['MED_REC_ID']];
                }
                $yzb_detailed = [];
                if (!empty($yzb_list[$value['_source']['MED_REC_ID']])) {
                    $yzb_detailed = $yzb_list[$value['_source']['MED_REC_ID']];
                }
                $data['detail'][] = [
                    'EMR_BL_BL01' => $emr_bl_01,
                    'FeeDetailed' => $fee_detailed,
                    'YZB' => $yzb_detailed,
            ...
            if (!empty($data['detail'])) {
                $medIds = array_column($data['detail'], 'MED_REC_ID');
                $patientInfo = PatientInfo::query()->whereIn('MED_REC_ID', $medIds)->get(['MED_REC_ID', 'AAA01'])->toArray();
                $patientInfo = array_column($patientInfo, null, 'MED_REC_ID');
                foreach ($data['detail'] as &$v) {
                    foreach ($v['EMR_BL_BL01'] as $k => $emr) {
                        if (empty($patientInfo[$v['MED_REC_ID']])) {
                            continue;
                        }
                        $name = $patientInfo[$v['MED_REC_ID']]['AAA01'];
                        $nameFileter = desensitize($name, 1, 0, '*');
                        $v['EMR_BL_BL01'][$k]['HJNR'] = str_replace($name, $nameFileter, $v['EMR_BL_BL01'][$k]['HJNR']);
                    }
                }
            }
        }
    ...
    {
        $excel_data[] = ['序号', '住院号', '年龄', '出院科室', '入院时间', '出院时间']; // 导出数据的标头
        $limit = 1000;
        $page = 1;
        $no = 1;
        while (true) {
            $data = $this->searchDataPublic($request, $page, $limit);
     ...
     */
    //高级搜索
    public function searchData_bak(Request $request)
    {
        ...
        $map['start_time'] = $request->post("AAC01_start", "");
        $map['end_time'] = $request->post("AAC01_end", "");
        $map['date_start'] = $request->post("AAC04_start", 0);
        $map['date_end'] = $request->post("AAC04_end", 0);
        $map['age_start'] = $request->post("AAA04_start", 0);
        $map['age_end'] = $request->post("AAA04_end", 0);
        $map['day_start'] = $request->post("AAA40_start", 0);
        $map['day_end'] = $request->post("AAA40_end", 0);
        $map['field'] = $request->post('field', '');
        $params = QualityService::getSearchDataParams($params, $map, $detail);
        if (empty($params['body']['query'])) {
            $params['index'] = 'quality';
        }
            ...
            foreach ($ret['hits']['hits'] as $value) {
                if (!isset($value['_source']['AAA28'])) {
                    $patientInfo = PatientInfo::query()
                        ->where('MED_REC_ID', '=', $value['_source']['MED_REC_ID'])
                        ->first();
                    if ($patientInfo) {
                        $value['_source'] = [
                            'AAA28' => $patientInfo->AAA28,
                ...
                $pacs_detailed = [];
                if (!empty($pacs_list[$value['_source']['MED_REC_ID']])) {
                    $pacs_detailed = $pacs_list[$value['_source']['MED_REC_ID']];
                }
                $data['detail'][] = [
                    'EMR_BL_BL01' => $emr_bl_01,
                    'FeeDetailed' => $fee_detailed,
        ...
        // 数据脱敏
        if (request()->post('is_tm')) {

            $quality = array_column($ret['hits']['hits'], '_source');
            $medEecIds = array_column($quality, 'MED_REC_ID');
            $patientInfo = PatientInfo::query()->whereIn('MED_REC_ID', $medEecIds)->get(['MED_REC_ID', 'AAA01'])->toArray();
            $patientInfo = array_column($patientInfo, null, 'MED_REC_ID');
            $patientAddressInfo = PatientAddressInfo::query()->whereIn('AAA28', $medEecIds)->get(['AAA28', 'AAA46', 'AAA47', 'AAA12'])->toArray();
            $patientAddressInfo = array_column($patientAddressInfo, null, 'AAA28');
            $filterData = []; // 需要过滤的数据
            foreach ($medEecIds as $medEecId) {
                $filterData[$medEecId] = array_merge($patientInfo[$medEecId], $patientAddressInfo[$medEecId]);
            }
            if ($data['detail']) {
                foreach ($data['detail'] as &$item) {
                    // 名字
                    $name = $filterData[$item['MED_REC_ID']]['AAA01'];
                    $nameFilter = desensitize($filterData[$item['MED_REC_ID']]['AAA01'], 1, 0, '*');
                    // AAA46
                    $AAA46 = $filterData[$item['MED_REC_ID']]['AAA46'];
                    $AAA46Filter = desensitize($filterData[$item['MED_REC_ID']]['AAA46'], 0, 0, '*');
                    // AAA47
                    $AAA47 = $filterData[$item['MED_REC_ID']]['AAA47'];
                    $AAA47Filter = desensitize($filterData[$item['MED_REC_ID']]['AAA47'], 0, 0, '*');
                    // AAA12
                    $AAA12 = $filterData[$item['MED_REC_ID']]['AAA12'];
                    $AAA12Filter = desensitize($filterData[$item['MED_REC_ID']]['AAA12'], 0, 0, '*');
                    foreach ($item['EMR_BL_BL01'] as &$v) {
                        $v['HJNR'][0] = str_replace($name, $nameFilter, $v['HJNR'][0]);
                        $v['HJNR'][0] = str_replace($AAA46, $AAA46Filter, $v['HJNR'][0]);
                        $v['HJNR'][0] = str_replace($AAA47, $AAA47Filter, $v['HJNR'][0]);
                        $v['HJNR'][0] = str_replace($AAA12, $AAA12Filter, $v['HJNR'][0]);
                    }
                }
            }
        }
     ...
     */
    //高级搜索
    public function searchDataPublic(Request $request, $page = 1, $limit = 10)
    {
//        $limit = $request->post('limit', 10);
//        $page = $request->post('page', 1);
        $detail = $request->post("detail", 1);// 1为详情，0为列表
        ...
        $map['day_end'] = $request->post("AAA40_end", 0);
        $map['field'] = $request->post('field', '');
        $map['AAA28'] = $request->post("AAA28", 0);
        $map['sort'] = $request->post('sort', '');
        $map['other_diagnosis_ICD10_ID1'] = $request->post("other_diagnosis_ICD10_ID1", "");
        $map['other_diagnosis_ICD10_NAME'] = $request->post("other_diagnosis_ICD10_NAME", "");
        $map['main_operation_ICD9_ID1'] = $request->post("main_operation_ICD9_ID1", "");
        $map['main_operation_ICD9_NAME'] = $request->post("main_operation_ICD9_NAME", "");
        $map['secondary_operation_ICD9_ID1'] = $request->post("secondary_operation_ICD9_ID1", "");
        $map['secondary_operation_ICD9_NAME'] = $request->post("secondary_operation_ICD9_NAME", "");
        $map['ICD9_ID1'] = $request->post("ICD9_ID1", "");
        $map['ICD9_NAME'] = $request->post("ICD9_NAME", "");

        $ret = QualityService::getSearchDataParams($params, $map, $detail);
            ...
            foreach ($ret['hits']['hits'] as $value) {
                ...
                if ($patientInfo) {
                    $arr = [
                        ...
                        'MED_REC_ID' => $patientInfo->MED_REC_ID,
                        'AAA04' => $patientInfo->AAA04,
                        'AAC11N' => $depMap[$patientInfo->AAC02C]??"",
                        'AAB01' => $patientInfo->AAB01,
                        'AAC01' => $patientInfo->AAC01
                    ];
                }
                ...
                if ($patientInfo) {
                    $info = QualityService::getSsInfoData($patientInfo->MED_REC_ID, $patientInfo->AAA28, $patientInfo->AAB01, $patientInfo->AAC01, $map['field']);
                }
                $data['detail'][] = [
                    'EMR_BL_BL01' => $emr_bl_01,
                    'AAA28' => $value['_source']['AAA28'] ?? '',
        ...
        if (request()->post('is_tm')) {
            ...
            $filterData = []; // 需要过滤的数据
            foreach ($medEecIds as $medEecId) {
                $filterData[$medEecId]['AAA01'] = empty($patientInfo[$medEecId]) ? '' : $patientInfo[$medEecId]['AAA01'];
                $filterData[$medEecId]['AAA46'] = empty($patientAddressInfo[$medEecId]) ? '' : $patientAddressInfo[$medEecId]['AAA46'];
                $filterData[$medEecId]['AAA47'] = empty($patientAddressInfo[$medEecId]) ? '' : $patientAddressInfo[$medEecId]['AAA47'];
                $filterData[$medEecId]['AAA12'] = empty($patientAddressInfo[$medEecId]) ? '' : $patientAddressInfo[$medEecId]['AAA12'];
            }
            if (!empty($data['detail'])) {
                foreach ($data['detail'] as &$item) {
                    // 名字
    ...
    public function searchSelect()
    {
        $config = [
            ...
            ['name' => '医疗常用表格', 'bllb' => 87],
            ['name' => '医嘱单', 'bllb' => 49],
            ['name' => '出院科室', 'bllb' => 'AAC11N'],
//            ['name' => '费用明细', 'bllb' => 'FYMC'],
            // ['name' => '报告单', 'bllb' => 2000002], // no
            ['name' => '主要诊断名称', 'bllb' => 'ICD10_NAME'], // no
            ['name' => '主要诊断编码', 'bllb' => 'ICD10_ID1'], // no
            ['name' => '手术名称', 'bllb' => 'ICD9_NAME'], // no
            ['name' => '手术编码', 'bllb' => 'ICD9_ID1'], // no
            ['name' => '其他手术名称', 'bllb' => 'secondary_operation_ICD9_NAME'], // no
            ['name' => '其他手术编码', 'bllb' => 'secondary_operation_ICD9_ID1'], // no
            ['name' => '主要手术名称', 'bllb' => 'main_operation_ICD9_NAME'], // no
            ['name' => '主要手术编码', 'bllb' => 'main_operation_ICD9_ID1'], // no
            ['name' => '其他诊断名称', 'bllb' => 'other_diagnosis_ICD10_NAME'], // no
            ['name' => '其他诊断编码', 'bllb' => 'other_diagnosis_ICD10_ID1'], // no
        ];
        return ToolsService::returnData(200, $config);
