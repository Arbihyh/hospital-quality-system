  ...
  >
    <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="100px" class="demo-ruleForm">
      <el-form-item label="数据库表" prop="table">
        <el-select v-model="ruleForm.table" filterable placeholder="请选择" style="width: 100%;" :disabled="true">
      ...
      <el-form-item v-if="ruleForm.table" label="表字段" prop="table_field">
        <el-select v-model="ruleForm.table_field" filterable placeholder="请选择" style="width: 100%;" :disabled="true">
...
export default {
  ...
  async created() {
    ...
    this.$set(this.ruleForm, 'table', tableId)
    await this.getData2(tableId, null)
    const tableFieldId = await this.getTableFieldId()
    this.$set(this.ruleForm, 'table_field', tableFieldId)
  },
  methods: {
    getTableId() {
      const { id } = this.table.filter(item => item.field_name === this.data.row.parent_fildname)[0]
      return id
    },
    getTableFieldId() {
      const { id } = this.table_field.filter(item => item.field_name === this.data.row.parent_field)[0]
      return id
    },
    async getData(field, field_name) {
      const params = {}
      if (field || field === 0) {
        params.field = field
      }
      ...
      await get_field_detail(params).then(res => {
        const { p } = res
        this.table = Array.isArray(p) ? p : []
      })
    },
    async getData2(field, field_name) {
      const params = {}
      if (field || field === 0) {
        params.field = field
      }
