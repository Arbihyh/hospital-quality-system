  ...
  <div>
    ...
    >
      ...
      >
        <el-row :gutter="24">
          <el-col :span="6">
            <el-form-item label="文书范围" prop="case_type">
          ...
          <el-col :span="6">
            <el-form-item label="问题级别" prop="error_level">
              <el-select
          ...
          <el-col :span="6">
            <el-form-item prop="disease" label="专病病种">
              <el-select
          ...
          <el-col :span="6">
            <el-form-item label="运行节点" prop="changjing">
              <el-select
          ...
          <!-- <el-col v-if="ruleForm.id" :span="5">
            <el-form-item label="质控来源" prop="changjing">
              <el-select
                v-model="ruleForm.data_source"
                clearable
                filterable
                :disabled="data.isParamsEx"
                placeholder="请选择"
                style="width: 100%;"
              >
                <el-option
          ...
          <el-col v-if="ruleForm.id" :span="6">
            <el-form-item label="规则来源" prop="is_ai">
              <el-input v-model="currentRuleComp" :disabled="data.isParamsEx" style="width: 100%;" />
            </el-form-item>
          </el-col>
          <el-col v-if="ruleForm.id" :span="6">
            <el-form-item label="规则ID" prop="id">
              <el-input v-model="ruleForm.id" :disabled="data.isParamsEx" style="width: 100%;" />
            </el-form-item>
          </el-col>
        ...
        <div v-if="data.isParamsEx">
          ...
          <div v-if="ruleForm.id" class="quality-control-section">
            <div class="section-header">
              <span class="section-title">质控效果</span>
              <!-- <span class="section-subtitle">近30天统计</span> -->
            </div>
            <div class="stats-grid">
              <div class="stat-card">
                <span class="stat-label">准确率</span>
                <span class="stat-value">{{ ruleForm.quality_accuracy_rate }}%</span>
              </div>
              <div class="stat-card">
                <span class="stat-label">触发次数</span>
                <span
                  class="stat-value"
                >{{ ruleForm.quality_count == "" ? 0 : ruleForm.quality_count }}</span>
              </div>
              <div class="stat-card">
                <span class="stat-label">人工纠正</span>
                <span
                  class="stat-value"
                >{{ ruleForm.artificial_correction_count == "" ? 0 : ruleForm.artificial_correction_count }}</span>
              </div>
              <div class="stat-card">
                <span class="stat-label">申诉纠正</span>
                <span
                  class="stat-value"
                >{{ ruleForm.appeal_correction_count == "" ? 0 : ruleForm.appeal_correction_count }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <span
        v-if="data.actionType === 'DETAIL' && titleStr !== '详情'"
        slot="footer"
        class="dialog-footer"
        style="margin-right: 10px;"
      >
        <el-button @click="titleStr = '详情'">取消</el-button>
      </span>
      ...
      <span v-if="titleStr === '详情'" slot="footer" class="dialog-footer">
        <el-button @click="data.bSwitch = false">关闭</el-button>
        <el-button type="primary" @click="titleStr = '编辑'">编辑</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  computed: {
    ...
    // },
    currentRuleComp() {
      if (this.ruleForm.is_ai === 1) { return '系统' } else if (this.ruleForm.is_ai === 2) { return '人工' } else if (this.ruleForm.is_ai === 3) { return '大模型' } else return '自定义'
    },
    filteredCategories() {
  ...
  methods: {
    initTitle() {
      if (this.data.row.id) {
        this.titleStr = this.data.actionType === 'DETAIL' ? '详情' : '编辑'
      } else {
        this.titleStr = '新增'
      }
    },
    ...
    getDetail(id) {
      get_rule_detail({ id }).then((res) => {
        ...
        this.quality_accuracy_rate = quality_accuracy_rate
        this.artificial_correction_count = artificial_correction_count
        this.appeal_correction_count = appeal_correction_count
